...
The canonical current book is the live **84-chapter** site. [Open the complete
book in Human view](https://corbensorenson.github.io/asi-stack-book/?view=human)
for the reader-facing projection, or use the
[84-chapter architecture index](https://corbensorenson.github.io/asi-stack-book/products/architecture-reference/)
for direct technical lookup. The generated reader pipeline currently derives
all 84 chapters and passes the reader-spine contract. Downloadable artifacts
from earlier releases remain in the GitHub release archive, but they are not
presented as the current book because they do not contain the complete current
manifest.
...
Releases are immutable historical snapshots; they remain available for exact
citation and reproduction but do not override the current live-book count,
chapter order, source inventory, or claim state.

- Quarto book structure is initialized and renders to HTML.
- All 84 manifest chapters exist as manuscript drafts across four manifest-driven parts.
...
- Appendix C now carries a generated "What would promote this" field for all 84 chapter core claims, derived from `docs/per_chapter_evidence_plan.md` and validated by `scripts/validate_core_claim_promotion_paths.py`, without changing any support state. `docs/core_claim_disposition_ledger.md` consolidates those per-chapter movement paths with accepted no-change transition records and accepted no-promotion decisions.
- `docs/chapter_consolidation_sequence.md` preserves the governed consolidation
  history, URL lineage, meaning-preservation checks, and no-support-state-change
  boundary. It is historical process evidence, not a competing description of
  the current 84-chapter architecture.
...
- Tracked reader-manuscript directories preserve immutable derivative history.
  The current reader path is generated from all 84 live chapters plus the
  semantic overlay manifest; old derivative manifests cannot override live-book
  claim, evidence, proof/test, implementation, or release authority.
...
- `editions/reader_manuscript/v1_0/chapter_review_matrix.json` and `docs/reader_chapter_review_matrix.md` preserve a historical curated-reader review queue. The current all-chapter reader check derives directly from `book_structure.json`; the frozen queue remains edition-specific and cannot define current coverage.
...
- `docs/evidence_laundering_prevention_case_studies.md` records three live no-promotion case studies plus one historical count-surface narrowing record. The historical record cannot define current breadth; the active 84-chapter manifest is derived from `book_structure.json`. `scripts/validate_evidence_laundering_case_studies.py` keeps those examples visible while preserving that no chapter core claim has been demoted or refuted.
...
| [docs/reader_chapter_review_matrix.md](docs/reader_chapter_review_matrix.md) | Historical curated-reader review queue and edition-specific release blockers; not current coverage authority. |
