...
- Appendix C now carries a generated "What would promote this" field for all 45 chapter core claims, derived from `docs/per_chapter_evidence_plan.md` and validated by `scripts/validate_core_claim_promotion_paths.py`, without changing any support state. `docs/core_claim_disposition_ledger.md` consolidates those per-chapter movement paths with accepted no-change transition records and accepted no-promotion decisions.
