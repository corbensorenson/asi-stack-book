...
- `scripts/validate_source_appendices.py` checks that Appendix G and Appendix H are independent top-level appendices: G contains only Corben-authored, Corben-supplied, and local project records, while H contains only external-source records by other authors generated from `sources/source_inventory.json`.
