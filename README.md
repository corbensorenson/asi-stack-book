...
- `editions/reader_overlays/README.md` and `editions/reader_overlays/v1_0/manifest.json` define the semantic reader-overlay layer for major-version human-edition deltas that should survive regeneration without forking the live book. The current v1.0 overlay set carries 46 active operations across 24 chapters for Human view and generated reader editions only; `assets/reader-overlays.html`, `docs/reader_continuity_audit.md`, and `docs/reader_chapter_review_matrix.md` are the generated review surfaces for the exact operation set.
