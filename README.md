...
- `scripts/validate_source_appendices.py` checks that Appendix G and Appendix H are independent top-level appendices: G contains only Corben-authored, Corben-supplied, and local-project source records, while H contains only external-source records by other authors generated from `sources/source_inventory.json`.
...
| [appendices/G_corben_source_corpus.qmd](appendices/G_corben_source_corpus.qmd) | Generated appendix for Corben-authored, Corben-supplied, and local project ASI Stack sources. |
