...
**What is currently evidenced:** all 51 chapter core claims remain at `argument`; three narrow non-core transitions are accepted and listed in [docs/non_core_evidence_ledger.md](docs/non_core_evidence_ledger.md); each v1.x safety-critical Lean module now has at least one derived/decomposed finite-record theorem, but the modules remain shallow formal envelopes rather than deployed safety proofs; all 51 chapters have source-noted external positioning records with 0 explicit external-baseline exceptions, but that is positioning only, not exhaustive literature synthesis or support-state promotion; Circle has one bounded local prototype-backed receipt slice plus a public ASI consumer-gate fixture checked by digest and negative controls, and Project Theseus now has one public-safe static architecture-gate report import checked by digest, but neither promotes a chapter core claim; independent external human review is requested in [GitHub issue #1](https://github.com/corbensorenson/asi-stack-book/issues/1), tracked in [docs/external_review_status.md](docs/external_review_status.md), and not yet accepted as review evidence.
...
- All 51 manifest chapters exist as manuscript drafts across four manifest-driven parts.
...
- `editions/reader_manuscript/v1_0/manifest.json` defines the curated reader-manuscript path for the point where generated reader output plus overlays are no longer enough. `editions/reader_manuscript/v1_0/curation_contract.json` and `docs/curated_reader_source_contract.md` define the curated-chapter record contract. The current manifest status is `drafting` with forty-nine drafting-only curated chapter records, and `scripts/validate_reader_manuscript_manifest.py` enforces that any curated manuscript remains a parallel derivative source for human prose, not an equal authority for claims, support states, source boundaries, proof/test status, implementation horizons, or release records.
...
- `editions/reader_manuscript/v1_0/audio_script_probe_manifest.json`, `docs/reader_audio_script_probe_manifest.md`, and `scripts/validate_reader_audio_script_probe_manifest.py` preserve the latest local audio-script review-workspace facts: 56 script files, implementation horizons preserved, 5 table notes, 58 Mermaid diagram notes, 1 image note, pronunciation/proof reading files present, and MP3/M4B/audio-embedded EPUB targets still not generated. This is a probe record, not audiobook approval.
...
- `docs/evidence_laundering_prevention_case_studies.md` records three live no-promotion case studies: the Project Theseus static import, Circle public consumer gate, and reader HTML artifact review each had tempting promotion pressure but stayed inside exact evidence and artifact boundaries. `scripts/validate_evidence_laundering_case_studies.py` keeps those examples visible while preserving that no true demotion/refutation case has happened yet.
...
- `claim_decisions/v1_0_core_claim_no_promotion.json`, `docs/core_claim_transition_coverage.md`, and `scripts/validate_core_claim_decisions.py` close the v1.0 chapter-core claim-state coverage gate: 24 chapter core claims have accepted no-change transition records, 27 have accepted explicit no-promotion decisions, and all 51 remain at `argument`.
...
| [docs/evidence_laundering_prevention_case_studies.md](docs/evidence_laundering_prevention_case_studies.md) | No-promotion case studies for evidence-laundering prevention; records the remaining demotion/refutation gap. |
...
| [docs/chapter_external_grounding_status.md](docs/chapter_external_grounding_status.md) | Generated 51-chapter grounding ledger tying each chapter to source-noted external comparators, explicit exceptions, and Corben/local sources to mine first. |
| [docs/per_chapter_evidence_plan.md](docs/per_chapter_evidence_plan.md) | 51-chapter evidence-lane backlog used as a menu for selecting 5-8 high-payoff v1.x lanes, not as a breadth-sweep checklist. |
| [docs/v1_x_active_evidence_cycle.md](docs/v1_x_active_evidence_cycle.md) | Active v1.x evidence-cycle ledger: seven selected lanes, forty-four planned-only lanes, and no chapter-core promotion. |
| [docs/chapter_consolidation_sequence.md](docs/chapter_consolidation_sequence.md) | Full governed consolidation sequence for the 54-to-44/47 critique, with the Part I pilot and conservative compression merge executed and remaining packages still decision-gated. |
...
| [docs/chapter_consolidation_decision_review.md](docs/chapter_consolidation_decision_review.md) | Current consolidation decision: Part I destination drafts are review-ready, but manifest merge waits on review and execution of the URL/history policy. |
| [docs/chapter_consolidation_url_history_policy.md](docs/chapter_consolidation_url_history_policy.md) | URL/history policy for future chapter merges and folds; no manifest merge, redirect, or stub has been executed. |
...
| [docs/chapter_consolidation_full_review_packet.md](docs/chapter_consolidation_full_review_packet.md) | Full decision-queue review packet for all review-ready consolidation packages and fold dispositions; request surface only, not a manifest edit. |
| [docs/chapter_consolidation_release_stability_review.md](docs/chapter_consolidation_release_stability_review.md) | Release-stability decision deferring unexecuted consolidation packages for the current reader-curation cycle; no merge/fold execution, support-state movement, or external-review claim. |
...
| [docs/chapter_consolidation_dry_run_intent_contracts.md](docs/chapter_consolidation_dry_run_intent_contracts.md) | Dry-run merge package for intent-to-execution plus command contracts; review artifact only, not a manifest edit. |
| [docs/chapter_consolidation_destination_draft_intent_contracts.md](docs/chapter_consolidation_destination_draft_intent_contracts.md) | Review-ready one-skeleton destination draft for Command Contracts: From Intent to Executable Work; not reviewed, not canonical, and not a manifest edit. |
| [docs/chapter_consolidation_dry_run_context_abi.md](docs/chapter_consolidation_dry_run_context_abi.md) | Dry-run merge package for Virtual Context ABI plus semantic pages/certificates; review artifact only, not a manifest edit. |
| [docs/chapter_consolidation_destination_draft_context_abi.md](docs/chapter_consolidation_destination_draft_context_abi.md) | Review-ready one-skeleton destination draft for The Virtual Context ABI: Typed Pages, Cells, and Certificates; not reviewed, not canonical, and not a manifest edit. |
| [docs/chapter_consolidation_dry_run_verification_review.md](docs/chapter_consolidation_dry_run_verification_review.md) | Dry-run merge package for Spinoza proof-carrying claims plus tribunal/adversarial review; review artifact only, not a manifest edit. |
| [docs/chapter_consolidation_destination_draft_verification_review.md](docs/chapter_consolidation_destination_draft_verification_review.md) | Review-ready one-skeleton destination draft for Proof-Carrying Claims and Adversarial Review; not reviewed, not canonical, and not a manifest edit. |
| [docs/chapter_consolidation_dry_run_planning_dag.md](docs/chapter_consolidation_dry_run_planning_dag.md) | Dry-run merge package for planning control plus PlanForge DAG/intelligence arbitrage; review artifact only, not a manifest edit. |
| [docs/chapter_consolidation_destination_draft_planning_dag.md](docs/chapter_consolidation_destination_draft_planning_dag.md) | Review-ready one-skeleton destination draft for Planning as a Control Layer: DAGs and Intelligence Arbitrage; not reviewed, not canonical, and not a manifest edit. |
| [docs/chapter_consolidation_fold_moecot_runtime.md](docs/chapter_consolidation_fold_moecot_runtime.md) | Fold disposition for MoECOT runtime into Routing Heads and Specialist Cores; not reviewed, not executed, and not a manifest edit. |
| [docs/chapter_consolidation_fold_simulation_fidelity.md](docs/chapter_consolidation_fold_simulation_fidelity.md) | Fold disposition for Simulation Fidelity and Physical Constraints into Resource Economics and Token Budgets as Simulation Fidelity and Claim Transport; not reviewed, not executed, and not a manifest edit. |
...
| [docs/reader_chapter_review_matrix.md](docs/reader_chapter_review_matrix.md) | Manifest-synced 51-chapter human-reader review queue and release blockers. |
...
| [docs/curated_reader_moecot_runtime_prose_pass.md](docs/curated_reader_moecot_runtime_prose_pass.md) | Drafting-only curated reader prose pass for MoECOT Runtime and Multi-Core Orchestration; no runtime replay, benchmark reproduction, fold decision, or reader release claim. |
