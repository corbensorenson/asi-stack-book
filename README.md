...
**What is currently evidenced:** all 47 chapter core claims remain at `argument`; three narrow non-core transitions are accepted and listed in [docs/non_core_evidence_ledger.md](docs/non_core_evidence_ledger.md); each v1.x safety-critical Lean module now has at least one derived/decomposed finite-record theorem, but the modules remain shallow formal envelopes rather than deployed safety proofs; all 47 chapters have source-noted external positioning records with 0 explicit external-baseline exceptions, but that is positioning only, not exhaustive literature synthesis or support-state promotion; Circle has one bounded local prototype-backed receipt slice plus a public ASI consumer-gate fixture checked by digest and negative controls, and Project Theseus now has one public-safe static architecture-gate report import checked by digest, but neither promotes a chapter core claim; independent external human review is requested in [GitHub issue #1](https://github.com/corbensorenson/asi-stack-book/issues/1), tracked in [docs/external_review_status.md](docs/external_review_status.md), and not yet accepted as review evidence.
...
- All 47 manifest chapters exist as manuscript drafts across four manifest-driven parts.
...
- Appendix C now carries a generated "What would promote this" field for all 47 chapter core claims, derived from `docs/per_chapter_evidence_plan.md` and validated by `scripts/validate_core_claim_promotion_paths.py`, without changing any support state.
- `docs/chapter_consolidation_sequence.md` records the accepted governed consolidation sequence for the 54-to-44/47 chapter-shape critique, the executed Part I 4-to-2 pilot, the executed conservative compression merge, the executed intent/contracts merge, the executed MoECOT runtime fold, the executed simulation-fidelity fold, the executed static context ABI merge, the current 47-chapter manifest, and the no-support-state-change boundary; `docs/chapter_consolidation_release_stability_review.md` records the current `deferred_for_release` reader-work outcome for remaining unexecuted consolidation packages without authorizing additional merges or folds; `scripts/validate_chapter_consolidation_sequence.py` keeps that sequence visible from the roadmap and public surfaces.
...
- `editions/reader_manuscript/v1_0/audio_script_probe_manifest.json`, `docs/reader_audio_script_probe_manifest.md`, and `scripts/validate_reader_audio_script_probe_manifest.py` preserve the latest local audio-script review-workspace facts: 52 script files, implementation horizons preserved, 7 table notes, 55 Mermaid diagram notes, 1 image note, pronunciation/proof reading files present, and MP3/M4B/audio-embedded EPUB targets still not generated. This is a probe record, not audiobook approval.
...
- `docs/chapter_external_grounding_status.md` and `scripts/validate_chapter_external_grounding_status.py` turn that placement audit into a 47-chapter grounding ledger: 47 chapters have source-noted external comparators, 0 carry explicit exceptions, and each row names the Corben/local sources to mine first before broader citation backfill.
...
| [docs/chapter_external_grounding_status.md](docs/chapter_external_grounding_status.md) | Generated 47-chapter grounding ledger tying each chapter to source-noted external comparators, explicit exceptions, and Corben/local sources to mine first. |
| [docs/per_chapter_evidence_plan.md](docs/per_chapter_evidence_plan.md) | 47-chapter evidence-lane backlog used as a menu for selecting 5-8 high-payoff v1.x lanes, not as a breadth-sweep checklist. |
| [docs/v1_x_active_evidence_cycle.md](docs/v1_x_active_evidence_cycle.md) | Active v1.x evidence-cycle ledger: seven selected lanes, forty planned-only lanes, and no chapter-core promotion. |
| [docs/chapter_consolidation_sequence.md](docs/chapter_consolidation_sequence.md) | Full governed consolidation sequence for the 54-to-44/47 critique, with the Part I pilot, conservative compression merge, intent/contracts merge, MoECOT runtime fold, simulation-fidelity fold, and static context ABI merge executed and remaining packages still decision-gated. |
...
| [docs/chapter_consolidation_full_review_packet.md](docs/chapter_consolidation_full_review_packet.md) | Full decision-queue review packet for remaining review-ready packages and fold dispositions; request surface only, not a manifest edit. |
| [docs/chapter_consolidation_release_stability_review.md](docs/chapter_consolidation_release_stability_review.md) | Release-stability decision deferring remaining unexecuted consolidation packages for the current reader-curation cycle; no additional merge/fold execution, support-state movement, or external-review claim. |
...
| [docs/reader_chapter_review_matrix.md](docs/reader_chapter_review_matrix.md) | Manifest-synced 47-chapter human-reader review queue and release blockers. |
