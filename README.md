...
| Architecture reference | Interfaces, invariants, failure routes, protocols, tests, proofs, and implementation horizons across all 59 working-manifest chapters. | [Open the architecture index](https://corbensorenson.github.io/asi-stack-book/products/architecture-reference/) |
...
- All 59 manifest chapters exist as manuscript drafts across four manifest-driven parts.
...
- Appendix C now carries a generated "What would promote this" field for all 59 chapter core claims, derived from `docs/per_chapter_evidence_plan.md` and validated by `scripts/validate_core_claim_promotion_paths.py`, without changing any support state. `docs/core_claim_disposition_ledger.md` consolidates those per-chapter movement paths with accepted no-change transition records and accepted no-promotion decisions.
...
| [docs/core_claim_disposition_ledger.md](docs/core_claim_disposition_ledger.md) | Per-chapter core-claim disposition ledger: 59 current dispositions, 0 promoted core claims, and explicit movement paths without support-state inflation. |
...
| [docs/chapter_external_grounding_status.md](docs/chapter_external_grounding_status.md) | Generated 59-chapter grounding ledger tying each chapter to source-noted external comparators, explicit exceptions, and Corben/local sources to mine first. |
| [docs/per_chapter_evidence_plan.md](docs/per_chapter_evidence_plan.md) | 59-chapter evidence-lane backlog used as a menu for selecting one flagship measured lane plus direct supports, not as a breadth-sweep checklist. |
