...
| Architecture reference | Interfaces, invariants, failure routes, protocols, tests, proofs, and implementation horizons across all 54 chapters. | [Open the architecture index](https://corbensorenson.github.io/asi-stack-book/products/architecture-reference/) |
...
The three projections share one source of truth but have different density and review contracts; see [docs/product_contracts.md](docs/product_contracts.md) and [docs/product_projection_artifacts.md](docs/product_projection_artifacts.md). The v2, v2.1, and post-v2.1 programs are complete and preserved in their roadmaps. The latest cycle is summarized by the [v2.2.0 completion declaration](docs/v2_2_completion_declaration.md) and its completed [Post-v2.1 Residual and Transfer Roadmap](docs/post_v2_1_residual_and_transfer_roadmap.md).
...
Fast audit path: [the v2.2.0 completion declaration](docs/v2_2_completion_declaration.md), [the completed Post-v2.1 Residual and Transfer Roadmap](docs/post_v2_1_residual_and_transfer_roadmap.md), its machine [status record](roadmap_records/post_v2_1_residual_and_transfer_status.json), [the v2.1.0 completion declaration](docs/v2_1_completion_declaration.md), [the completed Post-v2 Evidence Roadmap](docs/post_v2_evidence_roadmap.md), [the v2.0.0 completion declaration](docs/v2_0_completion_declaration.md), [the completed v2 roadmap](docs/asi_stack_completion_roadmap.md), [docs/core_claim_disposition_ledger.md](docs/core_claim_disposition_ledger.md), [docs/evidence_quality_vectors.md](docs/evidence_quality_vectors.md), [docs/non_core_evidence_ledger.md](docs/non_core_evidence_ledger.md), Appendix C, Appendix G, and Appendix H. The landing-page and README trust contract is guarded by `scripts/validate_trust_surface.py`.

## Current Status

`v2.2.0` is the latest completed immutable canonical live/research HTML release, bound to tag `v2.2.0`, source commit `e3d5348993cc5083604c85bd699bb0e36eb00de1`, and archive SHA-256 `037563bc62792ecd968cf923b94e3082b02597f1b97f81b63fa59c6d083ee2db`. It adds the completed post-v2.1 P1–P3 programs, fourteen conservative core-claim `no_change` decisions, six bounded transitions, eleven residual dispositions, six modern external-source routes, and the QCSA source synthesis across nine existing chapters; all 54 chapter-core claims remain at `argument`. Historical `v1.0.0`, `v2.0.0`, and `v2.1.0` releases remain intact. The root site and `/latest/` are mutable. EPUB/DOCX/PDF/audio and the divergent historical curated-reader workspace are outside the v2.2.0 format scope.
...
- Appendix C now carries a generated "What would promote this" field for all 54 chapter core claims, derived from `docs/per_chapter_evidence_plan.md` and validated by `scripts/validate_core_claim_promotion_paths.py`, without changing any support state. `docs/core_claim_disposition_ledger.md` consolidates those per-chapter movement paths with accepted no-change transition records and accepted no-promotion decisions.
- `docs/chapter_consolidation_sequence.md` records the accepted governed consolidation sequence for the 54-to-44/47 chapter-shape critique, the executed Part I 4-to-2 pilot, the executed conservative compression merge, the executed intent/contracts merge, the executed MoECOT runtime fold, the executed simulation-fidelity fold, the executed static context ABI merge, the executed verification/adversarial-review merge, the executed planning/DAG consolidation, the executed semantic-representation fold, the historical 44-chapter consolidation snapshot, and the no-support-state-change boundary; the active manifest has 54 chapters. `docs/chapter_consolidation_release_stability_review.md` records the current `deferred_for_release` reader-work outcome for remaining unexecuted consolidation packages without authorizing additional merges or folds; `scripts/validate_chapter_consolidation_sequence.py` keeps that sequence visible from the roadmap and public surfaces. <!-- canonical-status:historical -->
...
- `docs/chapter_external_grounding_status.md` and `scripts/validate_chapter_external_grounding_status.py` turn that placement audit into a 54-chapter grounding ledger: all 54 chapters have source-noted external positioning records, 0 carry explicit exceptions, and each row names the Corben/local sources to mine first before broader citation backfill.
...
| [docs/non_core_evidence_ledger.md](docs/non_core_evidence_ledger.md) | Public ledger for the fourteen accepted non-core upward evidence transitions, 40 accepted `blocks_promotion` no-promotion side-lane decisions, one historical count-surface narrowing record, and their no-chapter-core-promotion boundary. |
...
| [docs/per_chapter_evidence_plan.md](docs/per_chapter_evidence_plan.md) | 54-chapter evidence-lane backlog used as a menu for selecting one flagship measured lane plus direct supports, not as a breadth-sweep checklist. |
