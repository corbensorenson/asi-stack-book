...
- `scripts/validate_source_appendices.py` checks that Appendix G and Appendix H are independent top-level appendices with separate appendix-identity rows: G contains only Corben's own papers, Corben-supplied materials, recovered project records, and local project records, while H contains only external-source and third-party literature records by other authors or organizations generated from `sources/source_inventory.json`.
