...
- All 44 manifest chapters exist as manuscript drafts across four manifest-driven parts.
...
- Appendix C now carries a generated "What would promote this" field for all 44 chapter core claims, derived from `docs/per_chapter_evidence_plan.md` and validated by `scripts/validate_core_claim_promotion_paths.py`, without changing any support state.
- `docs/chapter_consolidation_sequence.md` records the accepted governed consolidation sequence for the 54-to-44/47 chapter-shape critique, the executed Part I 4-to-2 pilot, the executed conservative compression merge, the executed intent/contracts merge, the executed MoECOT runtime fold, the executed simulation-fidelity fold, the executed static context ABI merge, the executed verification/adversarial-review merge, the executed planning/DAG consolidation, the executed semantic-representation fold, the current 44-chapter manifest, and the no-support-state-change boundary; `docs/chapter_consolidation_release_stability_review.md` records the current `deferred_for_release` reader-work outcome for remaining unexecuted consolidation packages without authorizing additional merges or folds; `scripts/validate_chapter_consolidation_sequence.py` keeps that sequence visible from the roadmap and public surfaces.
...
- `editions/reader_overlays/README.md` and `editions/reader_overlays/v1_0/manifest.json` define the semantic reader-overlay layer for major-version human-edition deltas that should survive regeneration without forking the live book. The current v1.0 overlay set carries 31 active operations across opening-chapter prose, Efficient ASI, Human Intent, System Boundaries, Evidence States, Personal Compute Hives, Command Contracts, Planning, Verification Bandwidth, Runtime Adapters, Labor OS, Circle Contracts, Generate-Verify-Repair, Fast Generation, RankFold/NeuralFold, Mathematical and Search Substrates, Policy Optimization, Artifact Steward Agents, Executable Specifications, and Semantic Representation overlay operations have been retired after the semantic fold; the remaining overlays apply to Human view and generated reader editions only.
- `editions/reader_manuscript/v1_0/manifest.json` defines the curated reader-manuscript path for the point where generated reader output plus overlays are no longer enough. `editions/reader_manuscript/v1_0/curation_contract.json` and `docs/curated_reader_source_contract.md` define the curated-chapter record contract. The current manifest status is `drafting` with forty-four drafting-only curated chapter records, no active manifest chapter missing a curated reader file, and a machine-checked reader handoff contract covering the book thesis, part arcs, signature ideas, ten draft key-figure assets with text-equivalent chapter anchors, chapter stakes/payoffs, and Corben voice-pass slots. `scripts/build_curated_reader_edition.py --check` verifies that those tracked curated chapters can be assembled into a renderable local Quarto review workspace while keeping release blockers active, and `scripts/validate_reader_key_figures.py` checks that all ten draft key figures have accessible SVG metadata, live chapter text equivalents, curated reader placements, alt text, captions, and non-claim boundaries. There is still no release approval; `scripts/validate_reader_manuscript_manifest.py` enforces that any curated manuscript remains a parallel derivative source for human prose, not an equal authority for claims, support states, source boundaries, proof/test status, implementation horizons, or release records.
...
- `editions/reader_manuscript/v1_0/audio_script_probe_manifest.json`, `docs/reader_audio_script_probe_manifest.md`, and `scripts/validate_reader_audio_script_probe_manifest.py` preserve the latest local audio-script review-workspace facts: 49 script files, implementation horizons preserved, 11 table notes, 53 Mermaid diagram notes, 11 image notes, pronunciation/proof reading files present, and MP3/M4B/audio-embedded EPUB targets still not generated. This is a probe record, not audiobook approval.
...
- `scripts/validate_claim_ledger_revision.py`, `scripts/validate_proof_carrying_claims.py`, `scripts/validate_tribunal_review.py`, `scripts/validate_value_conflicts.py`, `scripts/validate_constitutional_alignment.py`, `scripts/validate_governance_rights.py`, `scripts/validate_agency_rights.py`, `scripts/validate_support_state_transitions.py`, `scripts/validate_authority_transitions.py`, `scripts/validate_security_kernel.py`, `scripts/validate_security_scif_commit_probe.py`, `scripts/validate_stable_capability_fields.py`, `scripts/validate_capability_replacement.py`, `scripts/validate_self_improvement_boundaries.py`, `scripts/validate_stack_layer_traceability.py`, `scripts/validate_plan_execution_contracts.py`, `scripts/validate_intent_recontract_probe.py`, `scripts/validate_cognitive_compilation_traces.py`, `scripts/validate_runtime_adapter_permissions.py`, `scripts/validate_runtime_adapter_effect_probe.py`, `scripts/validate_artifact_steward_lifecycle_probe.py`, `scripts/validate_artifact_graph_replay.py`, `scripts/validate_procedural_memory_loop.py`, `scripts/validate_routing_decision_lease.py`, `scripts/validate_cyclic_memory_contracts.py`, `scripts/validate_circle_cyclic_memory_receipt_slice.py`, `scripts/validate_context_transaction_memory_store.py`, `scripts/validate_simulation_transfer_boundaries.py`, `scripts/validate_context_admission_adequacy.py`, `scripts/validate_vcm_resolver_certificate_probe.py`, `scripts/validate_readiness_residual_gates.py`, `scripts/validate_benchmark_antigoodhart.py`, `scripts/validate_generation_mode_baselines.py`, `scripts/validate_fast_generation_task_bundle.py`, `scripts/validate_resource_budget_ledgers.py`, `scripts/validate_reference_trace.py`, `scripts/validate_capacity_smoothing.py`, `scripts/validate_resource_workflow_trace.py`, `scripts/validate_resource_live_probe.py`, `scripts/validate_theseus_support_replay_probe.py`, `scripts/validate_theseus_report_bundle_audit.py`, and `scripts/validate_compact_gvr_slice.py` run synthetic, deterministic, local replay, or local external-project receipt harnesses for claim-ledger revision discipline, proof-carrying claim discipline, tribunal-review record discipline, value-conflict record discipline, constitutional-predicate record discipline, governance-right record discipline, agency-right checklist discipline, support-state promotion/demotion/refutation conservatism, authority non-escalation, security-kernel receipt discipline, SCIF sanitized commit replay discipline, stable-capability-field qualification/routing discipline, replacement-transaction discipline, self-improvement transition discipline, stack layer traceability, plan/contract/job consistency, intent re-contract trigger discipline, cognitive compilation trace discipline, runtime-adapter permission/approval/receipt discipline, runtime-adapter temp-file effect replay, artifact-steward fixture-composed lifecycle route discipline, artifact-graph replay/audit/promotion-blocking discipline, procedural-memory loop-qualification discipline, routing decision lease discipline, cyclic-memory contract discipline, Circle cyclic-memory receipt traceability, bounded context-transaction memory-store discipline, VCM resolver/certificate fixture discipline, simulation-transfer boundary discipline, context admission/adequacy consistency, readiness/residual custody, benchmark anti-Goodhart discipline, generation-mode baseline accounting, Fast Generation task-bundle accounting, resource-budget ledger discipline, reference-trace continuity/stop-condition discipline, capacity-smoothing toy traces, Resource Economics workflow-trace accounting, Resource Economics local command replay, Cognitive Compilation trace accounting, Project Theseus support replay accounting, Project Theseus report-bundle audit discipline, and compact GVR synthetic receipt discipline; `experiments/phase5_harness_registry.json` plus `scripts/validate_phase5_harness_registry.py` keep the registry-controlled harness set wired to docs, fixtures, result records, Appendix E, and book validation; the Stack Layer Traceability Audit, Artifact Graph replay, Procedural Memory loop, Routing decision lease, Cyclic Memory contract, Circle Cyclic Memory Receipt Slice, Context Transaction Memory-Store, VCM Resolver/Certificate Probe, Simulation Transfer Boundary, Resource Workflow Trace, Resource Live Probe, Runtime Adapter Effect Replay Probe, Artifact Steward Lifecycle Probe, Intent Re-contract Trigger Probe, SCIF Sanitized Commit Replay Probe, Fast Generation Task-Bundle Validation, Cognitive Compilation Trace, Theseus Support Replay Probe, Theseus Report-Bundle Audit, and Compact GVR Synthetic Slice harnesses are book-gate-only and are not part of the Phase 5 registry runner. These are executable or traceability checks over fixtures, records, and local command output, not chapter-claim promotions or deployed-runtime evidence.
...
- `docs/evidence_laundering_prevention_case_studies.md` records three live no-promotion case studies plus one live claim-surface narrowing record: the Project Theseus static import, Circle public consumer gate, and reader HTML artifact review each had tempting promotion pressure but stayed inside exact evidence and artifact boundaries, while `claim_revisions/v1_x/manifest_core_claim_count_narrowing.json` narrows a stale public count surface from 54 to the current 44 manifest chapter core claims. `scripts/validate_evidence_laundering_case_studies.py` keeps those examples visible while preserving that no chapter core claim has been demoted or refuted.
- `scripts/validate_circle_public_replay.py` records the public ASI-side Circle consumer gate: `docs/circle_public_replay_consumer_gate.md`, `experiments/circle_public_replay/fixtures/valid/circle_rope_receipt.consumer.valid.json`, and `experiments/circle_public_replay/results/2026-06-29-local.json` validate the pinned `CC-AI-CONTRACT-ROPE-001` receipt boundary and reject digest-mismatch, missing-theorem, stale-contract, and unsupported-transfer mutation controls. This makes the Circle lane CI-verifiable inside this repository, but it does not create a fourth accepted transition, rerun Circle Lean here, vendor Circle, prove deployed proof-contract transport, or promote any chapter core claim.
- `claim_decisions/v1_0_core_claim_no_promotion.json`, `docs/core_claim_transition_coverage.md`, and `scripts/validate_core_claim_decisions.py` close the v1.0 chapter-core claim-state coverage gate: every active chapter core claim has either an accepted no-change transition record or an accepted explicit no-promotion decision, and all 44 remain at `argument`.
...
| [docs/non_core_evidence_ledger.md](docs/non_core_evidence_ledger.md) | Public ledger for the four accepted non-core evidence transitions, one count-surface narrowing record, and their no-chapter-core-promotion boundary. |
...
| [docs/chapter_external_grounding_status.md](docs/chapter_external_grounding_status.md) | Generated 44-chapter grounding ledger tying each chapter to source-noted external comparators, explicit exceptions, and Corben/local sources to mine first. |
| [docs/per_chapter_evidence_plan.md](docs/per_chapter_evidence_plan.md) | 44-chapter evidence-lane backlog used as a menu for selecting one flagship measured lane plus direct supports, not as a breadth-sweep checklist. |
...
- Curated reader builds use `scripts/build_curated_reader_edition.py` to assemble tracked curated chapter drafts into `build/curated_reader_edition/` for source-level review and local rendering. This is a review workspace, not a release record, and it keeps all curated reconciliation and format-artifact blockers intact.
