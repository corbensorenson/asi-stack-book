...
**What is currently evidenced:** all 48 chapter core claims remain at `argument`; three narrow non-core transitions are accepted and listed in [docs/non_core_evidence_ledger.md](docs/non_core_evidence_ledger.md); each v1.x safety-critical Lean module now has at least one derived/decomposed finite-record theorem, but the modules remain shallow formal envelopes rather than deployed safety proofs; all 48 chapters have source-noted external positioning records with 0 explicit external-baseline exceptions, but that is positioning only, not exhaustive literature synthesis or support-state promotion; Circle has one bounded local prototype-backed receipt slice plus a public ASI consumer-gate fixture checked by digest and negative controls, and Project Theseus now has one public-safe static architecture-gate report import checked by digest, but neither promotes a chapter core claim; independent external human review is requested in [GitHub issue #1](https://github.com/corbensorenson/asi-stack-book/issues/1), tracked in [docs/external_review_status.md](docs/external_review_status.md), and not yet accepted as review evidence.
...
- All 48 manifest chapters exist as manuscript drafts across four manifest-driven parts.
...
- Appendix C now carries a generated "What would promote this" field for all 48 chapter core claims, derived from `docs/per_chapter_evidence_plan.md` and validated by `scripts/validate_core_claim_promotion_paths.py`, without changing any support state.
- `docs/chapter_consolidation_sequence.md` records the accepted governed consolidation sequence for the 54-to-44/47 chapter-shape critique, the executed Part I 4-to-2 pilot, the executed conservative compression merge, the executed intent/contracts merge, the executed MoECOT runtime fold, the executed simulation-fidelity fold, the current 48-chapter manifest, and the no-support-state-change boundary; `docs/chapter_consolidation_release_stability_review.md` records the current `deferred_for_release` reader-work outcome for remaining unexecuted consolidation packages without authorizing additional merges or folds; `scripts/validate_chapter_consolidation_sequence.py` keeps that sequence visible from the roadmap and public surfaces.
...
- `docs/chapter_external_grounding_status.md` and `scripts/validate_chapter_external_grounding_status.py` turn that placement audit into a 48-chapter grounding ledger: 48 chapters have source-noted external comparators, 0 carry explicit exceptions, and each row names the Corben/local sources to mine first before broader citation backfill.
...
| [docs/chapter_external_grounding_status.md](docs/chapter_external_grounding_status.md) | Generated 48-chapter grounding ledger tying each chapter to source-noted external comparators, explicit exceptions, and Corben/local sources to mine first. |
| [docs/per_chapter_evidence_plan.md](docs/per_chapter_evidence_plan.md) | 48-chapter evidence-lane backlog used as a menu for selecting 5-8 high-payoff v1.x lanes, not as a breadth-sweep checklist. |
| [docs/v1_x_active_evidence_cycle.md](docs/v1_x_active_evidence_cycle.md) | Active v1.x evidence-cycle ledger: seven selected lanes, forty-one planned-only lanes, and no chapter-core promotion. |
| [docs/chapter_consolidation_sequence.md](docs/chapter_consolidation_sequence.md) | Full governed consolidation sequence for the 54-to-44/47 critique, with the Part I pilot, conservative compression merge, intent/contracts merge, MoECOT runtime fold, and simulation-fidelity fold executed and remaining packages still decision-gated. |
...
| [docs/reader_chapter_review_matrix.md](docs/reader_chapter_review_matrix.md) | Manifest-synced 48-chapter human-reader review queue and release blockers. |
