...
- All 45 manifest chapters exist as manuscript drafts across four manifest-driven parts.
...
- Appendix C now carries a generated "What would promote this" field for all 49 chapter core claims, derived from `docs/per_chapter_evidence_plan.md` and validated by `scripts/validate_core_claim_promotion_paths.py`, without changing any support state. `docs/core_claim_disposition_ledger.md` consolidates those per-chapter movement paths with accepted no-change transition records and accepted no-promotion decisions.
...
- `docs/chapter_external_grounding_status.md` and `scripts/validate_chapter_external_grounding_status.py` turn that placement audit into a 49-chapter grounding ledger: all 49 chapters have source-noted external positioning records, 0 carry explicit exceptions, and each row names the Corben/local sources to mine first before broader citation backfill.
...
| [docs/core_claim_disposition_ledger.md](docs/core_claim_disposition_ledger.md) | Per-chapter core-claim disposition ledger: 45 current dispositions, 0 promoted core claims, and explicit movement paths without support-state inflation. |
...
| [docs/per_chapter_evidence_plan.md](docs/per_chapter_evidence_plan.md) | 45-chapter evidence-lane backlog used as a menu for selecting one flagship measured lane plus direct supports, not as a breadth-sweep checklist. |
