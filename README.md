...
- `scripts/validate_source_appendices.py` checks that Appendix G and Appendix H are independent top-level appendices with explicit ownership-rule rows: G contains only Corben-authored, Corben-supplied, recovered, and local project records, while H contains only external-source records by other authors generated from `sources/source_inventory.json`.
...
- The live Human view uses the same reader-strip policy on the GitHub Pages site. Each chapter's `.asi-human-only` Human Reading Path bridge is hidden in default AI view, shown in Human view, and unwrapped into reader editions; `.asi-ai-only` blocks are removed from reader editions.
