...
**What is currently evidenced:** all 50 chapter core claims remain at `argument`; three narrow non-core transitions are accepted and listed in [docs/non_core_evidence_ledger.md](docs/non_core_evidence_ledger.md); each v1.x safety-critical Lean module now has at least one derived/decomposed finite-record theorem, but the modules remain shallow formal envelopes rather than deployed safety proofs; all 50 chapters have source-noted external positioning records with 0 explicit external-baseline exceptions, but that is positioning only, not exhaustive literature synthesis or support-state promotion; Circle has one bounded local prototype-backed receipt slice plus a public ASI consumer-gate fixture checked by digest and negative controls, and Project Theseus now has one public-safe static architecture-gate report import checked by digest, but neither promotes a chapter core claim; independent external human review is requested in [GitHub issue #1](https://github.com/corbensorenson/asi-stack-book/issues/1), tracked in [docs/external_review_status.md](docs/external_review_status.md), and not yet accepted as review evidence.
...
- All 50 manifest chapters exist as manuscript drafts across four manifest-driven parts.
...
- Appendix C now carries a generated "What would promote this" field for all 50 chapter core claims, derived from `docs/per_chapter_evidence_plan.md` and validated by `scripts/validate_core_claim_promotion_paths.py`, without changing any support state.
- `docs/chapter_consolidation_sequence.md` records the accepted governed consolidation sequence for the 54-to-44/47 chapter-shape critique, the executed Part I 4-to-2 pilot, the executed conservative compression merge, the executed MoECOT runtime fold, the current 50-chapter manifest, and the no-support-state-change boundary; `docs/chapter_consolidation_release_stability_review.md` records the current `deferred_for_release` reader-work outcome for remaining unexecuted consolidation packages without authorizing additional merges or folds; `scripts/validate_chapter_consolidation_sequence.py` keeps that sequence visible from the roadmap and public surfaces.
...
- `editions/reader_manuscript/v1_0/audio_script_probe_manifest.json`, `docs/reader_audio_script_probe_manifest.md`, and `scripts/validate_reader_audio_script_probe_manifest.py` preserve the latest local audio-script review-workspace facts: 55 script files, implementation horizons preserved, 5 table notes, 57 Mermaid diagram notes, 1 image note, pronunciation/proof reading files present, and MP3/M4B/audio-embedded EPUB targets still not generated. This is a probe record, not audiobook approval.
...
- `claim_decisions/v1_0_core_claim_no_promotion.json`, `docs/core_claim_transition_coverage.md`, and `scripts/validate_core_claim_decisions.py` close the v1.0 chapter-core claim-state coverage gate: 24 chapter core claims have accepted no-change transition records, 26 have accepted explicit no-promotion decisions, and all 50 remain at `argument`.
...
| [docs/chapter_external_grounding_status.md](docs/chapter_external_grounding_status.md) | Generated 50-chapter grounding ledger tying each chapter to source-noted external comparators, explicit exceptions, and Corben/local sources to mine first. |
| [docs/per_chapter_evidence_plan.md](docs/per_chapter_evidence_plan.md) | 50-chapter evidence-lane backlog used as a menu for selecting 5-8 high-payoff v1.x lanes, not as a breadth-sweep checklist. |
| [docs/v1_x_active_evidence_cycle.md](docs/v1_x_active_evidence_cycle.md) | Active v1.x evidence-cycle ledger: seven selected lanes, forty-three planned-only lanes, and no chapter-core promotion. |
| [docs/chapter_consolidation_sequence.md](docs/chapter_consolidation_sequence.md) | Full governed consolidation sequence for the 54-to-44/47 critique, with the Part I pilot, conservative compression merge, and MoECOT runtime fold executed and remaining packages still decision-gated. |
...
| [docs/reader_chapter_review_matrix.md](docs/reader_chapter_review_matrix.md) | Manifest-synced 50-chapter human-reader review queue and release blockers. |
