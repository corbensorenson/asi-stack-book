...
- `scripts/validate_source_appendices.py` checks that Appendix G and Appendix H are independent top-level appendices: G contains only Corben's source corpus and local-project records, while H contains only external-source records by other authors generated from `sources/source_inventory.json`.
...
| [appendices/G_corben_source_corpus.qmd](appendices/G_corben_source_corpus.qmd) | Generated appendix for Corben's ASI Stack source corpus and local project records. |
