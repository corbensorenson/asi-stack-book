...
- `scripts/validate_source_appendices.py` checks that Appendix G and Appendix H are independent top-level appendices with separate appendix-identity rows: G contains only Corben's papers, Corben-supplied materials, recovered project records, and local project records, while H contains only external-source and third-party literature records generated from `sources/source_inventory.json`.
...
| [appendices/G_corben_source_corpus.qmd](appendices/G_corben_source_corpus.qmd) | Generated appendix for Corben's sources, papers, and local project records. |
| [appendices/H_external_sources.qmd](appendices/H_external_sources.qmd) | Generated appendix for external sources and third-party literature. |
