...
- `editions/reader_overlays/README.md` and `editions/reader_overlays/v1_0/manifest.json` define the semantic reader-overlay layer for major-version human-edition deltas that should survive regeneration without forking the live book. The current v1.0 overlay set carries six active operations: two opening-chapter section replacements plus four Personal Compute Hives table-to-prose replacements for Human view and generated reader editions only.
