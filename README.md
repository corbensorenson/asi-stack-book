...
- `scripts/validate_reader_evidence_boundaries.py` checks that every generated reader chapter strips raw live core-claim markers while preserving the claim text and a plain-language support-state boundary in the Core Claim section.
...
| [appendices/G_corben_source_corpus.qmd](appendices/G_corben_source_corpus.qmd) | Generated appendix for Corben-authored, Corben-supplied, and local project source records. |
| [appendices/H_external_sources.qmd](appendices/H_external_sources.qmd) | Generated appendix for external literature, documentation, benchmarks, and third-party references by other authors. |
...
- The live Human view uses the same reader-strip policy on the GitHub Pages site. Each chapter's `.asi-human-only` Human Reading Path bridge is hidden in default AI view, shown in Human view, and unwrapped into reader editions; `.asi-ai-only` blocks are removed from reader editions; raw bracketed core-claim markers are visible in AI view but hidden in Human view and stripped from generated reader chapters while the claim text and support boundary remain.
