...
- `scripts/validate_support_state_transitions.py`, `scripts/validate_authority_transitions.py`, and `scripts/validate_plan_execution_contracts.py` run synthetic gate harnesses for support-state conservatism, authority non-escalation, and plan/contract/job consistency; they are executable behavior checks over fixtures, not claim promotions or deployed-runtime evidence.
