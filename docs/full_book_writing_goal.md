...
Update source notes, Appendix C, Appendix D/E/G/H/K as needed, keeping Appendix G reserved as its own top-level appendix for Corben-authored, Corben-supplied, and local project records and Appendix H reserved as its own top-level external-source appendix for records by other authors. Do not present H as a second part of G. Keep `_quarto.yml`, Appendix A, Appendix C, Appendix E, Appendix G, Appendix H, and Appendix K generated through `scripts/sync_scaffold.py`. Before reporting completion, run the full launch gate:
