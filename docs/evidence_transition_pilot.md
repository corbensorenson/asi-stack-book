...
| `resource-economics-and-token-budgets.core` | no change; remains `argument` | Exact source mappings, resource-budget schemas, external serving/resource context, and the deterministic generation-mode baseline harness support budget-record discipline, but they do not establish a budget scheduler, load stability, verification-tax optimization, KV-cache or serving performance, cost-quality economics, or runtime budget enforcement. |
