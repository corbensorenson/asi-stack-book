...
| `resource-economics-and-token-budgets.core` | no change; remains `argument` | Exact source mappings, resource-budget schemas, external serving/resource context, the deterministic generation-mode baseline harness, and the deterministic resource-budget ledger harness support budget-record discipline, but they do not establish a budget scheduler, review-queue optimizer, load stability, verification-tax optimization, KV-cache or serving performance, cost-quality economics, or runtime budget enforcement. |
