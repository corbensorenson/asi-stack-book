...
|---|---|---|---|---|---|---|
| `evidence-states-and-claim-discipline` | executed-narrow | Claim/evidence ledgers, explicit no-promotion decisions, terminal-state Lean envelope, and synthetic demotion/refutation fixtures | `docs/non_core_evidence_ledger.md`; `docs/core_claim_transition_coverage.md`; `claim_decisions/v1_0_core_claim_no_promotion.json`; `docs/defended_contribution_prior_art_positioning.md`; `lean/AsiStackProofs/EvidenceStates.lean`; `docs/support_state_transition_harness.md` | No-promotion decisions and support-state transition validators reject unsupported promotion, reasonless demotion, and unsupported refutation. | No chapter core promotion, demotion, deprecation, or refutation; core claim remains `argument`. | External review and a live claim demotion/refutation record before stronger methodology claims. |
...
| `resource-economics-and-token-budgets` | executed-narrow | Costed-route/resource-budget synthetic slice plus folded simulation-fidelity contract lane | `docs/costed_route_resource_slice.md`; `experiments/costed_route_resource_slice/results/2026-06-29-local.json`; `evidence_transitions/v1_0_measured/costed_route_resource_slice_synthetic_test_backed.json`; `schemas/simulation_contract_record.schema.json` | Cheaper failed route is rejected; adequate overkill baseline remains eligible; folded simulation contract records still block transfer beyond declared fidelity. | Non-core `resource-economics.costed_route_budget_slice` is `synthetic-test-backed`; chapter core remains `argument`. | Larger public trace with workload quality, displaced-cost accounting, fidelity calibration, and simulation-transfer negative cases. |
