...
| Governed visual edition | `visual_edition/` | Track Manim scene source, visual grammar, storyboards, narration, reviewed captions, descriptive transcripts, thumbnails, manifests, the canonical YouTube channel contract, the generated 84-chapter publication/revision ledger, exact generation-N supersession plans, and immutable render/platform receipts. The ledger derives append-only generation history from those receipts. Keep rendered video, narration audio, partial frames, and Manim caches in ignored `build/`; YouTube owns published binaries. |
...
| Build or update a chapter visual abstract | Update the packet under `visual_edition/chapters/<chapter-id>/`, regenerate `visual_edition/manifest.json` and `visual_edition/youtube_ledger.json`, validate with `scripts/validate_visual_edition.py`, and render only into ignored `build/visual_edition/`. A video becomes `published_current` only after an authorized YouTube receipt and synchronized Quarto embed exist. Regeneration preserves a published predecessor as `stale`; use `scripts/prepare_youtube_supersession.py`, `scripts/record_youtube_supersession_receipt.py`, and `scripts/reconcile_youtube_supersession_receipt.py` for a material revision. Each replacement requires its own exact plan-digest authority, keeps immutable generation receipts, makes the predecessor unlisted and points it to the successor, removes only its canonical playlist item, and never deletes either generation automatically. |
