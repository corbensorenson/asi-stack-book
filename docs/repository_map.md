...
| Governed visual edition | `visual_edition/` | Track Manim scene source, visual grammar, storyboards, narration, reviewed captions, descriptive transcripts, thumbnails, manifests, and exact render/platform receipts. Keep rendered video, narration audio, partial frames, and Manim caches in ignored `build/`; YouTube owns published binaries. |
...
| Build or update a chapter visual abstract | Update the packet under `visual_edition/chapters/<chapter-id>/`, regenerate `visual_edition/manifest.json`, validate with `scripts/validate_visual_edition.py`, and render only into ignored `build/visual_edition/`. A video becomes `published_current` only after an authorized YouTube receipt and synchronized Quarto embed exist. |
