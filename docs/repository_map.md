...
| `docs/evidence_transition_pilot.md` | Phase 3 evidence-transition summary, recording twenty-six no-change support-state decisions plus the separate bounded registry-runner and costed-route/resource-budget transitions. | tracked |
| `docs/first_measured_replayed_slice.md` | Accepted measured/replayed slice ledger: a narrow `synthetic-test-backed` repository-infrastructure transition and a bounded non-infrastructure costed-route/resource-budget transition, with non-claims. | tracked |
...
| `evidence_transitions/v1_0_measured/` | Accepted bounded measured/replayed evidence-transition records for the Phase 5 registry-runner infrastructure claim and the costed-route/resource-budget selector slice. | tracked |
