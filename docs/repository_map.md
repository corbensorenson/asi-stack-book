...
| Inspect or advance the P5 natural stateful-service campaign | Use `experiments/governed_operations_argument_exit/preregistration.json` as the sole prospective campaign authority, `schemas/governed_operations_campaign_preregistration.schema.json` as its record contract, and `docs/p5_natural_stateful_service_campaign_preregistration.md` as the reader decision. Validate with `scripts/validate_governed_operations_control_contract.py`. Preserve dated amendments before held-out opening; after opening, task/arm/metric/denominator changes are forbidden. The campaign cannot borrow P2/Q1/Q2 tasks, substitute for Theseus T4, create a public effect during trials, or move support/release state. |
