...
| `docs/chapter_consolidation_dry_run_planning_dag.md` | Dry-run merge package for `planning-as-a-control-layer` plus `planforge-dags-and-intelligence-arbitrage`; review artifact only, not a manifest edit or support-state change. | tracked |
| `docs/chapter_consolidation_destination_draft_planning_dag.md` | Review-ready destination draft for **Planning as a Control Layer: DAGs and Intelligence Arbitrage**; not reviewed, not canonical, and not a manifest edit. | tracked |
| `docs/chapter_consolidation_fold_moecot_runtime.md` | Fold disposition for `moecot-runtime-and-multi-core-orchestration` into a named MoECOT Runtime Crosswalk section inside `routing-heads-and-specialist-cores`; review artifact only, not a manifest edit or support-state change. | tracked |
| `docs/chapter_consolidation_fold_simulation_fidelity.md` | Fold disposition for `simulation-fidelity-and-physical-constraints` into a named Simulation Fidelity and Claim Transport section inside `resource-economics-and-token-budgets`; review artifact only, not a manifest edit or support-state change. | tracked |
| `docs/chapter_consolidation_fold_semantic_representation.md` | Fold disposition for `semantic-representation-and-tree-structured-models` into a named Semantic Representation Leasing section inside the compression/representation destination package; review artifact only, not a manifest edit or support-state change. | tracked |
| `docs/chapter_consolidation_decision_review.md` | Current Part I consolidation decision: destination drafts are review-ready, but manifest merge waits on human/external review plus execution of the URL/history policy in a future merge commit. | tracked |
| `docs/chapter_consolidation_url_history_policy.md` | URL/history policy for future chapter merges and folds, including continuity URLs, retired source URLs, redirect or historical-stub treatment, and chapter-history ledger requirements. | tracked |
...
| `docs/chapter_consolidation_full_review_packet.md` | Full review packet for deciding execute, revise, defer, or reject across the governed consolidation decision queue; request artifact only, not accepted review or manifest authorization. | tracked |
...
| `docs/curated_reader_planforge_dag_prose_pass.md` | Drafting-only curated reader prose-pass record for the PlanForge DAGs and Intelligence Arbitrage chapter; preserves support-state, proof/test, scheduler, route-selection, selected-tier adequacy, cost-quality, MoECOT runtime, consolidation, and release boundaries. | tracked |
