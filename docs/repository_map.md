...
| Inspect or advance the P5 natural stateful-service campaign | Use `experiments/governed_operations_argument_exit/preregistration.json` as the sole prospective campaign authority and `docs/p5_natural_stateful_service_campaign_preregistration.md` as its design decision. The authored implementation receipt is `experiments/governed_operations_argument_exit/qualification/2026-07-28-local.json`; its reader boundary is `docs/p5_natural_stateful_service_campaign_qualification.md`, and `scripts/validate_p5_natural_service_campaign_qualification.py` reruns and adversarially validates it. When an independently necessary eligible task is discovered before solution investigation, prepare its candidate envelope outside the repository and run `python3 scripts/admit_p5_natural_task.py --candidate /absolute/path/to/candidate.json`; inspect the prospective receipt, then repeat with `--write` only if the assignment and redaction are correct. `scripts/validate_p5_natural_task_admission.py` validates the seeded 15-development/40-heldout allocation, heldout redaction, source binding, exact custody/receipt reconciliation, and eighteen rejecting controls. Every admission invalidates the prior empty-custody qualification and requires requalification before execution. Preserve dated amendments before held-out opening; after opening, task/arm/metric/denominator changes are forbidden. The current canonical custody contains zero natural tasks and cannot borrow P2/Q1/Q2 tasks, substitute for Theseus T4, create a public effect, or move support/release state. |
