...
When revising chapter endings, add or preserve `## Handoff` sections. A good handoff should come after `Summary`, name the next manifest chapter title exactly, avoid numbered chapter references, and explain the architectural reason the next boundary follows. `python3 scripts/validate_chapter_handoffs.py` enforces this for all manifest chapters; the final chapter should close the book-level arc instead of naming a later chapter.
