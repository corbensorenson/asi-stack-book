...
When revising diagrams, add a concise `Diagram reading note` after the primary Mermaid block. The note should explain how to read the boundary, loop, state machine, or evidence flow without claiming that the diagram itself proves implementation, benchmark, safety, or runtime behavior. Part I foundation chapters, Part II planning/memory/reasoning/execution chapters, and Part III routing/compression/representation/substrate chapters are already under this validator ratchet; Part IV should converge to the same convention.
