...
The repeated-prose guard also rejects `The support state remains argument` and `The passage-reviewed mappings support discussion` as support-boundary formulas; write the evidence limit in chapter-specific language instead. For failure-mode sections, name the failure directly in the chapter's own vocabulary rather than opening with generic formulas such as `The subtle failure is`, `The subtle failure mode is`, or `Another failure is`.
