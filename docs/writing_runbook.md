...
When revising chapter endings, add or preserve `## Handoff` sections only where the handoff ratchet has been enabled. A good handoff should come after `Summary`, name the next manifest chapter title exactly, avoid numbered chapter references, and explain the architectural reason the next boundary follows. Part I is currently under this ratchet through `python3 scripts/validate_chapter_handoffs.py`; later parts should converge as their endings are polished.
