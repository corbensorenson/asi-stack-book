...
| Manuscript scale | 54 chapter files; 184,470 chapter words excluding YAML front matter; 192,059 raw chapter-file words including metadata and live scaffolding | Local word-count check on `chapters/*.qmd` |
...
| Test harnesses | Six synthetic harnesses are wired into book validation and the Phase 5 registry: the support-state transition harness checks 2 valid and 2 expected-invalid evidence-transition fixtures for no-change conservatism, upward-transition review gates, required evidence refs, and failed-verification blockers; the authority transition harness checks 3 valid and 3 expected-invalid authority-transition fixtures for non-escalation, permission separation, denial receipts, approval escalation, and confused-deputy shortcuts; the plan-execution contract harness checks 2 valid and 5 expected-invalid command-contract, plan-graph, DAG, semantic-atom, and typed-job fixtures for cross-record consistency, acyclic dependency order, dispatch receipts, requirement preservation, artifact traceability, and approval gating; the context admission/adequacy harness checks 3 valid and 5 expected-invalid context ABI, packet, certificate, transaction, and adequacy fixtures for admission/adequacy separation, conflict blocking, stale certificate rejection, deletion closure, escalation, and mode-confusion gates; the readiness/residual gate harness checks 4 valid and 5 expected-invalid costed-route, readiness-gate, and replacement-transaction fixtures for promotion gates, residual escrow custody, quarantine, authority bounds, expired-evidence reruns, fallback, and rollback readiness; the benchmark anti-Goodhart harness checks 2 valid and 5 expected-invalid benchmark-ratchet, policy-optimization, and steward-action fixtures for holdout/contamination/mutation or transfer checks, saturated-benchmark regression floors, blocked-ratchet policy promotion, reward-as-truth confusion, and release approval evidence. The registry guard checks commands, docs, fixture counts, result records, Appendix E rows, public status references, primary chapter mappings, non-claim boundaries, and main-validation wiring. None of these harnesses promotes live claims | `experiments/support_state_transitions/`; `experiments/authority_transitions/`; `experiments/plan_execution_contracts/`; `experiments/context_admission_adequacy/`; `experiments/readiness_residual_gates/`; `experiments/benchmark_antigoodhart/`; `experiments/phase5_harness_registry.json`; `docs/support_state_transition_harness.md`; `docs/authority_transition_harness.md`; `docs/plan_execution_contract_harness.md`; `docs/context_admission_adequacy_harness.md`; `docs/readiness_residual_harness.md`; `docs/benchmark_antigoodhart_harness.md`; `docs/phase5_harness_registry.md`; `python3 scripts/validate_support_state_transitions.py`; `python3 scripts/validate_authority_transitions.py`; `python3 scripts/validate_plan_execution_contracts.py`; `python3 scripts/validate_context_admission_adequacy.py`; `python3 scripts/validate_readiness_residual_gates.py`; `python3 scripts/validate_benchmark_antigoodhart.py`; `python3 scripts/validate_phase5_harness_registry.py` |
...
| Release surfaces | Live, research, reader, and audio profiles exist; reader/audio derivation scripts exist; the reader release has a tracked semantic overlay manifest as the editable delta source, generated reader delta report path as review output with a zero-active-operation note or operation digests and before/after excerpts, embedded live Human-view overlay payload for major-version human-edition deltas, and a generated reader-continuity audit with 3 high-priority heuristic review rows. The curated reader-manuscript manifest exists with `not_graduated` status so the future human-prose source can diverge only as a subordinate narrative derivative. The current v1.0 reader-overlay set carries 25 active operations: 2 opening-chapter operations, 1 Human Intent table-to-prose operation, 1 System Boundaries table-to-prose operation, 1 Evidence States table-to-prose operation, 4 Personal Compute Hives table-to-prose operations, 2 Command Contracts table-to-prose operations, 1 Planning table-to-prose operation, 1 Verification Bandwidth table-to-prose operation, 1 Runtime Adapters table-to-prose operation, 1 Labor OS table-to-prose operation, 1 Circle Contracts table-to-prose operation, 2 Fast Generation table/code-to-prose operations, 1 RankFold/NeuralFold table-to-prose operation, 2 Policy Optimization table-to-prose operations, 2 Artifact Steward Agents table-to-prose operations, and 2 Semantic Representation table-to-prose operations for Human view and generated reader editions only | `editions/release_profiles.json`; `editions/reader_overlays/v1_0/manifest.json`; `editions/reader_manuscript/v1_0/manifest.json`; `docs/reader_continuity_audit.md`; `assets/reader-overlays.html`; `docs/major_version_release_runbook.md`; `docs/reader_overlay_pilot.md`; `python3 scripts/sync_reader_overlay_asset.py --check`; `python3 scripts/validate_reader_overlays.py --check`; `python3 scripts/audit_reader_continuity.py --check`; `python3 scripts/validate_reader_manuscript_manifest.py` |
...
- Continue the final reader-facing editorial pass over both the generated reader manuscript and representative live Human view chapters. The opening-chapter, Human Intent, System Boundaries, Evidence States, Personal Compute Hives, Command Contracts, Planning, Verification Bandwidth, Runtime Adapters, Labor OS, Circle Contracts, Fast Generation, RankFold/NeuralFold, Policy Optimization, Artifact Steward Agents, and Semantic Representation overlays prove the semantic-delta path for reader-only prose, and the reader-continuity audit gives the first manual pass 3 high-priority heuristic rows, but full human continuity review still needs chapter-level decisions without weakening evidence boundaries.
