...
| Source appendix ownership | Appendix G (`Corben-Authored, Supplied, and Local Sources`) and Appendix H (`External Sources by Other Authors`) are independent top-level appendices with explicit ownership-rule and ownership-at-a-glance rows: G contains Corben-authored, Corben-supplied, recovered, and local project records; H contains external records by other authors marked `external_literature` | `python3 scripts/validate_source_appendices.py` |
