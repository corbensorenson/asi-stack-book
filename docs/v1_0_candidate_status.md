...
| Manuscript scale | 54 chapter files; 183,692 chapter words excluding YAML front matter; 191,191 raw chapter-file words including metadata and live scaffolding | Local word-count check on `chapters/*.qmd` |
...
| Source appendix ownership | Appendix G (`Corben-Authored, Supplied, and Local Sources`) and Appendix H (`External Sources by Other Authors`) are independent top-level appendices with explicit ownership-rule and appendix-local identity rows: G contains Corben-authored, Corben-supplied, recovered, and local project records; H contains external records by other authors marked `external_literature`; neither appendix renders the other source class as a second ownership row | `python3 scripts/validate_source_appendices.py` |
...
| Visual coverage | Every chapter has at least one substantive Mermaid diagram with at least 12 non-comment lines plus enough named states, edges, and labeled transitions to explain a mechanism; every chapter has required `Diagram reading note` prose for its primary diagram; landing page has a generated visual asset; browser Human-view gate checks rendered Mermaid SVG visibility | `python3 scripts/validate_visual_coverage.py`; `node scripts/validate_live_human_view_browser.js --all-chapters --all-viewports` |
| Chapter handoffs | All 54 manifest chapters now end with reader-facing `Handoff` sections: non-final chapters name the next manifest chapter title and avoid numbered chapter references, while the final chapter closes the book-level arc | `python3 scripts/validate_chapter_handoffs.py` |
