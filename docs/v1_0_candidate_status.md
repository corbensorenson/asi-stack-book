...
| Manuscript scale | 54 chapter files; 173,950 chapter words excluding YAML front matter; 181,449 raw chapter-file words including metadata and live scaffolding | Local word-count check on `chapters/*.qmd` |
...
- Every manifest chapter exists and has the required chapter contract in order: problem, insufficiency, core claim, mechanism, interfaces, invariants, failure modes, minimum viable implementation, beyond-state-of-the-art end state, test plan, source crosswalk, and summary. The chapter DoD guard also requires invariants and failure modes to clear a 90-word substance floor, the implementation endpoint pair to remain substantive, and chapter summaries to clear a 120-word floor while rejecting self-referential chapter phrasing, mechanical section handoffs, and live crosswalk references.
