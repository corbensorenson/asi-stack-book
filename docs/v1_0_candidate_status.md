...
| Manuscript scale | 54 chapter files; 183,509 chapter words excluding YAML front matter; 191,008 raw chapter-file words including metadata and live scaffolding | Local word-count check on `chapters/*.qmd` |
| Source inventory | 108 public-safe source records, each with a matching public source note; `sources/source_notes/` also contains a README and template | `sources/source_inventory.json`; `sources/source_notes/` |
...
| Test harnesses | The first synthetic support-state transition harness checks 2 valid and 2 expected-invalid evidence-transition fixtures for no-change conservatism, upward-transition review gates, required evidence refs, and failed-verification blockers; it is wired into book validation and does not promote live claims | `experiments/support_state_transitions/`; `docs/support_state_transition_harness.md`; `python3 scripts/validate_support_state_transitions.py` |
