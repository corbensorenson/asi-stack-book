...
| Manuscript scale | 54 chapter files; 183,611 chapter words excluding YAML front matter; 191,131 raw chapter-file words including metadata and live scaffolding | Local word-count check on `chapters/*.qmd` |
...
| Test harnesses | Two synthetic harnesses are wired into book validation: the support-state transition harness checks 2 valid and 2 expected-invalid evidence-transition fixtures for no-change conservatism, upward-transition review gates, required evidence refs, and failed-verification blockers; the authority transition harness checks 3 valid and 3 expected-invalid authority-transition fixtures for non-escalation, permission separation, denial receipts, approval escalation, and confused-deputy shortcuts. Neither harness promotes live claims | `experiments/support_state_transitions/`; `experiments/authority_transitions/`; `docs/support_state_transition_harness.md`; `docs/authority_transition_harness.md`; `python3 scripts/validate_support_state_transitions.py`; `python3 scripts/validate_authority_transitions.py` |
