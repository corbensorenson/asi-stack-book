...
| Manuscript scale | 54 chapter files; 183,796 chapter words excluding YAML front matter; 191,295 raw chapter-file words including metadata and live scaffolding | Local word-count check on `chapters/*.qmd` |
...
| Source appendix ownership | Appendix G (`Corben-Authored, Supplied, and Local Sources`) and Appendix H (`External Sources by Other Authors`) are independent top-level appendices with explicit ownership-rule rows: G contains Corben-authored, Corben-supplied, recovered, and local project records; H contains external records by other authors marked `external_literature` | `python3 scripts/validate_source_appendices.py` |
