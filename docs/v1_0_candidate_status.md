...
| Manuscript scale | 54 chapter files; 176,336 chapter words excluding YAML front matter; 183,835 raw chapter-file words including metadata and live scaffolding | Local word-count check on `chapters/*.qmd` |
...
| Source appendix ownership | Appendix G is the Corben-authored, Corben-supplied, and local project source corpus; Appendix H is a separate external-source appendix for records marked `external_literature` | `python3 scripts/validate_source_appendices.py` |
...
- Every manifest chapter exists and has the required chapter contract in order: problem, insufficiency, core claim, mechanism, interfaces, invariants, failure modes, minimum viable implementation, beyond-state-of-the-art end state, test plan, source crosswalk, and summary. The chapter DoD guard also requires `Problem` sections and summaries to clear a 120-word substance floor, `Interfaces` sections to clear a 130-word systems-handoff floor, invariants, failure modes, and minimum viable implementation sections to clear a 90-word substance floor, and the mature endpoint to remain substantive while rejecting self-referential chapter phrasing, mechanical section handoffs, and live crosswalk references.
