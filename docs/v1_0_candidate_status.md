...
| Manuscript scale | 54 chapter files; 183,699 chapter words excluding YAML front matter; 191,198 raw chapter-file words including metadata and live scaffolding | Local word-count check on `chapters/*.qmd` |
...
| Source appendix ownership | Appendix G (`Corben's Sources, Papers, and Local Projects`) and Appendix H (`External Sources and Third-Party Literature`) are independent top-level appendices with explicit ownership-rule and appendix-local identity rows: G contains Corben's papers, Corben-supplied materials, recovered project records, and local project records; H contains external records and third-party literature marked `external_literature`; neither appendix renders the other source class as a second ownership row | `python3 scripts/validate_source_appendices.py` |
