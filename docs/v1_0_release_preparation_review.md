...
- `docs/reader_chapter_review_matrix.md` records the manifest-synced
  54-chapter reader-review queue: 22 `reviewed`, 32 `spot_checked`, 0
  `not_started`, 20 active-overlay chapters, 43 no-immediate-action decisions,
...
- The reader chapter review matrix is a release-control queue only. It preserves
  ...
  full Part I sequence and first eight Part II chapters have chapter-text review records, but the matrix is not
...
2. The generated reader manuscript needs release-grade full human continuity
   review; the current matrix has 22 reviewed rows and no `not_started`
