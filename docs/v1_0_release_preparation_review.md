...
- `docs/reader_chapter_review_matrix.md` records the manifest-synced
  54-chapter reader-review queue: 34 `reviewed`, 20 `spot_checked`, 0
...
- The reader chapter review matrix is a release-control queue only. It preserves
  ...
  full Part I and Part II sequences plus the first four Part III chapters have chapter-text review records, but the matrix is not
...
2. The generated reader manuscript needs release-grade full human continuity
   review; the current matrix has 34 reviewed rows and no `not_started`
