...
- `docs/reader_chapter_review_matrix.md` records the manifest-synced
  54-chapter reader-review queue: 38 `reviewed`, 16 `spot_checked`, 0
  `not_started`, 20 active-overlay chapters, 50 no-immediate-action decisions,
...
- The reader chapter review matrix is a release-control queue only. It preserves
  ...
  full Part I and Part II sequences plus the first eight Part III chapters have chapter-text review records, but the matrix is not
...
2. The generated reader manuscript needs release-grade full human continuity
   review; the current matrix has 38 reviewed rows and no `not_started`
