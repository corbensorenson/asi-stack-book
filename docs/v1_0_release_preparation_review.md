...
- `docs/reader_chapter_review_matrix.md` records the manifest-synced
  54-chapter reader-review queue: 50 `reviewed`, 4 `spot_checked`, 0
  `not_started`, 20 active-overlay chapters, 53 no-immediate-action decisions,
...
- The reader chapter review matrix is a release-control queue only. It preserves
  ...
  full Part I, Part II, and Part III sequences plus the first five Part IV chapters have chapter-text review records, but the matrix is not
...
2. The generated reader manuscript needs release-grade full human continuity
   review; the current matrix has 50 reviewed rows and no `not_started`
