...
- `docs/reader_chapter_review_matrix.md` records the manifest-synced
  54-chapter reader-review queue: 41 `spot_checked`, 13 `not_started`, 20
  active-overlay chapters, 22 no-immediate-action decisions, 3 companion-note
...
2. The generated reader manuscript needs full human continuity review; the
   current matrix still has 13 `not_started` chapter rows and release blockers
   on every row.
