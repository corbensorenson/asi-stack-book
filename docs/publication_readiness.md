...
- All 80 live chapter-core claims remain at `argument`; publication
...
- The non-core evidence ledger exists at `docs/non_core_evidence_ledger.md`; `scripts/validate_non_core_evidence_ledger.py` checks that the twenty-five accepted non-core upward transitions, accepted blocking decisions, three historical `refuted` labels with N1/N2 interpretation ceilings, and the live count-surface narrowing record are visible, while the separate core-disposition and trust-surface checks keep all 80 live chapter core claims at `argument`.
...
- The chapter external-grounding status ledger exists at `docs/chapter_external_grounding_status.md`; `scripts/validate_chapter_external_grounding_status.py` checks all 80 manifest chapters against source-noted external records, explicit external-baseline exceptions, and first-pass Corben/local source-mining queues without claiming exhaustive literature coverage, reproduced external results, or support-state promotion.
...
- `docs/proof_artifact_audit.md` records the current proof artifact traceability audit across all 325 proof targets: 310 implemented and 15 deliberately planned Round 18 chapter targets.
...
- Appendix C now records exact source-note mappings for all 80 core claims without promoting support states.
- Appendix C now includes a generated "What would promote this" field for all 80 chapter core claims, derived from `docs/per_chapter_evidence_plan.md` and checked by `scripts/validate_core_claim_promotion_paths.py`; this is a reader-facing burden-of-proof field, not a support-state change.
- All 80 chapters have manuscript drafts from the source-of-truth manifest and drafting passes, kept at conservative support states; the taxonomy reconciliation added four complete argument-level chapters and thirteen routed section repairs without promoting support.
- All 80 chapters have `.asi-human-only` Human Reading Path bridges that are hidden from default AI view, visible as unheaded lead-in prose in live Human view, omitted from the page TOC, and unwrapped without the source-only heading into generated reader editions. Generated reader chapters retain manifest titles as their opening heading.
...
- Phase 6 external-positioning is now machine-tracked in `docs/external_sota_positioning_audit.md`: 80 of 80 chapters currently have in-prose `ext_*` positioning before Source crosswalk, 0 have explicit external-baseline exceptions, and 0 rows remain open under the stricter placement release gate. This closes the current placement gate only; it does not claim exhaustive literature synthesis, reproduced external results, or support-state promotion.
- The generated chapter external-grounding status ledger currently has 80 source-noted chapters, 0 explicit exceptions, 0 candidate-backlog rows, and 0 missing audit rows. It is a routing surface for future source mining and chapter writing, not an external-literature-backed support-state transition.
