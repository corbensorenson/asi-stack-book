...
- Reader, research, PDF, EPUB, DOCX, AZW3, MOBI, and audio editions are planned and scaffolded. Local HTML/EPUB/DOCX dry-run snapshots now exist with structural inspection for review, and an isolated PDF probe renders when `LANG` and `LC_ALL` are set to `en_US.UTF-8`. No human-reader major-version artifact or audiobook should be reported as a release artifact until the corresponding generated manuscript, render or conversion, human review, and release record exist.
