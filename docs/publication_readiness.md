...
- `docs/proof_artifact_audit.md` records the current proof artifact traceability audit across all 143 proof targets.
...
- The manifest now tracks 143 proof targets, all implemented as finite-record Lean targets. Appendix E publishes the current coverage/accounting breakdown from `proofs/proof_triage.json`, `docs/proof_artifact_audit.md` records traceability coverage, and `docs/proof_adequacy_review.md` records the semantic adequacy classification. Most finite-record modules remain useful but too narrow and do not prove broad system behavior.
- Twenty-nine synthetic or deterministic behavior/accounting harnesses are wired into validation, including twenty-two registry-checked harnesses plus seven book-gate-only harnesses for Artifact Graph replay, Procedural Memory loop qualification, Routing decision lease discipline, Cyclic Memory contract discipline, Context Transaction Memory-Store discipline, Simulation Transfer Boundary discipline, and Resource Workflow Trace discipline. The set includes claim-ledger revision, proof-carrying claim, tribunal-review discipline, value-conflict discipline, constitutional-predicate discipline, governance-right discipline, agency-right checklist discipline, security-kernel receipt discipline, stable-capability-field qualification/routing discipline, replacement-transaction discipline, self-improvement transition discipline, artifact-graph replay discipline, procedural-memory loop qualification, routing decision lease discipline, cyclic-memory contract discipline, bounded context-transaction memory-store discipline, simulation-transfer boundary discipline, resource-workflow-trace discipline, and reference-trace discipline. The first bounded non-infrastructure measured/replayed slice validates a synthetic costed-route/resource-budget selector with baseline, negative control, residual, fallback, and non-claim boundaries; the deterministic Resource Economics workflow trace validates selected-route cost recomputation, scheduler-ordering checks, displaced-cost residual controls, over-budget aggregate-resource-bill rejection, and physical-feasibility overclaim rejection without support-state movement; the first bounded imported external-prototype slice records a Circle rope-contract receipt replay from a clean local checkout. Most chapter-level Codex tests remain planned, synthetic, or not yet replayed as empirical/prototype runs, and no chapter core claim is promoted by either slice.
