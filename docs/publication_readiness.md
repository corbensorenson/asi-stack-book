...
- Seven synthetic behavior-gate harnesses are wired into validation and registry-checked, but most chapter-level Codex tests remain planned or synthetic rather than replayed empirical/prototype runs.
...
- Reader, research, PDF, EPUB, DOCX, AZW3, MOBI, and audio editions are planned and scaffolded. Local HTML/EPUB/DOCX dry-run snapshots now exist with structural inspection for review, `docs/reader_docx_probe_manifest.md` records a 514-page LibreOffice conversion probe for the generated DOCX, an isolated PDF probe renders when `LANG` and `LC_ALL` are set to `en_US.UTF-8`, `docs/reader_pdf_probe_manifest.md` records the current 535-page PDF probe and sampled source-card appendix pages, and `docs/reader_artifact_layout_review.md` records refreshed PDF/DOCX sampling plus a broader 28 page-view HTML layout/navigation probe. No human-reader major-version artifact or audiobook should be reported as a release artifact until the corresponding generated manuscript, render or conversion, broader human review, and release record exist.
