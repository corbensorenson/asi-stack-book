...
- The chapters have received broad coherence, transition, Human-view, and generated-reader chapter-text review passes, but the book still needs claim-to-mechanism support review and accepted evidence transitions before v1.0 evidence-release quality. The reader chapter review matrix now has all 54 current chapters at reviewed chapter-text status, but release-record and format-artifact blockers remain on every row.
- Reader, research, PDF, EPUB, DOCX, AZW3, MOBI, and audio editions are planned and scaffolded. Local HTML/EPUB/DOCX dry-run snapshots now exist with structural inspection for review, an isolated PDF probe renders when `LANG` and `LC_ALL` are set to `en_US.UTF-8`, and `docs/reader_artifact_layout_review.md` records representative PDF sampling plus a broader 28 page-view HTML layout/navigation probe. No human-reader major-version artifact or audiobook should be reported as a release artifact until the corresponding generated manuscript, render or conversion, broader human review, and release record exist.
