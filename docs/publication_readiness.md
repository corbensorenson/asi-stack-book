...
- Reader, research, PDF, EPUB, DOCX, AZW3, MOBI, and audio editions are planned and scaffolded. Local HTML/EPUB/DOCX dry-run snapshots now exist with structural inspection for review, an isolated PDF probe renders when `LANG` and `LC_ALL` are set to `en_US.UTF-8`, `docs/reader_pdf_probe_manifest.md` records the current 614-page PDF probe and its source-appendix table-collision blocker, and `docs/reader_artifact_layout_review.md` records refreshed PDF sampling plus a broader 28 page-view HTML layout/navigation probe. No human-reader major-version artifact or audiobook should be reported as a release artifact until the corresponding generated manuscript, render or conversion, broader human review, and release record exist.
