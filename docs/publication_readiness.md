...
- The non-core evidence ledger exists at `docs/non_core_evidence_ledger.md`; `scripts/validate_non_core_evidence_ledger.py` checks that the three accepted non-core upward transitions are visible, that all 47 chapter core claims remain at `argument`, and that README, landing page, and Appendix C point readers to the no-promotion boundary.
...
- The chapter external-grounding status ledger exists at `docs/chapter_external_grounding_status.md`; `scripts/validate_chapter_external_grounding_status.py` checks all 47 manifest chapters against source-noted external records, explicit external-baseline exceptions, and first-pass Corben/local source-mining queues without claiming exhaustive literature coverage, reproduced external results, or support-state promotion.
...
- Appendix C now records exact source-note mappings for all 47 core claims without promoting support states.
- Appendix C now includes a generated "What would promote this" field for all 47 chapter core claims, derived from `docs/per_chapter_evidence_plan.md` and checked by `scripts/validate_core_claim_promotion_paths.py`; this is a reviewer-facing burden-of-proof field, not a support-state change.
- All 47 chapters have manuscript drafts from the source-of-truth manifest and hand drafting passes, kept at conservative support states.
- All 47 chapters have `.asi-human-only` Human Reading Path bridges that are hidden from default AI view, visible as unheaded lead-in prose in live Human view, omitted from the page TOC, and unwrapped without the source-only heading into generated reader editions. Generated reader chapters now retain manifest titles as their opening heading.
