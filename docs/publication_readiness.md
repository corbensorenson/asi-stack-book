...
- `scripts/build_audio_script.py` can derive an audio-script review workspace, `audio_manifest.json`, `AUDIO_RELEASE_CHECKLIST.md`, `companion_notes.md`, `chapter_markers.md`, and pronunciation glossary under ignored `build/` without claiming audio exists; its check also verifies that generated chapter scripts retain both implementation-horizon sections.
...
- Phase 6 external-positioning is now machine-tracked in `docs/external_sota_positioning_audit.md`: 44 of 54 chapters currently have in-prose `ext_*` positioning before Source crosswalk, 10 have explicit external-baseline exceptions, and 0 rows remain open under the stricter placement release gate. This closes the v1.0 placement gate only; it does not claim exhaustive literature synthesis, reproduced external results, or support-state promotion.
- The generated chapter external-grounding status ledger currently has 44 source-noted chapters, 10 explicit exceptions, 0 candidate-backlog rows, and 0 missing audit rows. It is a routing surface for future source mining and chapter writing, not an external-literature-backed support-state transition.
