...
- The current v1.0 candidate reproducibility and citation note exists at `docs/release_reproducibility.md`; `CITATION.cff` records version `1.0.0-candidate`, DOI-pending status, and the public site/repository citation path; `scripts/validate_release_reproducibility.py` checks the pinned CI Quarto/Python/Node setup, Lean toolchain reference, locale notes, and non-release artifact boundary.
...
- The v1.0 release-gate audit exists at `docs/v1_0_release_gate_audit.md`; `scripts/validate_v1_release_gate_audit.py` checks all eleven Definition-of-Done gates, their evidence references, and the candidate-versus-final-release boundary without creating a final tag, DOI, archive, or support-state promotion.
...
- `CITATION.cff` is candidate metadata only. A final v1.0 evidence-release tag and DOI/Zenodo entry remain blocked until the release gate actually clears or the release record explicitly records DOI pending; the current reader HTML source tag and edition record do not create that final evidence release.
