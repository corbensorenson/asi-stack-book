...
- `docs/source_evidence_audit.md` records the current public-safe source evidence audit: 461 assigned source/chapter pairs, 461 exact claim-source mappings, complete source-note/chapter-listing coverage, and 81 passage-reviewed mappings recorded.
...
- Source-derived support still requires passage-level source review, claim-to-mechanism reconciliation, and an accepted evidence transition; the source evidence audit confirms mapping coverage and records the first 81 passage-reviewed mappings, but exact source-note mapping alone remains insufficient for support-state promotion.
