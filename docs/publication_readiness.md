...
- The non-core evidence ledger exists at `docs/non_core_evidence_ledger.md`; `scripts/validate_non_core_evidence_ledger.py` checks that the three accepted non-core upward transitions and the live count-surface narrowing record are visible, that all 44 chapter core claims remain at `argument`, and that README, landing page, and Appendix C point readers to the no-promotion boundary.
...
- The chapter external-grounding status ledger exists at `docs/chapter_external_grounding_status.md`; `scripts/validate_chapter_external_grounding_status.py` checks all 44 manifest chapters against source-noted external records, explicit external-baseline exceptions, and first-pass Corben/local source-mining queues without claiming exhaustive literature coverage, reproduced external results, or support-state promotion.
...
- `docs/proof_artifact_audit.md` records the current proof artifact traceability audit across all 144 proof targets.
...
- Appendix C now records exact source-note mappings for all 44 core claims without promoting support states.
- Appendix C now includes a generated "What would promote this" field for all 44 chapter core claims, derived from `docs/per_chapter_evidence_plan.md` and checked by `scripts/validate_core_claim_promotion_paths.py`; this is a reviewer-facing burden-of-proof field, not a support-state change.
- All 44 chapters have manuscript drafts from the source-of-truth manifest and hand drafting passes, kept at conservative support states.
- All 44 chapters have `.asi-human-only` Human Reading Path bridges that are hidden from default AI view, visible as unheaded lead-in prose in live Human view, omitted from the page TOC, and unwrapped without the source-only heading into generated reader editions. Generated reader chapters now retain manifest titles as their opening heading.
...
- The manifest now tracks 144 proof targets, all implemented as finite-record Lean targets. Appendix E publishes the current coverage/accounting breakdown from `proofs/proof_triage.json`, `docs/proof_artifact_audit.md` records traceability coverage, and `docs/proof_adequacy_review.md` records the semantic adequacy classification. Most finite-record modules remain useful but too narrow and do not prove broad system behavior.
- Thirty-five synthetic, deterministic, measured, or local replay harnesses are wired into validation, including twenty-two registry-checked harnesses plus thirteen book-gate-only chapter/support harnesses for Artifact Graph replay, Procedural Memory loop qualification, Routing decision lease discipline, Cyclic Memory contract discipline, Context Transaction Memory-Store discipline, Simulation Transfer Boundary discipline, Resource Workflow Trace discipline, Resource live probes, Resource workload-quality probing, Resource load-stability probing, Theseus support replay, Compact GVR, and Personal Compute Hive admission. The current set checks claim-ledger revision, proof-carrying claim, tribunal-review discipline, value-conflict discipline, constitutional-predicate discipline, governance-right discipline, agency-right checklist discipline, security-kernel receipt discipline, stable-capability-field qualification/routing discipline, replacement-transaction discipline, self-improvement transition discipline, artifact-graph replay discipline, procedural-memory loop qualification, routing decision lease discipline, cyclic-memory contract discipline, bounded context-transaction memory-store discipline, simulation-transfer boundary discipline, resource-workflow-trace discipline, resource-workload route selection, resource-load-stability route selection, reference-trace discipline, capacity smoothing, resource-budget ledger discipline, generation-mode baseline discipline, and hive admission discipline. The first bounded non-infrastructure measured/replayed slice validates a synthetic costed-route/resource-budget selector with baseline, negative control, residual, fallback, and non-claim boundaries; the deterministic Resource Economics workflow trace validates selected-route cost recomputation, scheduler-ordering checks, displaced-cost residual controls, over-budget aggregate-resource-bill rejection, and physical-feasibility overclaim rejection without support-state movement; the Resource workload-quality probe records a local five-sample scoped-validator route choice with a broader baseline and a rejected no-op negative control; the Resource load-stability probe records a local synthetic burst-review route choice with a broader admit-arrivals baseline, 7 residualized selected deferrals, and a rejected review-erasure negative control; the bounded Compact GVR slice validates exact compact-generation receipt accounting; the hive admission harness checks synthetic Personal Compute Hive admission records without claiming deployed hive behavior; and the first bounded imported external-prototype slice records a Circle rope-contract receipt replay from a clean local checkout. Most chapter-level Codex tests remain planned, synthetic, or not yet replayed as empirical/prototype runs, and no chapter core claim is promoted by these slices.
