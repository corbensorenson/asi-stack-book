...
- `editions/reader_manuscript/v1_0/curated_format_probe_manifest.json`, `docs/curated_reader_format_artifact_probe.md`, and `scripts/validate_curated_reader_format_probe_manifest.py` record the current tracked curated-reader HTML/EPUB/DOCX/PDF structural probe, including PNG figure fallbacks for DOCX/PDF, 50 Chrome-screenshot Mermaid fallbacks for PDF, a repaired-package EPUB content/navigation audit with 0 unresolved internal hrefs, a repaired-package DOCX document XML/relationship audit with 0 raw `.qmd` relationship targets, an all-page PDF visual raster audit with 511 pages, 0 blank pages, 1 low-ink page, and 0 near-edge pages, an extracted-text PDF reading-flow audit with 511 nonempty pages, and a Chromium PDF viewer smoke review with 2 nonblank screenshots and 4.479% changed pixels after scroll, while preserving the release blockers for final release approval, full publication packaging, and edition release records.
...
- `release_records/2026-07-05-v1-curated-reader-blocked-3e59bde3.json` and `scripts/validate_curated_reader_blocked_release_record.py` record the current curated reader manuscript as a blocked partial release candidate, naming exact local HTML/EPUB/DOCX/PDF/e-reader/audio blockers without approving or publishing artifacts.
