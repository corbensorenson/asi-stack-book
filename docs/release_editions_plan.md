...
- `editions/reader_manuscript/v1_0/curated_format_probe_manifest.json`, `docs/curated_reader_format_artifact_probe.md`, and `scripts/validate_curated_reader_format_probe_manifest.py` record the current tracked curated-reader HTML/EPUB/DOCX/PDF structural probe, including PNG figure fallbacks for DOCX/PDF and a repaired-package EPUB content/navigation audit with 0 unresolved internal hrefs, while preserving the release blockers for application/e-reader review and edition release records.
