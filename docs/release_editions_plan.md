...
- `editions/reader_manuscript/v1_0/manifest.json` preserves a frozen historical
  curated-reader workspace, its graduation criteria, allowed prose divergence,
  blocked evidence divergence, handoff contract, review records, and
  release-blocker history. It is not current-spine coverage. The current reader
  source is generated from all 84 live chapters through
...
- The v1.0 curated format probes preserve exact historical HTML/EPUB/DOCX/PDF
  preparation evidence and blockers for their named artifacts. They do not
  describe the current 84-chapter reader source or approve a current download.
...
- `scripts/sync_reader_chapter_review_matrix.py --check` checks the frozen
  edition-specific review queue against its own historical spine and blockers;
  current all-chapter coverage comes from the reader generator and continuity
  audit.
