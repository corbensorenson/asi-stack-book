...
| Source records | 140 | The corpus is substantial, with 59 Corben/local records and 81 external records by current appendices. |
...
| Core claim support states | 54 at `argument` | The book is still conservative. The v1.0 evidence-transition pilot now records seven accepted no-change decisions; no upward support transition exists. |
...
| Test appendix rows | 229 | Appendix E now has 213 chapter-level rows plus 16 repository-level check rows. |
| Planned/unrun chapter test rows | 152 | The biggest technical evidence gap is still tests beyond shape validation, synthetic gate fixtures, and narrow finite-record proofs. |
| Implemented or partial test/proof/check rows in Appendix E | 77 | Appendix E now has 60 implemented chapter rows, 1 partial chapter row, and 16 repository-level checks. Existing rows mostly validate fixtures, synthetic transition gates, synthetic plan-execution consistency, synthetic runtime-adapter permission consistency, synthetic context admission/adequacy consistency, synthetic readiness/residual gate consistency, synthetic benchmark anti-Goodhart consistency, the Phase 5 harness registry, the reader continuity audit, Lean build, or proof/source traceability mechanics. |
...
| Reader format readiness | HTML, EPUB, DOCX setup checks pass; a local dry run rendered 59 reader-site HTML files, 1 EPUB, and 1 DOCX, and snapshotted 81 HTML-site files/dependencies under ignored `build/reader_edition/format_artifacts/`; structural inspection passed for those snapshots; an isolated PDF probe failed without explicit locale settings but succeeded with `LANG=en_US.UTF-8 LC_ALL=en_US.UTF-8`; the first representative PDF/HTML layout spot check is recorded | `docs/reader_format_dry_run.md` and `docs/reader_artifact_layout_review.md` record local render, structural-inspection, PDF-probe, and representative layout-review outcomes, not reviewed or published release artifacts. |
...
| P1 | External literature normalization | Initial source-noted records now exist across alignment/control, governance/evals, planning/agent control, retrieval/context, formal methods, routing/MoE, compression/representation, benchmark science, fast generation, policy optimization, hives, and artifact stewardship. Retrieval/context now includes RAG, Lost in the Middle, MemGPT, LongBench, and RULER; formal methods now includes proof-carrying code, TLA+, Lean theorem proving, Dafny, and Reluplex; routing/MoE now includes sparse MoE, GShard, Switch Transformers, Expert Choice Routing, Mixtral, and an MoE-in-LLMs survey; compression now includes Deep Compression, LoRA, knowledge distillation, GPTQ, and QLoRA; benchmark science now includes MMLU, BIG-bench, HELM, GPQA, SWE-bench, and LiveBench as comparison vocabulary. | Coverage is still selective; behavior trees, GOAP, broader orchestration, provenance-aware RAG, context-compression evaluation, runtime assurance, proof-assistant adequacy, deployment model checking, router evaluation, task-level routing, program synthesis, residual coding, artifact-utility metrics, hidden-test operations, saturation analysis, contamination audits, benchmark-gaming, and evaluator-gaming work remain queued. | Add citation-normalized source records and source notes only after reading the sources, without turning external comparison vocabulary into support-state promotion. |
| P1 | Reader release dry run | Reader generation and HTML/EPUB/DOCX/PDF local render paths have been probed; `docs/reader_format_dry_run.md` records a local HTML/EPUB/DOCX dry run, structural inspection, and UTF-8 PDF probe with ignored snapshots; `docs/reader_artifact_layout_review.md` records the first representative layout spot check. | No actual reviewed reader artifact or release record is recorded. | Continue broader manual layout/navigation inspection on representative local snapshots, continue full reader review, then create an edition release record only if review passes. |
...
Each future harness should write public-safe results under a tracked result or report location only after it actually runs. If a test is negative, failed, or inconclusive, keep that result visible. The current harnesses remain synthetic record and cross-record gates only; they do not prove routing accuracy, deployed authorization, planner quality, adapter runtime safety, memory correctness, benchmark quality, policy-training quality, steward-agent behavior, runtime safety, or support-state promotion.
...
PDF probe, and representative PDF/HTML layout spot check are recorded in
