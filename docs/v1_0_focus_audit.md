...
| Reader format readiness | HTML, EPUB, DOCX setup checks pass; a local dry run rendered all three and snapshotted 61 HTML files, 1 EPUB, and 1 DOCX under ignored `build/reader_edition/format_artifacts/`; structural inspection passed for those snapshots; an isolated PDF probe failed without explicit locale settings but succeeded with `LANG=en_US.UTF-8 LC_ALL=en_US.UTF-8` | `docs/reader_format_dry_run.md` records local render, structural-inspection, and PDF-probe outcomes, not reviewed or published release artifacts. |
...
The first local HTML/EPUB/DOCX dry run, structural artifact inspection, and
UTF-8 PDF probe are recorded in `docs/reader_format_dry_run.md`. They produced
ignored snapshots and local reports for review, not a release. The continuing
sequence is:
