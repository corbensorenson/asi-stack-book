...
| Core claim support states | 44 at `argument` | The book is still conservative. The v1.0 evidence-transition gate now records twenty-two accepted no-change decisions, twenty-two explicit no-promotion decisions, two bounded `synthetic-test-backed` non-core transitions, and one bounded `prototype-backed` imported Circle receipt transition; no chapter core support transition exists. |
