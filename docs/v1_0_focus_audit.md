...
| Source records | 125 | The corpus is substantial, with 59 Corben/local records and 66 external records by current appendices. |
...
| P1 | External literature normalization | Initial source-noted records now exist across alignment/control, governance/evals, planning/agent control, retrieval/context, formal methods, routing/MoE, compression/representation, benchmark science, fast generation, policy optimization, hives, and artifact stewardship. | Coverage is still selective; behavior trees, GOAP, broader orchestration, deeper context engineering, runtime assurance, proof-assistant practice, modern routing surveys, quantization/distillation, contamination, saturation, and evaluator-gaming work remain queued. | Add citation-normalized source records and source notes only after reading the sources, without turning external comparison vocabulary into support-state promotion. |
