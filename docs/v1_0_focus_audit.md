...
| Source records | 143 | The corpus is substantial, with 59 Corben/local records and 84 external records by current appendices. |
...
| P1 | External literature normalization | Initial source-noted records now exist across alignment/control, governance/evals, planning/agent control, retrieval/context, formal methods, routing/MoE, compression/representation, benchmark science, fast generation, policy optimization, hives, and artifact stewardship. Planning now includes ReAct, Tree of Thoughts, PDDL, SHOP2, Integrated TAMP, behavior trees, GOAP/F.E.A.R., and AutoGen; retrieval/context now includes RAG, Lost in the Middle, MemGPT, LongBench, and RULER; formal methods now includes proof-carrying code, TLA+, Lean theorem proving, Dafny, and Reluplex; routing/MoE now includes sparse MoE, GShard, Switch Transformers, Expert Choice Routing, Mixtral, and an MoE-in-LLMs survey; compression now includes Deep Compression, LoRA, knowledge distillation, GPTQ, and QLoRA; benchmark science now includes MMLU, BIG-bench, HELM, GPQA, SWE-bench, and LiveBench as comparison vocabulary. | Coverage is still selective; PlanForge-translation comparison, provenance-aware RAG, context-compression evaluation, runtime assurance, proof-assistant adequacy, deployment model checking, router evaluation, task-level routing, routing-specific modular-agent orchestration, program synthesis, residual coding, artifact-utility metrics, hidden-test operations, saturation analysis, contamination audits, benchmark-gaming, and evaluator-gaming work remain queued. | Add citation-normalized source records and source notes only after reading the sources, without turning external comparison vocabulary into support-state promotion. |
