...
| Chapter body words | 185,053 | The book is already full-length; the main risk is not shortness. |
| Raw chapter-file words | 192,670 | Live scaffolding adds useful AI/research overhead beyond the reader spine. |
...
| Core claim support states | 54 at `argument` | The book is still conservative. The v1.0 evidence-transition pilot now records thirteen accepted no-change decisions; no upward support transition exists. |
...
| P0 | Claim-to-mechanism support review | 461 mappings are exact and passage-reviewed, with thirteen accepted no-change decisions now preserving support boundaries for book-method, control, fast-generation, efficiency, and resource-accounting claims. | No accepted evidence transitions have promoted claims above `argument`. | Evidence transition records for a small number of narrow claims, or explicit decisions to keep them at `argument`. |
