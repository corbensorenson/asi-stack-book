...
| Core claim support states | 54 at `argument` | The book is still conservative. The v1.0 evidence-transition pilot now records twenty-six accepted no-change decisions, twenty-eight explicit no-promotion decisions, two bounded `synthetic-test-backed` non-core transitions, and one bounded `prototype-backed` imported Circle receipt transition; no chapter core support transition exists. |
...
| Reader spine | 47 chapters, minimum 2040 words | The generated reader source is structurally substantial. |
| Reader continuity audit | 47 chapters, 1 high-priority and 3 medium-priority heuristic review rows | The audit gives Phase 2 a deterministic human-review queue without claiming release readiness; `docs/reader_continuity_review.md` records manual decisions for earlier medium rows while `docs/reader_continuity_audit.md` remains the current queue. |
| Human Reading Path bridges | 47, minimum 170 words | Every chapter has a human-entry bridge. |
...
| Audio script readiness | 52 script files generated for review | The check confirms script generation, not audio existence or review. |
