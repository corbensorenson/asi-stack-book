...
- Reader-only major-version prose deltas must live in the tracked semantic overlay source under `editions/reader_overlays/`, not in generated reader manuscripts or generated delta reports. Zero active overlays are acceptable when no reader-only delta exists. `reader_delta_report.md` is reviewed to understand the derivation, operation digests, zero-operation note, and generated before/after excerpts when active overlays exist; it is regenerated after overlay or canonical-source edits and is not an editable patch file.
- The normal-reader review queue must stay synced through `editions/reader_manuscript/v1_0/chapter_review_matrix.json` and `docs/reader_chapter_review_matrix.md`. `scripts/sync_reader_chapter_review_matrix.py --check` should pass after chapter, overlay, or review-decision changes, and the matrix should retain release blockers until full chapter review, artifact review, and release-record work actually happen.
- The normal-reader format review queue must stay synced through `editions/reader_manuscript/v1_0/format_review_matrix.json` and `docs/reader_format_review_matrix.md`. `scripts/sync_reader_format_review_matrix.py --check` should pass after reader-format render, structural inspection, application/e-reader inspection, PDF layout review, or release-record status changes, and the matrix should retain format blockers until artifact review and release-record work actually happen.
- Generated reader chapters must strip each live chapter's raw core-claim marker and repeated support boilerplate while preserving the claim text and a compact plain-language support-state boundary inline with the Core Claim sentence, so the Human view and reader releases cannot become more confident than the live evidence state after scaffold stripping. The generated reader spine should also read as direct manuscript prose: `scripts/validate_reader_spine.py --check` rejects live-only terms, internal-tool phrasing such as `Codex workflow`, scaffold labels such as `source crosswalk`, hyphenated source-review jargon such as `source-note`, self-referential reader meta-phrases such as `this chapter` and `the chapter`, repeated evidence-boundary paragraph openers, missing or generic reader Handoff continuity, and sections that fall below reader-facing word/prose-paragraph floors after stripping.
...
- Chapter `Problem` sections should give both humans and research agents a substantive entry point by naming the stakes, boundary, and failure pressure for the mechanism. `scripts/validate_chapter_dod.py` requires at least 130 words in every manifest chapter `Problem` section, and `scripts/validate_repeated_prose.py` rejects generic meta-openers such as `The book needs a place`, `The book needs`, `The stack needs`, `The ASI Stack needs`, and `The problem is`.
- Reader-visible prose should name the chapter's actual boundary instead of leaning on generic practical-purpose formulas. `scripts/validate_repeated_prose.py` rejects `The practical point is`, `The practical test is`, `The practical rule is`, `The practical purpose is`, and `The practical problem is`.
- Reader-visible causal transitions should name the actual layer, record, or authority change rather than relying on generic explanation scaffolding. `scripts/validate_repeated_prose.py` rejects `This is why` and `This matters because`.
- Reader-visible prose should name the concrete layer, record, gate, artifact, or accounting surface that carries an obligation instead of falling back to generic stack-level phrasing. `scripts/validate_repeated_prose.py` rejects `The stack should`.
- Record-bearing prose should name the actual ledger, receipt, packet, card, admission record, or claim record carrying a requirement instead of using a generic record obligation. `scripts/validate_repeated_prose.py` rejects `The record should`.
- Failure-mode prose should name the mitigation, governance control, protocol, residual, downgrade, or continuity path directly instead of repeating a generic response formula. `scripts/validate_repeated_prose.py` rejects `The response is` and `Each failure should`.
- Summary and transition prose should name the actual bridge, discipline, lifecycle, or artifact path instead of opening with a generic result formula. `scripts/validate_repeated_prose.py` rejects `The result is`.
- Chapter `Why existing approaches are insufficient` sections should make the insufficiency concrete enough for human readers to understand why the proposed layer is necessary rather than decorative. `scripts/validate_chapter_dod.py` requires at least 130 words in every manifest chapter insufficiency section, and `scripts/validate_repeated_prose.py` rejects the generic opener `The insufficiency is`.
...
- Chapter `Mechanism` and Core Claim support prose should carry the real architecture of the layer, not just a short list or diagram placeholder. `scripts/validate_chapter_dod.py` requires at least 300 words in every manifest chapter mechanism section, and `scripts/validate_repeated_prose.py` rejects generic evidence-boundary formulas such as `None of those passages show`, `The reviewed passages sharpen the`, and `The passage-reviewed mappings support discussion`.
- Chapter `Interfaces` sections should be usable as systems handoff contracts, naming the record surface, consumers, refusal path, and evidence boundary where relevant. `scripts/validate_chapter_dod.py` requires at least 130 words in every manifest chapter `Interfaces` section, and `scripts/validate_repeated_prose.py` rejects generic interface phrasing such as `The interface is the`, `The interface is a`, `The interface is an`, `The public schema now records`, `The interface should`, `The interface should also record`, `The interface should distinguish`, `The interface should expose`, `The interface should carry`, `The interface should also`, and `The contract should also`.
- Chapter `Invariants` and `Failure modes` sections should be substantial enough to name the preserved boundary and the concrete ways the mechanism can fail. `scripts/validate_chapter_dod.py` requires at least 110 words in both `Invariants` and `Failure modes` for every manifest chapter, and `scripts/validate_repeated_prose.py` rejects generic invariant and failure-section openers such as `Another invariant is`, `A second invariant is`, `The strongest invariant is`, `The key invariant is`, `The invariant is`, `The subtle failure is`, `The subtle failure mode is`, `Another failure is`, and `Each failure should`.
- Chapter `Minimum Viable Implementation` sections should identify the smallest honest artifact, fixture, trace, schema, or validation slice that can begin the idea without implying capability, safety, benchmark, or runtime success. `scripts/validate_chapter_dod.py` requires at least 125 words in every manifest chapter MVI section, and `scripts/validate_repeated_prose.py` rejects known MVI formulas such as `The next useful implementation step is`, `The next useful fixture set is`, `The next useful fixture is`, `The fixture validates`, `The fixture is not`, `The fixture is only`, `Passing the fixture`, `Passing schema validation only`, `That would`, `without claiming`, `does not prove`, `do not prove`, `proves only`, `prove only`, `cannot prove`, `The accompanying Lean module is`, `The Lean predicates do not`, `These proofs do not implement`, and `The Lean coverage stays at`.
- MVI prose should name the concrete first fixture, ledger, checklist, evaluation, or build slice directly rather than opening with a generic minimum-obligation formula. `scripts/validate_repeated_prose.py` rejects `The minimum should`.
- Chapter `Mature Research Target` sections should describe the mature product-level endpoint without implying that it already exists. `scripts/validate_chapter_dod.py` requires at least 200 words in every manifest chapter mature-endpoint section, and `scripts/validate_repeated_prose.py` rejects known mature-endpoint or support-boundary formulas such as `Operating mechanism:`, `At maturity,`, `The mature version is`, `The mature version of`, `The mature product surface would include:`, `The final product surface would include:`, `would expose:`, `The operational contract is`, `Support should stay at`, `At that mature boundary`, `In that final product`, `Failure closure:`, `detect and route failure modes such as`, `This is a target architecture, not a current-result claim`, `remains a target architecture, not a current-result claim`, `It remains beyond the chapter's present support state`, and `The support state remains argument`.
- Reader-visible prose should name the control mechanism rather than leaning on the reusable `keeps ... honest` cadence. `scripts/validate_repeated_prose.py` rejects that construction across chapter prose.
