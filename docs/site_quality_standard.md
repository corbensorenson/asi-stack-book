...
- Reader-only major-version prose deltas must live in the tracked semantic overlay source under `editions/reader_overlays/`, not in generated reader manuscripts or generated delta reports. `reader_delta_report.md` is reviewed to understand the derivation; it is regenerated after overlay or canonical-source edits.
...
- Failure-mode prose should name the mitigation, governance control, protocol, or continuity path directly instead of repeating a generic response formula. `scripts/validate_repeated_prose.py` rejects `The response is`.
