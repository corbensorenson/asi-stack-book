...
- **External-anchoring depth is uneven** (not presence — all 44 pass the book's positioning gate). Foundational chapters are *lightly* anchored: `capability-replacement` (1 `ext_`), `failure-modes` (2), `efficient-asi` (3), `system-boundaries` (3). They discuss the relevant external concepts in prose (canary/rollback, failure taxonomies, efficient inference, capability security) but anchor to only 1–2 tracked external records. For the masterwork standard, deepen the external anchoring in these foundational chapters — don't add citations where they already engage, but connect the concepts they discuss to tracked `ext_` records and name the specific prior art.
- **Proof coverage is uneven** (theorem *count*, not quality — the low-count chapters have real, sound theorems). Broad coverage: Authority (28), EvidenceStates (24), Efficiency (23), FailureModes (22), StackBoundaries/SelfImprovement (21). Narrow coverage for their importance: IntentContracts (8), StableCapabilityFields (9), Replacement (10). SCF and intent are central primitives whose formal *surface* deserves more scenarios/negative cases.
...
- **Weakness 1 (calibrated):** lightly anchored (3 `ext_` records) for a chapter *about efficient capability*. It reasons about routing/compression/verification cost in its own terms but doesn't connect to the efficient-inference / MoE / routing / speculative-decoding literature the book engages heavily in ch. 29 — a missed cross-link, not a missing engagement.
- **Weakness 2:** Beyond-SOTA still opens "Efficient ASI is best understood as an operating system for cognition" — the one surviving instance of the templated "is best understood as an operating system" cadence.
- **Actions:** (a) import the ch. 29/38 external efficiency literature into this chapter's Problem/Beyond-SOTA; (b) rewrite the Beyond-SOTA opener into chapter-specific prose.
...
- **Weakness:** **proof-light (IntentContracts, 8 theorems) and test-light (5 impl / 5 planned)** for a foundational chapter that everything downstream depends on. The Beyond-SOTA restates the core. This is a central chapter carried mostly by argument.
- **Actions:** (a) deepen the proof (intent→contract preservation, ambiguity-forces-clarification as derived invariants); (b) implement the 5 planned tests; (c) engage the instruction-following / preference-elicitation / RLHF-intent literature.
...
**Tier: A− (concrete, well-tested).** "Reversibility is the trust boundary" (human version) is memorable and correct. The replacement-as-transaction frame is strong and concrete. Tests 9/0, proof Replacement (10).
- **Weakness (calibrated):** the chapter *discusses* canary, rollback, and monitor concepts heavily in prose, but tracks only **1 `ext_` record** — so it's the most lightly *anchored* chapter, not the most "uncited." Its whole thesis is the AI-governance analog of canary / blue-green / progressive delivery, yet it never names that external prior art or connects to it.
- **Actions:** connect the deployment/MLOps prior art it already echoes (canary, progressive delivery, feature flags, regression-gating) to tracked `ext_` records and name it explicitly — a depth upgrade, since the concepts are already in the prose.
...
- **External-anchoring depth is uneven** (all positioned; the spread is depth): deeply anchored (virtual-context-abi 27, context-transactions 24, verification-bandwidth 14, planning 11) vs. lightly anchored (procedural-memory 1, labor-os 2, intent-execution 2, claim-ledgers 3). The lightly-anchored chapters engage their concepts in prose; deepen the tracked anchoring, don't "add citations."

## 13. Intent-to-Execution Contracts *(merged with command-contracts-and-semantic-interfaces)*
**Tier: B+ (clean merge, solid, lightly anchored).** "Command-contract operating spine… one semantic control language before work becomes executable" is a good frame; the merge reads as one chapter. IntentToExecution (11), tests 7/2. Human version is strong and concrete.
- **Weakness:** ext 2 (the typed-workflow / API-contract / DSL / structured-output literature is directly relevant); the "three lanes" scaffold is mechanical.
- **Actions:** engage structured-command / typed-workflow literature; dissolve the "lanes" framing into prose.
