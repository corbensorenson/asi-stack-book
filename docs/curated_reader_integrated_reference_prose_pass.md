...
Curated reader file:
`editions/reader_manuscript/v1_0/chapters/integrated-reference-architecture.qmd`

## Reader Promise

A human reader should leave this chapter understanding that the ASI Stack only
comes together when every layer participates in one inspectable trace. The
integrated reference architecture is a trace contract: it must preserve parent
artifacts, authority deltas, evidence deltas, residual deltas, stop conditions,
validation commands, source references, support-state effects, and non-claims
across both happy paths and blocked paths.

## Scope

This pass turns the generated reader baseline for Integrated Reference
Architecture into a first booklike curated reader draft. It is a reader-prose
derivative only.
...
- Reorganized the mechanism around Reference Trace Records, parentage,
  authority deltas, evidence deltas, residual deltas, stop conditions, and
  downstream rejection.
- Preserved the external orientation to planning/execution, distributed
  conditional computation, formal/model-checking, and runtime-assurance lanes
  without claiming an integrated local implementation.
