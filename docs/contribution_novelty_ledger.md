...
| Pattern language for governed cognition | `owner_spine_integrated_not_novelty_proven` | `ext_model_cards_2019`; `ext_datasheets_datasets_2021`; `ext_factsheets_ai_services_2019`; `ext_nist_ai_rmf_1_0_2023`; `ext_frontier_ai_regulation_2023` | Turns claim movement, authority, artifacts, evidence receipts, residual custody, and rollback into a reusable systems pattern instead of separate reporting practices; the opener now names it, Integrated Reference Architecture applies it at trace scale, and Living Book Methodology applies it to book changes. | Broader inline local-delta pass through non-owner chapters and outline entries; no standalone audit report by default. |
