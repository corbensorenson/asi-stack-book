...
| Residual honesty | `residual_conservation_fixture_backed_not_empirical` | `ext_helm_2022`; `ext_goodhart_variants_2018`; `ext_benchmark_contamination_2023`; `ext_model_evaluation_extreme_risks_2023`; `ext_nist_ai_rmf_1_0_2023` | Makes displaced burden a ledgered artifact that can block promotion even when visible metrics improve; the first residual-conservation fixture now accepts visible accepted/deferred/discharged residual records and rejects hidden, erased, unowned, support-promoting, and zero-residual-overclaim controls. | Real residual-ledger trace from a compact-generation, resource, or readiness workflow with owner handoff, discharge review, external/live workload context, and no-promotion decision. |
