...
- Chapter-core support effect: none; all 54 chapter core claims remain
  `argument`.
...
| Support-state discipline | `exact_non_core_refutations_recorded_chapter_core_unmoved` | `ext_model_cards_2019`; `ext_datasheets_datasets_2021`; `ext_ml_reproducibility_program_2021`; `ext_proof_carrying_code_1997`; `ext_agm_belief_revision_1985`; `ext_truth_maintenance_system_1979`; `ext_assumption_based_tms_1986` | Joins reporting, reproducibility, proof receipts, and belief revision into public claim-state transition discipline. The bounded bridge records synthetic narrowing, downgrade, and refutation routes, and the post-v2.3 cycle now records two exact live non-core QCSA refutations without chapter-core movement or support promotion. | A chapter-core narrowing, demotion, deprecation, or refutation only when future evidence requires it; do not manufacture one. Current artifacts include the transition bridge and `evidence_transitions/post_v2_3/qcsa_exact_synthetic_matched_advantage_refute.json` plus `evidence_transitions/post_v2_3/qcsa_active_questions_exact_fixture_value_refute.json`. |
