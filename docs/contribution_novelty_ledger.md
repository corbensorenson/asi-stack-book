...
|---|---|---|---|---|
| Verification bandwidth | `promising_not_defended` | `ext_lost_in_middle_2023`; `ext_longbench_2023`; `ext_ruler_2024`; `ext_helm_2022`; `ext_livebench_2024` | Frames verification capacity as an architectural budget that must be routed, escalated, and recorded before claims can move. | Record-level verification-capacity model with negative controls and Lean/Python agreement. |
| Pattern language for governed cognition | `needs_delta_audit` | `ext_model_cards_2019`; `ext_datasheets_datasets_2021`; `ext_factsheets_ai_services_2019`; `ext_nist_ai_rmf_1_0_2023`; `ext_frontier_ai_regulation_2023` | Turns claim movement, authority, artifacts, receipts, and rollback into a reusable systems pattern instead of separate reporting practices. | Chapter-by-chapter pattern-delta audit. |
...
| Residual honesty | `promising_not_defended` | `ext_helm_2022`; `ext_goodhart_variants_2018`; `ext_benchmark_contamination_2023`; `ext_model_evaluation_extreme_risks_2023`; `ext_nist_ai_rmf_1_0_2023` | Makes displaced burden a ledgered artifact that can block promotion even when visible metrics improve. | Residual-conservation fixture with accepted, deferred, hidden, and erased residual cases plus Lean invariant. |
