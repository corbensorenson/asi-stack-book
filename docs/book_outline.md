...
- Codex test: Resource workload-quality probe - Measure a scoped Resource workflow trace review route against a broader Resource Economics replay baseline, reject a cheaper no-op success-text route, record residuals and non-claims, and preserve the no-transition boundary (implemented; passing via `python3 scripts/validate_resource_workload_quality_probe.py`; local single-run repository task only; no stable-speedup, deployed-scheduler, model-quality, economic-outcome, external-review, or support-state-transition claim).
