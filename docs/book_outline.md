...
- Implemented executed grant-to-effect refinement: `AsiStackProofs.AuthorityEffectRefinement` models exact grant, principal, operation, target, ceiling, epoch, expiry, approval, dispatch, bounded-use effect, independent observation, revocation, and rollback custody. Its exact thirty-one-theorem surface preserves the state invariant over arbitrary runs, exposes trace validity, composes event batches, makes rejection state-noninterfering, clears effect accounting on exact rollback, preserves revoked IDs, and rules out approval, dispatch, or effect use by a revoked grant in every successful suffix. `python3 scripts/validate_authority_effect_refinement.py` recompiles that surface and independently checks 3 reachable traces/20 events, all 20 prefixes, 23 batch splits, 6 authority fixtures, 1 executed local effect, 2 pre-effect denials, 5 revocation entries, 9 governed repository scenarios, and 50/50 state-noninterfering semantic-mutation rejections. Support-state effect remains `none`; authentic identities/receipts, concurrent or distributed revocation, deployed enforcement, production security, reproduction, and transfer remain open.
