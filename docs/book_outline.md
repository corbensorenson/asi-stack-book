...
- Implemented no-promotion Project Theseus currentness import: `python3 scripts/validate_theseus_t0a_architecture_closure_currentness_import.py` separates completed historical `T0` from active successor `T0A` at clean Theseus `main` commit `653401de`, verifies that commit is published on `origin/main`, preserves the historical 123-artifact freeze as an immutable control, records 99 unchanged and 24 changed bound artifacts, records 7/7 current CPU/governance replay passes and 13/14 valid accelerator receipts, preserves the final optimizer-matched shard's guarded stop after reclaimable memory fell below its 4,096 MiB live reserve with zero swapout growth, and binds the failed attempt as non-qualifying calibration evidence for a prospective 6,144 MiB launch floor derived from the live reserve, 1,529.766 MiB observed peak, and a 512 MiB margin. It keeps `T1` blocked by `T0A` and rejects eighteen authority/currentness/resource/denominator/support mutations. It establishes no successful replay, replacement freeze, optimizer result, training readiness, behavior, capability, support, or release result.
