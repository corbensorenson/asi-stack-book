...
- No Lean target is assigned for the GitHub Pages CI attestation because the artifact is an external service-status record rather than a finite in-repo transition predicate; the formal side remains the existing artifact-record, receipt-faithfulness, repository-audit, challenge, live-attestation, randomized-attestation, and epistemic-TCB bridges.
