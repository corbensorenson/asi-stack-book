...
- Implemented no-promotion Project Theseus currentness import: `python3 scripts/validate_theseus_t0a_architecture_closure_currentness_import.py` separates completed historical `T0` from active successor `T0A` at clean Theseus `main` commit `24955417`, verifies that commit is published on `origin/main`, preserves the historical 123-artifact freeze as an immutable control, records 102 unchanged and 21 changed bound artifacts, records 7/7 current CPU/governance replay passes and 13/14 valid accelerator receipts, preserves the final optimizer-matched shard's guarded stop after reclaimable memory fell below its 4,096 MiB live reserve with zero swapout growth, keeps `T1` blocked by `T0A`, and rejects sixteen authority/currentness/denominator/support mutations. It establishes no replacement freeze, optimizer result, training readiness, behavior, capability, support, or release result.
