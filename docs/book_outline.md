...
- Implemented book-gate harness: `python3 scripts/validate_cyclic_memory_contracts.py` validates 3 valid and 6 expected-invalid synthetic cyclic-memory contract traces and independently reconstructs the 17-declaration Lean lifecycle: one residue-only collision, seven accepted fresh transitions, all eight trace splits, three epoch/residue/winding stale classifications routed through fallback, and eleven rejecting mutations with exact state preservation. This repository harness is not a deployed KV-cache checker, recurrence benchmark, learned-memory workload, retrieval-quality test, or long-context result.
...
classified as 1,845 derived/decomposed, 437 direct/projection, and 668
