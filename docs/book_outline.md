...
- Current minimum: two public record schemas, 3-valid/5-invalid constitutional
  ...
  effect-admission consumer under seven manifest targets. `AsiStackProofs.Alignment` now has 47 declarations and an exhaustive sixteen-pair two-predicate refinement consumer with nine admitted subset refinements, seven widening rejections, nine exact rollbacks, and one same-count/different-predicate collision. This is bounded
...
- Implemented synthetic Codex tests from both source chapters via `python3 scripts/validate_constitutional_alignment.py` and `python3 scripts/validate_agency_rights.py`; the constitutional harness compiles and independently reconstructs four accepted events plus five controls, while the agency-rights harness does the same for five correction-control events plus seven controls. Deployed constitutional alignment, affected-party standing, agency preservation, dignity preservation, consent quality, material rights usability, manipulation resistance, correction outcomes, and reviewer independence or competence remain open.
- Implemented Lean follow-through: `AsiStackProofs.Alignment` includes the finite missing-field admission router, a versioned proposal lifecycle, and a two-predicate migration refinement. Its 47 declarations cover accepted-step admissibility and exact advancement, constitutional identity/scope/proposer/ceiling custody, zero support/effect assignment, independent review before activation, conflict residualization, return to the recorded rollback version, arbitrary-run preservation, exact batch composition, one complete review-to-rollback witness, five rejecting substitutions, subset refinement, transitive composition, exact prior-set rollback, concrete widening rejection, and same-count predicate-set non-identifiability. The independent consumer exhausts all sixteen prior/candidate pairs. These authored semantics do not establish predicate meaning or completeness, moral correctness, deployed alignment, rights usability, reviewer competence, effect-complete rollback, consent quality, support-state promotion, or runtime policy behavior.
