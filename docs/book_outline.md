...
- Implemented no-promotion Project Theseus currentness import: `python3 scripts/validate_theseus_t0a_architecture_closure_currentness_import.py` separates completed historical `T0` from active successor `T0A` at clean Theseus `main` commit `9e545cf1`, verifies that commit is published on `origin/main`, preserves the historical 123-artifact freeze as an immutable control, records 99 unchanged and 24 changed bound artifacts, and records 7/7 current CPU/governance replay passes and 13/14 valid accelerator receipts. It preserves the prior optimizer-matched shard's guarded live-reserve stop with zero swapout growth as non-qualifying calibration evidence for a prospective 6,144 MiB launch floor derived from the 4,096 MiB live reserve, 1,529.766 MiB observed peak, and a 512 MiB margin. It also records the latest exact preflight: 3,874.8 MiB was available against that floor, so the child did not start and no model result exists. It keeps `T1` blocked by `T0A` and rejects twenty authority/currentness/resource/chronology/denominator/support mutations. It establishes no successful replay, replacement freeze, optimizer result, training readiness, behavior, capability, support, or release result.
...
- Mechanism: Keep completed historical `T0` distinct from active successor `T0A`; a changed source tree may retain the old package as a control but cannot inherit its training authority, so `T1` waits for a guarded replay, replacement content-addressed freeze, and current-book readiness rerun.
