...
| `lean:failure.recovery.closed_loop_refinement` | `AsiStackProofs.FailureRecoveryRefinement` | Every successful finite recovery run preserves exact nine-field incident/version identity and zero support/external-authority assignment, adds exactly one receipt per accepted event, exposes a valid trace, and composes across event batches; the five-stage lifecycle guards readmission and re-isolates a modeled recurrence, while the recompiling consumer rejects 31 mutations. | implemented |
