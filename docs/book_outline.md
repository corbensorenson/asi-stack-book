...
Proof boundary: the seven public targets group fifty-one theorem declarations. The two valid-summary projections are retired, and the independent scheduler-state and runtime-replan consumers bind to the retained admission, delta, and reachable-lifecycle families. The new model proves control-state reachability, arbitrary-run invariants, rejecting countermodels, denial noninterference, and two vertical refinements over trusted authored fields. Index ordering is not a general proof of DAG acyclicity or dependency truth. The declarations are useful mechanized control semantics, but natural decomposition and dependency quality, context prediction, route adequacy, executable planner production, observed outcomes, causal superiority, deployed runtime behavior, reproduction, and transfer remain open.
...
- Implemented generated audit: `docs/proof_depth_classification.md` records proof-depth classification. Current proof-depth snapshot: 330 proof targets, 130 Lean modules, 2828 theorem declarations, 1764 derived/decomposed, 432 direct/projection, 632 unknown/mixed, and 2/2 safety-critical chapter classifications present.
