...
- Implemented generated audit: Appendix E summarizes all 205 proof targets by status, triage class, and recommended route from `proofs/proof_triage.json`.
- Implemented generated audit: `docs/proof_artifact_audit.md` checks that all 205 proof targets are traceable through manifest, triage, Lean module, root import, chapter hook, limitation prose, and Appendix E coverage; this is not a semantic adequacy review.
- Implemented generated audit: `docs/proof_depth_classification.md` records proof-depth classification. Current proof-depth snapshot: 205 proof targets, 54 Lean modules, 1013 theorem declarations, 824 derived/decomposed, 185 direct/projection, 4 unknown/mixed, and 5/5 safety-critical chapter classifications present.
...
- Implemented reader release-candidate bridge: `python3 scripts/validate_reader_release_candidate_bridge.py`, result record `experiments/reader_release_candidate_bridge/results/2026-07-05-local.json`, and Lean route fixture `lean:living_book.methodology.reader_release_candidate_bridge` check that local reader artifacts cannot clear missing accessibility, audio, or release-approval blockers. The actual blocked curated candidate routes to accessibility review rather than release approval; synthetic controls cover audio-missing, approval-missing, support-promotion, and missing-non-claim overclaims. This is release-boundary discipline only, not reader artifact approval, not audiobook approval, and not support-state promotion.
...
- Implemented Codex test: Reader release-candidate bridge, via `python3 scripts/validate_reader_release_candidate_bridge.py`, with the current blocked curated candidate, 3 additional valid route cases, and 5 expected-invalid controls.
