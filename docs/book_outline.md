...
| `lean:failure.recovery.closed_loop_refinement` | `AsiStackProofs.FailureRecoveryRefinement` | Every successful finite recovery run preserves exact nine-field incident/version identity and zero support/external-authority assignment, adds exactly one receipt per accepted event, monotonically accounts for incident, recovery, and recurrence events, exposes a valid trace, and composes across event batches; accepted events require a valid control state, detection requires recorded failure class and boundary and opens one residual while disabling effects and promotion, readmission requires residual discharge and restores operation, and the recompiling consumer checks all six lifecycle splits while rejecting 117 mutations. | implemented |
...
- Formal audit: the 18 current declarations retain seven finite receipt, promotion, replay, and public-gate boundary results and add eleven lifecycle results for rejected-event noninterference, protected identities and zero authority, trace composition, descendant invalidation, persistent revocation, one reachable consume-then-revoke trace, post-revocation rejection, unrelated-lineage availability, and identity and parent-link countermodels. The existing independent validator recompiles the module, checks all eight trace splits, and rejects 16/16 lifecycle mutations with exact state preservation. None proves resolution soundness, statement equivalence, semantic refinement, authenticated or deployed transport, concurrent/distributed revocation, liveness, recovery, independent reproduction, downstream utility, or transfer.
...
| `lean:circle_contracts.versioned_transport.descendant_revocation` | `AsiStackProofs.ProofCarryingContracts` | The finite versioned transport state machine preserves theorem, parent, consumer, support, and external-effect fields; rejects invalid events without state change; composes traces; and makes root revocation persistently invalidate descendants while preserving unrelated-lineage availability. | implemented |
...
- Implemented generated audit: `docs/proof_depth_classification.md` records proof-depth classification. Current proof-depth snapshot: 330 proof targets, 130 Lean modules, 2981 theorem declarations, 1869 derived/decomposed, 439 direct/projection, 673 unknown/mixed, and 2/2 safety-critical chapter classifications present.
...
330-target live manifest; a 130-module Lean workspace with 2,981 declarations
classified as 1,869 derived/decomposed, 439 direct/projection, and 673
