...
- Codex test: Resource workflow trace harness - Check deterministic multi-step workflow fixtures for selected-route cost recomputation, high-risk-first scheduler ordering, protected review overhead, displaced-cost residual ownership, physical-feasibility overclaim rejection, no-promotion boundaries, and tracked result-record summary alignment with the finite Lean fixture (implemented; passing via `python3 scripts/validate_resource_workflow_trace.py`; no deployed scheduler, physical-feasibility, model-quality, or economic-outcome claim).
