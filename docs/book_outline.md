...
Cross-chapter aggregate evidence note: the selected Theseus/Fast support lane is an executable aggregate guarded by `python3 scripts/validate_theseus_fast_support_lane.py`. Its copied `AsiStackProofs.TheseusReference` formal mirror was retired after semantic audit; the immutable result, current digest checks, no-promotion decisions, and explicit no-clean-live-replay, no-model-quality, no-generation-speed, no-useful-solution, no-support-promotion, and no-chapter-core-promotion boundaries remain.
