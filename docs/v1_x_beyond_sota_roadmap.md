...
- `AsiStackProofs.CoilAttentionMemory` now has an exact 17-declaration finite
  address/freshness/fallback/recurrence surface. It retains the four negative
  cases and adds residue-only non-injectivity, exact epoch/residue/winding
  freshness, rejection noninterference, memory/request identity and zero
  support/effect authority preservation, trace composition, stale fallback,
  and recurrence-budget closure. The independent consumer recompiles the
  module, reconstructs seven accepted transitions and all eight trace splits,
  checks three stale mismatch paths, and rejects 11/11 mutations without state
  change. This closes all three public targets only as authored finite-record
...
- `AsiStackProofs.CyclicMixers` now has finite negative-case theorems for
  ...
  blockers.
- `AsiStackProofs.BenchmarkRatchets` now has an exact 15-declaration
  registered-to-closed instrument lifecycle with identity and zero-authority
  custody, exact accepted-event receipts, rejection noninterference, batch
  composition, contamination quarantine, saturation-to-floor routing,
  missing-evidence rejection, three reachable outcomes, and absorbing closure.
  The existing consumer recompiles the module, executes six clean transitions,
  checks all seven splits, and rejects 15/15 lifecycle mutations. This closes
  all three targets only as authored finite-record invariants; natural and
  hidden workloads, construct validity, contamination detection, transfer,
  capability and safety outcomes, independent reproduction, and support remain
  blockers.
...
- The Human-AI Organizations formal tranche is complete at finite
  authored-record scope. `AsiStackProofs.HumanAIOrganizations` now contains 42
  theorem declarations: the retained five-stage assignment review plus a
  ten-stage delegation-to-remedy lifecycle proving arbitrary-run nine-field
  identity custody, support/effect non-authority, exact receipts,
  contest/remedy monotonicity, accepted traces, batch composition, and
  absorbing closure. One adverse-path witness expires authority before
  reconstruction and remedy; the independent consumer checks all ten trace
  splits, reaches all 39 routes, and rejects 156/156 lifecycle mutations. The
  public target is adequate only as a narrow finite-record invariant; field
  truth, reviewer competence, automation bias, meaningful human control,
...
|---|---|---|
| `ai-supply-chain-integrity-and-lifecycle-provenance` | The new cross-lifecycle integrity layer must prevent an AI BOM, provenance statement, or signature from becoming a generic claim of artifact security, data fitness, model safety, legal compliance, or release permission. | Formal tranche deepened without promotion: source-noted NIST C-SCRM, SLSA Build Track, OpenSSF Model Signing, and SPDX AI Profile comparators; live/Human prose; and a no-promotion disposition remain. `AsiStackProofs.SupplyChainIntegrity` now has exactly 23 declarations: seven admission routes plus a reachable bind, clean-or-critical review, admit-or-quarantine, and revoke lifecycle with arbitrary-run artifact/provenance identity, authority-ceiling, non-authority, valid-trace, composition, receipt, quarantine-exclusion, and zero-authority revocation results. The existing affected-path consumer recompiles the exact surface, retains one graph and ten fixture mutations, closes clean and critical paths, and rejects eight lifecycle controls. Event truth and deployed effects remain open. Next run a natural public-safe supply-chain workload with independently generated artifact/content, supplier, signature, advisory/applicability, propagation, quarantine, restoration, revocation, residual, privacy/rights, availability, and cost evidence before any claim about complete lineage, artifact integrity, absence of compromise, data fitness, supplier trustworthiness, legal compliance, model safety, readiness, authority, deployment, or chapter-core support. |
| `asi-is-a-stack-not-a-model` | The finite boundary contract needed a quantified inverse and arbitrary-length handoff guarantee; the architectural thesis still needs integration-pressure evidence. | Formal tranche complete at finite authored-record scope: live and curated reader prose contrasts the ASI Stack against scale-only systems, generic agent loops, and compound/modular AI systems while preserving source-noted MRKL, LLM-agent, cognitive-architecture, and layered-control boundaries. The exact 34-theorem `AsiStackProofs.StackBoundaries` surface proves complete admitted-contract obligations, rejects unauthorized external action, preserves the boundary invariant and exact composition over arbitrary accepted runs, makes revocation absorbing for later authorization/dispatch/effect, and bounds every valid nonempty handoff chain by its initial source ceiling with adjacent artifact custody. `python3 scripts/validate_stack_boundary_effect_consumer.py` recompiles the surface, matches eighteen priority-ordered layer-contract routes, consumes six authority fixtures plus the contained effect and revocation traces, checks thirteen invariant prefixes and compositions, and rejects twelve semantic mutations. All three public targets are adequate finite-record invariants with support effect `none`. Remaining larger-system work belongs in Project Theseus or empirical lanes: authentic grants/receipts and target ownership, contract-change migration, integration-pressure and multi-agent traces, distributed clocks/partitions/retries, complete effect discovery, irreversible-effect handling, utility/cost comparison, reproduction, and transfer before any necessity, superiority, deployment, safety, support, AGI, or ASI claim. |
...
| `system-boundaries-and-authority` | Object-capability/confused-deputy lineage was not explicit enough despite strong authority proof coverage. | Finite formal tranche complete: external grounding names Saltzer-Schroeder protection principles, Levy capability-system boundaries, and Hardy's confused-deputy problem. The exact 31-theorem `AsiStackProofs.AuthorityEffectRefinement` lifecycle now owns all four public targets and proves one-step and arbitrary-run identity, non-authority, observation/effect, ceiling, epoch, revocation, approval, dispatch, and trace invariants; exact batch composition; persistent revoked IDs; successful-suffix exclusion of approval, dispatch, and effect under revoked grants; rejected-event noninterference; exact rollback accounting; and reachable one-use, two-use, and revocation witnesses. `python3 scripts/validate_authority_effect_refinement.py` recompiles that surface and independently checks six fixtures, three traces/20 events, every prefix and batch split, nine governed scenarios, and 50 state-noninterfering mutations. All four targets are adequate only as authored finite-record invariants, with support-state effect `none`. The earlier revocation trace and accepted no-promotion decision remain bounded cross-artifact records, not deployed enforcement. Remaining larger-system work belongs in Project Theseus or empirical lanes: authentic identity and receipts, complete mediation and observation, live adapter denial, concurrent/distributed revocation, trusted clocks and hardware roots, independently attacked tool wrappers, effect-complete rollback, utility/cost measurement, reproduction, and transfer. |
| `failure-modes-of-ungoverned-intelligence` | Stack failure vocabulary is not mapped tightly enough to established AI-safety failure taxonomies. | Closed by artifact for the reviewer weakness in the 2026-07-05 burn-down reconciliation: source-noted external grounding covers `ext_concrete_ai_safety_2016`, `ext_goal_misgeneralization_2022`, `ext_learned_optimization_risks_2019`, `ext_optimal_policies_power_2019`, and `ext_goodhart_variants_2018`, while the validator-guarded live and curated reader bridge maps the chapter taxonomy to established accident, goal-misgeneralization, learned-optimization, power-seeking, Goodhart, side-effect, safe-exploration, reward-hacking, and evidence-smoothing families without promotion. The deterministic detector fixture retains two valid synthetic incidents and seven expected-invalid controls. The exact 27-theorem `AsiStackProofs.FailureRecoveryRefinement` now adds arbitrary successful-run nine-field identity custody, zero support/external-authority assignment, exact receipts, monotone incident/recovery/recurrence accounting, valid traces, batch composition, rejected-state preservation, valid control-state admission, failure-class and boundary gates, residual opening and discharge, effect and promotion disablement during recovery, guarded readmission, and recurrence re-isolation; `python3 scripts/validate_failure_recovery_refinement.py` recompiles that surface, checks all six lifecycle splits, and rejects 117/117 state-preserving mutations. This strengthens only the bounded recovery target; the chapter's five-target proof class remains `useful but too narrow` and support remains `argument`. Follow-on work belongs in Project Theseus or empirical lanes: deployed fault injection and detector traces, actual effects and containment observation, detector/reviewer independence, runtime authority/context/evaluator/claim-verifier behavior, remediation and restart efficacy, recurrence and escape measurement, prevention evidence, reproduction, and transfer. |
...
| `constitutional-alignment-substrate` | Source lineage and finite predicate records must not be mistaken for moral or deployed alignment. | Partially executed: the Metaphysics lineage boundary audit keeps metaphysical and consciousness language in lineage, interpretation, speculation, or design-rationale lanes. The 47-declaration `AsiStackProofs.Alignment` surface now adds a finite two-predicate migration refinement: accepted changes are subset refinements, compose transitively without reintroducing removed predicates, store and restore the exact prior set, reject widening, and expose scalar-count non-identifiability. `python3 scripts/validate_constitutional_alignment.py` recompiles the module, reconstructs the existing lifecycle, and exhausts all sixteen prior/candidate pairs with nine admitted subset refinements, seven widening rejections, and nine exact rollbacks. This proves authored finite representation consequences only. Remaining work: predicate meaning and completeness, legitimate authorship, external safety review, deployed predicate behavior, rights-usability tests, reviewer quality and independence, effect-complete rollback, conflict behavior, natural workloads, reproduction, and transfer; no support-state promotion. |
| `moral-uncertainty-and-value-conflict` | Value conflict needed a concrete rights-as-interface example and an exact proof boundary between aggregate support and preserved party-specific dissent. | Partially executed: the Contestability worked example fixture remains guarded by `python3 scripts/validate_contestability_worked_example.py` for `contestability://synthetic-care-memory-export-001`. The 47-declaration `AsiStackProofs.ValueConflict` surface now adds a finite three-slot stakeholder-profile boundary: two distinct party-specific profiles collide under scalar support count, scalar aggregation is non-injective, no count-only decoder recovers every profile, and accepted modeled receipts preserve the full supplied profile and dissent payload exactly. `python3 scripts/validate_value_conflicts.py` recompiles the module, reconstructs the four-event lease and six controls, enumerates all eight finite support profiles, detects aggregation collisions, and rejects aggregate-equivalent dissent substitution plus missing standing. This proves authored finite representation consequences only and creates no support transition. Remaining book-local work is only a stronger semantic model if one can be stated honestly; stakeholder completeness, legitimate standing and representation, representative aggregation, reviewer competence and independence, real rights-interface usability, legal/institutional review, safe-fork review, deployed expiry and escalation, runtime authority enforcement, natural workloads, causal ablations, reproduction, and transfer belong to empirical or Project Theseus work before stronger claims. |
...
| `planning-as-a-control-layer` | Central merged chapter now has a substantive reachable control model, but natural planner quality and deployed system evidence remain missing. | Partially executed: the retained plan-control, plan-graph admission, scheduler-state, runtime-replan, and PlanForge routes are now joined by a 21-result reachable Planning lifecycle over command admission, node readiness, job lowering, feedback, scoped replanning, and blocking. Lean proves one-step and arbitrary-run authority, ordering, version, stop, dispatch, and residual invariants; one complete seven-event replan witness; eleven rejecting transition countermodels; denial noninterference; and admitted-plan/job-lowering refinement into `IntentExecutionRefinement`. The Planning scheduler-state probe remains a deterministic synthetic scheduler-state fixture: `python3 scripts/validate_planning_scheduler_state_probe.py` requires the exact 48-declaration Planning surface and independently checks two accepted/seven rejected records. The Planning runtime-replan delta audit remains a deterministic synthetic runtime-replan delta audit: `python3 scripts/validate_planning_runtime_replan_delta.py` requires the replan subset and checks two accepted/nine rejected deltas. Both accepted no-promotion decisions remain unchanged. The model trusts authored decomposition, dependency, context, adequacy, route, receipt, authority, stop, and residual fields. Remaining work belongs primarily in Project Theseus: natural multi-planner decomposition and dependency quality, context-demand prediction, selected-tier and route adequacy, scheduler usefulness/optimality, complete cost-quality denominators, live feedback, effect-tested stops and recovery, deployed replanning, causal ablations, reproduction, and transfer. No support-state promotion follows from this formal tranche. |
...
| `privacy-data-rights-and-information-flow-governance` | The admitted chapter establishes a distinct purpose-bounded information-use and executable-rights owner with nine reviewed sources, a finite transaction, rejecting controls, and a frozen campaign, but no natural privacy attack, rights workflow, deletion propagation, influence analysis, legal determination, independent reproduction, or transfer result has run. | Formal tranche deepened without promotion: `AsiStackProofs.PrivacyInformationFlow` now has exactly 27 declarations, retaining eleven routes and adding a reachable purpose/minimization/map/evaluation/rights/use/revocation/deletion-record lifecycle with arbitrary-run transaction identity, authority-ceiling, non-authority, receipt, valid-trace, composition, stage-order, and bounded known-copy disposition results. The independent consumer recompiles the exact surface, retains the twelve-surface transaction and twenty-six fixture mutations, closes one eight-event witness, and rejects eleven lifecycle controls. All event and known-copy fields remain authored; no privacy, lawful-purpose, consent, rights-fulfillment, deletion-in-fact, total-erasure, or compliance claim follows. Implement the six-arm natural campaign only after all fifteen competence gates freeze: qualify strong extraction, confidence and label-only membership, linkage, cross-user, and purpose-bypass attacks with positive controls; exercise access, correction, export, restriction, deletion, recipient, backup, descendant, and exception paths; measure utility, privacy, rights completion, latency, residual influence, operator work, and total cost; then pursue independent replay and cross-model, system, operator, and policy-module transfer before any bounded support review. |
...
| `circle-calculus-and-proof-carrying-ai-contracts` | The chapter abstracts away from Circle's most concrete proved results. | Partially executed: the chapter surfaces source-verified, public-safe Circle evidence for commit `63b0f511`, the pinned rope receipt, theorem IDs, fingerprints, consumer gate, and non-claims; archives nine contracts and four policy receipts with five rejected controls and a no-change decision; and now exposes `lean:circle_contracts.versioned_transport.descendant_revocation`. The exact 18-declaration module adds a seven-transition finite transport lifecycle with rejected-event noninterference, ten protected identity/authority fields, exact trace composition, persistent root and descendant revocation, a reachable consume-then-revoke witness, unrelated-lineage availability, and identity/parent-link countermodels. `python3 scripts/validate_circle_contract_pack_archive.py` recompiles the module, reconstructs seven accepted transitions, checks all eight trace splits, and rejects 16/16 lifecycle mutations with exact state preservation. This proves authored local semantics only: it does not resolve external theorem IDs, establish statement or semantic refinement, authenticate or deploy transport, invalidate real distributed caches, prove service liveness/recovery, reproduce independently, validate downstream utility, promote support, or change the chapter core. Remaining work: clean Circle replay from this repo, a real theorem resolver and semantic bridge, authenticated multi-consumer transport, deployed revocation/fallback/recovery, workload/baseline evidence, independent reproduction, and heterogeneous transfer. |
...
| `executable-specifications-and-lean-proof-envelope` | Proof-governance chapter needed to show the book's real proof layer more concretely. | Partially executed: added Proof-depth surface synchronization guarded by `python3 scripts/validate_proof_depth_surface.py`; the live chapter, reader chapter, outline, and roadmap now expose the current validator-reported proof-depth snapshot. Current proof-depth snapshot: 330 proof targets, 130 Lean modules, 2731 theorem declarations, 1712 derived/decomposed, 431 direct/projection, 588 unknown/mixed, and 2/2 safety-critical chapter classifications present. The chapter explains derived/decomposed versus direct/projection labels and projection-only traceability, using the book's Lean layer as a worked proof-etiquette example. A reader-only overlay now replaces dense Interfaces and Minimum Viable Implementation proof-envelope inventory prose in Human view and generated reader output, dropping the generated-reader heuristic score from 9 to 7 while preserving the canonical live evidence table, rendered mechanism diagram, and support boundaries. The semantic proof adequacy review is validator-checked against the current manifest and no-promotion boundary language. This does not prove broad semantic adequacy, does not validate source interpretation, does not prove deployed enforcement, and does not promote proof-envelope support. Remaining work: upgrade high-value projection hooks where the proof model is too shallow and add consumer-gate receipts before stronger formal-methods claims. |
