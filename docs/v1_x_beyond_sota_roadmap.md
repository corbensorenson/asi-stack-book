...
Quarto site, and ten narrow non-core evidence transitions. The current v1.x
...
| P0 | Formal proof count is no longer the bottleneck; executable-model depth is. | `docs/proof_depth_classification.md` now records 1030 theorem declarations, 185 direct/projection-style, 841 derived/decomposed, 4 unknown/mixed, and 106 safety-critical theorem declarations after the Resource aggregate flagship invariant pass, Theseus/Fast support-lane aggregate bridge, Capability replacement identity-sequence bridge, Resource governance-tax trade-off bridge, receipt repository audit bridge, claim-state transition bridge, Circle public consumer-gate fixture bridge, residual-ledger storage/replay bridge, Artifact Graphs live attestation bridge, Artifact Graphs randomized artifact attestation audit bridge, Theseus simulation-fidelity receipt-suite bridge, Theseus module definition-of-done import bridge, and Theseus project-registry import bridge. The first anti-projection sweep is real, but most theorems still verify finite records and decision routes rather than traces of a running system. | Stop optimizing for theorem-count growth. Pick one proof/evidence bridge and prove a property over transitions or fixture equivalence, then run the same cases through the executable harness where possible. |
...
| P2 | The project has many ledgers but still few promotions. | The v1.0.0 release was honest, and the current 44 core claims still remain `argument`; ten narrow non-core claims moved upward, including the finite synthetic load-smoothing selector, local empirical scoped-route selector, Theseus artifact-retention import, Theseus governance-rights receipt-suite import, Theseus simulation-fidelity receipt-suite import, module definition-of-done import, and project-registry import transitions. | Future roadmap work should close evidence gaps, not multiply status documents. Add ledgers only when they make support-state decisions clearer or enforceable. |
...
- `python3 scripts/validate_chapter_review_burndown.py` passes and proves that
  ...
  dated blocker.
- `python3 scripts/validate_proof_depth.py` now classifies 1030 theorem
  declarations, including 841 derived/decomposed declarations and 185 direct
...
- source-to-chapter and claim-source mappings are complete for the current
  manifest;
- ten narrow non-core evidence transitions are recorded;
...
6. make the honesty system legible in 60 seconds from README, landing page, and
   Human view, and keep the ten bounded non-core evidence transitions visible
...
- Update the README, landing page, and live Human view entry path so a first-time
  ...
  - ten bounded non-core evidence transitions exist and are narrow;
...
- The generated proof-depth report records 1030 theorem declarations, 841
...
- `docs/proof_depth_classification.md` records 1030 theorem declarations, 841
  derived/decomposed theorem declarations, 185 direct/projection-style theorem
...
| `executable-specifications-and-lean-proof-envelope` | Proof-governance chapter needed to show the book's real proof layer more concretely. | Partially executed: added Proof-depth surface synchronization guarded by `python3 scripts/validate_proof_depth_surface.py`; the live chapter, reader chapter, outline, and roadmap now expose the current validator-reported proof-depth snapshot. Current proof-depth snapshot: 207 proof targets, 54 Lean modules, 1034 theorem declarations, 844 derived/decomposed, 186 direct/projection, 4 unknown/mixed, and 5/5 safety-critical chapter classifications present. The chapter explains derived/decomposed versus direct/projection labels and projection-only traceability, using the book's Lean layer as a worked proof-etiquette example. A reader-only overlay now replaces dense Interfaces and Minimum Viable Implementation proof-envelope inventory prose in Human view and generated reader output, dropping the generated-reader heuristic score from 9 to 7 while preserving the canonical AI/research details, rendered mechanism diagram, and support boundaries. The semantic proof adequacy review is now validator-checked by `python3 scripts/validate_proof_adequacy_review.py` against the 206-target manifest and no-promotion boundary language. This does not prove broad semantic adequacy, does not validate source interpretation, does not prove deployed enforcement, and does not promote proof-envelope support. Remaining work: upgrade high-value projection hooks where the proof model is too shallow and add consumer-gate receipts before stronger formal-methods claims. |
...
- The remaining stronger milestone work is a clean Project Theseus replay or
  archived public release fixture, current work-board import beyond the
  project-registry import, missing-artifact and private/publication permission
  closure, artifact-truth review, external review, plus any separate accepted
  evidence-transition record if a stronger bounded non-core claim is later
  promoted.
...
- `docs/curated_reader_format_artifact_probe.md` and
  ...
  511-page PDF with sample pages rendered to PNG. The curated DOCX/PDF renderer
  ...
  renders all 511 pages at 72 dpi with
  0 blank pages, 1 low-ink page, and 0 near-edge pages. The PDF extracted-text
  reading-flow review now checks 511 text pages, 511 nonempty text pages, 44
  ordered chapter headings, 3 ordered appendix headings, and 0 replacement
  characters. The PDF page-by-page release-preparation review now checks 511
  page rows, 511 text pages, 511 word-box pages, 511 raster pages, 0 failed
...
- Test 2: one blocked curated-reader release-candidate record now exists beyond
  ...
  a 511-page PDF sample-page render with 50 Chrome-screenshot Mermaid
  ...
  renders all 511 pages with 0 blank pages, 1 low-ink page, and 0 near-edge
  pages; its PDF extracted-text reading-flow review checks 511 nonempty text
  ...
  Its PDF page-by-page release-preparation review checks 511 page rows, 0
...
2. **Release unblock ladder.** The blocked curated-reader release candidate
   ...
   `docs/curated_reader_pdf_page_review.md`: all 511 pages have text,
...
3. **Evidence ladder still needs ascent, not only more refusals.** The current
   ten accepted upward non-core transitions and 39 accepted
   ...
   evidence, or the first Theseus parity imports (governance-tax measurement,
   reference-trace export) under the cross-project contract above.
