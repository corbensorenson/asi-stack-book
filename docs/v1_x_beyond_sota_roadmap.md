...
Quarto site, and nine narrow non-core evidence transitions. The current v1.x
...
| P0 | Formal proof count is no longer the bottleneck; executable-model depth is. | `docs/proof_depth_classification.md` now records 1013 theorem declarations, 185 direct/projection-style, 824 derived/decomposed, 4 unknown/mixed, and 106 safety-critical theorem declarations after the Resource aggregate flagship invariant pass, Theseus/Fast support-lane aggregate bridge, Capability replacement identity-sequence bridge, Resource governance-tax trade-off bridge, receipt repository audit bridge, claim-state transition bridge, Circle public consumer-gate fixture bridge, residual-ledger storage/replay bridge, Artifact Graphs live attestation bridge, Artifact Graphs randomized artifact attestation audit bridge, Theseus simulation-fidelity receipt-suite bridge, and Theseus module definition-of-done import bridge. The first anti-projection sweep is real, but most theorems still verify finite records and decision routes rather than traces of a running system. | Stop optimizing for theorem-count growth. Pick one proof/evidence bridge and prove a property over transitions or fixture equivalence, then run the same cases through the executable harness where possible. |
...
| P2 | The project has many ledgers but still few promotions. | The v1.0.0 release was honest, and the current 44 core claims still remain `argument`; nine narrow non-core claims moved upward, including the finite synthetic load-smoothing selector, local empirical scoped-route selector, Theseus artifact-retention import, Theseus governance-rights receipt-suite import, and Theseus simulation-fidelity receipt-suite import transitions. | Future roadmap work should close evidence gaps, not multiply status documents. Add ledgers only when they make support-state decisions clearer or enforceable. |
...
- `python3 scripts/validate_chapter_review_burndown.py` passes and proves that
  ...
  dated blocker.
- `python3 scripts/validate_proof_depth.py` now classifies 1013 theorem
  declarations, including 824 derived/decomposed declarations and 185 direct
...
- source-to-chapter and claim-source mappings are complete for the current
  manifest;
- nine narrow non-core evidence transitions are recorded;
...
- The generated proof-depth report records 1013 theorem declarations, 824
...
- `docs/proof_depth_classification.md` records 1013 theorem declarations, 824
...
| `executable-specifications-and-lean-proof-envelope` | Proof-governance chapter needed to show the book's real proof layer more concretely. | Partially executed: added Proof-depth surface synchronization guarded by `python3 scripts/validate_proof_depth_surface.py`; the live chapter, reader chapter, outline, and roadmap now expose the current validator-reported proof-depth snapshot. Current proof-depth snapshot: 205 proof targets, 54 Lean modules, 1019 theorem declarations, 830 derived/decomposed, 185 direct/projection, 4 unknown/mixed, and 5/5 safety-critical chapter classifications present. The chapter explains derived/decomposed versus direct/projection labels and projection-only traceability, using the book's Lean layer as a worked proof-etiquette example. A reader-only overlay now replaces dense Interfaces and Minimum Viable Implementation proof-envelope inventory prose in Human view and generated reader output, dropping the generated-reader heuristic score from 9 to 7 while preserving the canonical AI/research details, rendered mechanism diagram, and support boundaries. The semantic proof adequacy review is now validator-checked by `python3 scripts/validate_proof_adequacy_review.py` against the 205-target manifest and no-promotion boundary language. This does not prove broad semantic adequacy, does not validate source interpretation, does not prove deployed enforcement, and does not promote proof-envelope support. Remaining work: upgrade high-value projection hooks where the proof model is too shallow and add consumer-gate receipts before stronger formal-methods claims. |
...
- Test 3: 9 accepted narrow transitions, including the finite synthetic
...
3. **Evidence ladder still needs ascent, not only more refusals.** The current
   nine accepted upward non-core transitions and 38 accepted
