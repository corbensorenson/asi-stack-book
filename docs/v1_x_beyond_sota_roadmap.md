...
| P0 | Formal proof count is no longer the bottleneck; executable-model depth is. | `docs/proof_depth_classification.md` now records 829 theorem declarations, 157 direct/projection-style, 668 derived/decomposed, 4 unknown/mixed, and 102 safety-critical theorem declarations after the CommandContracts field-confidence bridge pass. The first anti-projection sweep is real, but most theorems still verify finite records and decision routes rather than traces of a running system. | Stop optimizing for theorem-count growth. Pick one proof/evidence bridge and prove a property over transitions or fixture equivalence, then run the same cases through the executable harness where possible. |
...
- `python3 scripts/validate_chapter_review_burndown.py` passes and proves that
  ...
  dated blocker.
- `python3 scripts/validate_proof_depth.py` now classifies 829 theorem
  declarations, including 668 derived/decomposed declarations and 157 direct
...
- The generated proof-depth report records 829 theorem declarations, 668
...
- `docs/proof_depth_classification.md` records 829 theorem declarations, 668
...
| `coil-attention-cyclic-memory-and-recurrence-contracts` | Specialist chapter is sound but could show concrete Circle backing. | Surface source-verified Circle coil-attention proof artifacts or record an explicit blocker for unavailable artifacts; keep the structure-versus-quality boundary explicit and do not infer retrieval quality, task quality, or memory scaling from recurrence structure. |
...
| `executable-specifications-and-lean-proof-envelope` | Proof-governance chapter should show the book's real proof layer more concretely. | Surface current validator-reported proof-depth numbers, explain derived/projection labels, and use the book's Lean layer as a worked proof-etiquette example without freezing stale counts into release claims. |
