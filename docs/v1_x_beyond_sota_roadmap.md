...
|---|---|---|
| `asi-is-a-stack-not-a-model` | Beyond-SOTA previously restated the core thesis more than it argued against alternative framings. | Added live and curated reader mature-endpoint prose that contrasts the ASI Stack against scale-only systems, generic agent loops, and compound/modular AI systems, while preserving the source-noted comparator boundary for MRKL, LLM-agent surveys, cognitive architectures, and layered control. Remaining work: finish source-to-layer traceability and claim-support audits only with recorded commands and non-claims. |
...
| `evidence-states-and-claim-discipline` | The chapter undersells the novelty of support-state discipline. | Reframe it as a methodological contribution paired with Living Book Methodology; position it against model cards, datasheets, reproducibility checklists, and proof-carrying-code only through source-noted records; finish planned tests without turning clearer prose into stronger evidence. |
...
| `virtual-context-abi` | Minor merge-scaffold residue around the "four lanes" framing. | Smooth lane wording into natural mechanism prose in both live and curated reader surfaces while preserving the ABI/certificate/refusal boundary, source mappings, and existing evidence limits. |
...
| `spinoza-verification-and-proof-carrying-claims` | Proof-carrying, autoformalization, LLM-judge, debate, and "Spinoza" naming need outside-reader grounding. | Add source-noted proof-carrying-code, autoformalization, debate, and LLM-as-judge positioning; evaluate whether the chapter name earns its cognitive load in reader prose, and if retained, explain it without grandiosity or ungrounded authority. |
...
| `artifact-steward-agents-and-living-project-governance` | Former chapter-level Lean module mapping bug hid a distinctive steward proof lane from the chapter metadata. | Metadata mapping is fixed; surface the steward proof lane in proof-hook/status surfaces, then polish the chapter around bounded continuity, federation, treasury, release gate, sunset, and stewardship evidence without implying autonomous governance safety. |
...
| `living-book-methodology` | The demonstrated living-book method is undersold. | Reframe it with Evidence States as the methodological contribution: manifest-driven source of truth, claim ledger, proof manifest, evidence transitions, no-axiom discipline, release records, and reader-edition derivation; compare to executable/reproducible books and literate-programming lineage only through source-noted records. |
...
- `docs/resource_workload_quality_probe.md` adds a local measured
  ...
  replays the route commands, checks output digests and tracked artifacts,
  verifies that the selected route preserves the required quality surface for
  the scoped task, and rejects the cheaper no-op route. This is local
  single-run repository evidence only: it is not a stable-speedup result,
...
- The next evidence work should deepen the flagship Resource Economics lane by
  replacing local single-run workload-quality evidence, local command replay,
