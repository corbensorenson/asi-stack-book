...
Quarto site, and five narrow non-core evidence transitions. The current v1.x
...
| P1 | Project Theseus is the right implementation reference; the first imports are intentionally narrow. | `docs/local_project_mining_theseus_circle.md` records public-safe Theseus mining and source notes; `docs/theseus_report_import_slice.md` records one static digest-verified architecture-gate report import; `docs/theseus_generation_mode_import_slice.md` records one static digest-verified generation-mode import; `docs/theseus_support_replay_probe.md` records a local replay of both ASI-side validators with command-output and artifact digests; and `docs/theseus_public_task_bundle_import.md` records one bounded public task-bundle summary import with 64 public BigCodeBench metadata-only tasks, 0 public training rows, 0 task-level regressions, visible artifact gaps, and seven expected-invalid controls. The local checkout still had private/dirty surfaces, so no clean live Theseus replay or support-state transition exists, and the import does not prove model quality. | Keep the static imports, support probe, and public task-bundle import as implementation-reference evidence only, then pursue a clean replay or archived public fixture with environment notes, publication permission, missing-artifact closure, and external review before any stronger transition. |
...
| P2 | The project has many ledgers but still few promotions. | The v1.0.0 release was honest, and the current 44 core claims still remain `argument`; five narrow non-core claims moved upward, including the finite synthetic load-smoothing selector transition. | Future roadmap work should close evidence gaps, not multiply status documents. Add ledgers only when they make support-state decisions clearer or enforceable. |
...
| Evidence / validated substance | B | A steady cadence of *accepted* narrow transitions earned by real measurements and imports — refusals recorded honestly, but the ladder visibly climbs; every chapter's named evidence lane eventually executes. | Strengthen one measured lane per cycle to an accepted narrow transition (load-stability and workload-quality are closest). Import the next real Circle/Theseus result each cycle. Keep the flagship-first pacing (one flagship, at most two supporting lanes) so the 44-lane plan never becomes a breadth trap. |
...
- source-to-chapter and claim-source mappings are complete for the current
  manifest;
- five narrow non-core evidence transitions are recorded;
...
6. make the honesty system legible in 60 seconds from README, landing page, and
   Human view, and keep the five bounded non-core evidence transitions visible
...
- Update the README, landing page, and live Human view entry path so a first-time
  ...
  - five bounded non-core evidence transitions exist and are narrow;
...
| Non-core evidence visibility | Appendix C or sibling surface names the five current non-core transitions and keeps all 44 core claims at `argument` unless separately promoted. | Readers cannot tell what evidence exists. |
...
- Test 3: 5 accepted narrow transitions, including the new finite synthetic
  load-smoothing selector transition; roughly 39 of 44 lanes have not yet
  executed; core-claim
