...
| P0 | Formal proof count is no longer the bottleneck; executable-model depth is. | `docs/proof_depth_classification.md` now records 1046 theorem declarations, 186 direct/projection-style, 856 derived/decomposed, 4 unknown/mixed, and 106 safety-critical theorem declarations after the Resource aggregate flagship invariant pass, Theseus/Fast support-lane aggregate bridge, Capability replacement identity-sequence bridge, Resource governance-tax trade-off bridge, receipt repository audit bridge, claim-state transition bridge, Circle public consumer-gate fixture bridge, residual-ledger storage/replay bridge, Artifact Graphs live attestation bridge, Artifact Graphs randomized artifact attestation audit bridge, Theseus simulation-fidelity receipt-suite bridge, Theseus module definition-of-done import bridge, Theseus project-registry import bridge, Theseus RLDS/Minari trace-export import bridge, Theseus book-crosswalk pointer boundary, Theseus work-board metadata import boundary, and Theseus assistant reference-trace import bridge. The first anti-projection sweep is real, but most theorems still verify finite records and decision routes rather than traces of a running system. | Stop optimizing for theorem-count growth. Pick one proof/evidence bridge and prove a property over transitions or fixture equivalence, then run the same cases through the executable harness where possible. |
...
- `python3 scripts/validate_chapter_review_burndown.py` passes and proves that
  ...
  dated blocker.
- `python3 scripts/validate_proof_depth.py` now classifies 1046 theorem
  declarations, including 856 derived/decomposed declarations and 186 direct
...
|---|---|---|
| `ai-supply-chain-integrity-and-lifecycle-provenance` | The new cross-lifecycle integrity layer must prevent an AI BOM, provenance statement, or signature from becoming a generic claim of artifact security, data fitness, model safety, legal compliance, or release permission. | Partially executed: added source-noted NIST C-SCRM, SLSA Build Track, OpenSSF Model Signing, and SPDX AI Profile comparators; live/Human prose; a no-promotion disposition; and `AsiStackProofs.SupplyChainIntegrity` finite routes that quarantine a requested artifact with an unresolved critical advisory or required unverified signature. Remaining work: build a public-safe synthetic supply-chain workload with artifact identity/digest, lineage, supplier scope, provenance, signature, BOM freshness, advisory, derivative, revocation, residual, missing-lineage, stale-BOM, unverified-signature, critical-advisory, revoked-signer, changed-derivative, and missing-residual controls before any claim about complete lineage, artifact integrity, absence of compromise, data fitness, supplier trustworthiness, legal compliance, model safety, readiness, authority, deployment, or chapter-core support. |
...
| `failure-modes-of-ungoverned-intelligence` | Stack failure vocabulary is not mapped tightly enough to established AI-safety failure taxonomies. | Closed by artifact for the reviewer weakness in the 2026-07-05 burn-down reconciliation: source-noted external grounding already covered `ext_concrete_ai_safety_2016`, `ext_goal_misgeneralization_2022`, `ext_learned_optimization_risks_2019`, `ext_optimal_policies_power_2019`, and `ext_goodhart_variants_2018`, and this pass added a validator-guarded live and curated reader taxonomy bridge mapping goal misbinding, context pollution, authority creep, evaluator drift, hidden optimization pressure, tool/action overreach, and residual hiding to accident-risk, goal-misgeneralization, learned-optimization, power-seeking/option-preservation, Goodhart/proxy-failure, side-effect, safe-exploration, reward-hacking, and evidence-smoothing families without support-state promotion. The Failure taxonomy detector and mitigation-boundary probe remains implemented by `python3 scripts/validate_failure_taxonomy_detector_probe.py` as a deterministic synthetic failure-taxonomy detector fixture with two valid synthetic incidents, seven expected-invalid controls, result artifact `experiments/failure_taxonomy_detector/results/2026-07-02-local.json`, Lean bridge `lean:failure.taxonomy.detector_probe_bridge`, and no support-state promotion. Follow-on stronger-evidence blockers are preserved outside this closed burn-down weakness: deployed detector traces, runtime authority/context/evaluator/claim-verifier behavior, real recurrence detection, mitigation-effectiveness evidence, prevention evidence, and external review before stronger detection or prevention claims. |
...
| `human-factors-and-meaningful-control-in-oversight` | The newly admitted human-control layer can still mistake an approval event, nominal authority, or post-hoc accountability record for comprehension, feasible intervention, and meaningful operational control. | Build a prospectively frozen human-in-the-loop program spanning workload, alarm quality, time pressure, automation bias, comprehension, intervention latency, safe-state reachability, authority, responsibility fit, and degraded conditions; include direct, assisted, sham-assistance, and no-intervention controls, independent outcome scoring, and explicit no-promotion decisions until representative behavior rather than approval shape supports the chapter. |
...
| `inter-stack-protocols-identity-and-economic-exchange` | The new cross-stack boundary must prevent protocol conformance, structured messages, identity records, credentials, delegation, reservations, and receipts from becoming ambient claims of peer trust, authorization, task truth, settlement, fairness, privacy, safety, or deployment readiness. | Partially executed: added source-noted MCP, A2A, W3C DID Core, W3C Verifiable Credentials Data Model, and Interledger comparators; live/Human prose; a no-promotion disposition; and `AsiStackProofs.InterStackProtocols` finite routes that deny required-invalid credentials and route value-bearing requests without reserved budget to repair. Remaining work: build the public-safe exchange workload with protocol/schema, endpoint/capability, sender/receiver/principal, delegation, credential, audience/scope/expiry, reservation/consideration, receipt, dispute, revocation, and residual controls; reject missing identity, audience mismatch, expired/revoked/invalid credential, missing reservation, disputed receipt, and missing residual owner before any peer-trust, authorization, task/artifact-truth, payment, settlement, fairness, privacy, safety, deployment, or chapter-core support claim. |
...
| `replaceable-cognitive-substrates-beyond-transformer-monoculture` | The accepted post-activation chapter has a source-reviewed ABI argument and taxonomy but no implemented kernel substitution, conformance proof, or matched architecture result. | Build three heterogeneous Cognitive Kernel ABI implementations and an independently encoded malicious-kernel checker; test proposal/effect separation, authority, checkpoint migration, revocation, fallback, and lineage; then run matched equal-active-parameter and equal-total-lifecycle-cost tournaments plus the frozen-core ratchet before any support promotion. |
| `relational-dimension-compilation-and-polyadic-cognition` | The complete relational-compiler owner remains an architectural proposal without an implementation, complete natural-task denominator, independent evaluator, matched lower-order tournament, reproduction, or transfer result. | Closed by manuscript artifact for structural coverage: typed roles, proposal and qualification lifecycle, lower-order rescues, compilation, contraction, RODIE evaluation, sources, figure, proof route, and non-claims are integrated. Next implement the deterministic typed-relation compiler and independent evaluator; log every proposal; compare reified-node, message-passing, sequence, retrieval, tool, and ordinary-model rescues under matched lifecycle budgets; then evaluate unseen topology, natural usefulness, contraction, reproduction, and transfer before support review. |
| `governed-model-training-distributed-optimization-and-scaling` | The admitted chapter closes the training-run ownership gap with a seven-source review, authored transaction fixture, schema, validator, formal invariants, and a frozen protocol, but it has no executed distributed-training, fault-recovery, resume-equivalence, independent-qualification, reproduction, or transfer result. | Implement the prospectively frozen five-arm campaign without opening protected qualification early: run competent natural multi-seed baselines, exercise all thirteen fault families and twelve competence gates, retain the complete attempt and checkpoint denominator, measure joint quality, recovery, throughput, latency, resources, operator work, and governance cost, then require independent replay and cross-topology transfer before any bounded support review. |
| `privacy-data-rights-and-information-flow-governance` | The admitted chapter establishes a distinct purpose-bounded information-use and executable-rights owner with nine reviewed sources, a finite transaction, rejecting controls, and a frozen campaign, but no natural privacy attack, rights workflow, deletion propagation, influence analysis, legal determination, independent reproduction, or transfer result has run. | Implement the six-arm natural campaign only after all fifteen competence gates freeze: qualify strong extraction, confidence and label-only membership, linkage, cross-user, and purpose-bypass attacks with positive controls; exercise access, correction, export, restriction, deletion, recipient, backup, descendant, and exception paths; measure utility, privacy, rights completion, latency, residual influence, operator work, and total cost; then pursue independent replay and cross-model, system, operator, and policy-module transfer before any bounded support review. |
| `confidential-and-verifiable-ai-computation` | The protected-computation owner now separates guarantees and evidence statements, but no FHE, MPC, zero-knowledge, confidential-execution, or hybrid path has been implemented, independently appraised, leakage-tested, or compared under matched lifecycle cost. | Closed by manuscript artifact for coverage: guarantee vectors, distrust boundaries, attestation and proof semantics, fallback, sources, diagram, evidence plan, and non-claims are integrated. Implement one bounded protected inference transaction with a separately written verifier; inject replay, artifact, tokenizer, preprocessing, policy, epoch, relying-party, and fallback mismatches; inventory side channels; compare native and protected paths under matched task and concurrency; and publish unsuccessful attempts and residuals before any security or privacy review. |
...
| `governed-deliberation-and-test-time-scaling` | The new deliberation control plane has source-noted comparators and finite routes, but no local reasoning workload, verifier, trace-to-action check, cost measurement, or independent review. | Partially executed: added primary source notes for Tree of Thoughts and test-time compute scaling, scoped reasoning-RL and early-exit comparators, live/Human prose, and `AsiStackProofs.Deliberation` routes that send a high-risk requested execution handoff without independent verification to review and an exhausted budget to residual escrow. Remaining work: build a public-safe workload comparing direct, revision, and search modes with matched quality, verifier-cost, branch, latency, residual, trace-to-action, independent-review, and negative-control records before any stronger reasoning, efficiency, verifier, safety, or chapter-core claim. |
...
| `executable-specifications-and-lean-proof-envelope` | Proof-governance chapter needed to show the book's real proof layer more concretely. | Partially executed: added Proof-depth surface synchronization guarded by `python3 scripts/validate_proof_depth_surface.py`; the live chapter, reader chapter, outline, and roadmap now expose the current validator-reported proof-depth snapshot. Current proof-depth snapshot: 324 proof targets, 113 Lean modules, 1272 theorem declarations, 854 derived/decomposed, 183 direct/projection, 235 unknown/mixed, and 1/1 safety-critical chapter classifications present. The chapter explains derived/decomposed versus direct/projection labels and projection-only traceability, using the book's Lean layer as a worked proof-etiquette example. A reader-only overlay now replaces dense Interfaces and Minimum Viable Implementation proof-envelope inventory prose in Human view and generated reader output, dropping the generated-reader heuristic score from 9 to 7 while preserving the canonical AI/research details, rendered mechanism diagram, and support boundaries. The semantic proof adequacy review is validator-checked against the current manifest and no-promotion boundary language. This does not prove broad semantic adequacy, does not validate source interpretation, does not prove deployed enforcement, and does not promote proof-envelope support. Remaining work: upgrade high-value projection hooks where the proof model is too shallow and add consumer-gate receipts before stronger formal-methods claims. |
...
| `model-weight-custody-and-hardware-roots-of-trust` | The new artifact-custody layer needs to prevent encrypted storage or attestation from becoming a generic claim of model security, safety, or release permission. | Partially executed: added source-noted RAND model-weight-security, NIST confidential-computing draft, NVIDIA lifecycle, and formal release-scheme comparators; live/Human prose; a no-promotion disposition; and `AsiStackProofs.ModelWeightCustody` finite routes that block a requested load with required invalid attestation and route missing lineage to custody repair. Remaining work: build a public-safe synthetic custody workload with model/artifact identity, lineage, policy, measurement, attestation, access/release scope, incident/revocation, residual, stale/invalid-attestation, changed-policy, widened-scope, and irreversible-release controls before any claim about protected weights, hardware trust, confidentiality, extraction resistance, security, model safety, release desirability, readiness, authority, deployment, or chapter-core support. |
...
| `perception-sensor-fusion-and-observation-trust` | The new observation layer must not convert multiple agreeing channels into independent evidence when sensors share data, models, clocks, calibration errors, environments, or attack surfaces. | Partially executed: full live and Human Reading Path prose, ImageBind and robustness/calibration source grounding, Platonic World Model lineage, a figure, implementation horizons, explicit limitations, and planned correlated-agreement target `lean:perception.correlated_agreement_no_independent_promotion` are in place. Remaining work: build a multimodal workload with declared and hidden common causes, missing and stale channels, spoofed agreement, disagreement, clock skew, distribution shift, calibrated fusion baselines, interventions, independent evaluators, downstream decision costs, and abstention; then evaluate whether the dependence model detects the failures it is supposed to govern before implementing the finite admission rule. |
| `human-ai-organizations-delegation-and-accountability` | The organizational chapter can otherwise become a role-chart narrative that assigns humans nominal responsibility after information, competence, time, attention, or intervention authority has already been delegated away. | Partially executed: full live and Human Reading Path prose, organizational and meaningful-control source grounding, Talos lineage, a figure, implementation horizons, explicit limitations, and planned accountability-capacity target `lean:human_ai_org.accountability_requires_authority` are in place. Remaining work: build representative mixed human-AI workflows with delegation, escalation, shift handoff, workload, automation-bias, delayed intervention, contested decisions, authority expiry, incident reconstruction, appeal, and remedy; evaluate decision quality, intervention reachability, accountability gaps, latency, worker burden, and governance cost before implementing the narrow authored-record guard. |
| `human-ai-symbiosis-neurotechnology-and-cognitive-sovereignty` | The longitudinal coupling owner has complete argument-level prose but no human-subject, neural-interface, clinical, withdrawal, post-exit, subgroup, independent-reproduction, or cognitive-sovereignty implementation evidence. | Closed by manuscript artifact for coverage: three-arm complementarity, coupling levels, neural and inferred mental data, human-state rollback, cognitive sovereignty, sources, figure, proof route, and non-claims are integrated. Build a low-risk synthetic or researcher-operated rehearsal; preregister human-alone, AI-alone, combined, and simpler-intervention baselines; obtain appropriate ethics review before participants; measure skill, dependence, calibration, well-being, subgroup effects, withdrawal, and practical exit longitudinally; preserve adverse events and irreversible residuals before any benefit claim. |
| `embodied-agency-real-time-control-and-physical-safety` | The new embodiment layer risks mistaking a complete command record or safety-filter result for physical safety despite estimator error, missed deadlines, actuator limits, irreversible effects, and plant-model mismatch. | Partially executed: full live and Human Reading Path prose, control-barrier and safe-RL source grounding, VIEA lineage, a figure, implementation horizons, explicit limitations, and planned control-admission target `lean:embodiment.missing_safety_state_blocks_control` are in place. Remaining work: implement closed-loop simulations and hardware-in-the-loop boundaries with competent controller baselines, state-estimation error, latency and deadline misses, saturation, sensor faults, distribution shift, safety-filter conflict, emergency stops, degraded modes, fallback reachability, rollback versus compensation, and irreversible effects; measure violations, useful throughput, recovery, latency, and governance cost before treating the checklist as safety evidence. |
| `multi-agent-dynamics-collective-intelligence-and-systemic-risk` | The new population layer must prevent valid pairwise contracts, high average utility, or local cooperation from being laundered into systemic safety, beneficial collective intelligence, institutional legitimacy, or preserved human agency. | Partially executed: full live and Human Reading Path prose, cooperative-AI and gradual-disempowerment source grounding, Coherence Exchange lineage, a figure, implementation horizons, explicit limitations, and planned population-boundary target `lean:multi_agent.pairwise_validity_no_systemic_promotion` are in place. Remaining work: build population workloads with communication and no-communication baselines, shared-model correlation, coalitions, collusion, manipulation, resource competition, entry and exit, distribution shift, mechanism variants, human veto and appeal, independent evaluators, tail-risk metrics, inequality and option-value measures, and causal ablations before implementing the finite no-systemic-promotion guard. |
...
| `human-ai-communication-persuasion-and-epistemic-security` | The influence lifecycle is now explicit, but correction reach, audience autonomy, and evidence-provenance behavior remain argument-level rather than measured. | Partially executed: full live and Human Reading Path prose, source mappings, a figure, explicit limits, and a planned bounded influence-envelope proof target are in place. Remaining work: deepen the communication and persuasion source crosswalk, implement a public-safe communication-packet fixture with manipulation and provenance-negative controls, and evaluate comprehension, autonomy, amplification, and correction reach without promoting the chapter core claim. |
| `governed-objective-formation-value-learning-and-goal-integrity` | The target, proxy, constraint, and amendment distinctions are written, but objective drift and contested-value handling do not yet have an executable workload. | Partially executed: full live and Human Reading Path prose, source mappings, a figure, explicit limits, and a planned finite objective-amendment proof target are in place. Remaining work: ground the target/proxy distinctions with additional value-learning sources, implement objective-packet mutation and amendment fixtures, and evaluate proxy drift, veto preservation, disagreement, and rollback under distribution shift without claiming solved value learning. |
| `institutions-international-coordination-and-public-legitimacy` | The institutional layer is present, but jurisdiction conflicts, excluded publics, enforcement asymmetries, and remedy capacity remain prose-level governance requirements. | Partially executed: full live and Human Reading Path prose, source mappings, a figure, explicit limits, and a planned finite legitimacy-boundary proof target are in place. Remaining work: deepen comparative institutional and international-governance sources, map authority, enforcement, appeal, and remedy across jurisdictions, and audit excluded-public and capture controls in a bounded institutional packet before any legitimacy claim. |
| `adversarial-machine-learning-and-model-attack-surface` | The lifecycle attack surface is mapped, but adaptive, compositional, transfer, and post-deployment attacks have not been exercised against a common evaluated system. | Partially executed: full live and Human Reading Path prose, source mappings, a figure, explicit limits, and a planned finite adversarial-admission proof target are in place. Remaining work: deepen the attack taxonomy and primary-source set, implement a lifecycle attack-packet fixture with adaptive and compositional negative controls, and evaluate transfer, persistence, detection, recovery, and useful-throughput costs without treating checklist coverage as model security. |
| `autonomous-replication-proliferation-and-containment` | Component permissions and end-to-end proliferation control are distinguished, but no closed workload yet tests hidden copies, provider disagreement, persistence, or containment recovery. | Partially executed: full live and Human Reading Path prose, source mappings, a figure, explicit limits, and a planned finite replication-authority proof target are in place. Remaining work: tighten component versus end-to-end claim boundaries, build a closed synthetic provider and identity fixture, and evaluate census recall, unauthorized persistence, revocation propagation, containment, and recovery without claiming real-world proliferation control. |
...
| `ai-deployment-transition-distribution-and-human-agency` | The transition chapter connects rollout to distribution and agency, but affected-person denominators, option value, burden shifting, and remedy remain unmeasured. | Partially executed: full live and Human Reading Path prose, source mappings, a figure, explicit limits, and a planned finite transition-remedy proof target are in place. Remaining work: deepen labor, distribution, and technology-transition sources, map affected-person denominators and burden transfers, and evaluate staged rollout, agency, reversibility, appeal, remedy, inequality, and governance cost before any beneficial-deployment claim. |
| `learning-theory-generalization-and-scaling-science` | The chapter separates interpolation, transfer, extrapolation, and architectural change, but its prospective forecasts and breakpoint tests have not been registered or run. | Partially executed: full live and Human Reading Path prose, source mappings, a figure, explicit limits, and a planned finite scaling-inference proof target are in place. Remaining work: deepen the learning-theory and scaling-science source map, build a prospective forecast registry with frozen hypotheses, and evaluate breakpoints, data shifts, architecture changes, uncertainty, and failed extrapolations without laundering curve fit into mechanism. |
| `physical-compute-infrastructure-energy-and-environmental-constraints` | Hardware, grid, water, material, and rebound constraints are integrated conceptually, but workload-to-capacity accounting and resilience have not been measured together. | Partially executed: full live and Human Reading Path prose, source mappings, a figure, explicit limits, and a planned finite physical-budget proof target are in place. Remaining work: deepen hardware, grid, water, materials, and lifecycle sources, implement a workload-capacity packet, and evaluate metering uncertainty, rebound effects, supply interruption, thermal limits, resilience, and useful throughput before making sustainability or capacity claims. |
| `scientific-discovery-and-experimental-governance` | Discovery claims are separated from suggestion and validation, but preregistration, null retention, replication, laboratory control, and dual-use review remain untested as one workflow. | Partially executed: full live and Human Reading Path prose, source mappings, a figure, explicit limits, and a planned finite discovery-claim proof target are in place. Remaining work: deepen autonomous-laboratory and scientific-inference sources, implement a preregistered experiment packet with null and leakage controls, and evaluate calibration, replication, causal identification, laboratory safety, dual-use review, and correction before any discovery claim. |
| `dangerous-capability-domains-and-misuse-uplift` | The new domain owner must prevent capability, propensity, safeguard bypass, actor uplift, and realized harm from collapsing into one danger score or a false negative from weak elicitation. | Closed by manuscript artifact for taxonomy coverage: the complete live and Human Reading Path prose, eight current source-note mappings, diagram, handoffs, implementation horizon, no-promotion decision, evidence plan, and planned admission boundary are integrated. Stronger-evidence work remains explicit: build prospectively frozen domain dossiers with competent positive controls, realistic actor and accessible-frontier baselines, adaptive elicitation, independent challenge, information-hazard custody, complete attempt denominators, expiry, and joint capability/uplift/safeguard/cost measurement before any result claim. |
| `military-ai-autonomous-weapons-and-strategic-stability` | The command-and-interaction owner is structurally complete but has no operational system, crisis evidence, doctrine-robust simulation result, legal determination, operator study, proliferation result, or strategic-stability evidence. | Closed by manuscript artifact for coverage: decision-role taxonomy, meaningful judgment, escalation interaction, safe posture, secrecy boundaries, sources, figure, proof route, and non-operational guardrails are integrated. Build only publicly safe, independently designed simulations that vary doctrine, adversary assumptions, sensor correlation, communication loss, time pressure, operator workload, off-ramps, and reciprocal adoption; compare human-only, non-AI interlock, advisory, and automation baselines; keep all outputs non-operational and retain legal, civilian-harm, and strategic residuals. |
| `societal-resilience-and-misuse-defense` | The new social-defense owner must not mistake a provider safeguard, takedown count, tabletop, or fast internal response for population protection, lawful coordination, equitable recovery, or acceptable residual harm. | Closed by manuscript artifact for taxonomy coverage: the complete live and Human Reading Path prose, four current source-note mappings, diagram, cross-organization lifecycle, handoffs, implementation horizon, no-promotion decision, evidence plan, and planned admission boundary are integrated. Stronger-evidence work remains explicit: build bounded multi-organization exercises with representative exposed cohorts, missing participants, jurisdiction conflict, false intervention, privacy and civil-liberties constraints, harmed-party remedy, correlated failure, attacker adaptation, independent observers, and recovery outcomes before any resilience claim. |
| `open-weight-release-and-post-release-control` | The new release owner must keep pre-release custody, deliberate irreversible publication, derivative adaptation, accessible-frontier comparison, post-release influence, and impossible universal revocation conceptually separate. | Closed by manuscript artifact for taxonomy coverage: the complete live and Human Reading Path prose, six current source-note mappings, access-form ladder, diagram, handoffs, implementation horizon, no-promotion decision, evidence plan, and planned admission boundary are integrated. Stronger-evidence work remains explicit: build harmless artifact-specific release cases that compare no release, escrow, gated access, hosted inference, delayed release, reduced artifacts, and full weights under competent malicious adaptation, derivative-lineage, cumulative-risk, distributional-benefit, incident, post-release measurement, and independent-review tests before any release recommendation. |
| `content-authenticity-watermarking-and-synthetic-media-integrity` | The new information-integrity owner must prevent provenance validity, watermark presence, detector output, disclosure, semantic truth, consent, legality, and remedy from being laundered into one authenticity badge. | Closed by manuscript artifact for taxonomy coverage: the complete live and Human Reading Path prose, C2PA 2.3, current EU guidance, international synthesis, diagram, transformation lifecycle, handoffs, implementation horizon, no-promotion decision, evidence plan, and planned admission boundary are integrated. Stronger-evidence work remains explicit: build a public-safe interoperable corpus and evaluate signing, bindings, edits, stripping, laundering, regeneration, compromised signers, validly signed false claims, detector shift, accessible disclosure, privacy, correction, and harmed-party remedy across independent tools and channels before any robustness, compliance, truth, or trust claim. |
