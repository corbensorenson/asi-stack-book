...
- The Integrated Reference Architecture row now has validated fixtures and a
  narrative showpiece: one bounded book-maintenance request followed hop by hop
  through intent, governance, planning, context, routing, verification,
  execution/audit, evidence, residuals, and SCF no-promotion review, plus the
  blocked authority path. The remaining blocker is no longer craft prose; it is
  a replayed or live trace from an actual command, artifact bundle, validator
  run, and blocked-path stop condition.
