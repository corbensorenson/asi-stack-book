...
| `failure-modes-of-ungoverned-intelligence` | Stack failure vocabulary is not mapped tightly enough to established AI-safety failure taxonomies. | Closed by artifact for the reviewer weakness in the 2026-07-05 burn-down reconciliation: source-noted external grounding covers `ext_concrete_ai_safety_2016`, `ext_goal_misgeneralization_2022`, `ext_learned_optimization_risks_2019`, `ext_optimal_policies_power_2019`, and `ext_goodhart_variants_2018`, while the validator-guarded live and curated reader bridge maps the chapter taxonomy to established accident, goal-misgeneralization, learned-optimization, power-seeking, Goodhart, side-effect, safe-exploration, reward-hacking, and evidence-smoothing families without promotion. The deterministic detector fixture retains two valid synthetic incidents and seven expected-invalid controls. The exact 19-theorem `AsiStackProofs.FailureRecoveryRefinement` now adds arbitrary successful-run nine-field identity custody, zero support/external-authority assignment, exact receipt accounting, valid traces, batch composition, rejected-state preservation, guarded readmission, and recurrence re-isolation; `python3 scripts/validate_failure_recovery_refinement.py` recompiles that surface and rejects 31/31 state-preserving mutations. This strengthens only the bounded recovery target; the chapter's five-target proof class remains `useful but too narrow` and support remains `argument`. Follow-on work belongs in Project Theseus or empirical lanes: deployed fault injection and detector traces, actual effects and containment observation, detector/reviewer independence, runtime authority/context/evaluator/claim-verifier behavior, remediation and restart efficacy, recurrence and escape measurement, prevention evidence, reproduction, and transfer. |
...
| `executable-specifications-and-lean-proof-envelope` | Proof-governance chapter needed to show the book's real proof layer more concretely. | Partially executed: added Proof-depth surface synchronization guarded by `python3 scripts/validate_proof_depth_surface.py`; the live chapter, reader chapter, outline, and roadmap now expose the current validator-reported proof-depth snapshot. Current proof-depth snapshot: 324 proof targets, 130 Lean modules, 2626 theorem declarations, 1635 derived/decomposed, 425 direct/projection, 566 unknown/mixed, and 1/1 safety-critical chapter classifications present. The chapter explains derived/decomposed versus direct/projection labels and projection-only traceability, using the book's Lean layer as a worked proof-etiquette example. A reader-only overlay now replaces dense Interfaces and Minimum Viable Implementation proof-envelope inventory prose in Human view and generated reader output, dropping the generated-reader heuristic score from 9 to 7 while preserving the canonical live evidence table, rendered mechanism diagram, and support boundaries. The semantic proof adequacy review is validator-checked against the current manifest and no-promotion boundary language. This does not prove broad semantic adequacy, does not validate source interpretation, does not prove deployed enforcement, and does not promote proof-envelope support. Remaining work: upgrade high-value projection hooks where the proof model is too shallow and add consumer-gate receipts before stronger formal-methods claims. |
