...
| Formal layer | A− | Executable-model depth, not count: properties over *sequences* of transitions, Lean/Python fixture equivalence on shared fixtures, and adversarial negative cases tied to live evidence lanes; projection bridges stay labeled. | The Capability replacement identity-sequence bridge is the current concrete movement from trace-summary alignment toward sequence invariants: the Python trace and Lean summary now agree on field identity across canary/rollback, monitor-failure default blocking, prior restoration, authority-envelope preservation, residual ownership, four rejected sequence controls, and no support-state promotion. Next cycles should repeat that pattern for another live evidence lane, preferably a live/external replay or a context-transaction sequence rather than another field-presence bridge. |
...
| `executable-specifications-and-lean-proof-envelope` | Proof-governance chapter needed to show the book's real proof layer more concretely. | Partially executed: added Proof-depth surface synchronization guarded by `python3 scripts/validate_proof_depth_surface.py`; the live chapter, reader chapter, outline, and roadmap now expose the current validator-reported proof-depth snapshot. Current proof-depth snapshot: 191 proof targets, 54 Lean modules, 971 theorem declarations, 786 derived/decomposed, 181 direct/projection, 4 unknown/mixed, and 5/5 safety-critical chapter classifications present. The chapter explains derived/decomposed versus direct/projection labels and projection-only traceability, using the book's Lean layer as a worked proof-etiquette example. A reader-only overlay now replaces dense Interfaces and Minimum Viable Implementation proof-envelope inventory prose in Human view and generated reader output, dropping the generated-reader heuristic score from 9 to 7 while preserving the canonical AI/research details, rendered mechanism diagram, and support boundaries. The semantic proof adequacy review is now validator-checked by `python3 scripts/validate_proof_adequacy_review.py` against the 191-target manifest and no-promotion boundary language. This does not prove broad semantic adequacy, does not validate source interpretation, does not prove deployed enforcement, and does not promote proof-envelope support. Remaining work: upgrade high-value projection hooks where the proof model is too shallow and add consumer-gate receipts before stronger formal-methods claims. |
