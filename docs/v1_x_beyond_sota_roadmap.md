...
- The ten draft SVG key figures are now embedded in live chapters and
  ...
  matches. The remaining blocker is not placement, source contrast, rendered DOM
  presence, or package/text survival; it is visual polish, real e-reader/app/PDF
  inspection, audio integration, and release review before any
...
| `executable-specifications-and-lean-proof-envelope` | Proof-governance chapter needed to show the book's real proof layer more concretely. | Partially executed: added Proof-depth surface synchronization guarded by `python3 scripts/validate_proof_depth_surface.py`; the live chapter, reader chapter, outline, and roadmap now expose the current validator-reported proof-depth snapshot. Current proof-depth snapshot: 200 proof targets, 54 Lean modules, 994 theorem declarations, 807 derived/decomposed, 183 direct/projection, 4 unknown/mixed, and 5/5 safety-critical chapter classifications present. The chapter explains derived/decomposed versus direct/projection labels and projection-only traceability, using the book's Lean layer as a worked proof-etiquette example. A reader-only overlay now replaces dense Interfaces and Minimum Viable Implementation proof-envelope inventory prose in Human view and generated reader output, dropping the generated-reader heuristic score from 9 to 7 while preserving the canonical AI/research details, rendered mechanism diagram, and support boundaries. The semantic proof adequacy review is now validator-checked by `python3 scripts/validate_proof_adequacy_review.py` against the 200-target manifest and no-promotion boundary language. This does not prove broad semantic adequacy, does not validate source interpretation, does not prove deployed enforcement, and does not promote proof-envelope support. Remaining work: upgrade high-value projection hooks where the proof model is too shallow and add consumer-gate receipts before stronger formal-methods claims. |
...
- The ten draft key figures are now embedded/adapted into the curated reader
  ...
  manual page-by-page PDF/PDF-viewer review, audio-specific inspection, and
...
- `scripts/validate_reader_key_figure_pdf_layout.py` now records
  ...
  page-by-page PDF review, PDF viewer review, final figure-artifact approval,
...
- `docs/curated_reader_format_artifact_probe.md` and
  ...
  504-page PDF with sample pages rendered to PNG. The curated DOCX/PDF renderer
  ...
  with 0 failures while preserving dedicated e-reader approval as a release
  blocker. The repaired DOCX package audit now checks document XML, required
  ...
  as a release blocker. The PDF key-figure layout probe checks 10 caption
  ...
  standard deviation while still preserving manual PDF page-by-page review and
  PDF viewer review as release blockers. EPUB, DOCX, PDF, e-reader, and
...
- Ten draft key-figure assets tied to the reader handoff contract now exist and
  ...
  standard deviation, and 116 minimum quantized colors, and PDF key-figure
  ...
  polish, real e-reader/app inspection, Word/LibreOffice GUI/Google Docs
  review, manual page-by-page PDF/PDF-viewer review, and release review still
...
- Test 2: one blocked curated-reader release-candidate record now exists beyond
  ...
  a 504-page PDF sample-page render with 50 Chrome-screenshot Mermaid
  ...
  minimum luminance standard deviation. PDF remains probe-level until real
  PDF-viewer reading-flow/layout review and an edition release record exist; audio now has
  ...
  remains blocked on keyboard-only review, screen-reader review, WCAG
  conformance review, real e-reader/application review, audiobook review, and
  reader release approval. Automated PNG raster fallback review now exists
