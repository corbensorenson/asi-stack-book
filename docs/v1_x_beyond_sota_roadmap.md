...
1. **Pattern-language integration**: name the governed-cognition pattern once
   ...
   standalone audit. This is the highest-priority idea-depth action because it
   turns the repeated record/lifecycle/ceiling/gate/receipt/rollback pattern
   into an explicit contribution while preventing every chapter from pretending
   to be a separate novelty claim.
...
| Pattern language for governed cognition | Opening stack chapter, Integrated Reference Architecture, Living Book Methodology | Owner-spine integration now exists: the opener names the pattern, Integrated Reference Architecture applies it at trace scale, Living Book Methodology applies it reflexively to book changes, and the contribution novelty ledger records this as positioning rather than novelty proof. Many non-owner chapters still need concise local-delta prose. | Continue the inline chapter/outline/reader pass through non-owner chapters so each affected chapter distinguishes signature mechanisms from pattern applications and states only the local delta it actually owns. | A standalone pattern report, blanket novelty language, adding new terminology to every chapter, or treating each pattern application as an independent contribution. |
