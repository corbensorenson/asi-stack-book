...
Quarto site, and twelve narrow non-core evidence transitions. The current v1.x
...
| P0 | Formal proof count is no longer the bottleneck; executable-model depth is. | `docs/proof_depth_classification.md` now records 1034 theorem declarations, 186 direct/projection-style, 844 derived/decomposed, 4 unknown/mixed, and 106 safety-critical theorem declarations after the Resource aggregate flagship invariant pass, Theseus/Fast support-lane aggregate bridge, Capability replacement identity-sequence bridge, Resource governance-tax trade-off bridge, receipt repository audit bridge, claim-state transition bridge, Circle public consumer-gate fixture bridge, residual-ledger storage/replay bridge, Artifact Graphs live attestation bridge, Artifact Graphs randomized artifact attestation audit bridge, Theseus simulation-fidelity receipt-suite bridge, Theseus module definition-of-done import bridge, Theseus project-registry import bridge, and Theseus RLDS/Minari trace-export import bridge. The first anti-projection sweep is real, but most theorems still verify finite records and decision routes rather than traces of a running system. | Stop optimizing for theorem-count growth. Pick one proof/evidence bridge and prove a property over transitions or fixture equivalence, then run the same cases through the executable harness where possible. |
...
- `python3 scripts/validate_chapter_review_burndown.py` passes and proves that
  ...
  dated blocker.
- `python3 scripts/validate_proof_depth.py` now classifies 1034 theorem
  declarations, including 844 derived/decomposed declarations and 186 direct
...
- source-to-chapter and claim-source mappings are complete for the current
  manifest;
- twelve narrow non-core evidence transitions are recorded;
...
6. make the honesty system legible in 60 seconds from README, landing page, and
   Human view, and keep the twelve bounded non-core evidence transitions visible
...
- Update the README, landing page, and live Human view entry path so a first-time
  ...
  - twelve bounded non-core evidence transitions exist and are narrow;
...
- The generated proof-depth report records 1034 theorem declarations, 844
...
- `docs/proof_depth_classification.md` records 1034 theorem declarations, 844
...
3. **Evidence ladder still needs ascent, not only more refusals.** The current
   twelve accepted upward non-core transitions and 39 accepted
