...
Quarto site, and seven narrow non-core evidence transitions. The current v1.x
...
| P1 | Circle evidence is real but not yet a clean upstream replay. | `docs/circle_external_receipt_slice.md` records a local clean checkout and accepted rope receipt, `docs/circle_public_replay_consumer_gate.md` adds a CI-verifiable ASI-side consumer gate with negative controls, and the Circle KV-cache, recurrence, sparse-attention, and cyclic-mixer receipt slices now record pinned public-safe structural/accounting imports with accepted `blocks_promotion` no-change decisions. The ASI repo still does not rerun the external checkout in CI or vendor a public replay pack. | Treat the consumer gate and no-promotion decisions as accountability closure, then pursue a public contract pack, archived evidence bundle, or clean replay before stronger claims. |
...
- source-to-chapter and claim-source mappings are complete for the current
  manifest;
- seven narrow non-core evidence transitions are recorded;
...
6. make the honesty system legible in 60 seconds from README, landing page, and
   Human view, and keep the seven bounded non-core evidence transitions visible
...
- Update the README, landing page, and live Human view entry path so a first-time
  ...
  - seven bounded non-core evidence transitions exist and are narrow;
...
- `editions/reader_manuscript/v1_0/audio_script_probe_manifest.json`,
  ...
  `TBD` marker rows, 66 narration notes, 1,078,567 checked text characters, 44
...
- Test 3: 7 accepted narrow transitions, including the finite synthetic
...
2. **Release unblock ladder.** The blocked curated-reader release candidate
   ...
   treatment review checks 49 ordered scripts, 66 narration notes, 1,078,567
...
3. **Evidence ladder still needs ascent, not only more refusals.** The current
   seven accepted upward non-core transitions and 34 accepted
