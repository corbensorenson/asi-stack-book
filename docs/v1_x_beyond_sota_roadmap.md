...
| P0 | Formal proof count is no longer the bottleneck; executable-model depth is. | `docs/proof_depth_classification.md` now records 927 theorem declarations, 177 direct/projection-style, 746 derived/decomposed, 4 unknown/mixed, and 103 safety-critical theorem declarations after the Typed job durable lifecycle probe pass. The first anti-projection sweep is real, but most theorems still verify finite records and decision routes rather than traces of a running system. | Stop optimizing for theorem-count growth. Pick one proof/evidence bridge and prove a property over transitions or fixture equivalence, then run the same cases through the executable harness where possible. |
...
- `python3 scripts/validate_chapter_review_burndown.py` passes and proves that
  ...
  dated blocker.
- `python3 scripts/validate_proof_depth.py` now classifies 927 theorem
  declarations, including 746 derived/decomposed declarations and 177 direct
...
- The generated proof-depth report records 927 theorem declarations, 746
...
- `docs/proof_depth_classification.md` records 927 theorem declarations, 746
...
| `executable-specifications-and-lean-proof-envelope` | Proof-governance chapter needed to show the book's real proof layer more concretely. | Partially executed: added Proof-depth surface synchronization guarded by `python3 scripts/validate_proof_depth_surface.py`; the live chapter, reader chapter, outline, and roadmap now expose the current validator-reported proof-depth snapshot. Current proof-depth snapshot: 177 proof targets, 54 Lean modules, 927 theorem declarations, 746 derived/decomposed, 177 direct/projection, 4 unknown/mixed, and 5/5 safety-critical chapter classifications present. The chapter explains derived/decomposed versus direct/projection labels and projection-only traceability, using the book's Lean layer as a worked proof-etiquette example. This does not prove semantic adequacy, does not validate source interpretation, does not prove deployed enforcement, and does not promote proof-envelope support. Remaining work: perform semantic proof adequacy review, upgrade high-value projection hooks where the proof model is too shallow, and add consumer-gate receipts before stronger formal-methods claims. |
