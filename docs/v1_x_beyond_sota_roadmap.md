...
| P0 | Formal proof count is no longer the bottleneck; executable-model depth is. | `docs/proof_depth_classification.md` now records 986 theorem declarations, 183 direct/projection-style, 799 derived/decomposed, 4 unknown/mixed, and 103 safety-critical theorem declarations after the Resource aggregate flagship invariant pass, Theseus/Fast support-lane aggregate bridge, Capability replacement identity-sequence bridge, Resource governance-tax trade-off bridge, receipt repository audit bridge, claim-state transition bridge, and Circle public consumer-gate fixture bridge. The first anti-projection sweep is real, but most theorems still verify finite records and decision routes rather than traces of a running system. | Stop optimizing for theorem-count growth. Pick one proof/evidence bridge and prove a property over transitions or fixture equivalence, then run the same cases through the executable harness where possible. |
...
- `python3 scripts/validate_chapter_review_burndown.py` passes and proves that
  ...
  dated blocker.
- `python3 scripts/validate_proof_depth.py` now classifies 986 theorem
  declarations, including 799 derived/decomposed declarations and 183 direct
...
- The ten draft SVG key figures are now embedded in live chapters and
  ...
  `editions/reader_manuscript/v1_0/key_figure_contrast_manifest.json`. The
  remaining blocker is not placement, source contrast, or rendered DOM
...
1. **Pattern-language integration**: preserve the completed owner spine, first
   ...
   novelty claim.
2. **Receipt faithfulness beyond fixture shape**: current ownership decision
   for the next implementation pass is section-owned in Artifact Graphs, with
   back-propagation into Proof-Carrying Claims, Integrated Reference
   Architecture, and Recursive Self-Improvement. The live and reader chapters
   already contain receipt-faithfulness prose; the next movement is to make
   Artifact Graphs carry a showpiece record-reality section and pursue live or
   externally reviewable attestation/audit evidence. Split into a new chapter
   only if the section-level treatment fails the split test by creating
   duplication, burying a distinct evidence lane, or making the surrounding
   chapters longer and less clear.
...
| Record-reality gap | Artifact Graphs, Proof-Carrying Claims, Integrated Reference Architecture, Bounded Self-Improvement | Receipt-faithfulness and epistemic-TCB fixtures exist, and the receipt repository audit now checks selected real repository receipt records at `experiments/receipt_repository_audit/results/2026-07-03-local.json` for artifact refs, tracked digest integrity, command pass status, Circle external rope receipt fingerprints, non-claims, and no support-state promotion; live and curated Artifact Graphs prose already carries the receipt-faithfulness argument; open-world receipt faithfulness and verifier correctness remain unproven. | Treat Artifact Graphs as the current owner and strengthen it into a showpiece record-reality section in both editions, with outline/ledger/source/proof routing synchronized; split into a new manifest-backed chapter only if the section-level treatment fails the split test by creating duplication, burying a distinct evidence lane, or making the surrounding chapters longer and less clear. Then add live/external attestation, randomized audit, independent verifier-quality, trust-base storage/replay, and incentive-limit evidence. | Schema-valid receipts, self-checking by the same component, unbounded attestation language, trust-base naming without roots and residuals, another repository-only audit over the same records without stronger replay/evidence, or adding a new chapter that only repeats the Artifact Graphs owner material. |
...
real repository residual-ledger trace, and receipt faithfulness now has both a
first bounded adversarial fixture and a first receipt repository audit at
...
traces into live/external storage-and-replay evidence, move receipt faithfulness
...
1. **Receipt faithfulness / the record-reality gap** — current ownership is
   ...
   and a section-versus-split stress test after the owner section is upgraded.
   Current status is
   `repository_receipt_audit_backed_not_open_world`; the fixture and repository
   audit do not prove open-world receipt faithfulness, deployed attestation,
   deployed audit behavior, external project truth, or chapter-core support.
...
|---|---|---|---|
| Receipt faithfulness / record-reality gap | Current owner: `artifact-graphs-audit-logs-and-replay`, with back-references from Proof-Carrying Claims, Integrated Reference Architecture, and Bounded Self-Improvement. Split only if the upgraded owner section creates duplication, hides a distinct evidence lane, or makes adjacent chapters less clear. | Upgrade the major section into the book's record-reality showpiece: state the objection, best current answer, limits of attestation/traps/cross-checks, repository receipt audits, incentive limits, and no-deployed-faithfulness boundary in both editions. | Live or externally reviewable attestation/audit replay, randomized audit fixture beyond the current repository audit, independent verifier-quality record, or dated blocker. |
...
- The generated proof-depth report records 986 theorem declarations, 799
...
- `docs/proof_depth_classification.md` records 986 theorem declarations, 799
...
| `executable-specifications-and-lean-proof-envelope` | Proof-governance chapter needed to show the book's real proof layer more concretely. | Partially executed: added Proof-depth surface synchronization guarded by `python3 scripts/validate_proof_depth_surface.py`; the live chapter, reader chapter, outline, and roadmap now expose the current validator-reported proof-depth snapshot. Current proof-depth snapshot: 196 proof targets, 54 Lean modules, 987 theorem declarations, 800 derived/decomposed, 183 direct/projection, 4 unknown/mixed, and 5/5 safety-critical chapter classifications present. The chapter explains derived/decomposed versus direct/projection labels and projection-only traceability, using the book's Lean layer as a worked proof-etiquette example. A reader-only overlay now replaces dense Interfaces and Minimum Viable Implementation proof-envelope inventory prose in Human view and generated reader output, dropping the generated-reader heuristic score from 9 to 7 while preserving the canonical AI/research details, rendered mechanism diagram, and support boundaries. The semantic proof adequacy review is now validator-checked by `python3 scripts/validate_proof_adequacy_review.py` against the 196-target manifest and no-promotion boundary language. This does not prove broad semantic adequacy, does not validate source interpretation, does not prove deployed enforcement, and does not promote proof-envelope support. Remaining work: upgrade high-value projection hooks where the proof model is too shallow and add consumer-gate receipts before stronger formal-methods claims. |
...
- The ten draft key figures are now embedded/adapted into the curated reader
  ...
  checks rendered curated-reader HTML DOM presence for all ten. Remaining work
  is visual figure polish, EPUB/DOCX/PDF/e-reader/audio-specific inspection,
...
- Ten draft key-figure assets tied to the reader handoff contract now exist and
  ...
  `scripts/validate_reader_key_figure_contrast.py`, and recorded in
  `docs/reader_key_figure_artifact_review.md`. They remain
...
Validator-enforced status fragments have grown pathological: the
`v1_0_candidate_status.md` "Proof envelope" cell is now a single multi-
thousand-word run-on sentence enumerating ~90 envelope names, and similar
enumerating cells exist on other status surfaces. These are unreadable,
unreviewable, and make every increment's regeneration diff enormous. The
rule: when an enforced fragment exceeds roughly one hundred words, restructure
it into a generated count-plus-ledger-link form (the detailed enumeration
lives in a generated ledger document; the status cell carries counts and a
link), updating the enforcing validators in the same commit. Apply this first
to the Proof envelope cell and the corresponding
`validate_v1_status_snapshot.py` / `validate_book.py` fragments.
