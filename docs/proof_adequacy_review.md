...
proof shape, not semantic adequacy. The current baseline is 143 theorem
declarations: 112 direct/projection-style, 31 derived/decomposed, and 0
unknown or mixed. The v1.0 safety-critical modules now contain 14 theorem
declarations: 10 remain direct/projection-style, and `Alignment`,
`Corrigibility`, `GovernanceRights`, and `SelfImprovement` each have one
derived/decomposed finite-record theorem.
