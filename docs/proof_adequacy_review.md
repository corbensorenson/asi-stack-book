...
proof shape, not semantic adequacy. The current baseline is 139 theorem
declarations: 112 direct/projection-style, 27 derived/decomposed, and 0
unknown or mixed. All 10 theorem declarations in the v1.0 safety-critical
modules `Alignment`, `Corrigibility`, `GovernanceRights`, `SelfImprovement`,
and `ValueConflict` are direct/projection-style.
...
|---|---|---|
| `constitutional-alignment-substrate` | `AsiStackProofs.Alignment` | Projection-only traceability; requires richer transition/review semantics before it can support alignment-behavior claims. |
