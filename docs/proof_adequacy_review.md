...
proof shape, not semantic adequacy. The current baseline is 141 theorem
declarations: 112 direct/projection-style, 29 derived/decomposed, and 0
unknown or mixed. The v1.0 safety-critical modules now contain 12 theorem
declarations: 10 remain direct/projection-style, and `Alignment` plus
`Corrigibility` each have one derived/decomposed finite-record theorem.
