...
proof shape, not semantic adequacy. The current baseline is 152 theorem
declarations: 112 direct/projection-style, 40 derived/decomposed, and 0
unknown or mixed. The v1.0 safety-critical modules now contain 23 theorem
...
| `governance-rights-fork-exit-and-audit` | `AsiStackProofs.GovernanceRights` | Mixed finite-record envelope: two projection-only hooks plus one derived/decomposed constrained-fork audit-path theorem; still requires richer access-path, fork-safety, appeal, and institutional semantics before it can support governance-rights claims. |
...
### Governance Rights Constrained-Fork Envelope

The Governance Rights follow-through increment adds a finite
`ForkGovernanceDecision` envelope and the theorem
`constrained_fork_without_audit_path_routes_to_review`. The theorem uses the
declared governance-safety predicate to show that a modeled constrained fork
with no preserved audit path must route to `blockedForReview`.

This moves `AsiStackProofs.GovernanceRights` from entirely projection-style to
...
review semantics`; this does not prove real access-path availability, legal
rights, fork safety, institutional contestability, redaction quality, or SCF
