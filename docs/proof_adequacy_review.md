...
proof shape, not semantic adequacy. The current baseline is 144 theorem
declarations: 112 direct/projection-style, 32 derived/decomposed, and 0
unknown or mixed. The v1.0 safety-critical modules now contain 15 theorem
declarations: 10 remain direct/projection-style, and all five safety-critical
modules have one derived/decomposed finite-record theorem.
...
| `recursive-self-improvement-boundaries` | `AsiStackProofs.SelfImprovement` | Mixed finite-record envelope: two projection-only hooks plus one derived/decomposed authority-widening review-route theorem; still requires richer canary, monitor, rollback, evaluator, and boundary-delta semantics before it can support recursive self-improvement safety claims. |
...
### Self-Improvement Authority-Widening Envelope

The Self-Improvement follow-through increment adds a finite
`ImprovementBoundaryDecision` envelope and the theorem
`authority_widening_without_review_route_is_blocked`. The theorem uses the
declared improvement-boundary predicate to show that a modeled proposed change
that widens authority with no explicit review route must route to
`blockedForReview`.

This moves `AsiStackProofs.SelfImprovement` from entirely projection-style to
mixed finite-record depth in the proof-depth classifier: two theorem
declarations remain projection-only traceability, while one is
derived/decomposed. The chapter remains in `needs richer state-machine or
review semantics`; this does not prove evaluator independence in practice,
runtime self-improvement safety, rollback effectiveness, verification-budget
preservation, boundary-delta adequacy, or phase-gate completion.
