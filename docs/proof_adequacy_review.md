...
proof shape, not semantic adequacy. The current baseline is 155 theorem
declarations: 112 direct/projection-style, 43 derived/decomposed, and 0
unknown or mixed. The v1.0 safety-critical modules now contain 26 theorem
...
`AsiStackProofs.GovernanceRights` now has four, while
...
| `moral-uncertainty-and-value-conflict` | `AsiStackProofs.ValueConflict` | Mixed finite-record envelope: two projection-only hooks plus one derived/decomposed unresolved-conflict revisit-path theorem; still requires richer tribunal, stakeholder, reversibility, and uncertainty semantics before it can support value-conflict safety claims. |
