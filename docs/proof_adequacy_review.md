...
proof shape, not semantic adequacy. The current baseline is 142 theorem
declarations: 112 direct/projection-style, 30 derived/decomposed, and 0
unknown or mixed. The v1.0 safety-critical modules now contain 13 theorem
declarations: 10 remain direct/projection-style, and `Alignment`,
`Corrigibility`, and `GovernanceRights` each have one derived/decomposed
finite-record theorem.
