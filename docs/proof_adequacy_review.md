...
proof shape, not semantic adequacy. The current baseline is 149 theorem
declarations: 112 direct/projection-style, 37 derived/decomposed, and 0
unknown or mixed. The v1.0 safety-critical modules now contain 20 theorem
...
`AsiStackProofs.Alignment` now has four, and
...
| `agency-dignity-and-corrigibility` | `AsiStackProofs.Corrigibility` | Mixed finite-record envelope: two projection-only hooks plus one derived/decomposed high-impact review-path theorem; still requires richer lifecycle, rights-usability, and correction-pathway semantics before it can support corrigibility claims. |
...
### Corrigibility High-Impact Review Envelope

The Corrigibility follow-through increment adds a finite
`AgencyActionDecision` envelope and the theorem
`high_impact_action_without_usable_review_routes_to_review`. The theorem uses
the declared corrigibility predicate to show that a modeled high-impact action
with no usable review path must route to `blockedForReview`.

This moves `AsiStackProofs.Corrigibility` from entirely projection-style to
...
review semantics`; this does not prove human dignity, manipulation resistance,
runtime corrigibility, institutional review quality, consent quality, material
usability of real interfaces, or deployed high-impact approval behavior.
