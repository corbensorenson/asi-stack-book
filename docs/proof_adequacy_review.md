...
This review classifies the 115 implemented Lean proof targets by adequacy for the claims they are attached to. It does not change any support state and does not claim broad system correctness.
...
| useful but too narrow | 36 | The predicate is a useful guard, but the chapter needs state-machine, trace, integration, empirical, or benchmark tests before the proof can support the larger boundary. |
...
All 115 targets still build as narrow finite-record predicates. The review changes the project backlog, not the theorem status.
...
proof shape, not semantic adequacy. The current baseline is 200 theorem
declarations: 115 direct/projection-style, 85 derived/decomposed, and 0
