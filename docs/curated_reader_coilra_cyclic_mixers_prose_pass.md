# Curated Reader Prose Pass: CoilRA, MultiCoil RoPE, and Cyclic Mixers

Date: 2026-06-30

Chapter: `coilra-multicoil-rope-and-cyclic-mixers`

Reader manuscript file:
`editions/reader_manuscript/v1_0/chapters/coilra-multicoil-rope-and-cyclic-mixers.qmd`

Live source file:
`chapters/coilra-multicoil-rope-and-cyclic-mixers.qmd`

## Reader Promise

This pass should let a human reader understand that cyclic substrates are
allowed into the architecture as evaluated candidates, not as shortcuts around
baselines.

The reader throughline is:

> structure can earn a receipt before it earns adoption.

## Curation Scope

- Reworked the generated reader baseline into a more continuous chapter about
  cyclic-substrate adoption, ledger separation, baseline symmetry, and
  tradeoff packets.
- Centered the chapter on separating structural guarantees from model quality,
  context length, runtime, memory, training stability, hardware efficiency, and
  deployment claims.
- Preserved CoilRA, MultiCoil RoPE, RoPE position distinguishability, cyclic
  mixer, residue/winding, alias/load diagnostic, parameter-accounting,
  hardware-refusal, baseline-matrix, negative-control, and canary-route
  boundaries.
- Kept the Mermaid diagram as a cyclic-substrate evaluation flow from candidate
  to structural invariant, receipt boundary, diagnostics, baselines, measured
  tradeoff, and adoption decision.
- Preserved the minimum viable implementation and beyond-state-of-the-art
  endpoint without implying a deployed cyclic-substrate evaluation lane.
- Preserved the proof/test boundary for `AsiStackProofs.CyclicMixers` and the
  cyclic-mixer evaluation fixture boundary.

## Meaning-Preservation Checks

- The live chapter core claim remains a design rationale at `argument` support.
- The curated reader chapter does not change Appendix C, Appendix K, source
  mappings, proof status, test status, evidence level, or release records.
- The prose keeps structural receipts, parameter counts, baseline metrics,
  quality, runtime, memory, context length, hardware fit, training stability,
  and transfer as separate claim axes.
- Circle references remain contract-family and receipt-boundary context, not
  local evidence of model quality, cyclic-mixer performance, RoPE deployment
  behavior, or downstream adoption.
- Theseus transfer references remain source-reported transfer-boundary context;
  no private payload, report artifact, router-head trace, runtime measurement,
  or promotion evidence is imported.
- RoFormer/RoPE, LoRA, Mamba, and RetNet remain external comparator families
  only; no external result is reproduced or promoted here.
- The implemented cyclic-mixer fixture remains a schema/protocol-shape
  validation only, not a RoPE certifier run, cyclic mixer benchmark, MLX
  experiment, hardware-kernel benchmark, downstream quality evaluation,
  Circle contract pack, or Theseus transfer consumer.

## Non-Claims

- This pass does not edit the live AI/research chapter.
- This pass does not change `book_structure.json`.
- This pass does not alter Appendix C or Appendix K.
- This pass does not add, remove, normalize, or promote source records.
- This pass does not claim a RoPE certifier run, sidecar regeneration, Circle
  contract-pack generation, cyclic mixer benchmark, MLX model experiment,
  hardware-kernel benchmark, downstream quality evaluation, external Circle
  Lean build, Theseus transfer consumer, model-quality result, context-length
  result, runtime result, memory-savings result, training-stability result,
  hardware-efficiency result, deployment-readiness result, or transfer result.
- This pass does not promote any chapter core claim above `argument`.
- This pass does not approve the curated reader manuscript or any EPUB, DOCX,
  PDF, HTML, audio, or audio-embedded EPUB artifact for release.

## Remaining Blockers

- Human editorial review has not approved this chapter for reader release.
- A real cyclic-substrate packet remains planned: a structural receipt accepted
  for diagnostics, a quality-promotion request rejected for missing baselines,
  a hardware mismatch narrowing deployment scope, a wrong-period or nonperiodic
  negative control, and a canary route allowed only after baseline-symmetric
  metrics exist.
- Stronger cyclic-substrate claims require accepted evidence-transition records
  with exact workloads, baselines, hardware notes, negative controls, commands,
  limitations, counterevidence, and non-claims.
- Any future use of external comparator results requires direct source review
  and local or recorded artifact boundaries appropriate to the claim.
