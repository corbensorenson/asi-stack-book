...
technical-route edges. EM4 now owns only public presentation, legacy routing,
browser/accessibility review, and the explicit no-support/no-release cutover
record.
...
| M7 — Editorial product migration, independent narrative, and derivative disposition | in progress; EM0-EM3 and EM4 crosswalk complete | Preserve `reader-2026-07-18`, the 84-chapter virtual derivative, and the current candidate as immutable historical receipts while maintaining all 87 research identities. EM0 manifest-derived count truth, EM1 exact metadata-first 54+2/16+2+0/7/5/1 disposition, seven no-cutover EM2 composition packages covering all sixteen publication nests and two method-detail nests, a terminal no-merge adjudication for Prototype Roadmap/Project Theseus, all twenty-six target-length EM3 units at 133,658 visible words, and the generated 87-owner conclusion/claim crosswalk are complete. Finish EM4: public Human Reader presentation, legacy routing, browser/accessibility review, and cutover validation; retain the fifteen-minute utility route, three runnable task recipes, optional-depth marking, and unit-level use/avoid/conclusion-change guidance. Defer the primer and full EPUB/PDF/DOCX/audio build until major-version content freeze. P7.3 remains separately owned and cannot substitute for or block narrative completion. |
