...
Status: started across all priority queues. Initial backfill passes added forty-five primary external source records and source notes across alignment/control, AI governance/evaluation, planning/agent control, retrieval/context, formal methods, routing/MoE, compression/representation, and benchmark science, summarized in `docs/external_literature_backfill_phase6.md`. The planning slice now includes ReAct, Tree of Thoughts, PDDL, SHOP2, Integrated TAMP, behavior trees, GOAP/F.E.A.R., and AutoGen as comparison vocabulary only. The retrieval/context slice now includes RAG, Lost in the Middle, MemGPT, LongBench, RULER, ALCE, Self-RAG, and LongLLMLingua as context-interface, citation-support, adaptive-retrieval, long-context-evaluation, and prompt-compression vocabulary only. The formal-methods slice now includes proof-carrying code, TLA+, Lean theorem proving, Dafny, and Reluplex as proof, specification, and property-verification vocabulary only. The routing/MoE slice now includes sparse MoE, GShard, Switch Transformers, Expert Choice Routing, Mixtral, and a MoE-in-LLMs survey as model-routing and systems vocabulary only. The compression slice now includes Deep Compression, LoRA, knowledge distillation, GPTQ, and QLoRA as compression/adaptation vocabulary only. The benchmark-science slice now includes MMLU, BIG-bench, HELM, GPQA, SWE-bench, and LiveBench as benchmark-design and evaluation-governance vocabulary only. No claim support state, reproduction result, compliance claim, imported formal artifact, proof-assistant import, verifier run, planner run, motion-planning run, context benchmark run, citation-evaluation run, context-compression run, MoE training or inference run, compression experiment, finetuning run, benchmark run, or evidence transition changed.
...
6. Router evaluation, modular-agent orchestration, task-level routing, governance-aware route selection, and model/system routing.
