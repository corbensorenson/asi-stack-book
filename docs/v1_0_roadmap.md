...
| The first measured support transition was missing | Resolved for two narrow claims. `docs/first_measured_replayed_slice.md` and `evidence_transitions/v1_0_measured/phase5_harness_runner_synthetic_test_backed.json` record `living-book-methodology.phase5_harness_registry_runner` moving from `argument` to `synthetic-test-backed`; `docs/costed_route_resource_slice.md` and `evidence_transitions/v1_0_measured/costed_route_resource_slice_synthetic_test_backed.json` record `resource-economics.costed_route_budget_slice` moving from `argument` to `synthetic-test-backed`; all 54 chapter core claims remain at `argument`, 26 chapter core claims have accepted no-change records, and 28 chapter core claims have accepted explicit no-promotion decisions in `claim_decisions/v1_0_core_claim_no_promotion.json` | Phase 3B now has bounded infrastructure and non-infrastructure synthetic transitions. The next evidence work should move toward public-safe prototype, empirical measurement, imported trace, or replay lanes before any chapter core claim can move. |
...
| Phase 3B | v1.0-complete for bounded infrastructure and non-infrastructure slices | Accepted transitions now cover the narrow Phase 5 registry-runner claim and the bounded costed-route/resource-budget selector claim; stronger prototype or empirical lanes remain needed before chapter core claims can move. |
...
| Phase 5 | v1.0-complete for first non-infrastructure synthetic slice; deeper prototype/empirical evidence remains v1.x unless promoted | Harness runner, coverage drift guard, and costed-route/resource-budget measured-slice validation are implemented. Next Phase 5 work should move toward public-safe prototype traces, empirical measurements, or imported receipts. |
...
| Phase 7 | v1.0-blocking for final tag metadata; candidate accessibility/progress ledgers complete | Public-site accessibility readiness and compact progress ledger are recorded; final release-record metadata still waits for the actual tag/archive facts. |
...
Status: complete for v1.0 claim-state coverage. Twenty-six no-change evidence-transition records are recorded under `evidence_transitions/v1_0_pilot/`, summarized in `docs/evidence_transition_pilot.md`, and validated by `scripts/validate_evidence_transitions.py`. The remaining twenty-eight chapter core claims have accepted explicit no-promotion decisions in `claim_decisions/v1_0_core_claim_no_promotion.json`, summarized in `docs/core_claim_transition_coverage.md`, and validated by `scripts/validate_core_claim_decisions.py`. Separate measured/replayed records under `evidence_transitions/v1_0_measured/` accept the narrow repository-infrastructure transition `living-book-methodology.phase5_harness_registry_runner` and the bounded non-infrastructure transition `resource-economics.costed_route_budget_slice` from `argument` to `synthetic-test-backed`. All 54 chapter core claims remain at `argument`.
...
3. Create or update evidence transition records. The no-change pilot now has twenty-six JSON records under `evidence_transitions/v1_0_pilot/`; the measured/replayed set has two accepted bounded records under `evidence_transitions/v1_0_measured/`.
...
5. Update Appendix C only after a chapter-core transition is accepted. No Appendix C chapter support-state update was made because the accepted upward transitions are scoped to the repository-infrastructure runner claim and the bounded synthetic costed-route/resource-budget selector claim, not to any chapter core claim.
...
Status: bounded infrastructure and non-infrastructure slices accepted.
...
`synthetic-test-backed`. No chapter core claim has moved above `argument`, and
no deployed runtime, scheduler, model-quality, safety, benchmark, economic, or
source-interpretation claim is promoted.
...
- At least one measured or replayed slice exists with a public-safe result record and an accepted transition, or the v1.0 release explicitly downgrades itself from evidence release. Current v1.0 status satisfies this for the bounded registry-runner infrastructure claim and the bounded synthetic costed-route/resource-budget selector claim.
...
Next Phase 5 evidence work should move from synthetic record gates and selector slices toward replayable empirical slices or imported prototype traces where the source artifacts are available and public-safe.
...
- Preserve the current explicit harness result records, the first registry-runner transition, and the bounded costed-route/resource-budget selector transition, but prioritize the next increment toward prototype or empirical measured/replayed evidence: a real route-quality/resource-budget trace, a compression/RankFold measurement, a context-admission replay, a planner/runtime adapter trace, or another public-safe prototype slice with command, environment, input, output, negative control, and non-claim boundaries.
...
- Maintain `docs/v1_progress_ledger.md` so completed Phase 2, Phase 5, and Phase 6 history can move out of the execution roadmap over time; the roadmap should foreground remaining blockers, acceptance criteria, and release gates.
...
depth from bounded synthetic slices toward prototype or empirical receipts,
