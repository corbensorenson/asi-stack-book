...
- The matrix currently records 54 chapter rows: 14 `reviewed` rows from the first four full generated-reader chapter-text review passes, 40 `spot_checked` rows from existing representative reader-review notes plus the first Part II, Part III, and Part IV review passes, 0 `not_started` rows, 20 chapters with active reader overlays, 41 no-immediate-action decisions, 3 companion-note candidates, and 1 curated-manuscript candidate.
- The 14 reviewed Part I rows retain reader-release and format-artifact blockers. The remaining 40 rows still carry release blockers for missing full chapter review, missing reader release record, and missing artifact review. The matrix is a review queue, not a reviewed reader release.
