...
and 8 curated-manuscript candidates. `docs/reader_format_dry_run.md` records a
...
- The matrix currently records 54 chapter rows: 54 `reviewed` rows from the full generated-reader chapter-text review passes, 0 `spot_checked` rows, 0 `not_started` rows, 20 chapters with active reader overlays, 54 no-immediate-action decisions, 3 companion-note candidates, and 8 curated-manuscript candidates.
