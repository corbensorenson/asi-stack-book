...
Status: started. The generated reader baseline was produced and recorded in `docs/reader_manuscript_review.md`; the active semantic reader-overlay log is recorded in `docs/reader_overlay_pilot.md` with two opening-chapter operations, one Efficient ASI table-to-prose operation, one Human Intent table-to-prose operation, one System Boundaries table-to-prose operation, one Evidence States table-to-prose operation, six Personal Compute Hives table/prose operations, two Command Contracts table-to-prose operations, one Planning table-to-prose operation, one Verification Bandwidth table-to-prose operation, one Runtime Adapters table-to-prose operation, one Labor OS table-to-prose operation, two Circle Contracts table-to-prose operations, one Generate-Verify-Repair table-to-prose operation, two Fast Generation table/code-to-prose operations, one RankFold/NeuralFold table-to-prose operation, one Mathematical and Search Substrates table-to-prose operation, two Policy Optimization table-to-prose operations, three Artifact Steward Agents table/prose operations, one Executable Specifications prose operation, and two Semantic Representation table-to-prose operations; the generated heuristic continuity audit is recorded in `docs/reader_continuity_audit.md`; the first manual medium-priority decisions are recorded in `docs/reader_continuity_review.md`; the first Part I matrix review pass is recorded in `docs/reader_part_i_review_pass.md`; the first Part II matrix review pass is recorded in `docs/reader_part_ii_review_pass.md`; the first Part III matrix review pass is recorded in `docs/reader_part_iii_review_pass.md`; the first Part IV matrix review pass is recorded in `docs/reader_part_iv_review_pass.md`; the first opening full-review pass is recorded in `docs/reader_opening_full_review_pass.md`; the second boundary/intake full-review pass is recorded in `docs/reader_boundary_full_review_pass.md`; the third normative full-review pass is recorded in `docs/reader_normative_full_review_pass.md`; the Part I full-review completion pass is recorded in `docs/reader_part_i_full_review_completion.md`; the first Part II contracts full-review pass is recorded in `docs/reader_part_ii_contracts_full_review_pass.md`; the synced chapter review matrix is recorded in `editions/reader_manuscript/v1_0/chapter_review_matrix.json` and summarized in `docs/reader_chapter_review_matrix.md` with 18 `reviewed` chapters, 36 `spot_checked` chapters, 0 `not_started` chapters, 20 chapters carrying active reader overlays, 43 no-immediate-action decisions, 3 companion-note candidates, and 1 curated-manuscript candidate; `docs/reader_format_dry_run.md` records a local HTML/EPUB/DOCX render dry run, basic structural artifact inspection, and UTF-8 PDF probe with ignored snapshots; and `docs/reader_artifact_layout_review.md` records the first representative PDF/HTML layout spot check. Release-grade full chapter review for the remaining 36 chapters, broader reader overlays, broader artifact layout/navigation inspection, audio, and release records remain open.
...
- The matrix currently records 54 chapter rows: 18 `reviewed` rows from the first five full generated-reader chapter-text review passes, 36 `spot_checked` rows from existing representative reader-review notes plus the first Part II, Part III, and Part IV review passes, 0 `not_started` rows, 20 chapters with active reader overlays, 43 no-immediate-action decisions, 3 companion-note candidates, and 1 curated-manuscript candidate.
- The reviewed rows retain reader-release and format-artifact blockers. The remaining 36 rows still carry release blockers for missing full chapter review, missing reader release record, and missing artifact review. The matrix is a review queue, not a reviewed reader release.
