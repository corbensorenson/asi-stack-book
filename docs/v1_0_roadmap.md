...
7. Record actual render outcomes without claiming publication until review and release records exist. The first local dry run rendered HTML, EPUB, and DOCX and snapshotted them under ignored `build/reader_edition/format_artifacts/`, but did not attempt PDF and did not create a release record.
...
Status: preparation reviewed; still future and blocked on reviewed reader manuscript. `docs/v1_0_release_preparation_review.md` records passing release-profile, reader-spine, reader-boundary, reader-overlay, reader-edition-check, reader-format-check, reader-format dry-run, and audio-script-check commands. `docs/reader_format_dry_run.md` records local HTML/EPUB/DOCX snapshots in ignored `build/` space, but no tag, reviewed reader release, PDF artifact, audiobook, or edition release record was produced.
