...
- The audit measures 54 reader chapters, 121,684 reader words, 33 active/applied reader-overlay operations, 0 table rows, 58 Mermaid diagrams, 0 non-Mermaid code blocks, 0 paragraphs at or above 160 words, and 0 repeated first-sentence stems under the current heuristic.
...
- The matrix currently records 54 chapter rows, 49 `spot_checked` rows from existing representative reader-review notes plus the first Part I, Part II, and Part III review passes, 5 `not_started` rows, 20 chapters with active reader overlays, 30 no-immediate-action decisions, 3 companion-note candidates, and 1 curated-manuscript candidate.
