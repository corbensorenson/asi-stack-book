...
7. `appendices/G_corben_source_corpus.qmd` for Corben's authored/supplied sources and local project references, and `appendices/H_external_sources.qmd` for external-literature references by other authors when citations or source placeholders are in scope.
