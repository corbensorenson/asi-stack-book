...
| `system-boundaries-and-authority.formal.authority-ceiling-failure-blocks-promotion` | `formal_target` | `formal` | `semantically_reviewed` | Revoked grant IDs persist and cannot commit an effect in any successful suffix; the independent consumer checks one-use, two-use, and revocation traces, every prefix and batch split, and 50 state-noninterfering widening, substitution, freshness, receipt, observation, rollback, revocation, and reuse mutations. |
