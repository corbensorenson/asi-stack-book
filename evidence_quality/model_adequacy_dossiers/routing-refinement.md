# Model-adequacy dossier: routing refinement

## Decision

Adequate for a finite authored request-to-closure routing invariant and its countermodels. Inadequate for natural-task usefulness, substrate comparison, deployed routing, or recursive architectural self-improvement.
...
- `scripts/validate_routing_refinement.py` independently reimplements route priority.
- `scripts/validate_routing_decision_lease.py` preserves 3 valid and 7 expected-invalid fixtures.
- `scripts/validate_readiness_residual_gates.py` preserves 4 valid and 5 expected-invalid fixtures.
- `scripts/validate_post_v2_routing_deliberation.py` preserves the exact bounded study, including zero fallback activations, fifteen extra-compute harms, and no support-state change.

## Trusted assumptions

The finite model trusts the authored identifiers, booleans, digests, outcome records, runtime/replay reference-presence flags, and consumer acknowledgment. It does not independently establish semantic task fit, evaluator independence, runtime execution, replay fidelity, answer correctness, or revocation propagation outside the model.
