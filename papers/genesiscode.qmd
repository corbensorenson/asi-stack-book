...
| Exact published-source SHA-256 | `dcc3902d01fc6b82ae4693fccd59f399db5a66399c62412b49d98fa93ba09ca6` |
| Exact published-source bytes | 60,187 |
| Exact source text | [Download/view the tracked Markdown source](source/genesiscode.md) |
| Book's source note | [Read the bounded mining note](https://github.com/corbensorenson/asi-stack-book/blob/main/sources/source_notes/genesiscode.md) |
...
[System Boundaries and Authority](../chapters/system-boundaries-and-authority.html), [Command Contracts: From Intent to Executable Work](../chapters/intent-to-execution-contracts.html), [Cognitive Compilation and Semantic IR](../chapters/cognitive-compilation-and-semantic-ir.html), [Proof-Carrying Claims and Adversarial Review](../chapters/spinoza-verification-and-proof-carrying-claims.html), [Labor OS and Typed Jobs](../chapters/labor-os-and-typed-jobs.html), [Artifact Graphs, Audit Logs, and Replay](../chapters/artifact-graphs-audit-logs-and-replay.html), [Runtime Adapters, Tool Permissions, and Human Approval](../chapters/runtime-adapters-tool-permissions-and-human-approval.html), [Mathematical and Search Substrates](../chapters/mathematical-and-search-substrates.html), [Executable Specifications and Lean Proof Envelope](../chapters/executable-specifications-and-lean-proof-envelope.html), [Open Research Agenda and Bibliography Plan](../chapters/open-research-agenda-and-bibliography-plan.html)
...
dispatch(c, msg):
  r = c.handler(msg)
  if is_sealed_unhandled(r) and c.proto != nil:
...
I’ve also included templates for AGENTS.md and a Codex Skill you can drop into .agents/skills/ so the agent reliably follows the same workflow every time. Codex explicitly supports project-level instruction files (AGENTS.md / AGENTS.override.md) and skill folders with SKILL.md. (OpenAI Developers)
________________
...
genesiscode/
  ...
  crates/
    gc_coreform/        # AST + parser/printer + canonicalization
    gc_kernel/          # Gλ evaluator + values + env + primitives + seals
    gc_prelude/         # contracts + dispatch/extend/explain + sealed protocol
    gc_effects/         # effect IR + runner interface + effect log + replay
    gc_obligations/     # obligations engine + evidence store + policy
    gc_patches/         # semantic patch schema + apply/validate
    gc_cli/             # CLI executable wiring everything
  prelude/
  ...
  docs/
    PAPER_v0.2.md      # the paper you wrote
...
Codex Desktop is explicitly built to run multiple tasks in parallel with worktrees and built-in Git tooling. (OpenAI Developers) Use that structure from the start:
...
### Mission
Implement GenesisCode v0.2 exactly per docs/PAPER_v0.2.md and docs/TECH_HANDOFF.md.
...
Codex will automatically prefer AGENTS.override.md over AGENTS.md in a directory and will layer from the git root down to the working directory. (OpenAI Developers)
________________


13) Codex Skills (optional but HIGH leverage)
Codex skills are a supported way to package instructions/resources/scripts so the agent can reliably follow workflows; skills use progressive disclosure and are discovered in .agents/skills/ directories (scanned upward toward repo root). (OpenAI Developers)
...
You can explicitly invoke a skill in Codex (e.g., with $ selection) or let it be auto-chosen based on its description. (OpenAI Developers)
________________
...
GPT‑5.3‑Codex is explicitly described as being able to handle long-running tasks with tool use and complex execution, and the Codex app is designed for supervising parallel agent work via threads/worktrees. (OpenAI)
________________
