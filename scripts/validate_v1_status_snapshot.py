...
def main() -> None:
    ...
    expected_fragments = [
        ...
        f"| Support states | {evidence_counts.get('argument', 0)} chapter core claims at `argument`; the v1.0 claim-state coverage gate records twenty-six accepted no-change transition records plus twenty-eight accepted explicit no-promotion decisions, and the separate measured/replayed set records two bounded `synthetic-test-backed` transitions for `living-book-methodology.phase5_harness_registry_runner` and `resource-economics.costed_route_budget_slice`; no chapter core claim support-state promotion |",
