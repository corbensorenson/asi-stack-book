...
def main() -> None:
    ...
    expected_fragments = [
        ...
        f"| Support states | {evidence_counts.get('argument', 0)} chapter core claims at `argument`; the initial v1.0 evidence-transition pilot records four accepted no-change decisions and no support-state promotion |",
        ...
        "the reader release has a tracked semantic overlay manifest as the editable delta source, generated reader delta report path as review output with a zero-active-operation note or operation digests and before/after excerpts, embedded live Human-view overlay payload for major-version human-edition deltas, and a generated reader-continuity audit with 0 high-priority heuristic review rows",
        "The current v1.0 reader-overlay set carries 28 active operations: 2 opening-chapter operations, 1 Efficient ASI table-to-prose operation, 1 Human Intent table-to-prose operation, 1 System Boundaries table-to-prose operation, 1 Evidence States table-to-prose operation, 4 Personal Compute Hives table-to-prose operations, 2 Command Contracts table-to-prose operations, 1 Planning table-to-prose operation, 1 Verification Bandwidth table-to-prose operation, 1 Runtime Adapters table-to-prose operation, 1 Labor OS table-to-prose operation, 1 Circle Contracts table-to-prose operation, 1 Generate-Verify-Repair table-to-prose operation, 2 Fast Generation table/code-to-prose operations, 1 RankFold/NeuralFold table-to-prose operation, 1 Mathematical and Search Substrates table-to-prose operation, 2 Policy Optimization table-to-prose operations, 2 Artifact Steward Agents table-to-prose operations, and 2 Semantic Representation table-to-prose operations for Human view and generated reader editions only",
        "The curated reader-manuscript manifest exists with `not_graduated` status so the future human-prose source can diverge only as a subordinate narrative derivative.",
