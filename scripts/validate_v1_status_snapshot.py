...
def main() -> None:
    ...
    expected_fragments = [
        ...
        f"| Support states | {evidence_counts.get('argument', 0)} chapter core claims at `argument`; the v1.0 evidence-transition pilot records twenty-three accepted no-change decisions and no support-state promotion |",
        "`python3 scripts/validate_evidence_transitions.py`",
        "| Test harnesses | Twenty synthetic or deterministic harnesses are wired into book validation and the Phase 5 registry:",
