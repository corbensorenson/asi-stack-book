...
def main() -> None:
    ...
    expected_fragments = [
        ...
        f"| Support states | {evidence_counts.get('argument', 0)} chapter core claims at `argument`; the v1.0 evidence-transition pilot records thirteen accepted no-change decisions and no support-state promotion |",
        "`python3 scripts/validate_evidence_transitions.py`",
        "| Test harnesses | Ten synthetic or deterministic harnesses are wired into book validation and the Phase 5 registry:",
        ...
        "`docs/reader_docx_probe_manifest.md` records the current 514-page, 8,190,127-byte DOCX LibreOffice conversion probe, expected title/evidence-boundary/source-card text, refreshed sampled source-card pages, and remaining full-format-review blocker",
        ...
        "`docs/reader_format_review_matrix.md` records format-level blockers for HTML, EPUB, DOCX, and PDF.",
