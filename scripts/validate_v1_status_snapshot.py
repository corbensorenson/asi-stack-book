...
def main() -> None:
    ...
    expected_fragments = [
        ...
        "| Test harnesses | Thirty-three synthetic, deterministic, or local replay harnesses are wired into book validation; twenty-two are wired into the Phase 5 registry and eleven chapter-specific/support harnesses are book-gate-only: the artifact graph replay harness, the procedural memory loop harness, the routing decision lease harness, the cyclic memory contract harness, the context transaction memory-store harness, the simulation transfer boundary harness, the resource workflow trace harness, the resource live probe harness, the Theseus support replay probe, the Compact GVR synthetic slice, and the hive admission harness;",
