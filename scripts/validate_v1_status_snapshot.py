...
def main() -> None:
    ...
    expected_fragments = [
        ...
        "| Test harnesses | Twenty-eight synthetic or deterministic harnesses are wired into book validation; twenty-two are wired into the Phase 5 registry and six chapter-specific harnesses are book-gate-only: the artifact graph replay harness, the procedural memory loop harness, the routing decision lease harness, the cyclic memory contract harness, the context transaction memory-store harness, and the simulation transfer boundary harness;",
