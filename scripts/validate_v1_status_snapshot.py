...
def main() -> None:
    ...
    expected_fragments = [
        ...
        "| Source appendix ownership | Appendix G is reserved for Corben-authored, supplied, and local source records; Appendix H is reserved for external source records marked `external_literature` |",
        ...
        f"must be at least 130 words excluding the source-only heading, with the current minimum at {human_min_words} words",
    ]
    ...
    if human_min_words < 130:
        errors.append(f"Human Reading Path prose minimum is {human_min_words}, below 130.")
