...
def main() -> None:
    ...
    expected_fragments = [
        ...
        "| Test harnesses | Thirty synthetic, deterministic, or local replay harnesses are wired into book validation; twenty-two are wired into the Phase 5 registry and eight chapter-specific harnesses are book-gate-only: the artifact graph replay harness, the procedural memory loop harness, the routing decision lease harness, the cyclic memory contract harness, the context transaction memory-store harness, the simulation transfer boundary harness, the resource workflow trace harness, and the resource live probe harness;",
