...
def main() -> None:
    ...
    expected_fragments = [
        ...
        "| Source appendix ownership | Appendix G (`Corben-Authored, Supplied, and Local Sources`) and Appendix H (`External Sources by Other Authors`) are independent top-level appendices with explicit ownership-rule rows: G contains Corben-authored, Corben-supplied, recovered, and local project records; H contains external records by other authors marked `external_literature` |",
        ...
        "| Chapter handoffs | All 30 Part I and Part II chapters now end with manifest-driven `Handoff` sections that name the next manifest chapter title and avoid numbered chapter references; Parts III and IV remain ratchet targets |",
