...
def main() -> None:
    ...
    expected_fragments = [
        ...
        f"| Support states | {evidence_counts.get('argument', 0)} chapter core claims at `argument`; the v1.0 claim-state coverage gate records twenty-six accepted no-change transition records plus twenty-eight accepted explicit no-promotion decisions, and the separate measured/replayed set records one bounded `synthetic-test-backed` transition for `living-book-methodology.phase5_harness_registry_runner`; no chapter core claim support-state promotion |",
        ...
        "The synced chapter review matrix records 54 reader-review rows with 54 `reviewed`, 0 `spot_checked`, 0 `not_started`, 20 active-overlay chapters, 54 no-immediate-action decisions, 3 companion-note candidates, 1 curated-manuscript candidate, and release blockers on every row until reader release records exist.",
        ...
        "`docs/reader_format_review_matrix.md` records HTML blocked only on an edition release record while EPUB, DOCX, and PDF retain format-specific review blockers.",
