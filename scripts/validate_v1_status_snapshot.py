...
def main() -> None:
    ...
    expected_fragments = [
        ...
        f"| Support states | {evidence_counts.get('argument', 0)} chapter core claims at `argument`; the v1.0 evidence-transition pilot records six accepted no-change decisions and no support-state promotion |",
        ...
        f"| Proof envelope | {proof_targets} proof targets, all implemented as narrow finite-record Lean predicates; current proof adequacy review classifies 8 targets as adequate only for narrow finite-record claims, 27 useful-but-too-narrow, 20 needing richer state or review semantics, 41 needing executable tests first, 10 needing empirical or baseline tests first, and 6 remaining research-agenda until artifact import; follow-through increments add a record-aware allow/deny/escalate authority decision envelope and a record-aware planning control envelope without promoting System Boundaries or Planning above `argument` |",
