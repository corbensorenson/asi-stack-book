...
def main() -> None:
    ...
    expected_fragments = [
        ...
        f"| Proof envelope | {proof_targets} proof targets, all implemented as narrow finite-record Lean predicates; current proof adequacy review classifies 8 targets as adequate only for narrow finite-record claims, 31 useful-but-too-narrow, 19 needing richer state or review semantics, 38 needing executable tests first, 10 needing empirical or baseline tests first, and 6 remaining research-agenda until artifact import; follow-through increments add a record-aware allow/deny/escalate authority decision envelope, a record-aware planning control envelope, a terminal/downgrade evidence-state envelope, a benchmark-ratchet decision envelope, an artifact-steward lifecycle-route envelope, finite claim-ledger and proof-carrying-claim record envelopes, a synthetic runtime adapter permission harness, deterministic generation-mode/resource-budget accounting coverage, a deterministic resource-budget ledger harness, and a capacity-smoothing toy harness without promoting System Boundaries, Planning, Evidence States, Benchmark Ratchets, Artifact Steward Agents, Claim Ledgers, Spinoza, Runtime Adapters, Fast Generation, or Resource Economics above `argument` |",
