...
def main() -> None:
    ...
    expected_fragments = [
        ...
        "| Test harnesses | Thirty-seven synthetic, deterministic, measured, or local replay harnesses are wired into book validation; twenty-two are wired into the Phase 5 registry and fifteen chapter-specific/support harnesses are book-gate-only: the stack layer traceability audit, the artifact graph replay harness, the procedural memory loop harness, the routing decision lease harness, the cyclic memory contract harness, the context transaction memory-store harness, the simulation transfer boundary harness, the resource workflow trace harness, the resource live probe harness, the resource workload-quality probe, the resource load-stability probe, the Theseus support replay probe, the Compact GVR synthetic slice, the hive admission harness, and the cognitive compilation trace harness;",
