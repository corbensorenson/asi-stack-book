...
def main() -> None:
    ...
    expected_fragments = [
        ...
        "| Test harnesses | Twenty-nine synthetic or deterministic harnesses are wired into book validation; twenty-two are wired into the Phase 5 registry and seven chapter-specific harnesses are book-gate-only: the artifact graph replay harness, the procedural memory loop harness, the routing decision lease harness, the cyclic memory contract harness, the context transaction memory-store harness, the simulation transfer boundary harness, and the resource workflow trace harness;",
        ...
        f"The curated reader-manuscript manifest exists with `{reader_manifest.get('status')}` status and {len(curated_records)} drafting-only curated chapter records; its reader handoff contract records the book thesis, part arcs, signature ideas, key-figure targets, Corben voice-pass slots, and chapter stakes/payoffs without release approval; retired standalone reader drafts are archived as history, and the active manifest remains a subordinate narrative derivative whose reconciliation report keeps release blockers active until reconciliation, format review, and an edition release record exist.",
