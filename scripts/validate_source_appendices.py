...
Appendix G is reserved for Corben's papers, Corben-supplied materials, recovered
project records, and local project source records.
Appendix H is reserved for external sources and third-party literature.
...
def main() -> None:
    ...
    if corpus_in_h:
        errors.append(f"Appendix H includes Corben-side or local project source ids: {sorted(corpus_in_h)}")
    if missing_from_g:
        errors.append(f"Appendix G is missing Corben-side or local project source ids: {sorted(missing_from_g)}")
    if missing_from_h:
    ...
    print(
        ...
        f"{len(corpus_ids)} Corben-side or local project source records in Appendix G, "
        f"{len(external_ids)} external-source or third-party literature records in Appendix H."
    )
