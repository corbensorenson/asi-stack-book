...
def errors(data: dict[str, Any]) -> list[str]:
    ...
    if status["execution_readiness"].get("immediate_book_packet") != "P6.5-R16-post-activation-integration-debt" or status["execution_readiness"].get("structural_admission_freeze") is not True:
        out.append("execution board did not preserve terminal operations and A1/A2 while freezing A3 behind Round 16 integration debt")
