...
UNITS = [
    {
        ...
        "artifacts": ["scripts/validate_governed_operations_control_contract.py", "schemas/governed_operations_control_packet.schema.json", "schemas/governed_operations_campaign_preregistration.schema.json", "tests/fixtures/protocol_records/governed_operations_control_packet.valid.json", "experiments/governed_operations_argument_exit/preregistration.json", "docs/p5_natural_stateful_service_campaign_preregistration.md", "lean/AsiStackProofs/GovernedOperations.lean", "lean/AsiStackProofs/GovernedOperationsRefinement.lean", "lean/AsiStackProofs.lean", "chapters/governed-operations-incident-command-and-graceful-degradation.qmd", "evidence_quality/proof_model_dossiers/governed-operations-incident-command-and-graceful-degradation.md"],
        "input_contract": "One authored identity-bound authority-to-effect control case, an eleven-class packet state inventory, a prospectively frozen unexecuted five-arm/forty-task natural-service campaign with fourteen campaign state classes, and a finite static-plus-eight-stage lifecycle refinement model.",
        "output_contract": "Require five-dimensional authority narrowing, independent containment, exact state/effect reconciliation, fresh independent acceptance, qualified fallback, emergency expiry, twenty-six Lean declarations, two explicit static-contract refinement theorems, seven accepted lifecycle transitions, bounded recurrence, one completed development positive control, a sealed natural task population, strong matched baseline, separate evaluator/monitor, and eighteen packet plus fifteen campaign plus forty-four lifecycle rejecting mutations.",
        "output_assertions": ["1 authored safe-hold case", "5 authority dimensions narrow", "8 lifecycle stages", "7 accepted lifecycle transitions", "44 lifecycle mutations reject", "11 packet state classes", "14 campaign state classes", "5 campaign arms", "40 heldout tasks unopened", "9 competence gates", "26 Lean declarations", "77 mutations reject", "support/release/publication none"],
        "claim_scope": "Governed-operations packet shape, finite degradation/recovery routes, and bounded incident-lifecycle refinement semantics only.",
        "negative_controls": "validator_owned_seventy_seven_lifecycle_packet_and_campaign_stage_identity_replay_authority_containment_state_effect_acceptance_expiry_fallback_source_custody_denominator_matching_independence_and_non_authority_mutations",
        "negative_control_cases": ["five authority widenings", "identity deletion", "command loss", "cooperation dependency", "state omission", "stale acceptance", "dependent verifier", "active emergency lease", "fallback loss", "task or outcome opening", "denominator shrink", "strong baseline deletion", "public-effect widening", "independence invention", "T4 laundering", "support and release laundering"],
    ...
    },
    {
        ...
        "artifacts": ["scripts/validate_p7_2_t4_governed_operations_reader_integration.py", "evidence_quality/p7_2_t4_governed_operations_reader_integration.json", "docs/p7_2_t4_governed_operations_reader_integration.md", "book_structure.json", "sources/source_inventory.json", "proofs/proof_manifest.json", "lean/AsiStackProofs/GovernedOperationsRefinement.lean", "roadmap_records/post_v2_3_maintenance_transfer_and_publication_status.json", "index.qmd", "appendices/B_glossary.qmd", "chapters/safety-cases-and-structured-assurance.qmd", "chapters/policy-optimization-and-learning-from-feedback.qmd", "chapters/integrated-reference-architecture.qmd"],
        "input_contract": "The admitted Operations chapter, nine exact source mappings, three implemented targets, two Lean modules with twenty-six declarations, one authored safe-hold case, one prospectively frozen five-arm/forty-task campaign with protected outcomes closed, and adjacent reader surfaces.",
        ...
        "output_assertions": ["4 first-tranche chapters terminal", "9 source mappings", "3 implemented targets", "26 Lean declarations", "8 lifecycle stages", "44 lifecycle mutations reject", "authored case not flagship T4", "5 arms and 40 protected tasks frozen", "0 natural incidents", "13 integration mutations reject", "support effect none"],
    ...
    },
    {
        ...
        "input_contract": "The reconciled four-module resource/artifact/steward/compression cluster with 33 public targets and 44 theorem declarations.",
        ...
        "output_assertions": ["4 adequate modules", "33 public targets", "44 theorem declarations", "6 semantic separations", "4 consumers pass", "12 mutations reject", "all 6 P4 clusters terminal"],
...
def main() -> None:
    ...
    for owner, path_key, digest_key in ((audit, "chapter_path", "chapter_sha256"), (audit["formalization"], "module_path", "module_sha256"), (audit["formalization"], "refinement_module_path", "refinement_module_sha256"), (audit["record_contract"], "schema_path", "schema_sha256"), (audit["record_contract"], "fixture_path", "fixture_sha256"), (audit["claim_bearing_empirical_lane"], "protocol_path", "protocol_sha256")):
