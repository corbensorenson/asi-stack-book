...
def errors(data: dict) -> list[str]:
    ...
    if data["active_roadmaps"]:
        out.append("a roadmap still carries the active canonical marker")
    ...
    for path, body in data["public"].items():
        for phrase in [ROADMAP, STATUS, TERMINAL_RECORD, "v2.3.0", "No successor roadmap is active"]:
            if phrase.casefold() not in body.casefold():
                out.append(f"{path} missing terminal public truth: {phrase}")
        if "is the active canonical successor roadmap" in body or "Active canonical successor roadmap:" in body:
            out.append(f"{path} retains an obsolete active-roadmap claim")
    return out
...
def main() -> None:
    ...
    hidden = copy.deepcopy(base); hidden["active_roadmaps"] = ["docs/fake_roadmap.md"]; mutations.append(("hidden successor", hidden))
    ...
    print("Post-v2.3 handoff/reader/evidence closure passed: P0-P4, M0-M8, HTML+DOCX local approval, EPUB/PDF blockers, 54 argument cores, v2.3.0 unchanged, no active successor, and 8 rejecting mutations.")
