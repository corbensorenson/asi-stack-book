...
from run_resource_live_probe import (
    NON_CLAIMS,
    PROBE_ID,
    REPLAY_COMMANDS,
    RESULT,
...
def fail(errors: list[str]) -> None:
    print("Resource live probe validation failed:")
    for error in errors:
        print(f" - {error}")
    sys.exit(1)
...
def validate_record_shape(value: dict[str, Any], errors: list[str]) -> None:
    ...
    if not isinstance(summary, str) or "five local Resource Economics validators" not in summary:
        errors.append(f"{rel(RESULT)}: summary must describe the five local validator replay.")
    ...
    if artifact_refs != expected_refs:
        errors.append(f"{rel(RESULT)}: artifact_refs must match the tracked Resource Economics replay surface.")
...
def validate_doc(errors: list[str]) -> None:
    ...
    required = [
        ...
        RESULT_COMMAND,
        rel(RESULT),
        "five local Resource Economics validators",
        "Support-state effect | `none`",
        "Chapter-core support effect | `none`",
        "This is not a deployed scheduler result",
...
def main() -> None:
    ...
    if errors:
        fail(errors)
    print(
        "Resource live probe validation passed: "
