...
def load_base() -> Any:
    spec = importlib.util.spec_from_file_location("p4_gu_sweep_v3_base", BASE_RUNNER)
    ...
    module.DESIGN = base / "difficulty_sweep_v3_design.json"
    module.TASKS = base / "difficulty_sweep_v3_tasks.json"
    module.RUBRICS = base / "difficulty_sweep_v3_rubrics.json"
    module.DEFAULT_OUTPUT = base / "results" / "difficulty_sweep_v3_result.json"
    ...
    def require_open_gate() -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        prior = [module.load(base / "results" / name) for name in ("difficulty_sweep_result.json", "difficulty_sweep_v2_result.json")]
        if instrument.get("protocol_outcome") != "instrument_adequate_for_terminal_eligibility_and_residual_contract_only":
            raise SystemExit("V3 tuning gate closed: local instrument adequacy absent.")
        if any(row.get("protocol_outcome") != "non_estimable_operating_range_repair_required" for row in prior):
            raise SystemExit("V3 tuning gate closed: required prior diagnoses absent.")
        if design.get("state") != "frozen_tuning_only_after_second_non_estimable_result" or design.get("entry_gate", {}).get("gate_open") is not True:
            raise SystemExit("V3 tuning gate closed: repair design not prospectively frozen.")
        if design.get("entry_gate", {}).get("outcome_bearing_denominator_open") is not False:
            raise SystemExit("V3 tuning refuses an outcome-bearing denominator.")
