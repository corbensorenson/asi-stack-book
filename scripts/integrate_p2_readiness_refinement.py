...
def main():
 ...
 c['codex_tests'].append({'name':'Readiness candidate-to-terminal refinement','implementation_status':'implemented','result_status':'passes an exact 24-theorem surface, arbitrary finite-run identity and non-authority invariants, exact batch composition, absorbing terminal closure, three exact suites, 40 routes, seven reachable stages, and 45/45 rejecting mutations; one ordinary release, quarantine, and terminal closure remain separate; support-state effect none; no evaluator adequacy, calibration, natural usefulness, effect-complete rollback, deployed quarantine/revocation, transfer, or support claim'})
 STRUCTURE.write_text(json.dumps(s,indent=2)+'\n')
 t=json.loads(TRIAGE.read_text())
 for row in t['records']:
  if row['tag'] in TARGETS:row['module']=MODULE;row['formal_target']=TARGETS[row['tag']];row['rationale']='Exact 24-theorem seven-stage readiness lifecycle with rejection noninterference, arbitrary-run identity and non-authority invariants, composition, absorbing terminal closure, three bounded suites, 40 routes, and 45 rejecting mutations.'
