...
EXPECTED_THEOREMS={
 ...
 'run_events_compose',
 'terminal_event_is_rejected',
 'terminal_state_is_absorbing',
 'incomplete_baseline_blocks_shadow',
 ...
 'missing_useful_throughput_blocks_qualification',
 'missing_independent_evaluation_blocks_default',
 'missing_transfer_blocks_default',
 ...
 'incomplete_revocation_blocks_termination',
...
def formal_surface()->dict[str,Any]:
 ...
 if theorem_names!=EXPECTED_THEOREMS:
  raise RuntimeError(f'Lean theorem surface drifted; missing={sorted(EXPECTED_THEOREMS-theorem_names)}, extra={sorted(theorem_names-EXPECTED_THEOREMS)}')
 command='lake env lean AsiStackProofs/ReadinessRefinement.lean'
 p=subprocess.run(command.split(),cwd=ROOT/'lean',text=True,stdout=subprocess.PIPE,stderr=subprocess.STDOUT)
 if p.returncode:raise RuntimeError(p.stdout)
 compile_receipt={'command':command,'exit_code':p.returncode,'output_sha256':hashlib.sha256(p.stdout.encode()).hexdigest()}
 return {'theorem_count':len(theorem_names),'lean_module':str(LEAN.relative_to(ROOT)),'lean_compile_receipt':compile_receipt,'arbitrary_run_identity_custody':True,'arbitrary_run_no_support_or_external_effect':True,'exact_run_composition':True,'terminal_state_absorbing':True}
...
def main()->None:
 ...
 for f in ('inductive Stage','def routeFor','run_events_preserve_exact_identity','run_events_cannot_assign_support_or_external_effect','run_events_compose','terminal_state_is_absorbing','full_readiness_lifecycle_reaches_terminal_state'):
 ...
 print(f"Readiness refinement passed: 3 exact suites, 7 stages, 40 routes, 45/45 mutations rejected, {result['formal_surface']['theorem_count']} Lean theorems, support effect none.")
