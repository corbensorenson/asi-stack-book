...
def validate(data: dict) -> list[str]:
    ...
    if vectors.get("summary", {}).get("dimension_state_counts", {}).get("reproducibility") != {"adjacent_local_replay": 36, "not_demonstrated_for_claim": 19}:
    ...
    for decision in decisions:
        ...
        refs = vector.get("dimensions", {}).get("reproducibility", {}).get("evidence_refs", [])
        if vector.get("summary_support_state") != "argument" or not set(decision["transition_refs"]).issubset(refs):
