...
def registry_required_artifacts() -> list[str]:
    if not REGISTRY.exists():
        raise SystemExit(f"Missing declarative validation registry: {REGISTRY.relative_to(ROOT)}")
    value = json.loads(REGISTRY.read_text(encoding="utf-8"))
    artifacts = value.get("required_artifacts") if isinstance(value, dict) else None
    if not isinstance(artifacts, list) or not all(isinstance(path, str) and path for path in artifacts):
        raise SystemExit("validation/registry.json must contain a non-empty required_artifacts list")
    return artifacts


REQUIRED = registry_required_artifacts()
...
def main() -> None:
    ...
    print("Book base validation passed.")
