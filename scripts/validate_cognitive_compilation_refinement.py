...
EXPECTED_THEOREMS = {
    ...
    "accepted_bound_step_preserves_source_identity",
    "successful_bound_run_preserves_source_identity",
    ...
    "compilation_runs_compose",
    "successful_run_preserves_complete_compilation_custody",
    "accepted_step_plan_version_monotone",
    "successful_run_plan_version_monotone",
    "accepted_repair_increments_plan_and_ledger_versions",
    "accepted_target_requires_current_plan_validation",
    "accepted_target_closes_residuals",
}
...
def initial() -> dict[str, Any]:
    return {"stage": "raw", "plan_id": 0, "plan_version": 1,
            ...
            "validated_plan_version": 0,
            "lowering_receipt": False, "validation_receipt": False,
            "repair_receipt": False, "residual_count": 0,
...
def event(kind: str, source: str, target: str, time: int) -> dict[str, Any]:
    return {"kind": kind, "from_stage": source, "to_stage": target,
            ...
            "residual_count": 0, "support_promotion_requested": False,
            "external_effect_requested": False, "block_receipt": False,
            "logical_time": time}
...
def trace() -> list[dict[str, Any]]:
    rows = [event("bind_source", "raw", "source_bound", 1),
            ...
            event("apply_repair", "repair_required", "lowered", 6),
            event("validate_target", "lowered", "validated", 7),
            event("accept_target", "validated", "accepted", 8)]
    rows[2]["lowering_receipt"] = True
    rows[3]["validation_receipt"] = True
    rows[4].update(repair_invalidates_obligation=True, residual_count=1)
    rows[5].update(output_plan_version=2, repair_localized=True,
                   output_ledger_version=2, ledger_update_receipt=True)
    rows[6].update(input_plan_version=2, output_plan_version=2,
                   input_ledger_version=2, validation_receipt=True)
    rows[7].update(input_plan_version=2, output_plan_version=2,
                   input_ledger_version=2)
    return rows
...
def errors(state: dict[str, Any], row: dict[str, Any]) -> list[str]:
    ...
    if row.get("support_promotion_requested") is not False: out.append("support_promotion_requested")
    if row.get("external_effect_requested") is not False: out.append("external_effect_requested")
    kind = row["kind"]
    if kind == "bind_source":
        ...
        if row["output_plan_version"] != row["input_plan_version"]: out.append("bind_version_changed")
        if row["residual_count"] != 0: out.append("bind_residual")
    elif kind == "type_ir":
        ...
        if row["output_plan_version"] != row["input_plan_version"]: out.append("type_version_changed")
        if row["residual_count"] != state["residual_count"]: out.append("type_residual_changed")
        ...
        if row["output_plan_version"] != row["input_plan_version"]: out.append("lower_version_changed")
        if row["residual_count"] != state["residual_count"]: out.append("lower_residual_changed")
        ...
        if row["output_plan_version"] != row["input_plan_version"]: out.append("validation_version_changed")
        if row["residual_count"] != state["residual_count"]: out.append("validation_residual_changed")
        ...
        if row["output_plan_version"] != row["input_plan_version"]: out.append("detection_version_changed")
        if row["residual_count"] <= 0: out.append("repair_residual_missing")
    elif kind == "apply_repair":
        if (row["from_stage"], row["to_stage"]) != ("repair_required", "lowered"): out.append("repair_stage")
        if not source_matches(state, row) or row["target_hash"] != state["target_hash"]: out.append("target_or_source_substitution")
        ...
        if row["output_plan_version"] != state["plan_version"] + 1: out.append("plan_not_incremented")
        ...
        if row["residual_count"] != 0: out.append("repair_residual_open")
        ...
        if not row["obligations_preserved"]: out.append("obligation_loss")
        if state["validated_plan_version"] != state["plan_version"]: out.append("stale_validation")
        if row["output_plan_version"] != row["input_plan_version"]: out.append("accept_version_changed")
        ...
        if row["output_plan_version"] != row["input_plan_version"]: out.append("block_version_changed")
        if row["residual_count"] != state["residual_count"]: out.append("block_residual_changed")
...
def apply(state: dict[str, Any], row: dict[str, Any]) -> dict[str, Any]:
    ...
    if row["kind"] == "validate_target": nxt["validated_plan_version"] = row["output_plan_version"]
...
def mutations(base: list[dict[str, Any]]) -> list[tuple[str, list[dict[str, Any]]]]:
    ...
    m("bind_authority_widened", 0, "requested_authority", 4)
    m("bind_time", 0, "logical_time", 0)
    m("bind_version_laundering", 0, "output_plan_version", 2)
    m("bind_residual", 0, "residual_count", 1)
    for key in IDENTITY: m("type_" + key + "_substitution", 1, key, 999)
    m("type_authority_changed", 1, "requested_authority", 2)
    m("type_time", 1, "logical_time", 1)
    m("type_version_laundering", 1, "output_plan_version", 2)
    m("type_residual_laundering", 1, "residual_count", 1)
    for key in IDENTITY: m("lower_" + key + "_substitution", 2, key, 999)
    for name, key, value in [
        ("target_missing", "target_hash", 0),
        ("authority_changed", "requested_authority", 2),
        ("obligation_loss", "obligations_preserved", False),
        ("receipt_missing", "lowering_receipt", False),
        ("version_laundering", "output_plan_version", 2),
        ("residual_laundering", "residual_count", 1),
    ]: m("lower_" + name, 2, key, value)
    for key in (*IDENTITY, "target_hash"):
        m("validate_" + key + "_substitution", 3, key, 999)
    m("validate_obligation_laundering", 3, "obligations_preserved", False)
    m("validate_receipt_missing", 3, "validation_receipt", False)
    m("validate_version_laundering", 3, "output_plan_version", 2)
    m("validate_residual_laundering", 3, "residual_count", 1)
    m("detect_target_substitution", 4, "target_hash", 999)
    m("detect_no_delta", 4, "repair_invalidates_obligation", False)
    m("detect_ledger_mismatch", 4, "input_ledger_version", 2)
    m("detect_version_laundering", 4, "output_plan_version", 2)
    m("detect_missing_residual", 4, "residual_count", 0)
    for name, key, value in [
        ("target_substitution", "target_hash", 999),
        ("global", "repair_localized", False),
        ("before_after", "after_obligation_hash", 999),
        ("wrong_obligation", "before_obligation_hash", 999),
        ("ledger_input", "input_ledger_version", 2),
        ("ledger_increment", "output_ledger_version", 1),
        ("plan_increment", "output_plan_version", 1),
        ("receipt", "ledger_update_receipt", False),
        ("obligation_loss", "obligations_preserved", False),
        ("residual_open", "residual_count", 1),
        ("wrong_stage", "to_stage", "validated"),
    ]: m("repair_" + name, 5, key, value)
    for key in (*IDENTITY, "target_hash"):
        m("revalidate_" + key + "_substitution", 6, key, 999)
    m("revalidate_receipt_missing", 6, "validation_receipt", False)
    m("revalidate_stale_plan", 6, "output_plan_version", 1)
    m("revalidate_residual_reopened", 6, "residual_count", 1)
    for name, key, value in [
        ("target_substitution", "target_hash", 999),
        ("obligation_loss", "obligations_preserved", False),
        ("residual", "residual_count", 1),
        ("authority", "requested_authority", 2),
        ("time", "logical_time", 7),
        ("version_laundering", "output_plan_version", 3),
    ]: m("accept_" + name, 7, key, value)
    for index in range(len(base)):
        m(f"support_request_{index}", index, "support_promotion_requested", True)
        m(f"external_effect_request_{index}", index, "external_effect_requested", True)
    return out
...
def lean_surface(issues: list[str]) -> tuple[int, list[str]]:
    ...
    if len(names) != 33:
        issues.append(f"Lean theorem count drift: expected 33, observed {len(names)}")
    if missing:
...
def build() -> tuple[dict[str, Any], list[str]]:
    ...
    paths = sorted(FIXTURES.glob("*.json")); atom_schema = load(ATOM_SCHEMA)
    receipts = []
    for path in paths:
    ...
    if (
        not accepted
        or final["stage"] != "accepted"
        or final["plan_version"] != 2
        or final["ledger_version"] != 2
        or final["validated_plan_version"] != 2
        or final["residual_count"] != 0
        or final["support_authority"] is not False
        or final["external_effect_authority"] is not False
    ):
        issues.append("reference trace rejected")
    mutation_receipts = []
    ...
    result = {"schema_version": "asi_stack.cognitive_compilation_refinement.v2", "result_id": "cognitive-compilation-refinement-2026-07-15-local",
              ...
              "lean_theorem_count": theorem_count, "lean_required_theorems_missing": missing_theorems,
...
def main() -> None:
    ...
    print(f"Cognitive compilation refinement passed: {result['accepted_fixture_count']} accepted/{result['rejected_fixture_count']} rejected fixtures, {result['lean_theorem_count']} Lean theorems, {result['reachable_trace_event_count']} events, {result['mutation_rejection_count']} mutations rejected, support effect none.")
