...
This record gives context memory transactional semantics for reads, writes, derivations, branches, commits, deletion closure, taint propagation, declassification references, derivatives, materialization state, contradictions, faults, replay boundary, non-claims, and audit.
...
This record keeps optional mathematical, search, cyclic, and representation substrates from becoming core architecture before baselines, controls, proof boundaries, experiment requirements, consumer gates, axis-specific evidence, falsification conditions, evidence, residuals, and non-claims are visible.
...
This record governs promotion, quarantine, split, merge, retirement, and retraining decisions by tying lifecycle state to field identity, workload family, freshness, evidence, regressions, residual escrow, inherited residuals, authority, route permissions, blockers, quarantine conditions, fallback, expiry, review, stale-gate triggers, and non-claims.
...
This record separates a durable semantic capability boundary from replaceable implementations, while keeping authority, qualification lease, evaluator policy, evaluator independence, route validity, evidence, incidents, review triggers, regression, migration, rollback obligations, and non-claims visible.
...
This record turns capability replacement into an evidence-gated transaction with identity preservation, prior state, candidate state, regressions, evaluator independence, residuals, rollback receipt, approval, monitoring, decision, and non-claims attached.
...
This record packages theorem-linked structural facts as receipts with finite-model scope, theorem references, proof status, fingerprints, deterministic fields, resolver/replay status, consumer permissions, failure behavior, evidence references, and explicit non-claims. It records receipt shape only; it does not imply external theorem replay, model-quality evidence, runtime evidence, or substrate adoption.
