...
Claim labels and support states are separate. A claim can be a `Hypothesized` claim with `argument` support, or a `Measured` claim with `synthetic-test-backed` support after a recorded test run. Claim scope, lifecycle state, surface refs, contradiction state, revision history, promotion blockers, and non-claims keep repeated prose from laundering confidence. Conversation-mined author intent can explain lineage and drafting intent, but it is not by itself source-derived evidence.
...
This record makes proof or justification claims tiered and auditable, including interpretation mapping, artifact validity state, semantic adequacy, verifier result, limitations, consumer requirements, downgrade behavior, claim-validity effect, residual route, ledger impact, and non-claims.
