...
| `evidence_matrices` | Appendices, source notes, schemas, release records, proof manifest, test specs, Corben source corpus, external literature, implementation horizons, changelog, and lineage. | Retain for live/research releases; include selected human-useful appendices in reader releases. |
...
It keeps the main argument, diagrams, problem statements, mechanisms, interfaces, invariants, failure modes, minimal implementation discussion, summaries, glossary, Corben source-corpus path, and external-literature path. The result should read as a book while still preserving uncertainty in the prose itself.

The live-site `Human view` uses the same strip-heading list at render time. `scripts/validate_reading_mode_toggle.py` checks that the toggle asset, view-mode classes, local-storage key, URL query parameter, live-TOC marker, section-number hiding, assistive description/status contract, release policy, and generated Quarto config stay aligned with `reader_release.strip_headings`. `scripts/validate_human_reading_paths.py` checks that each chapter has exactly one Human Reading Path bridge and that reader generation unwraps it into ordinary prose. `scripts/validate_live_human_view.py` runs after HTML render and checks that each rendered chapter page includes the toggle, URL mode contract, assistive status strings, the live-only headings and TOC targets needed for runtime hiding, and any source view-mode block classes such as `.asi-human-only`.
