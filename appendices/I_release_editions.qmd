...
The live site now has two reading modes. `AI view` is the default canonical view with the full live/research scaffold. `Human view` hides the same repeated live-only chapter sections used by the reader-release strip policy, so a reader can stay on the site and read the prose spine without downloading a separate EPUB, PDF, or DOCX. Future chapters can also use `.asi-human-only` fenced divs for human-specific prose digests and `.asi-ai-only` fenced divs for AI/research-only notes; reader generation unwraps the human blocks and removes the AI-only blocks. This is a convenience projection of the live book, not a reviewed major-version reader release.
...
The live-site `Human view` uses the same strip-heading list at render time. `scripts/validate_reading_mode_toggle.py` checks that the toggle asset, view-mode classes, release policy, and generated Quarto config stay aligned with `reader_release.strip_headings`. `scripts/validate_live_human_view.py` runs after HTML render and checks that each rendered chapter page includes the toggle, the live-only headings needed for runtime hiding, and any source view-mode block classes such as `.asi-human-only`.
