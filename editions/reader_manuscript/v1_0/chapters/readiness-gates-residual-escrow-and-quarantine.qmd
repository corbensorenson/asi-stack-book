...
The previous layer made a route into a lease. Readiness asks whether the
...
Readiness contributes the routability rule. A capability can remain the same
capability while its allowed use changes: draft, shadow, canary, default,
diagnostic-only, quarantined, retired, or superseded. The gate turns evidence
and residuals into permission for ordinary use, not just a nicer status label.
...
Readiness joins those surfaces into one transition record. Benchmaxxing
