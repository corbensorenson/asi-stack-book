...
Fast generation handles the cost of producing output over time. Artifact
compression handles the cost of keeping, moving, and reusing the things the
stack has already produced or collected. The same rule applies in both places:
a shortcut earns authority only after acceptance, fallback, residuals, and
consumer policy remain visible.
...
The live book now also records a narrow RankFold artifact import. Three
existing local `.rfa` archives point to the same 100,000,000-byte decoded
artifact digest,
`2b49720ec4d78c3c9fabaee6e4179a5e997302b3a70029f30f2d582218c024a8`. The
smallest recorded archive is 36,148,844 bytes, an observed decoded/archive
ratio of 2.76634019 for that artifact set. RankFold's read-only verifier
reported 1 OK, 0 failed for each archive, and inspection recorded the `NEURAL0`
codec path.

This matters as concrete artifact evidence, but only inside a tight boundary.
It does not prove RankFold codec correctness, deterministic decoder correctness
beyond the recorded local decoded digest observations, benchmark performance,
downstream utility, fallback execution, deployed compression behavior, or
support-state promotion. It does not promote the RankFold chapter core claim.
The core claim remains an architectural argument. The no-promotion record for
the artifact import keeps the import at `argument` and blocks stronger claims
until fresh compression, clean or archived source state, decoder review,
downstream utility, fallback execution, and independent review exist.

The live book also records a separate RankFold public-safe replay probe. This
one is a fresh local run over a generated 3,936-byte synthetic text file: pack
it, verify it, list it, unpack it, check roundtrip-exact byte identity, then
run a single-byte archive mutation and confirm that verification rejects the
corrupted archive.

That sounds small because it is small. The point is not to claim a
breakthrough compression result. The local CLI reported `RAW0` storage because
NeuralFold was disabled by license, and the archive was larger than the input.
The probe therefore proves only replay discipline and a negative control. It
does not prove NeuralFold compression, compression advantage, RankFold codec
correctness, downstream utility, fallback execution, deployed compression
behavior, or support-state promotion. The no-promotion record for the
public-safe replay keeps the replay at `argument` and blocks stronger claims
until an enabled NeuralFold path, public-safe corpus, decoder review,
downstream utility, fallback execution, and independent review exist.
...
The repository now has two narrow concrete surfaces, neither of which promotes the chapter core claim. The RankFold public-safe replay probe performs one fresh RAW0 pack/verify/list/unpack roundtrip over a 3,936-byte synthetic file, records no compression advantage, and rejects a single-byte corrupt archive. The RankFold artifact import records three existing `.rfa` archive observations for one 100,000,000-byte decoded digest, with `rfa verify` reporting 1 OK and 0 failed and `NEURAL0` inspect metadata. Together, they make the evidence boundary more concrete while still leaving NeuralFold compression, codec correctness, downstream utility, fallback execution, corpus benchmarking, and deployed compression behavior unproved.
