...
locality, more owned capacity, and more places where work can live. Once that
substrate exists, compactness becomes tempting. A seed might stand in for a
file, a rule for a repeated procedure, a graph for scattered context, or a
generator for a family of artifacts. If the smaller object is honest, it can
lower storage, routing, verification, replay, and movement cost across the
fabric.

But compactness is also a perfect hiding place. A representation can look small
because it captured real structure, or because it pushed missing work into a
verifier, a repair stream, a human reader, a fallback path, or a later debugging
session. The book therefore uses a blunt standard: small only counts after the
remainder is named.

A compact representation should say what must be reconstructed, verified,
repaired, expanded, or refused. It should also carry the fallback and authority
limits that decide who may rely on it. That rule covers ordinary compression,
semantic compression, generated reconstruction, and compact local controllers.
A tree, graph, semantic token, prompt, summary, seed, or generator can help the
stack think only when its grounding, permitted use, residual uncertainty,
consumer policy, and path back to fuller context remain visible.

Honest compactness is therefore a governance problem before it is a size
problem. The smaller object has to carry enough receipts to show when it is
safe to use, when it needs repair, and when the full artifact must return.
Compactness earns trust only when reconstruction duties, verification duties,
and residual duties travel with every use.

The local contribution is residual custody. Many chapters in the book use
records and receipts; this one asks where the burden went. If a compact seed,
generator, summary, graph, or repaired artifact is smaller, the reader should
still be able to see the reconstruction cost, verification cost, repair
channel, fallback path, consumer policy, and owner of whatever burden remains.
A small representation that hides those costs is not efficient in the stack's
sense. It is just incomplete accounting.

The residual-conservation fixture makes that rule concrete without pretending
to solve the whole problem. It accepts three honest residual records: one
visible residual that is accepted with an owner, one deferred residual with a
due condition, and one discharged residual with a receipt. It rejects five
dishonest controls: hidden burden after an apparent metric gain, erased burden
with no discharge, burden moved to no owner, a support-state promotion attempt,
and a zero-residual claim while work remains.

That is not a proof that all residuals can be found. It is a smaller and more
useful boundary: residual burden can be carried, priced, deferred, or
discharged, but it cannot simply disappear because the compact route looks
elegant.
...
That is still not proof that a deployed ledger exists. It is proof of a modest
but important discipline: current repository artifacts can carry their unpaid
burdens forward instead of letting those burdens vanish inside a success
metric.
...
release-reviewed art and does not establish compression ratios, task quality,
...
A compact generative system should answer six questions:
...
The raw LLM fits this frame as a compressed generative engine, not as the whole intelligence. It can generate across many targets, but the stack still needs routing, verification, residuals, authority, and evidence around its outputs. In the ASI Stack, generation is a capability; admissible use is a governed decision.
...
Compactness is not the invariant; accountability is. A compact representation may be useful, but only after generation, verification, correction, fallback, and governance costs are visible enough to compare with alternatives.
...
Residual honesty also requires retaining the expensive reference path when the compact path becomes the default. Otherwise the first successful shortcut quietly becomes the only route anyone remembers how to audit.
...
Compactness should first appear as a compact generative record and fixture. That artifact is not evidence that a compact core works. It is a ledger that says what would have to be true before a smaller structure could be trusted: target, seed, rules, memory, generation status, residuals, correction mechanism, verification contract/status, verifier independence, governance, authority boundary, use envelope, burden ledger, cost accounting, leverage, hidden complexity, fallback path/status, residual-burden status, promotion blockers/state, retirement condition, source refs, support-state effect, evidence references, and non-claims.

The repository fixtures validate that this public contract can be represented consistently. They also give later experiments a stable place to attach metrics: rate, reconstruction fidelity, verification cost, residual volume, utility, latency, and fallback frequency. The point is to make future evidence land in the right field instead of letting a compactness story outrun its tests.

`AsiStackProofs.CompactGenerativeSystems` adds bounded residual-honesty and exactness predicates, compact-admission route checks, and finite bridges to the Compact GVR synthetic slice, residual-conservation fixture, residual-ledger trace, and residual-ledger storage replay. The Lean bridges prove only modeled fixture relations: selected compact receipts and residual records preserve the fields the fixture names, and malformed controls are rejected inside that bounded world. The remaining engineering burden is still explicit: a real codec, generator, independent verifier, downstream utility test, corpus, fallback harness, live residual detector, and deployed residual ledger must exist before compact-core behavior can carry stronger evidence.
...
The Circle seed-rule receipt gives the reader an external structural example of the same discipline. The slice is not a claim that the book has a useful codec or a deployed compact generator. It records one finite exact-regeneration and storage-accounting fixture from Circle commit `63b0f511`: contract `CC-AI-CONTRACT-SEED-RULE-001`, kind `seed_rule_exact_regeneration`, fingerprint `836594a5f1d448900797e595cb98f0e476c0b9cbd7365fe333cf7ae2622f13c5`, `fixture_n=128`, `exact_regeneration=true`, `generator_length=383`, `explicit_length=454`, `storage_saving=71`, `bounded_search_candidate_count=3`, `bounded_search_exact_candidate_count=2`, and `bounded_search_best_shorter_generator_shorter=true`. Its checked theorem spine includes `GEN-T0001`, `GEN-T0040`, `GEN-T0041`, `GEN-T0046`, `GEN-T0048`, and `GEN-T0050`, with `theorem_count=32`; its planner recommendations are `SEED-RULE-USE-EXACT-REGENERATION-RECIPE` and `SEED-RULE-SELECT-BOUNDED-SHORTER-CANDIDATE`. The local evidence commands reported `2 passed in 4.52s` for the seed-rule tests and `1 passed in 2.80s` for the contract-ready recommendation test.

That makes the seed-rule receipt useful as storage accounting: a declared finite generator can exactly regenerate a declared finite object, and the record can say what the generator and explicit forms cost. It does not promote any chapter core claim, does not create a support-state transition, and does not prove useful compression, codec correctness, semantic utility, deployed generator behavior, fallback execution, downstream utility, optimal search, model quality, context length, runtime speed, memory scaling, benchmark performance, transfer, safety, ASI, or support-state-transition. The no-promotion decision is `circle_seed_rule_receipt_no_change.json`.
...
residual movement; it does not establish that all residuals are discoverable,
does not establish safety, and does not promote the chapter core claim.
...
The implemented residual-ledger storage replay checks a narrower but more
operational shape. `python3 scripts/validate_residual_ledger_storage_replay.py`
replays a bounded append-only event log, computes a digest chain, preserves
owner handoff, requires discharge review, keeps workload context, rejects five
invalid controls, and writes
`experiments/residual_ledger_storage_replay/results/2026-07-04-local.json`.
It records a `blocks_promotion` decision, so it is not proof of a deployed
residual ledger, live residual detection, safety, model quality, or the
chapter core claim.

It is not proof of a deployed residual ledger.
...
Compact generative systems earn their place as a burden-accounting layer for compression and generation. They let the stack use compact seeds, generators, procedures, learned representations, and local cores only when their reconstruction, decision, verification, fallback, governance, and residual costs are explicit.

At maturity, compactness would not be treated as a property of the artifact alone. It would be a governed account of the total route. A mature compact record would bind the target, seed, rule system, memory state, generator, correction mechanism, verifier, residual channel, fallback path, authority boundary, use envelope, burden ledger, cost accounting, promotion state, and retirement condition before the stack trusts a compact core. That would make compact generation a governed accounting surface rather than a slogan for smaller artifacts.
...
Compact systems are useful when they generate, reconstruct, or represent more value than their remaining burdens cost. Residual-honest accounting therefore keeps seed, rule system, memory, generation, verification, repair, semantic grounding, residuals, governance, fallback, consumer policy, and evidence in the same record.
