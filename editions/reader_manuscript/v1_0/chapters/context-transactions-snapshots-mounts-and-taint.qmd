...
The synthetic memory-store harness now checks bounded fixture cases for
committed-read visibility, mount faults, branch isolation, deletion blocking,
taint handling, replay boundaries, and support non-promotion. A companion
sequence bridge checks ordered write/read and tainted-derivative sequences,
then rejects read-before-write ordering, missing replay boundaries,
undeclassified taint materialization, and support-promotion attempts.

Those artifacts support record discipline. They do not prove deployed memory
behavior. The next honest implementation slice is a runtime-backed context
store that can replay real context views, enforce branch and mount visibility,
and show deletion closure across summaries, cached packets, artifacts, and
claims.
