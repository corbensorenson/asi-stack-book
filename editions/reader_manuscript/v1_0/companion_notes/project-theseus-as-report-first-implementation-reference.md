...
| Public task-bundle import | A bounded summary with 64 public BigCodeBench metadata-only tasks, 0 public training rows, 0 task-level regressions, visible gaps, clean live Theseus replay remaining unclaimed, and accepted no-promotion decision `evidence_transitions/v1_x_measured/theseus_public_task_bundle_import_no_change.json`. | It is not a model-quality result, benchmark-superiority result, generation-speed result, useful-solution-per-second result, or upward support-state transition. |
