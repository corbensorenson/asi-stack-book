...
`editions/reader_manuscript/` is the tracked path for curated human-reader manuscript work. Its v1.0 manifest currently records `drafting` with forty-four curated chapter records: zero drafting and forty-four reconciled for prose meaning, with no active manifest chapter missing a curated reader file. Generated reader source plus overlays remain the active release path until format review and an edition release record exist; the curated manuscript is reconciled for prose meaning but not release-approved. The curation contract at `editions/reader_manuscript/v1_0/curation_contract.json` defines the required record fields, allowed prose divergence, blocked evidence divergence, meaning-preservation checks, and pre-release blockers for any curated chapter file. The chapter review matrix at `editions/reader_manuscript/v1_0/chapter_review_matrix.json` tracks the 44-chapter human-reader review queue, overlay dispositions, companion-note candidates, curated-manuscript candidates, and release blockers while staying synced to `book_structure.json`. `scripts/build_curated_reader_edition.py --check` now verifies that the tracked curated chapters assemble into a renderable local Quarto review workspace; a full local review build writes to ignored `build/curated_reader_edition/` and remains a review input, not a release artifact. The companion-note routing manifest at `editions/reader_manuscript/v1_0/companion_note_routing.json` records how dense proof/governance chapters should be treated in reader, e-reader, and audio companion notes without moving meaning-critical boundaries out of the reader spine; drafting companion notes can live under `editions/reader_manuscript/v1_0/companion_notes/`. If the normal reader edition needs sustained prose editing chapter by chapter, the curated manuscript remains a tracked parallel derivative source for narrative only, with reconciliation back to the live manifest, support states, source boundaries, proof/test status, implementation horizons, and release records.
