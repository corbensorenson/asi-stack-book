...
<span>v2.2.0 candidate selected; v2.1.0 remains the latest immutable release until publication completes</span>
</div>
<div>
...
`v2.2.0` is the selected live/research HTML release candidate for the completed Post-v2.1 Residual and Transfer Roadmap. It carries three bounded empirical programs, fourteen conservative core-claim `no_change` decisions, six bounded transitions, eleven residual dispositions, six modern external-source routes, and the QCSA design synthesis while all 54 chapter-core claims remain at `argument`. The candidate is not immutable until its exact tag, tested bundle, deployment, public attestation, archive, digest, release record, citation, and rights snapshot agree. Until then, `v2.1.0` at commit `cb3b86051c3f4bd82e8b3128c0fdf180e8a7cfa5` remains the latest immutable evidence release. The root site and `/latest/` are mutable publication channels. EPUB, DOCX, PDF, audio, and a curated-reader edition remain outside the v2.2.0 release scope unless a later exact release record approves them.
