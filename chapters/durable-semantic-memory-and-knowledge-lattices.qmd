...
A memory write is a durable behavioral intervention, not clerical storage. The
Adaptive Commit Boundary treats memory as one candidate locus among several and
requires the write to retain its lesson identity, evidence, scope, commitment
profile, authority, qualification conditions, expiry, monitor, and descendant
links. A fact that belongs in a dated memory may not belong in weights; a
procedural defect may require a test or tool rather than another retrieval.

Deletion must follow descendants and reliance. Removing one source node does
not erase summaries, compiled context, cached routes, tools, or institutional
decisions derived from it. Anything that cannot be removed, deoptimized,
compensated, or requalified remains an explicit residual.
