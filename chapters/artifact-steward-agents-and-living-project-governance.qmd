...
| Test state | Record-shape fixtures for the steward charter, work contract, contribution ledger entry, action decision, sunset review, treasury policy, and event-taint record validate locally; twelve finite Lean route reductions cover lifecycle, contribution-ledger separation, and federation contracts, while reachable work-contract dispatch and release-gate routes remain planned; `python3 scripts/validate_benchmark_antigoodhart.py` validates synthetic release evidence and approval handoff behavior; `python3 scripts/validate_artifact_steward_lifecycle_probe.py` validates the Artifact steward lifecycle probe over `valid_clean_release_review_proposal`, `valid_sunset_review_route`, and six expected-invalid controls. No steward bot, treasury policy engine, event-taint workflow, contributor-ledger service, governance runner, release runner, sunset protocol, or project federation harness has been run. |
...
The exact current minimum is seven schema-and-fixture record families; twelve
Lean declarations reducing lifecycle, contribution-ledger, and federation
route functions; one deterministic public-safe lifecycle probe with two valid
routes and six expected-invalid controls; and one adjacent synthetic release
evidence-and-approval handoff. Work-contract dispatch and release-gate route
targets remain planned. No steward bot, real event intake,
...
| Work contract authority denial test | Confirm a steward cannot dispatch work when the contract lacks authority, allowed tools, forbidden-tool handling, verification requirements, or non-claims. | record-shape fixture implemented; reachable formal dispatch route planned; no steward dispatch run |
...
| Release evidence gate test | Confirm a stewarded release publication requires test, evidence, changelog, residual, and approval records. | expected-invalid lifecycle trace implemented; reachable formal publication route planned; no release runner exists |
...
| Artifact steward lifecycle probe | Confirm fixture-composed lifecycle routes for `valid_clean_release_review_proposal`, `valid_sunset_review_route`, and six expected-invalid controls covering event taint, over-policy treasury spend, contribution-governance laundering, unscoped federation authority inheritance, missing release evidence, and sunset ordinary-work blocking. | implemented by `python3 scripts/validate_artifact_steward_lifecycle_probe.py`; no steward bot, treasury executor, event-taint workflow, contributor-ledger service, governance runner, project federation harness, release runner, sunset protocol, or support-state transition exists |
...
**Formal audit.** The module now contains twelve declarations: four
lifecycle-route reductions, four contribution-ledger reductions, and four
federation-route reductions. The eighth C6 rationalization tranche removed
four assumption-restating implication theorems and their otherwise-unused
record predicates. The treasury and sunset targets now name retained route
reductions. The work-contract and release-gate targets are planned until
reachable decision functions derive repair or refusal from missing fields;
their schema and executable-trace evidence remains separate. None of the
twelve retained declarations is a direct projection of steward behavior or a
...
The deterministic lifecycle probe is a separate fixture evidence lane with two
valid routes and six rejecting controls; it does not turn the route functions
