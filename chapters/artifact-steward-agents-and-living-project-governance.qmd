...
| Test state | Record-shape fixtures for the steward charter, work contract, contribution ledger entry, action decision, sunset review, treasury policy, and event-taint record validate locally; finite Lean predicates now include work-contract, protected-action, release-gate, sunset-review, lifecycle-route, contribution-ledger separation, and federation-contract envelopes; `python3 scripts/validate_benchmark_antigoodhart.py` validates synthetic release evidence and approval handoff behavior; `python3 scripts/validate_artifact_steward_lifecycle_probe.py` validates the Artifact steward lifecycle probe over `valid_clean_release_review_proposal`, `valid_sunset_review_route`, and six expected-invalid controls. No steward bot, treasury policy engine, event-taint workflow, contributor-ledger service, governance runner, release runner, sunset protocol, or project federation harness has been run. |
...
sixteen Lean declarations derived from seven hand-authored finite predicates
or route functions; one deterministic public-safe lifecycle probe with two
valid routes and six expected-invalid controls; and one adjacent synthetic
release evidence-and-approval handoff. No steward bot, real event intake,
...
**Formal audit.** All sixteen declarations are derived consequences or
reductions of seven hand-authored finite predicates or route functions: four
record-boundary implications, four lifecycle-route reductions, four
contribution-ledger reductions, and four federation-route reductions. None is
a direct projection of steward behavior or a real project outcome. This finite record does not prove mission correctness, source or event truth, injection resistance,
...
This chapter owns **78** structured claim atoms in `CF-08`. The
frozen repository-wide terminal audit records 78 `blocked_after_full_attempt`. Its core
atom `artifact-steward-agents-and-living-project-governance.core` remains at `argument` with terminal
disposition `blocked_after_full_attempt`. The authoritative per-atom rows
are the `artifact-steward-agents-and-living-project-governance` slice of
...
at `argument` support. Across its 78 structured claims,
78 still lack required proof lanes,
0 were narrowed, and
...
- **Failure or non-promotion case:** 78 atoms
...
|---|---:|---|---|
| Core: `artifact-steward-agents-and-living-project-governance.core` | 1 | `causal`, `empirical`, `executable`, `formal`, `source-synthesis` | `blocked_after_full_attempt`; missing or unproved: `normative`, `transfer`. |
| blocked after full attempt | 78 | `causal`, `empirical`, `executable`, `formal`, `source-synthesis` | Missing or unproved lanes across this set: `normative`, `transfer`. These are residual proof obligations, not false claims. |
