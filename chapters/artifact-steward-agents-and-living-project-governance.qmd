...
| Primary source records | `viea`, `talos`, `planforge`, `vcm_public`, `spinoza`, `benchmaxxing`, `rmi`, `cognitive_loop_closure`, `tokenmana`, `coherence_exchange`, `project_theseus_whitepaper`, `theseus_operator_os`, `scf`, `field_of_god_ai_constitution` |
| Claim label | Design rationale |
| Evidence level | argument |
| Source loading state | All assigned sources have public source notes, but the steward-agent architecture is a synthesis. The browser note is local-only planning context. |
| Test state | Record-shape fixtures for the steward charter, work contract, contribution ledger entry, action decision, and sunset review validate locally, and two finite Lean predicates build. No steward bot, treasury policy engine, governance runner, or project federation harness has been run. |

## Drafting guardrail

This chapter should not be read as a claim that autonomous project managers are safe, legally authorized, or ready to control funds. It argues for a bounded project-steward layer with explicit work contracts, evidence gates, treasury limits, contribution records, governance checks, and sunset paths. Adjacent examples such as CI systems, issue trackers, fiscal-hosting tools, grant systems, public-good funding, compute markets, or on-chain governance require source records and source notes before they can be used as evidence.
...
The core claim remains at `argument`. VIEA, Talos, PlanForge, VCM, Spinoza, Benchmaxxing, RMI, Cognitive Loop Closure, TokenMana, Coherence Exchange, Project Theseus, Hive Operator OS, SCF, and Field of God governance each support pieces of the design. The repository now includes record-shape schemas, valid fixtures, and narrow Lean predicates for the first steward records, but the assembled artifact-steward layer has not been implemented or behaviorally tested. The local browser note supplied author intent and terminology; it is not a source-derived evidence artifact.

## Mechanism

An artifact steward is built from four project records: `Artifact Steward Charter`, `Project Work Contract`, `Contribution Ledger Entry`, and `Steward Action Decision`.
...
The browser note raised useful external candidates: repository automation, fiscal sponsorship, public-goods funding, compute markets, decentralized governance, and workflow-injection research. These belong in the research queue, not in the evidence column, until source records and source notes exist. The book should later separate three questions: which tooling patterns already work, which governance patterns have known capture risks, and which pieces need new ASI Stack-specific schemas.
