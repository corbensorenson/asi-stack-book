...
open_evidence_gaps:
  ...
  - "Record-shape schemas, valid fixtures, narrow finite Lean predicates, finite steward lifecycle, contribution-ledger, and federation-contract route envelopes, and the benchmark anti-Goodhart harness's synthetic release evidence/approval handoff are implemented; no steward bot, treasury policy engine, event-taint workflow, governance system, contributor-ledger service, project federation harness, release runner, or sunset protocol has been executed from this repository."
...
These are not proofs of good governance. They are finite-record safety checks over declared work-contract, protected-action, release, sunset, lifecycle-route, contribution-ledger, and federation-contract fields. The lifecycle route model checks only that unreviewed tainted events quarantine, unreviewed sunset criteria open sunset review, autonomy escalation without charter approval routes to approval, and over-policy treasury spend routes to approval. The contribution route model checks only record-level separation of authorship, review, evidence, compensation, reputation, governance effect, conflict disclosure, collapsed-score rejection, and support-transition routing. The federation route model checks only scoped contract presence, worker-authority boundaries, tool/data/budget gates, approval routing for external spend, evidence-bundle requirements, and scoped dispatch. It does not prove institutional capture resistance, legal authority, treasury safety, workflow-injection resistance, event scanner quality, contribution-quality scoring, federation behavior, release safety, steward autonomy, or human project quality.
