...
open_evidence_gaps:
  - "The artifact-steward browser note is author-intent context only; it is not cited evidence and is kept in the ignored local source inbox."
  - "Only record-shape schemas, valid fixtures, and narrow finite Lean predicates have been implemented; no steward bot, treasury executor, contributor ledger service, governance system, or sunset protocol has been run."
...
| Primary source records | `viea`, `talos`, `planforge`, `vcm_public`, `spinoza`, `benchmaxxing`, `rmi`, `cognitive_loop_closure`, `tokenmana`, `coherence_exchange`, `project_theseus_whitepaper`, `theseus_operator_os`, `scf`, `field_of_god_ai_constitution`, plus external source notes for GitHub webhooks, self-hosted runners, OpenZeppelin Governor, Akash, Golem, agentic workflow injection, and DAO delegation concentration |
| Claim label | Design rationale |
| Evidence level | argument |
| Source loading state | All assigned sources have public source notes, including external tooling and governance notes. The steward-agent architecture remains a synthesis. The browser note is local-only planning context. |
| Test state | Record-shape fixtures for the steward charter, work contract, contribution ledger entry, action decision, and sunset review validate locally, and four finite Lean predicates build. No steward bot, treasury policy engine, governance runner, release gate, or project federation harness has been run. |
...
The core claim remains at `argument`. VIEA, Talos, PlanForge, VCM, Spinoza, Benchmaxxing, RMI, Cognitive Loop Closure, TokenMana, Coherence Exchange, Project Theseus, Hive Operator OS, SCF, and Field of God governance each support pieces of the design. External source notes now ground event-driven repository automation, self-hosted CI, decentralized compute, on-chain governance machinery, agentic workflow-injection risk, and DAO delegation concentration risk. The repository includes record-shape schemas, valid fixtures, and narrow Lean predicates for the first steward records, but the assembled artifact-steward layer has not been implemented or behaviorally tested. The local browser note supplied author intent and terminology; it is not a source-derived evidence artifact.

## Mechanism

An artifact steward is built from five project records: `Artifact Steward Charter`, `Project Work Contract`, `Contribution Ledger Entry`, `Steward Action Decision`, and `Sunset Review Record`.
...
Initial source records and conservative source notes now exist for repository events, self-hosted CI runners, decentralized compute markets, on-chain governance machinery, agentic workflow-injection risk, and DAO delegation concentration risk. Remaining queue items include fiscal hosting, grants, sponsorships, bounty platforms, package-maintainer sustainability, software supply-chain security, and legal/tax treatment of stewarded treasuries. The book should later separate three questions: which tooling patterns already work, which governance patterns have known capture risks, and which pieces need new ASI Stack-specific schemas.
