...
The previous chapter keeps learning inside policy-update gates. This chapter applies the same rule to projects themselves: durable artifacts can have memory, roadmaps, workers, funds, releases, and feedback loops, but none of those surfaces gives an AI steward ownership. Stewardship is a continuity interface, not a sovereignty claim.

::: {.asi-human-only}
## Human Reading Path

Policy learning updates behavior inside the stack. Artifact stewardship asks how long-lived projects can be cared for without giving an AI steward ownership over the artifact, money, maintainers, or community. The steward is a bounded service layer, not a sovereign.

The human reason this matters is continuity. Real projects need issue triage, release memory, funding constraints, contributor records, and sunset criteria. A steward can help only if its powers are leased, audited, appealable, and tied to explicit work contracts.
:::
...
```{mermaid}
flowchart TD
  charter["Artifact Steward Charter\nmission, authority, budget, governance, sunset"] --> intake["Intake\nissues, papers, PRs, failures, user goals"]
  vcm["Project memory\nsource refs, decisions, taint, open questions"] --> intake
  intake --> plan["PlanForge roadmap and dependency plan"]
  plan --> contract["Project Work Contract"]
  contract --> workers["Humans, agents, CI, hives, rented compute"]
  workers --> artifacts["Submitted artifacts\ncode, prose, proofs, tests, diagrams"]
  artifacts --> verify["Verification gates\nreview, tests, proofs, benchmarks, evidence"]
  verify --> decision["Steward Action Decision"]
  decision --> ledger["Contribution and evidence ledgers"]
  decision --> release["Release, rollback, defer, or sunset proposal"]
  ledger --> vcm
  release --> vcm
  charter --> gov["Governance checks\nowner, maintainer, community, guardian"]
  gov --> decision
```
...
The mature version of Artifact Steward Agents and Living Project Governance is not the minimal artifact with more scale. It is the chapter's idea turned into a product-grade, evidence-bearing boundary that the rest of the ASI Stack can depend on under ordinary operation and upgrade pressure.

At that end state, the chapter's claim becomes an operational contract: Every durable artifact should be able to carry a bounded steward agent that preserves mission, memory, roadmap, budget, contributors, compute, evidence, governance, and sunset policy across the artifact lifecycle while acting only through explicit work contracts, verification gates, treasury limits, contribution ledgers, and governance rules. The mature surface would include:

- Operating mechanism: Bind a steward to an artifact mission, non-goals, authority ceiling, governance model, budget policy, evidence requirements, and sunset criteria; Run an explicit lifecycle from inception to bootstrap, build, release, maintenance, governance, decline, and archive instead of treating stewardship as one long task list; Convert roadmap goals into project work contracts with required outputs, allowed tools, forbidden tools, worker requirements, budget, verification, payment, and reputation fields; Orchestrate humans, agents, personal hives, project hives, CI runners, rented compute, reviewers, and maintainers through typed jobs and evidence bundles; Use treasury policy and event-taint records to keep spending and untrusted repository events inside explicit review boundaries; Separate reputation, governance shares, economic credits, authorship, review credit, and evidence credit so contribution, voting, money, and truth claims cannot collapse into one gameable token; Move across manual, assisted, bounded-autonomous, community-governed, and sunset modes only through charter, policy, evidence, rollback, and approval records; Treat webhooks, issues, pull requests, comments, worker outputs, benchmark artifacts, and release events as typed and tainted intake until the steward records review.
- Stack interface: VCM preserves project memory, source documents, decisions, issues, releases, and unresolved questions; PlanForge and Talos turn goals into milestones, issues, jobs, reviews, artifacts, and releases; Spinoza, Benchmaxxing, and evidence ledgers decide whether submitted work changes claim or readiness state; TokenMana and treasury policies bound spending, compute rental, bounties, and recurring operations; SCF, governance rights, and constitutional predicates prevent steward authority from replacing owner, maintainer, contributor, or community authority.
- Preserved invariants: The steward is not the owner; Mission, non-goals, and sunset criteria remain durable; Spending is a bounded actuator governed by treasury policy; Untrusted issues, pull requests, comments, and worker outputs enter as tainted context until reviewed; Major governance changes require explicit proposal, review, delay, quorum, appeal, or guardian paths; A project that no longer has value, users, maintainers, or funds can enter sunset mode rather than continuing activity for its own sake.
- Failure closure: detect and route failure modes such as Mission drift from optimizing activity rather than purpose; Treasury drain through low-value compute, bounty spam, or runaway recurring costs; Contribution gaming and reputation capture; Governance capture by large holders, early maintainers, or steward-controlled scoring; Agentic workflow injection through untrusted repository event context; AI maintainer monopoly that makes the project opaque to humans; Zombie project continuation after user value disappears to quarantine, rollback, escalation, or residual records instead of hidden degradation.

For Artifact Steward Agents and Living Project Governance, this is a target architecture, not a current-result claim. Its support state should not rise until the source mappings, schemas, Lean predicates, tests, benchmark records, runtime traces, review receipts, or governance artifacts needed for this specific end state exist and validate the claim.
...
The next chapter zooms back out to the integrated architecture. Stewardship is one governance surface inside the larger trace from intent to plan, context, claim, job, artifact, evidence, release, and bounded improvement.
