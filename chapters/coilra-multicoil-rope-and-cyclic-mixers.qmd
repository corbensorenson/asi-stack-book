...
open_evidence_gaps:
  ...
  - "Pinned Circle project references support structural-contract, receipt, theorem-id, baseline, and non-claim discussion, and the separate public-safe Circle receipt lanes now give the CoilRA chapter one concrete RoPE receipt boundary plus one cyclic-mixer structural/accounting receipt boundary. The cyclic-mixer slice records Circle commit `63b0f511`, `CC-AI-CONTRACT-MIXER-001`, `max_abs_dense_delta=0`, `block_to_dense_ratio=0.0625`, seven theorem IDs, two planner recommendations, targeted Circle CLI outputs `3 passed in 2.49s` and `1 passed in 1.47s`, and no support-state transition; this ASI Stack repo still did not import a generated Circle contract pack, validate external Circle Lean proofs locally, run a cyclic mixer benchmark, run an MLX model experiment, run a hardware-kernel benchmark, or produce a quality/context-length/runtime result."
