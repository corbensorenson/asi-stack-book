...
These Lean hooks are implemented as finite-record predicates over cyclic mixer claim reviews and substrate-promotion reviews. The public hooks now bind to the derived negative cases `cyclic_mixer_claim_missing_claim_partition_rejected` and `cyclic_substrate_promotion_without_baselines_or_tradeoffs_rejected` rather than to assumption-restating projections. The module also proves bounded negative cases for alias review without winding or visible residuals, an incomplete tradeoff packet, and hardware mismatch without a refusal path. These cases reject malformed records; they do not prove CoilRA, MultiCoil RoPE, cyclic mixer quality, context length, runtime, memory, hardware-kernel behavior, deployment readiness, or parameter efficiency.

The formal audit counts five theorem declarations, all derived finite negative
cases covering missing claim partitions, promotion without baselines or
tradeoffs, alias review without winding or visible residuals, an incomplete
tradeoff packet, and hardware mismatch without a refusal path. The two former
assumption-restating projections were retired after their public targets moved
to the corresponding negative cases. None proves cyclic mechanism semantics, functional equivalence
