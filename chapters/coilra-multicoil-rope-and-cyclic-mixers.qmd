...
External baseline exception: the text names comparison families such as dense layers, LoRA variants, RoPE, recurrent memory, and state-space models as required future baselines, but the current source queue has not assigned source-noted external records to this layer. The cyclic-mixer claim therefore remains local-design and research-agenda material until those sources are triaged.
