...
open_evidence_gaps:
  ...
  - "The cyclic_mixer_evaluation_record fixture validates schema/example consistency for workload target, receipt refs, hardware refusal path, baseline symmetry, negative controls, resource costs, metrics status, tradeoff packet ref, consumer policy, adoption rationale, and non-claims; AsiStackProofs.CyclicMixers implements finite-record structural separation and baseline/tradeoff gates only; no RoPE certifier command, sidecar regeneration, Circle contract-pack generation, cyclic mixer benchmark, MLX model experiment, hardware-kernel benchmark, downstream quality evaluation, or external Circle Lean build has been run from this repo."
...
| Test state | `cyclic_mixer_evaluation_record.valid.json` passes repository-level protocol fixture validation for workload target, receipt refs, hardware refusal path, baseline symmetry, negative controls, resource costs, metrics status, tradeoff packet ref, consumer policy, adoption rationale, and non-claims; `AsiStackProofs.CyclicMixers` implements finite structural-claim separation and baseline/tradeoff promotion gates; RoPE boundary, baseline matrix, alias diagnostic, and parameter/quality/runtime tests remain planned. |
...
A Cyclic Mixer Evaluation Record keeps those layers separated. It can report a workload target, structural invariant, receipt refs, receipt boundary, alias/load diagnostics, parameter accounting, hardware notes, baseline symmetry, negative controls, resource costs, metrics status, consumer policy, and hardware refusal path while still refusing to claim quality, runtime, memory, training stability, context length, or deployment readiness without measured tradeoffs.
...
The smallest useful implementation is a cyclic mixer evaluation schema and fixture. The current fixture records workload target, structural invariant, receipt refs, proof or receipt boundary, alias and load diagnostics, parameter accounting, hardware notes, hardware refusal path, baselines, baseline symmetry, negative controls, resource costs, required metrics, metrics status, tradeoff packet ref, consumer policy, adoption state and rationale, non-claims, and evidence references.
...
| Test | Purpose | Status |
|---|---|---|
| Cyclic mixer evaluation record fixture validation | Validate that a cyclic-substrate evaluation records workload target, structural invariant, receipt refs, proof/receipt boundary, alias/load diagnostics, parameter accounting, hardware notes, hardware refusal path, baselines, baseline symmetry, negative controls, resource costs, required metrics, metrics status, tradeoff packet ref, consumer policy, adoption state/rationale, non-claims, and evidence references. | implemented; passing via `python3 scripts/validate_protocol_examples.py` |
