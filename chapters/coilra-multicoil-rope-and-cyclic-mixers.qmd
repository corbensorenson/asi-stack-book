...
The previous chapter applies structural contracts to memory and recurrence. This chapter applies the same boundary to model-internal cyclic structure: adapters, phase banks, position encodings, route heads, and mixers. The core question is not whether a cyclic invariant exists; it is whether that invariant survives contact with baselines, kernels, parameter accounting, failure cases, and workload metrics.
...
The chapter's job is to keep mechanism and evidence separate. A cyclic mixer can be interesting as structure, but it does not become better context, faster inference, lower memory, or stronger reasoning until the stack can show the relevant workload, baseline, metric, artifact, and boundary.
:::
...
At that end state, the chapter's claim becomes an operational contract: CoilRA, MultiCoil RoPE, and cyclic mixers should be treated as optional specialist substrates whose structural guarantees must be paired with dense, LoRA, RoPE, learned, recurrent, or state-space baselines before adoption. The mature surface would include:
...
The practical standard is that cyclic structure should make evaluation more precise, not more permissive. If the receipt makes it easier to overclaim than to reject the candidate, the evaluation boundary is wrong.
