...
The insufficiency is semantic proof drift. A finite predicate can be correct and still be the wrong predicate for the intended ASI Stack boundary. That gap belongs in a semantic adequacy review, not in the Lean build status.
...
**Diagram reading note:** The proof envelope starts by triaging whether a claim has an operational predicate at all. Lean modules, schemas, fixtures, validators, blocked targets, and residuals are separate lanes, and a passing artifact updates only the narrow proof/spec state it actually checks.
