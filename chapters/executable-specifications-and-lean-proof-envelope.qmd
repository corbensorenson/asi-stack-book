...
The current local verification gate includes `cd lean && lake build`; a passing build confirms the Lean modules in this repository, not every prose claim that references them. The proof-envelope module adds a narrow record-level invariant for implemented status and non-operational routing. Appendix E now publishes the generated coverage summary from `proofs/proof_triage.json`: 112 proof targets are covered by triage and all 112 are implemented as formal-invariant Lean candidates. The proof artifact traceability audit checks that those targets are wired through manifest records, triage records, Lean modules, root imports, chapter hooks, limitation prose, and Appendix E coverage. Semantic proof adequacy review remains a separate planned check.
