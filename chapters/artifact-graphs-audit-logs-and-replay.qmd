...
| Test state | Artifact graph fixture validation now includes context transaction refs, semantic certificate refs, replay grade, evidence gate, residuals, and non-claims; finite provenance predicates are implemented in `AsiStackProofs.ArtifactGraph`; `python3 scripts/validate_artifact_graph_replay.py` checks synthetic parentage, replay metadata, audit reconstruction, stale-certificate, and promotion-blocking cases; deployed replay, audit-reconstruction, and produced-artifact completeness services remain planned. |

## Drafting guardrail

Artifacts are provenance-bearing work products, not automatic evidence. Six local-cache source mappings have been passage-reviewed for the architecture surface; `moecot` remains connector/source-note mapped. The repo validates an artifact-graph fixture, proves two finite Lean predicates over provenance records, and runs a synthetic artifact-graph replay harness over parentage, context transaction, semantic certificate, replay metadata, audit reconstruction, stale-certificate, and promotion-blocking cases. It has not run a deployed replay engine, audit reconstruction service, produced-artifact checker, or provenance completeness service for this layer.
...
That fixture validates the shape of an artifact node. `AsiStackProofs.ArtifactGraph` adds a bounded predicate requiring produced artifacts to carry parent-job and source/context references, plus a bounded promotion-blocking predicate for missing required provenance. `python3 scripts/validate_artifact_graph_replay.py` adds a synthetic cross-record replay packet check across artifact records, typed jobs, context transactions, semantic certificates, audit reconstruction, replay grade, stale-certificate status, and promotion-blocking boundaries. The referenced render, deployed replay engine, audit service, and provenance completeness of any real produced artifact remain outside that result.
...
Fixture-shape validation, two bounded Lean predicates, and a synthetic cross-record replay harness are implemented. The remaining tests require produced artifacts, audit logs, and replay attempts from a deployed or imported artifact graph service.
