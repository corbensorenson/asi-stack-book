...
open_evidence_gaps:
  ...
  - "Protocol schema/fixture validation, finite Lean provenance and route predicates, and `python3 scripts/validate_artifact_graph_replay.py` are implemented; the harness checks synthetic parentage, context transaction, semantic certificate, replay metadata, audit reconstruction, stale-certificate, and promotion-blocking cases, while the Lean route model covers missing parent/source/context/transaction/certificate/tool/claim/test/audit/replay/evidence/non-claim fields and blocked promotion. No deployed artifact-graph replay service, audit reconstruction service, or produced-artifact completeness service has been run."
...
| Test state | Artifact graph fixture validation now includes context transaction refs, semantic certificate refs, replay grade, evidence gate, residuals, and non-claims; finite provenance predicates plus route and negative-case theorems are implemented in `AsiStackProofs.ArtifactGraph` for produced artifacts, missing trace refs, insufficient replay grade, missing claim/test links, missing audit/replay/evidence/non-claim fields, stale certificates, blocked promotion, and complete admission; `python3 scripts/validate_artifact_graph_replay.py` checks synthetic parentage, replay metadata, audit reconstruction, stale-certificate, and promotion-blocking cases; deployed replay, audit-reconstruction, and produced-artifact completeness services remain planned. |

## Drafting guardrail

Artifacts are provenance-bearing work products, not automatic evidence. Six local-cache source mappings have been passage-reviewed for the architecture surface; `moecot` remains connector/source-note mapped. The repo validates an artifact-graph fixture, proves finite Lean predicates over provenance and artifact-admission routes, and runs a synthetic artifact-graph replay harness over parentage, context transaction, semantic certificate, replay metadata, audit reconstruction, stale-certificate, and promotion-blocking cases. It has not run a deployed replay engine, audit reconstruction service, produced-artifact checker, or provenance completeness service for this layer.
...
That fixture validates the shape of an artifact node. `AsiStackProofs.ArtifactGraph` adds bounded predicates requiring produced artifacts to carry parent-job and source/context references, blocking promoted support when required provenance is missing, ranking replay grades, and routing artifact admission through parent, source, context transaction, semantic certificate, tool, claim, test, audit, replay, evidence-gate, stale-certificate, promotion, and non-claim checks. The module now has 24 theorem declarations, including 22 derived/decomposed route or negative-case declarations; those declarations show which modeled missing fields block admission and when complete non-promoted or approved-promoted artifact records are admitted. `python3 scripts/validate_artifact_graph_replay.py` adds a synthetic cross-record replay packet check across artifact records, typed jobs, context transactions, semantic certificates, audit reconstruction, replay grade, stale-certificate status, and promotion-blocking boundaries. The referenced render, deployed replay engine, audit service, and provenance completeness of any real produced artifact remain outside that result.
...
Fixture-shape validation, 24 bounded Lean theorem declarations over finite provenance, replay-grade, and artifact-admission route predicates, and a synthetic cross-record replay harness are implemented. The remaining tests require produced artifacts, audit logs, and replay attempts from a deployed or imported artifact graph service.
...
These proof hooks are implemented as finite-record predicates plus route and negative-case theorems over modeled artifact records. They cover parent/source/context refs, replay-grade sufficiency, claim/test links, audit and replay metadata, replay limits, evidence gates, stale certificates, promotion permission, non-claim boundaries, and complete modeled admission. They do not prove replayability, audit reconstruction, artifact-type-specific completeness, provenance completeness in a deployed service, or source interpretation adequacy.
