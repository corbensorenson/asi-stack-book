...
open_evidence_gaps:
  ...
  - "Only the generation_mode_record fixture, narrow finite Lean promotion predicates, bounded negative cases for accepted-output accounting, verifier-cost accounting, failed acceptance, residual recording, fallback routing, high-risk fast-mode review, the finite generation-mode admission lifecycle route, the Project Theseus static import Lean/Python fixture bridge, the Theseus support replay probe over the ASI-side validators, the public-safe fast-generation task-bundle accounting probe, and the aggregate Theseus/Fast support-lane bridge have been implemented; no local decoding, diffusion, acceptance-rate, KV-cache, useful-solution-per-second model result, route-selector, live Theseus, or serving benchmark has been run."
---
...
| Test state | `generation_mode_record.valid.json` passes repository-level protocol fixture validation; `AsiStackProofs.FastGeneration` implements finite-record promotion predicates plus bounded negative cases for missing accepted-output/verifier-cost evidence, failed acceptance without fallback or residual handling, high-risk fast-mode selection without verifier, override, or slower-fallback records, a generation-mode admission lifecycle route for missing mode, context, risk, quality, verifier, acceptance, baseline, output, cost, fallback, residual, override, budget, evidence-transition, and non-claim-boundary records, a finite Project Theseus import fixture checked against the public JSON summary, and a finite fast-generation task-bundle fixture bridge; `scripts/validate_theseus_support_replay_probe.py` reruns the generation-mode import validator as part of a two-validator Theseus support probe with command-output and artifact digests; `python3 scripts/validate_fast_generation_task_bundle.py` validates result `fast_generation_task_bundle_2026_07_02_local`, selecting `route://fast-template-verified` at 264 deterministic cost units vs baseline 632 while rejecting cheaper `route://latency-only-proxy`; `python3 scripts/validate_theseus_fast_support_lane.py` checks the selected Theseus/Fast support-lane aggregate `theseus-fast-support-lane-2026-07-03-local`, 4 command replays, 16 tracked artifact digests, 68 public task records, 14 expected-invalid or rejected controls, two accepted no-promotion decisions, and `theseusFastSupportAggregateFixture`; `evidence_transitions/v1_x_measured/fast_generation_task_bundle_no_change.json` records an accepted no-promotion decision for that task-bundle slice; this is no model-speed or deployment claim, no useful-solution-per-second model claim, and no support-state promotion. |
...
The claim is architectural. The current support state is `argument`. It does not say that one acceleration family wins everywhere. It says the generation layer should expose multiple modes behind a policy interface, measure them with accepted-output and task-success metrics, and promote them only when the evidence shows that they improve useful output without hiding verification, memory, or repair costs.
...
The Project Theseus generation-mode import adds the first public-safe implementation-reference slice for this boundary. The imported static gate summary records 18 modes, 13 comparisons, zero hard gaps, zero modes with missing report refs, five hard boundary gates passing, and zero promotable comparisons. Five comparisons saw accepted-span speed lift, but every candidate task-pass count stayed at zero and useful solution per second stayed at zero, so the import functions as negative evidence against raw-throughput promotion rather than as a speedup result. The ASI repository validates only the sanitized summary and digests, rejects hard boundary-gate failure and missing-report-ref overclaims, and now checks those public summary fields against a finite `AsiStackProofs.FastGeneration` fixture so the book-side proof layer cannot drift from the imported counts. The Theseus support replay probe reruns the ASI-side generation-mode validator and the connected architecture-gate validator, then records command-output and artifact digests. It does not copy private task rows, candidate outputs, prompts, training rows, checkpoints, traces, or benchmark payloads, and it does not rerun Theseus.
...
| Generation-mode admission lifecycle route | Prove that a finite fast-generation admission review routes missing mode records, context packets, risk tiers, quality targets, verifiers, acceptance predicates, baselines, accepted outputs, verifier costs, fallback and residual handling, high-risk overrides, budget records, evidence transitions, and non-claim boundaries to explicit outcomes. | implemented in `AsiStackProofs.FastGeneration`; no route-selector behavior, speedup, verifier-quality, fallback-runtime, or benchmark claim |
| Project Theseus generation-mode import validation | Validate a public-safe static Theseus generation-mode gate summary with 18 modes, 13 comparisons, zero hard gaps, zero modes with missing report refs, zero promotable comparisons, Lean fixture alignment, and mutation controls for hard boundary-gate failure, private payloads, missing-report-ref overclaim, support overclaim, raw-speed promotion, and useful-speed overclaim. | implemented; passing via `python3 scripts/validate_theseus_generation_mode_import.py`; finite `AsiStackProofs.FastGeneration` fixture matches the public summary; no live Theseus replay, generation-speed result, useful-solution-per-second win, or support-state promotion |
...
The implemented rows validate record discipline, finite admission gates, one static Theseus import boundary, and a local replay probe over the ASI-side Theseus validators. The remaining rows are a backlog for turning the design rationale into prototype-backed or external-literature-backed claims.
...
| `lean:fast_generation.task_bundle_fixture_bridge` | `AsiStackProofs.FastGeneration` | The public-safe fast-generation task-bundle fixture preserves baseline and candidate task quality, shows candidate deterministic cost-accounting improvement over baseline, rejects the latency-only negative control, and blocks support promotion within the finite result summary. | implemented |

These Lean hooks are implemented as finite-record predicates over promotion fields, speed-evidence fields, acceptance-accounting fields, failed-acceptance handling, high-risk fast-mode review, admission-lifecycle routing, the public Project Theseus generation-mode import summary, and the local public-safe fast-generation task-bundle result summary. `AsiStackProofs.FastGeneration` now includes derived negative-case and route theorems for missing accepted-output or verifier-cost evidence, failed accelerated drafts without fallback or residual handling, high-risk fast-mode selections without verifier, risk-override, or slower-fallback records, incomplete admission reviews, a fixture bridge showing that the imported accepted-span speed-lift warnings do not permit promotion when useful-solution evidence and promotable comparisons are zero, and a fixture bridge showing that the task bundle preserves candidate quality, improves deterministic cost accounting, rejects the latency-only proxy, and blocks support promotion in the finite record. The cross-chapter support aggregate is checked in `AsiStackProofs.TheseusReference` as `theseusFastSupportAggregateFixture`, so the Fast Generation lane can rely on the selected support-lane aggregate without duplicating the proof tag. The modules still do not prove that any decoder is faster, that any verifier preserves quality for model output, that the route selector is adequate, that fallback execution works outside the fixture, that Theseus has been freshly replayed from a live task bundle, or that any external literature claim has been reproduced; the support replay probe only reruns the ASI-side static-import validators.
...
Fast generation reduces time pressure at the output boundary, but stored and routed artifacts still need compression contracts of their own. RankFold, NeuralFold, and Artifact Compression narrows the focus to reusable artifacts: how archive forms, deterministic decode checks, probes, fallback, utility, and cost accounting decide whether a compressed artifact can safely replace or accompany its fuller source.
