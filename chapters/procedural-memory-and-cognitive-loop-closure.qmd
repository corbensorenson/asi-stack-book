...
open_evidence_gaps:
  ...
  - "Protocol schema/fixture validation, finite Lean gates with negative-case theorems, and the synthetic procedural-memory loop harness are implemented; no deployed loop detector, tool synthesizer, generated-tool correctness check, routing monitor, regression-quality benchmark, or retirement automation exists."
...
`AsiStackProofs.ProceduralMemory` adds finite record gates for generated tools and regression promotion reviews. The new negative-case theorems show that a generated tool missing source traces, parameters, or a verification result cannot satisfy the closure-artifact predicate, and that a failed-regression review cannot both satisfy the promotion-blocking predicate and keep routable status promoted.
...
Fixture-shape validation, four finite Lean theorem declarations over two record gates, and the synthetic procedure-qualification harness are implemented. Remaining tests need deployed repeated-trace mining, synthesized tool candidates, generated-tool correctness checks, regression-quality benchmarks, routing monitors, and retirement automation.
...
These Lean hooks are implemented as finite-record predicates plus negative-case theorems over generated-tool records and regression-promotion reviews. They prove only the record-level gates: a generated tool records source traces, parameters, and a verification result; a generated-tool record missing any closure artifact cannot satisfy that predicate; a failed regression blocks routable-status promotion; and a failed-regression record cannot remain promoted while satisfying the blocking predicate. The companion synthetic harness makes loop-qualification record discipline executable, but neither Lean nor the harness proves deployed loop detection, invariant abstraction quality, parameter-discovery quality, synthesized tool correctness, monitoring quality, regression quality, or retirement automation.
