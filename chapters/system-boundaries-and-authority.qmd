...
open_evidence_gaps:
  ...
  - "The authority transition harness validates synthetic authority-transition gate semantics for scoped allow, over-ceiling denial, approval escalation, missing receipts, permission-class collapse, and confused-deputy shortcuts. The executed grant-to-effect refinement checks three reachable traces, twenty invariant prefixes, twenty-three batch compositions, and fifty state-noninterfering semantic mutations while binding the existing local effect, denial, revocation, and governed-repository records. Deployed authorization enforcement, authentic identity and receipts, concurrent revocation, complete observation, live runtime adapters, and tool-wrapper security remain unimplemented."
  - "`AsiStackProofs.AuthorityEffectRefinement` now owns the first two proof targets with arbitrary-run invariant and caller-ceiling preservation, valid-trace extraction, event-batch composition, revoked-ID persistence, and revoked-suffix effect exclusion. `AsiStackProofs.Authority` retains finite ceiling and lifecycle route lemmas; one projection-only declaration was physically retired. Neither module proves deployed authorization enforcement or production security."
...
| Test state | `authority_transition_record.valid.json` passes protocol fixture validation; `AsiStackProofs.Authority` and `AsiStackProofs.AuthorityEffectRefinement` build locally; and `python3 scripts/validate_authority_effect_refinement.py` passes over 6 authority fixtures, 3 reachable traces/20 events, 20 invariant-prefix checks, 23 batch-composition checks, 1 executed local effect, 1 independent observation, 1 exact rollback, 2 pre-effect denials, 5 revocation entries, 9 governed repository scenarios, and 50 of 50 state-noninterfering mutation rejections. Earlier authority, runtime-adapter, and revocation validators remain passing. Deployed permission enforcement, authentic identity and receipts, concurrent revocation, complete observation, production security, and transfer remain planned. |
...
| `lean:authority.ceiling.failure_blocks_promotion` | `AsiStackProofs.AuthorityEffectRefinement` | Revoked grant IDs persist and cannot commit an effect in any successful suffix; the independent consumer checks one-use, two-use, and revocation traces, every prefix and batch split, and 50 state-noninterfering widening, substitution, freshness, receipt, observation, rollback, revocation, and reuse mutations. | implemented |
...
The first two targets are now implemented by the reachable model in `AsiStackProofs.AuthorityEffectRefinement`; the lifecycle route and historical revocation-surface bridge remain in `AsiStackProofs.Authority`. The stronger module's 27 declarations prove exact one-step consequences plus arbitrary-run invariant and caller-ceiling preservation, valid-trace extraction, event-batch composition, revoked-ID persistence, and exclusion of revoked-grant effects from every successful suffix. Its one-use, two-use, and revocation witnesses cover 20 events. The independent Python consumer checks every witness prefix and 23 batch splits, refines the model against existing executed local records, and rejects 50/50 source/model mutations without changing the pre-rejection state. The older general ceiling lemmas and lifecycle negative cases remain useful at their exact local scope. The projection-only declaration that merely extracted audit and non-claim fields from an assumed-valid record was physically retired. This finite proof packet does not prove natural-language authority extraction, real identity or receipt authenticity, wise issuance, complete observation, concurrent or distributed revocation, tool-wrapper enforcement, deployed approval services, production security, reproduction, transfer, or chapter-core support.
...
three witnesses, arbitrary-run theorems, and 50 rejected mutations show exact finite routing consequences;
