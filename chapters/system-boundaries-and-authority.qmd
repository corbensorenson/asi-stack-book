...
open_evidence_gaps:
  ...
  - "The authority_transition_record fixture validates schema/example consistency only; full authority-ceiling, permission-separation, and confused-deputy tests remain unimplemented."
...
| Test state | `authority_transition_record.valid.json` passes protocol fixture validation; full authority-ceiling, permission-separation, and confused-deputy tests remain planned. |
...
The claim remains at `argument` support. VIEA, SCF, Talos, Ladon/Manhattan, and GenesisCode all push toward typed boundaries and effect control, but the public claim only strengthens when this repo has exact source mappings, denial fixtures, or mechanized predicates.
...
These proof targets remain planned. The already implemented stack-boundary Lean module covers the narrower external-action handoff predicate introduced in the previous chapter; this chapter's full transition model still needs denial fixtures and formalization.
