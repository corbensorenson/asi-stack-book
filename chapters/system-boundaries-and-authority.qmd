...
open_evidence_gaps:
  ...
  - "The authority transition harness validates synthetic authority-transition gate semantics for scoped allow, over-ceiling denial, approval escalation, missing receipts, permission-class collapse, and confused-deputy shortcuts. The executed grant-to-effect and delegation refinements check three effect traces, one two-hop delegation trace, twenty-six total batch compositions, sixty-seven state-noninterfering semantic mutations, one lossy-summary authority collision, and twenty complete-transport mutations while binding the existing local effect, denial, revocation, and governed-repository records. Deployed authorization enforcement, authentic identity and receipts, concurrent or distributed delegation/revocation, complete observation, live runtime adapters, and tool-wrapper security remain unimplemented."
  - "`AsiStackProofs.AuthorityEffectRefinement` still owns all four proof targets through an exact thirty-one-theorem grant-to-effect surface. The exact 59-declaration authority surface now includes a transitive delegation-chain refinement with arbitrary-run custody, attenuation, epoch/expiry continuity, non-authority, composition, a two-hop witness, closed substitutions, summary-loss impossibility, and complete transport, beside the original finite ceiling and lifecycle routes. One projection-only declaration was physically retired. Neither module proves deployed authorization enforcement or production security."
...
### Worked authority check: a scope widens between plan and effect
...
adequate only for its bounded reachable grant-to-local-effect semantics and
`Authority` as adequate only for bounded sequential delegation-chain semantics. The
four witness families, arbitrary-run theorems, and 67 rejected mutations show exact finite routing consequences;
