...
open_evidence_gaps:
  ...
  - "The authority transition harness validates synthetic authority-transition gate semantics for scoped allow, over-ceiling denial, approval escalation, missing receipts, permission-class collapse, and confused-deputy shortcuts; deployed authorization enforcement, runtime adapter behavior, revocation propagation, and tool-wrapper security remain unimplemented."
...
| Test state | `authority_transition_record.valid.json` passes protocol fixture validation, `AsiStackProofs.Authority` builds locally, and `python3 scripts/validate_authority_transitions.py` passes with 3 valid and 3 expected-invalid synthetic authority-transition fixtures; the Authority lifecycle route proof builds over finite records; deployed permission enforcement, runtime adapter behavior, revocation propagation, and tool-wrapper security remain planned. |
...
This authority type system is still an architectural target. The synthetic harness exercises narrow transition-gate behavior, but authority support remains at `argument` until deployed denial paths, revocation propagation, runtime adapter enforcement, tool-call traces, and adversarial confused-deputy probes show that authority cannot expand silently by implication.
...
| Confused-deputy scenario | Check that a low-authority requester cannot borrow a tool's broader raw capability. | implemented for expected-invalid synthetic fixture; runtime adapter probe not run |
