...
The first half of Part I builds the safety frame for authority. The operational path from human purpose into the stack begins here. It asks how a request becomes a contract without letting ambiguity become permission.
...
This is why intent must become a formal input rather than a conversational preface. Later layers should not have to infer whether a goal was real, whether a method was allowed, or whether a missing constraint was intentionally omitted. They should receive a record that can be challenged, narrowed, cited, and revised before work becomes difficult to unwind.
...
**Diagram reading note:** The request is narrowed before it becomes work. Ambiguity, missing authority, or high risk branches toward clarification or residual review, while only a bounded Intent Contract moves into alignment checks, command contracts, and authorized typed work.
...
Human intent enters the mature stack as a structured agreement, not a prompt-shaped wish: goals, constraints, authority, artifacts, evidence requirements, ambiguities, and stop conditions travel together. The intent compiler carries that agreement through:
...
- Intent closure: goal misbinding, scope creep, and implicit permission to act should stop at intake review, re-confirmation, denial, escalation, or residual records before planning treats the request as authorized.

This intent compiler remains a target architecture, not a current-result claim. Keep intent formalization at `argument` until intent-parser fixtures, ambiguity and stop-condition tests, authorization handoff traces, scope-creep probes, and acceptance-criteria checks show that downstream execution cannot turn desire into unbounded permission.
