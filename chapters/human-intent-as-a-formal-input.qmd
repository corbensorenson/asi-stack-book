...
open_evidence_gaps:
  ...
  - "The intent_contract fixture validates schema/example consistency only; record-level Lean predicates now include preservation, missing-authority, and finite intent-resolution routing checks, but ambiguity parsing, authority extraction, and end-to-end stop-condition preservation tests remain unimplemented."
...
The current formal slice models only the structured routing decision after an intake record exists. It checks that missing intent text or prohibited actions route to rejection, ambiguity or non-goal conflicts route to clarification, and high-impact work with missing authority or irreversible effects routes to review before command compilation. That is useful for the gate shape, but it does not parse the raw request, judge semantic ambiguity, or validate a deployed intake UI.
...
| Intent resolution route proof | Check that a structured intent-resolution record routes missing text, prohibited actions, ambiguity, conflicts, and high-impact authority gaps to rejection, clarification, or review before compilation. | implemented in Lean; finite record only |
