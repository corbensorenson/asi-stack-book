...
open_evidence_gaps:
  ...
  - "`AsiStackProofs.IntentResolutionRefinement` now has 25 declarations for event write ownership, arbitrary-run root and authority-ceiling custody, valid traces, batch composition, and request-to-contract/re-contract semantics. Its independent consumer checks four traces, fourteen invariant prefixes, eighteen batch compositions, and forty state-noninterfering mutations while binding the existing intake, re-contract, and plan fixtures. Deployed natural-language parsing, semantic completeness, authentic authority extraction, prompt-injection containment, approval/runtime services, natural workloads, reproduction, and transfer remain unimplemented."
...
| Test state | `AsiStackProofs.IntentContracts` retains 20 finite resolution/admission branches, while `AsiStackProofs.IntentResolutionRefinement` supplies 25 declarations for exact event write ownership, arbitrary-run root/ceiling custody, valid traces, batch composition, and versioned re-contract semantics. `python3 scripts/validate_intent_resolution_refinement.py` passes over 4 traces/14 events, 14 invariant prefixes, 18 batch compositions, 4 valid/6 invalid intake cases, 2 valid/7 invalid re-contract cases, 13 plan fixtures, and 40/40 state-noninterfering mutations. No natural-language-understanding, semantic-completeness, authentic-authority-extraction, prompt-injection-containment, deployed-dispatch, user-satisfaction, reproduction, transfer, or support-promotion claim. |
...
The executed resolution-to-contract refinement now composes those surfaces into reachable finite state. Its 25 declarations preserve root-intent identity, the original authority ceiling, and approved-authority boundedness over arbitrary successful runs; extract recursively valid traces; compose event batches; and restrict contract payload writes to parse or accepted re-contract transitions. Four witnesses cover direct acceptance with re-contract, clarification, unchanged continuation, and rejection. Its independent consumer binds the intake, re-contract, and complete 13-fixture plan inventory by digest, checks all 14 witness prefixes and 18 batch splits, and rejects 40/40 semantic mutations without changing the accepted prefix state. The resulting evidence is stronger at the structured-record boundary, not at natural-language semantics: hashes, labels, authority, and receipts remain trusted inputs, and the model stops before dispatch or effect.
...
The two headline targets and intake bridge now resolve to `AsiStackProofs.IntentResolutionRefinement`; the general finite resolution/admission branches remain in `AsiStackProofs.IntentContracts`. The reachable model proves exact bounded write ownership, arbitrary-run root/ceiling/approved-authority preservation, valid-trace extraction, material-delta custody, batch composition, and versioned re-contract consequences, while the independent consumer binds the existing structured records and 40 state-noninterfering mutations. Five assumption-restating or literal-summary declarations were physically retired with frozen lineage preserved. This proof packet does not prove natural-language parsing, semantic equivalence, legitimate authority extraction, user preference fidelity, prompt-injection containment, private-source safety, end-to-end lowering, deployed dispatch/effects, user satisfaction, natural-workload usefulness, reproduction, transfer, or chapter-core support.
