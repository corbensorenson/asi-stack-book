...
open_evidence_gaps:
  ...
  - "`AsiStackProofs.IntentResolutionRefinement` now has 30 declarations for event write ownership, arbitrary-run root and authority-ceiling custody, valid traces, batch composition, request-to-contract/re-contract semantics, and lowering information boundaries. It proves a concrete collision under a thin four-field lowering, impossibility of decoding both collision witnesses, and injectivity of the modeled full ten-field lowering. Its independent consumer checks four traces, fourteen invariant prefixes, eighteen batch compositions, forty state-noninterfering lifecycle mutations, six omitted-field collisions, and ten full-field mutations while binding the existing intake, re-contract, and plan fixtures. Deployed natural-language parsing, semantic adequacy, authentic authority extraction, valid consent, prompt-injection containment, approval/runtime services, natural workloads, reproduction, and transfer remain unimplemented."
...
| Test state | `AsiStackProofs.IntentContracts` retains 20 finite resolution/admission branches, while `AsiStackProofs.IntentResolutionRefinement` supplies 30 declarations for exact event write ownership, arbitrary-run root/ceiling custody, valid traces, batch composition, versioned re-contract semantics, a concrete thin-lowering collision, decoder impossibility over that collision, and injectivity of the modeled full lowering. `python3 scripts/validate_intent_resolution_refinement.py` passes over 4 traces/14 events, 14 invariant prefixes, 18 batch compositions, 4 valid/6 invalid intake cases, 2 valid/7 invalid re-contract cases, 13 plan fixtures, 40/40 state-noninterfering lifecycle mutations, 6 omitted-field collisions, and 10/10 full-field mutations. No natural-language-understanding, semantic-adequacy, authentic-authority-extraction, valid-consent, prompt-injection-containment, deployed-dispatch, user-satisfaction, reproduction, transfer, or support-promotion claim. |
...
30-declaration finite Lean refinement grouped with the route model under five manifest proof targets; a
...
shape, finite route consequences, representational information loss, origin and preservation checks,
...
The executed resolution-to-contract refinement now composes those surfaces into reachable finite state. Its 30 declarations preserve root-intent identity, the original authority ceiling, and approved-authority boundedness over arbitrary successful runs; extract recursively valid traces; compose event batches; and restrict contract payload writes to parse or accepted re-contract transitions. Four witnesses cover direct acceptance with re-contract, clarification, unchanged continuation, and rejection. A separate lowering boundary makes information loss explicit: a four-field command projection omits forbidden means, authority basis, affected parties, privacy boundary, acceptance evidence, and permitted consumers, so distinct ten-field intent records can map to the same command and no decoder can recover both collision witnesses. The corresponding full modeled lowering is injective, and changing affected-party or privacy fields changes its output. The independent consumer binds the intake, re-contract, and complete 13-fixture plan inventory by digest, checks all 14 witness prefixes and 18 batch splits, rejects 40/40 lifecycle mutations without changing the accepted prefix state, reconstructs six omitted-field collisions, and rejects mutations to all ten full fields. The resulting evidence is stronger at the structured-record boundary, not at natural-language semantics: the theorem proves information preservation only for the modeled fields, while field meanings, hashes, labels, authority, consent, and receipts remain trusted inputs and the model stops before dispatch or effect.
...
| Executed Human Intent resolution-to-contract refinement | Check reachable root, version, constraint/stop, authority, ambiguity, material-delta, re-contract custody, and modeled lowering information boundaries against the bounded intake, re-contract, and plan-fixture surfaces. | implemented by `python3 scripts/validate_intent_resolution_refinement.py`; 30 Lean declarations, 4 valid/6 invalid intake cases, 6 signals, 2 valid/7 invalid re-contract cases, 13 plan fixtures, a 5-event version-2 witness, 40/40 rejected lifecycle mutations, 6 thin-lowering collisions, and 10/10 rejected full-field mutations; support-state effect `none`; no natural-language understanding, semantic adequacy, authentic authority, valid consent, deployed dispatch, reproduction, or transfer claim |
...
The two headline targets, intake bridge, and lowering boundary now resolve to `AsiStackProofs.IntentResolutionRefinement`; the general finite resolution/admission branches remain in `AsiStackProofs.IntentContracts`. The reachable model proves exact bounded write ownership, arbitrary-run root/ceiling/approved-authority preservation, valid-trace extraction, material-delta custody, batch composition, versioned re-contract consequences, one explicit lossy-projection collision, decoder impossibility for the colliding pair, and injectivity of the modeled full projection. The independent consumer binds the existing structured records, rejects 40 state-noninterfering lifecycle mutations, reconstructs six omitted-field collisions, and rejects ten full-field mutations. Five assumption-restating or literal-summary declarations were physically retired with frozen lineage preserved. This proof packet does not prove natural-language parsing, semantic adequacy of the modeled fields, legitimate authority extraction, valid consent, user preference fidelity, prompt-injection containment, private-source safety, deployed end-to-end lowering, dispatch/effects, user satisfaction, natural-workload usefulness, reproduction, transfer, or chapter-core support.
