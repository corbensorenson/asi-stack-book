...
open_evidence_gaps:
  ...
  - "`AsiStackProofs.IntentResolutionRefinement` now has 36 declarations for event write ownership, arbitrary-run root and authority-ceiling custody, valid traces, batch composition, request-to-contract/re-contract semantics, and command/route transport information boundaries. It proves a concrete collision under a thin four-field command lowering, impossibility of decoding both collision witnesses, and injectivity of the modeled full ten-field lowering. It also proves two route-changing collisions under a thin two-field resolution transport, an exact-router impossibility result, and round-trip, injectivity, and static-route preservation for a complete seven-field transport. Its independent consumer checks four traces, fourteen invariant prefixes, eighteen batch compositions, forty state-noninterfering lifecycle mutations, six omitted command-field collisions, two route-changing transport collisions, ten full command-field mutations, and seven complete route-transport mutations while binding the existing intake, re-contract, and plan fixtures. Deployed natural-language parsing, semantic sufficiency, authentic authority extraction, valid consent, prompt-injection containment, approval/runtime services, natural workloads, reproduction, and transfer remain unimplemented."
...
### Worked intent contract: “clean up the repo”
...
36-declaration finite Lean refinement grouped under five manifest proof targets; a
