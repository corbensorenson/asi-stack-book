...
open_evidence_gaps:
  ...
  - "The intent_contract fixture validates schema/example consistency only; record-level Lean predicates now include preservation, missing-authority, finite intent-resolution routing, and expanded intent-admission lifecycle checks for hidden overrides, unresolved ambiguity, constraint precedence, stop-condition preservation, authority ceilings, re-contract triggers, and non-claim boundaries; natural-language ambiguity parsing, authority extraction, and end-to-end runtime stop-condition preservation remain unimplemented."
...
| Test state | `intent_contract.valid.json` passes protocol fixture validation; `AsiStackProofs.IntentContracts` implements finite preservation, missing-authority, intent-resolution, and intent-admission lifecycle route predicates; `python3 scripts/validate_plan_execution_contracts.py` now includes synthetic intent-origin checks for explicit-constraint preservation, unresolved-ambiguity dispatch blocking, hidden-override rejection, authority-ceiling preservation, stop-condition preservation, and re-contract trigger preservation; natural-language ambiguity parsing, authority extraction quality, and end-to-end runtime preservation tests remain planned. |
...
The current formal slice models only structured records after intake fields exist, but it is no longer limited to the first resolution branch. It checks the older resolution cases for missing text, prohibited actions, ambiguity, conflict, high-impact authority, and irreversibility; it also checks an admission lifecycle where hidden overrides reject, unresolved ambiguity and missing constraints request clarification, constraint-precedence and preservation gaps block admission, missing or widened authority routes to review, downstream changes trigger re-contracting, missing non-claim boundaries block admission, and complete records admit. That is useful for the gate shape, but it does not parse the raw request, judge semantic ambiguity, or validate a deployed intake UI.
...
| Stop-condition preservation test | Check that stop conditions survive lowering into plans, semantic atoms, and jobs. | partially implemented by synthetic plan-execution contract harness for plan-level preservation only; runtime preservation not run |
| Re-contract trigger test | Check that changed means, authority, publication surface, affected parties, evidence requirements, or stop conditions return to the contract boundary. | partially implemented by synthetic plan-execution contract harness for command-contract trigger preservation only; runtime re-contract behavior not run |
...
These Lean hooks are implemented as finite-record predicates over intent-compilation, authority-compilation, intent-resolution, and intent-admission reviews. They prove only the record-level gates: declared constraints and stop conditions are preserved by a valid compilation review, a contract missing required authority cannot compile to an executable job, selected resolution cases route away from command compilation, and selected admission cases route hidden overrides, unresolved ambiguity, preservation gaps, authority gaps, re-contract triggers, and missing non-claim boundaries away from accepted admission. They do not solve natural-language parsing, infer hidden authority, resolve ambiguity from raw text, judge reviewer quality, or test end-to-end lowering into plans, semantic atoms, or typed jobs.
