...
External baseline exception: the current source queue for human intent is Corben/local architecture material about turning request, authority, and acceptance conditions into formal inputs. No source-noted external requirements-engineering, preference-learning, or human-computer-interaction baseline has been assigned here yet, so the v1.0 candidate records the gap instead of citing from memory.
