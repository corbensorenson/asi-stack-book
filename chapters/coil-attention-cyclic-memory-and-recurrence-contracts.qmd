...
**Diagram reading note:** The cyclic-memory contract is a chain-of-custody record from request to slot mapping, winding, coverage, freshness, and recurrence budget. Passing the structural contract admits the representation as a diagnostic guardrail, not as proof of retrieval quality or reasoning success.
...
The fixture is only a structural receipt. It shows that the book can represent the cyclic-memory claim without blurring it into model quality or runtime memory performance.

A compact trio of traces would extend the minimum: one slot reuse with winding preserved, one stale read blocked or residualized, and one recurrence schedule that exits and falls back. A later workload harness can then ask whether those structural protections improve task behavior.
...
Coil memory earns a mature role by providing structural guardrails for slots, lag, freshness, coverage, recurrence, and work budgets while leaving retrieval quality and reasoning quality to separate evidence. The recurrence-control plane exposes those guardrails through:
...
- Recurrence closure: alias hiding, non-terminating recurrence, uncovered lags, stale-read misuse, freshness-as-quality laundering, missing ordinary baselines, and authority widening through memory remapping become blocked admission, fallback attention, residual escrow, or downgraded support state.

This recurrence contract remains a target architecture, not a current-result claim. The claim should remain at `argument` until alias-boundary tests, retrieval-quality records, stale-memory probes, recurrence-contract fixtures, reset behavior, and promotion reviews show that cyclic memory improves continuity without letting old state become hidden authority.
