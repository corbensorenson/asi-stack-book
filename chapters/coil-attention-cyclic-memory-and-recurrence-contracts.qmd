...
| Source loading state | source notes and passage-reviewed mappings exist for all assigned sources; `vcm_public` uses local raw cache, while Circle and Theseus transfer mappings use pinned local Circle project passages; the Circle cyclic-memory receipt slice now imports one external-project residue/winding fixture without vendoring Circle or claiming learned-memory behavior |
...
| `circle_ai_contract_suite` | Theorem-linked contract families and consumer fields for KV-cache freshness, sparse attention, recurrence schedules, cyclic memory residue/winding, multicoil phase, and explicit non-claims. | Separate external Circle receipt slices are recorded for RoPE in `docs/circle_external_receipt_slice.md` and for cyclic memory in `docs/circle_cyclic_memory_receipt_slice.md`; the cyclic-memory slice is a local external-project residue/winding receipt only, not a learned-memory, long-context, runtime, or support-state result. |
