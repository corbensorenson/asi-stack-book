...
These Lean hooks are implemented as finite-record predicates over cyclic memory alias reviews, retrieval-quality promotion reviews, recurrence-admission reviews, and stale-read admission reviews. The public hooks now bind to derived negative cases rather than assumption-restating projections: reused slots with missing residue/winding and no visible residual are rejected, and structure-only retrieval-quality promotion is rejected. The module also rejects recurrence admission without work-budget, exit-condition, or fallback records and stale reads admitted as fresh without residual escrow. These results do not prove retrieval quality, KV-cache behavior, sparse-attention coverage, recurrence-exit behavior, freshness performance, long-context improvement, memory savings, or learned-model quality.

The module contains four theorem declarations, all derived negative cases for
hidden aliasing, structure-only promotion, recurrence without
budget/exit/fallback, and stale-read admission without residual escrow. The two
former assumption-restating projections were retired after their public targets
moved to the corresponding negative cases. None proves cache semantics,
cross-request isolation, coverage completeness, recurrence liveness, progress,
fallback reachability, useful memory, retrieval, reasoning, latency, memory
efficiency, independent reproduction, or transfer. Future formal work should
add transition, invalidation, progress, liveness, noninterference, or
runtime-refinement semantics only where those obligations constrain a real
consumer.
