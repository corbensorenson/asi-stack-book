...
**Diagram reading note:** Compactness is useful only when the generator, verifier, residual channel, fallback path, and cost ledger remain visible together. The residual burden audit is the guard against treating a small seed or elegant rule system as if it had carried every hidden cost.
...
Compactness becomes useful only when the generator, verifier, fallback, governance burden, reconstruction burden, decision burden, and residual burden are all counted. The compact-generation ledger exposes that burden in:
...
- Residual-honesty closure: false lossless claims, hidden residual displacement, utility-damaging compression, self-promoting generators, and recursive instability become downgraded claims, residual escrow, blocked promotions, fallback routes, quarantine, or SCF replacement reviews.

This compact-generation boundary remains a target architecture, not a current-result claim. The compactness claim stays at `argument` until exactness records, verifier outputs, reconstruction tests, residual-burden accounts, lossy-ablation cases, and fallback traces show which information compact generation preserved and which obligations it left behind.
...
Compact systems become sharper when the claim changes from useful generation to exact reconstruction. Generate-Verify-Repair Compression takes residual honesty into that stricter setting. It requires a generator, declared reconstruction contract, verifier, repair stream, literal fallback, consumer policy, and cost ledger so a smaller representation cannot claim exactness unless the missing remainder is actually paid.
