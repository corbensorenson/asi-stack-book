...
| Test state | `compact_generative_record.valid.json`, `compression_receipt.valid.json`, and `semantic_node_record.valid.json` pass repository-level protocol fixture validation; finite residual/exactness predicates are implemented in `AsiStackProofs.CompactGenerativeSystems`; finite reconstruction/verification predicates are implemented in `AsiStackProofs.GenerateVerifyRepair`; finite semantic provenance/supersession predicates are implemented in `AsiStackProofs.SemanticRepresentation`; behavioral CGS utility, fallback, reconstruction-quality, repair-cost, bounded-search, semantic grounding, hierarchy-revision, representation-utility, consumer-policy, and downstream tests remain planned. |
...
These Lean hooks are implemented as finite-record predicates. The validated JSON fixtures and Lean predicates do not prove that compact representations preserve utility, reduce cost, reconstruct real artifacts, improve compression rate, ground semantic graphs, preserve semantic utility, improve model quality, or block promotion under all executable conditions.
