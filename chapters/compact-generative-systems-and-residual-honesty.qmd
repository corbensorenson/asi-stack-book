...
The `qcsa_whitepaper` therefore adds a falsification burden to Semantic
Representation Leasing. Plural atlases, active questions, certificates, and
round-trip checks must beat matched flat, random/frequency-tree, single-tree,
and direct-generation baselines on a useful frontier. Until they do, QCSA is
design rationale and the existing `argument` boundary remains unchanged.
