...
The general contract and a candidate codec should not compete as equal
top-level explanations. Compact Generative Systems owns the architecture-wide
question: when may a smaller representation stand in for a larger target, and
which reconstruction, verification, repair, fallback, consumer, and residual
obligations survive that substitution? [RankFold, NeuralFold, and Artifact
Compression](rankfold-neuralfold-and-artifact-compression.qmd) owns a narrower
implementation family: tensor-oriented archive construction, low-rank and
learned front ends, manifests, deterministic decoding, access-pattern probes,
WORM assumptions, and the Compressed Artifact Admission Lease.

That method family matters because it supplies a concrete place where the
general contract can fail. Exact sequential reconstruction does not imply
acceptable random access. A good transform error does not imply a small final
archive once residuals and metadata are counted. A compact archive does not
inherit the source artifact's evidentiary authority. Decoder, platform, access
pattern, fallback, and time window remain part of the use decision.

The technical route therefore remains directly addressable and independently
testable. Its local artifacts, proof targets, replay probe, negative result,
source mappings, and open benchmark burden stay with RankFold/NeuralFold.
Compact Generative Systems supplies the shared residual-honesty and
representation-leasing frame; it does not inherit a codec result or imply that
RankFold/NeuralFold has qualified for production use.
...
| `treellm` | TreeLLM correction lineage | semantic_representation | Structured semantic tokens, residual fingerprints, versioned graph coordinates, exact/approximate fallbacks, adaptive short-id caching, and semantic-update obligations after rejecting frozen roots, canonical truth, fixed-width adequacy, and unsupported performance claims. | source note available; local raw cache available |
...
| `ext_raptor_2024` | RAPTOR | hierarchical_retrieval_and_abstraction | External comparator for recursively clustered/summarized retrieval trees and multi-resolution compression. | source note available |
...
RankFold, NeuralFold, and Artifact Compression remains a stable technical-detail route after publication nesting because it owns the technique-facing artifact and tensor-compression lane. It inherits the compact-system residual-honesty, semantic-leasing, and consumer-policy requirements while asking a different question: when may a compressed artifact stand in for a full artifact for a specific task?
