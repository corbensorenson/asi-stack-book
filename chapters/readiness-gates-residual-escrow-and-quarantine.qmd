...
open_evidence_gaps:
  ...
  - "Protocol schema/fixture validation, narrow Lean gate/quarantine predicates plus negative cases for failed promoted gates, stronger transitions missing freshness/residual/fallback/expiry records, quarantined ordinary routing or diagnostic routing without fallback, stale-gate reuse without rerun/residual records, and the readiness/residual gate harness's synthetic transition, residual-escrow, quarantine, expired-rerun, fallback, and rollback-readiness checks are implemented; no readiness engine, residual-ledger storage, benchmark harness, live quarantine routing, or MoECOT replay has been run for this chapter."
...
| Test state | Readiness gate fixture validation includes field identity, workload family, freshness window, inherited residuals, route permissions, fallback path, expiry, and non-claims; finite gate/quarantine predicates and negative cases for failed promoted gates, incomplete stronger transitions, quarantined route misuse, and stale-gate reuse are implemented in `AsiStackProofs.ReadinessGates`; `python3 scripts/validate_readiness_residual_gates.py` validates synthetic transition, residual-escrow, quarantine, expired-rerun, fallback, and rollback-readiness checks. Readiness-engine behavior, residual-ledger storage, benchmark evidence, live quarantine routing, and MoECOT replay remain planned. |
...
Fixture-shape validation, two bounded Lean predicates, and the readiness/residual synthetic harness are implemented. Remaining tests require a deployed lifecycle transition checker, residual ledger, benchmark harness, and routing harness that respects quarantine state.
...
These proof hooks are implemented as finite-record predicates over gate decisions, route selections, transition records, quarantine routing reviews, and stale-gate reuse reviews. The derived negative cases are `promoted_decision_with_failed_required_gates_rejected`, `accepted_stronger_transition_missing_required_record_rejected`, `quarantined_target_ordinary_or_unbacked_diagnostic_route_rejected`, and `stale_gate_reuse_without_rerun_or_residual_rejected`. They prove only record-level gates: promoted decisions cannot survive failed required gates; accepted stronger transitions require fresh evidence, residual escrow, fallback, and expiry records; quarantined targets cannot be ordinary-routed or diagnostic-routed without fallback; and stale gate reuse requires rerun and residual records. They do not prove benchmark quality, residual-ledger storage, lifecycle transition execution, live routing enforcement, MoECOT replay, or current readiness of any module.
