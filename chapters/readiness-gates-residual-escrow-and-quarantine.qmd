...
open_evidence_gaps:
  ...
  - "Protocol schema/fixture validation, narrow Lean gate/quarantine predicates plus negative cases for failed promoted gates, stronger transitions missing freshness/residual/fallback/expiry records, quarantined ordinary routing or diagnostic routing without fallback, stale-gate reuse without rerun/residual records, finite lifecycle transitions missing regression/authority/supersession/retirement records, the readiness/residual gate harness's synthetic transition/residual/quarantine checks, and the readiness lifecycle probe's six valid synthetic transitions plus twelve expected-invalid controls are implemented; no deployed readiness engine, residual-ledger storage, benchmark harness, live quarantine routing, or MoECOT replay has been run for this chapter."
...
| Test state | Readiness gate fixture validation includes field identity, workload family, freshness window, inherited residuals, route permissions, fallback path, expiry, and non-claims; finite gate/quarantine predicates, stale-gate predicates, and lifecycle-transition predicates are implemented in `AsiStackProofs.ReadinessGates` for failed promoted gates, incomplete stronger transitions, quarantined route misuse, stale-gate reuse, default promotion without regression/authority, quarantined lifecycle transitions with ordinary routing, supersession without a record, retirement without a receipt, and transition from retired state; `python3 scripts/validate_readiness_residual_gates.py` validates synthetic transition, residual-escrow, quarantine, expired-rerun, fallback, and rollback-readiness checks; `python3 scripts/validate_readiness_lifecycle_probe.py` validates six valid synthetic readiness lifecycle transitions and twelve expected-invalid controls at `experiments/readiness_lifecycle_probe/results/2026-07-02-local.json`. Readiness-engine behavior, residual-ledger storage, benchmark evidence, live quarantine routing, terminal-state governance, and MoECOT replay remain planned. |
...
The readiness sources support gate state, residual escrow, regression preservation, and quarantine. Eight mappings now have a narrower source-to-claim trace, and the Project Theseus architecture-gate lane has one static digest-verified report import. The book still has not run a live readiness gate, inspected a current Theseus dashboard, imported MoECOT runtime artifacts, or proved that any current Theseus, MoECOT, or book module passes readiness gates.
