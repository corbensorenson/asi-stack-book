...
open_evidence_gaps:
  - "The roadmap distinguishes completed historical T0 from active successor T0A and records the exact partial replay state on published Theseus `main`: 7/7 CPU/governance passes and 13/14 valid accelerator receipts. The prior final-shard attempt stopped below its live-memory reserve and has calibration authority only; the latest exact guard observed 3,874.8 MiB below the corrected 6,144 MiB launch floor, so the child did not start and no model result exists. T1 remains blocked until the unchanged final shard passes, a current content-addressed freeze exists, and the 84-chapter readiness rerun passes; none of this is optimizer, training, or capability evidence."
...
The natural Theseus flagship is the concrete late-phase consumer of this
roadmap contract. `ASI-THESEUS-FLAGSHIP-01` may be preregistered only after its
five matched route families, competence dossiers, evaluator independence,
attack families, resource denominators, rescue rules, freeze boundary, and
terminal result classes resolve to versioned artifacts. Completing this
roadmap chapter does not complete that experiment, and imported Theseus reports
do not satisfy its natural-task gates. The handoff is one-way until the
protected campaign is opened: the roadmap may authorize a frozen run contract,
while only the flagship's later evidence and an accepted evidence transition
may change a bounded claim.

Its first dependency is now stated at the right granularity. Historical `T0`
is complete and preserved as an immutable 57M architecture control. Successor
`T0A` is the active finite closure: the candidate docket and factorized
selection are organized, but 19 of 123 artifacts bound by the historical
package have changed, so that package supplies no authority to `T1`. The next
unlock is a guarded independent replay, a replacement content-addressed
freeze, and a readiness rerun against the 84-chapter book—not another
unbounded architecture intake and not a premature training or capability
claim.
...
Three proof targets are implemented by nine Lean declarations. One is a derived negative case over a hand-authored claim-promotion predicate, five reduce one hand-authored phase-route function, and three normalize a finite fixture-bridge summary or preserve its non-claims. A separate synthetic readiness/residual check covers expired-evidence rerun/reject behavior. These artifacts establish record and routing boundaries only.
...
These Lean hooks are implemented as finite-record predicates over claim-promotion, phase-route, and phase-gate fixture-bridge reviews. The former unlock conjunction projection and its otherwise unused record model are retired. The v1.x follow-through route envelope proves only record-level gates: missing source-matrix readiness rejects a phase route; self-improvement without an independent evaluator is rejected; failed acceptance gates keep a phase research-only; requested support promotion without an accepted evidence-transition record is rejected; a reached milestone with no evidence cannot promote; a fully accepted non-promoting phase can route to integration; and the complete synthetic phase-gate fixture summary can be accepted while missing non-claim boundaries reject the bridge. The prototype validator binds the migrated public targets to the retained negative-case and route theorem names. It does not prove that any prototype phase is complete, that dependent prototypes are safe to run, that evaluator independence exists in practice, that any acceptance gate is adequate, or that any capability claim has earned stronger support.

### Formal-proof audit boundary

The module contains nine declarations. One is a derived negative case over the hand-authored claim-promotion predicate. Five reduce a single hand-authored route function for missing source readiness, evaluator absence, failed gates, unsupported promotion, and accepted non-promoting integration. The final three normalize one finite fixture summary and preserve its declared non-claims. None proves that the chosen phase graph is correct, that a gate is adequate, that evaluator independence exists, that any real phase completed, that useful work improved, or that rollback, governance, reproduction, or transfer succeeds.
