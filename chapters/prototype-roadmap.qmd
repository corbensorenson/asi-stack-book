...
| Source loading state | source notes: all assigned sources; local raw caches cover the named Corben paper families; authenticated connector text is passage-reviewed for `moecot` and `moecot_md`; `road_to_agi` and `coherence_exchange` remain connector/recovery bounded; pinned local references: Project Theseus at `1ad88a22`, Circle Math at `63b0f511` |
