...
open_evidence_gaps:
  - "The roadmap distinguishes completed historical T0 from active successor T0A and records the exact partial replay state on published Theseus `main`: 7/7 CPU/governance passes, 13/14 valid accelerator receipts, and one final shard stopped by the external guard after the live-memory reserve was breached. T1 remains blocked until the unchanged final shard passes, a current content-addressed freeze exists, and the 84-chapter readiness rerun passes; none of this is optimizer, training, or capability evidence."
