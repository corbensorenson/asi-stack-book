...
open_evidence_gaps:
  - "The 2026-07-27 T0/T0A currentness import records clean published Theseus `main` at `653401de`, separates completed historical T0 from active successor T0A, finds 24 changed artifacts in the historical 123-artifact freeze, records 7/7 current CPU/governance replays and 13/14 valid accelerator receipts, preserves the final optimizer-matched shard's guarded live-memory-reserve stop, and binds that failed attempt as non-qualifying calibration evidence for a prospective 6,144 MiB launch floor. It does not establish a successful replay, current replacement freeze, training readiness, or T1 authority."
...
clean, published Theseus `main` tree at `653401de`; it therefore supplies no
...
remaining optimizer-matched accelerator shard passed its former 5,120 MiB
launch reserve and started, but the external guard stopped it after reclaimable memory
...
and swapout growth was zero. That is a safety result, not a negative optimizer
or architecture result and not completion. The failed attempt now has
calibration authority only: the 4,096 MiB live reserve plus the 1,529.766 MiB
observed peak plus a 512 MiB safety margin requires 6,137.766 MiB, rounded up
to a prospective 6,144 MiB launch floor. Rejecting controls prevent lowering
that floor or treating the failed receipt as qualification. `T1` remains
blocked until the same workload passes under that corrected guard, a replacement
content-addressed freeze is published, and the broader readiness gate is rerun
against the current 84-chapter book.
