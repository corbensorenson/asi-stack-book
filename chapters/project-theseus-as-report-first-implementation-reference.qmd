...
open_evidence_gaps:
  - "The 2026-07-27 T0/T0A currentness import records clean published Theseus `main` at `9e545cf1`, separates completed historical T0 from active successor T0A, finds 24 changed artifacts in the historical 123-artifact freeze, and records 7/7 current CPU/governance replays and 13/14 valid accelerator receipts. It preserves the prior optimizer-matched shard's guarded live-memory-reserve stop as non-qualifying calibration evidence for a prospective 6,144 MiB launch floor, then records the latest exact preflight observation: 3,874.8 MiB was available, below that floor, so the child did not start and no model result exists. It does not establish a successful replay, current replacement freeze, training readiness, or T1 authority."
...
The architecture-freeze handoff makes that distinction concrete. The
2026-07-27 public-safe import separates a completed historical `T0` package
from the active successor `T0A` closure. The old package remains a useful
immutable control, but 24 of its 123 bound artifacts differ from the current
clean, published Theseus `main` tree at `9e545cf1`; it therefore supplies no
authority for the successor campaign. The current implementation records a
GREEN factorized selection, acceleration selector, and project registry, plus
7/7 current CPU/governance replays and 13/14 valid accelerator receipts. The
prior optimizer-matched accelerator attempt passed its former 5,120 MiB launch
reserve and started, but the external guard stopped it after reclaimable memory
fell to 3,708.391 MiB below its declared 4,096 MiB live reserve. Maximum
inferred unified memory was 1,529.766 MiB, maximum process RSS was 231.75 MiB,
and swapout growth was zero. That safety stop remains calibration evidence,
not a negative optimizer or architecture result and not completion: the
4,096 MiB live reserve plus the 1,529.766 MiB observed peak plus a 512 MiB
safety margin requires 6,137.766 MiB, rounded up to a prospective 6,144 MiB
launch floor. On the latest exact retry, the guard observed only 3,874.8 MiB
available against that 6,144 MiB floor. The child therefore did not start,
was not terminated by the guard, returned no exit code, and produced no
optimizer, architecture, training, or capability observation. Rejecting
controls preserve both events without collapsing them into one story: they
reject erasing the earlier child start, inventing a child start or sufficient
memory for the latest preflight, lowering the floor, or treating either receipt
as qualification. `T1` remains blocked until the same exact guard itself
observes at least 6,144 MiB, the unchanged workload passes while preserving the
4,096 MiB live reserve, a replacement content-addressed freeze is published,
and the broader readiness gate is rerun against the current 84-chapter book.
A generic host-memory percentage is not launch authority.
