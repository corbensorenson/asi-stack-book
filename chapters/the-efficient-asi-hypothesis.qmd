...
This chapter owns **24** structured claim atoms in `CF-01`. The
frozen repository-wide terminal audit records 19 `blocked_after_full_attempt`; 5 `retained_after_full_attempt`. Its core
atom `the-efficient-asi-hypothesis.core` remains at `argument` with terminal
disposition `blocked_after_full_attempt`. The authoritative per-atom rows
are the `the-efficient-asi-hypothesis` slice of
...
at `argument` support. Across its 24 structured claims,
19 still lack required proof lanes,
0 were narrowed, and
...
| Exact accepted transitions in this chapter | `v1_0_pilot.efficient_asi.no_change` |
...
- **Failure or non-promotion case:** 19 atoms
...
|---|---:|---|---|
| Core: `the-efficient-asi-hypothesis.core` | 1 | `executable`, `formal`, `source-synthesis` | `blocked_after_full_attempt`; missing or unproved: `causal`, `empirical`, `normative`, `transfer`. |
| retained after full attempt | 5 | `executable`, `formal`, `source-synthesis` | All atom-required local routes were present at the frozen audit boundary, but no accepted upward transition was inferred. |
| blocked after full attempt | 19 | `executable`, `formal`, `source-synthesis` | Missing or unproved lanes across this set: `causal`, `empirical`, `executable`, `formal`, `normative`, `transfer`. These are residual proof obligations, not false claims. |
