...
External comparison sharpens the accounting target. Sparse MoE work such as `ext_sparse_moe_2017` shows conditional computation as a scale-efficiency mechanism, FrugalGPT-style routing in `ext_frugalgpt_2023` frames query-dependent cost-quality tradeoffs, and BIG-bench in `ext_bigbench_2022` shows why broad task suites can pressure capability claims without resolving hidden route cost or residual burden. The route-ledger design uses those records as baseline vocabulary for routed cognition and benchmark pressure, not as reproduced efficiency evidence for the ASI Stack.
...
Capability improves only when routed, compressed, reused, governed, and tested cognition leaves visible cost, quality, fallback, and residual records. The mature stack uses minimum viable intelligence for subtasks, turns repeated cognition into durable artifacts, tools, or procedural memory, models efficiency through the seed/router/search/generator/verifier/residual loop plus memory and governance, and exposes residual burden when routing or compression is incomplete.

Planning requests capability profiles, routing selects specialists, and evidence decides whether efficiency claims survived quality and cost tests. Efficiency claims include quality and cost, compression exposes residuals, and fallback routes remain available. Cheap inference mistaken for intelligence, critical context compressed away, and benchmark gains that increase hidden residual burden should create residual-cost records, fallback routes, demotion, or benchmark review.

This efficiency operating system is still an architectural hypothesis. The efficient-ASI claim remains at `argument` until route ledgers, lower-cost comparison traces, residual-accounting tests, hidden-cost audits, and quality/cost benchmarks show that cheaper cognition actually preserves the required predicates.
