...
The important distinction is between thrift and recklessness. The book is not arguing that every task should be made cheap. It is arguing that cost, quality, authority, uncertainty, and residual work should be visible together, so the system can choose the smallest adequate route without pretending that adequacy has been proved when it has not. Efficiency is governed only when saved work remains visible.

The target is not austerity; it is measured adequacy under a trace that later layers can challenge. Efficiency becomes trustworthy only when the saving remains visible after verification and repair costs are paid.

A cheap answer earns use only when the accounting survives review. Efficiency must pay its debt in evidence, not vibes. That debt keeps thrift accountable when efficiency looks too easy.
:::
...
Another failure is efficiency laundering: the system chooses a faster path, reports the speedup, and omits the verification, fallback, review, and failure-case costs that made the path acceptable. If those costs are outside the bill, the architecture may prefer a route that is cheaper only because the expensive parts became invisible.
...
The next useful fixture is a three-route comparison: one cheap route rejected because verification fails, one expensive route accepted because it satisfies the predicate, and one compressed route blocked because repair burden exceeds savings. Passing that fixture would show accounting discipline only, not an efficiency result.

The minimum is useful when a rejected low-cost path remains as visible as the accepted path.

## Beyond the State of the Art

Efficient ASI is best understood as an operating system for cognition. It routes, compresses, reuses, verifies, and accounts for work so capability grows by removing waste and preserving residuals, not by assuming that more parameters or more tokens are the only path forward.
