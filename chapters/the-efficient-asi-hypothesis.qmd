...
**Diagram reading note:** The route ledger starts with the task contract, quality predicate, and authority ceiling before it selects any route. The cheap path is valid only when verification passes; partial results loop back as residuals and fallback work rather than being counted as efficiency.
...
Capability improves only when routed, compressed, reused, governed, and tested cognition leaves visible cost, quality, fallback, and residual records. The efficiency operating system shows that discipline in:
...
- Efficiency closure: cheap inference mistaken for intelligence, critical context compressed away, and benchmark gains that increase hidden residual burden should create residual-cost records, fallback routes, demotion, or benchmark review.

This efficiency operating system remains a target architecture, not a current-result claim. The efficient-ASI hypothesis remains `argument` until route ledgers, lower-cost comparison traces, residual-accounting tests, hidden-cost audits, and quality/cost benchmarks show that cheaper cognition actually preserves the required predicates.
...
Efficiency only helps if the system knows where permission stops. System Boundaries and Authority takes the route ledger's quality and cost discipline and adds the missing control question: which principal, layer, grant, lease, or ceiling authorizes the next action? The book has now separated the stack from the model and efficient cognition from brute force; the next boundary prevents efficient routes from becoming permission leaks.
