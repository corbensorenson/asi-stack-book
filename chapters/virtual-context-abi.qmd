...
open_evidence_gaps:
  ...
  - "Source notes, schemas, fixtures, a synthetic context admission/adequacy harness, and finite Lean predicates support static context record discipline only; they do not prove resolver correctness, context compiler quality, semantic summary fidelity, certificate truthfulness, deletion closure, transaction isolation, verification bandwidth, model-facing context performance, or deployed memory-store behavior."
...
| Test state | `context_abi_record.valid.json`, `semantic_page_certificate.valid.json`, and related context packet/adequacy/transaction fixtures pass protocol validation; `python3 scripts/validate_context_admission_adequacy.py` passes synthetic admission/adequacy, certificate-use, stale-certificate, deletion-closure, and mode-confusion checks; `AsiStackProofs.VirtualContextABI` and `AsiStackProofs.ContextCertificates` implement finite record predicates including a context-admission route envelope; resolver, context compiler, summary-fidelity, certificate-truthfulness, deletion-closure, transaction-isolation, and model-facing memory tests remain planned. |
...
The next practical slice is a resolver fixture set: one valid request that
resolves to a leased materialization, one mandatory miss that emits a typed
fault, one resolved packet rejected as inadequate for verification, one packet
denied by mount policy, and one lease-expired reuse attempt blocked. It would
sharpen the ABI semantics without pretending to be a full memory product.
...
review states. They do not prove open-domain source selection, semantic
fidelity, certificate truthfulness, deployed resolver behavior, transaction
isolation, deletion enforcement, or model use.
