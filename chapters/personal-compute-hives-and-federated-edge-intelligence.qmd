...
open_evidence_gaps:
  ...
  - "Only record-shape schemas, valid fixtures, and narrow finite Lean predicates have been implemented; no device registry, scheduler, federation protocol, family-governance policy engine, or rented-node sandbox has been run."
...
| Primary source records | `field_of_god_ai_constitution`, `scf`, `talos`, `vcm_public`, `planforge`, `octopus_router`, `rmi`, `tokenmana`, `project_theseus_whitepaper`, `theseus_operator_os`, `ladon_manhattan` |
| Claim label | Design rationale |
| Evidence level | argument |
| Source loading state | All assigned sources have public source notes, but the core claim is still a design synthesis. The browser note is local-only planning context. |
| Test state | Record-shape fixtures for `DeviceResourceCard`, `HiveJobContract`, and `HiveSchedulingDecision` validate locally, and two finite Lean predicates build. No hive scheduler, live registry, federation, or rented-node test has been run. |

## Drafting guardrail

This chapter uses source-note material for VCM, Talos, Octopus Router, RMI, TokenMana, Project Theseus, the Hive Operator OS, SCF, Field of God governance, PlanForge, and Ladon/Manhattan. It does not claim that a complete personal compute hive exists. It also does not treat commercial mesh networks, orchestration systems, volunteer-compute systems, rented-compute systems, or file-sync tools as cited evidence until they have source records and source notes.
...
The core claim has not been promoted above `argument`. The assigned source notes support pieces of the design: VCM for governed context and taint, Talos for typed jobs and audit, Octopus/RMI for routed specialists and lifecycle state, TokenMana for resource budgets, Project Theseus and Hive Operator OS for trusted-machine node registries and operator surfaces, SCF for capability boundaries, Field of God governance for consent and least sufficient power, PlanForge for scheduling, and Ladon/Manhattan for handle-based secret use. The repository now includes record-shape schemas, valid fixtures, and narrow Lean predicates for the first hive records, but the combined hive architecture remains this book's synthesis until behavioral prototypes, scheduler tests, or external literature mappings are added.

## Mechanism

A Personal Compute Hive is built around three records: a `Device Resource Card`, a `Hive Job Contract`, and a `Hive Scheduling Decision`.
...
This repository now contains the record-level first step for that experiment: `device_resource_card.schema.json`, `hive_job_contract.schema.json`, and `hive_scheduling_decision.schema.json`, each with a matching valid fixture. These fixtures prove only that the public record shapes can be validated by the local protocol validator.
...
The browser note proposed several adjacent systems to examine later: mesh VPNs, container orchestrators, small-cluster schedulers, distributed compute frameworks, volunteer compute, rented compute markets, file-sync networks, and content-addressed storage. These are useful research leads, but they are not cited evidence in this chapter. Before they support claims, each needs a source record, source note, and explicit mapping to a hive claim.
