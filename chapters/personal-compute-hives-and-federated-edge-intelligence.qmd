...
open_evidence_gaps:
  - "Appendix C now records exact passage-reviewed mappings for all assigned Personal Compute Hives sources; support remains at argument because the new hive admission harness is synthetic record validation only and no live device registry, policy-first scheduler, portal approval service, federation protocol, family-governance policy engine, rented-node sandbox, or deployed behavioral tests exist."
  - "Record-shape schemas, valid fixtures, narrow finite Lean predicates, the synthetic hive admission harness, and the partitioned-authority fixture have been implemented; no personal hive scheduler, network overlay, device registry, approval portal, rented-node sandbox, federation run, partition test, energy measurement, dropout recovery run, privacy guarantee, or security result has been executed from this repository."
  - "External tooling sources are reviewed as adjacent substrate references only; they do not prove the ASI Stack hive design or any safety property."
---
...
| Primary source records | `field_of_god_ai_constitution`, `scf`, `talos`, `vcm_public`, `planforge`, `octopus_router`, `rmi`, `tokenmana`, `project_theseus_whitepaper`, `theseus_operator_os`, `ladon_manhattan`, plus external source notes for Tailscale, Kubernetes, K3s, Nomad, Ray Core, BOINC, Syncthing, IPFS, Akash, Golem, GitHub self-hosted runners, and CAP-style partition trade-offs via `ext_cap_theorem_gilbert_lynch_2002` |
| Claim label | Design rationale |
| Evidence level | argument |
| Source loading state | All assigned sources have public source notes. Internal/local raw passages and tracked local-project passages have now been passage-reviewed for the chapter; official external tooling pages were reviewed as adjacent substrate references. The core claim remains a design synthesis. The browser note is local-only planning context. |
| Test state | Record-shape fixtures for `DeviceResourceCard`, `HiveJobContract`, `HiveSchedulingDecision`, `PortalCard`, `HiveApprovalReceipt`, `HiveJobBid`, and `HiveFederationLease` validate locally; `AsiStackProofs.PersonalComputeHives` builds finite predicates plus negative-case theorems rejecting high-risk execution without a bound approval receipt, external access with a missing lease boundary, and partitioned stale-authority dispatch without no-mutation evidence. `python3 scripts/validate_hive_admission.py` passes on 2 valid and 8 expected-invalid synthetic hive admission fixtures. `python3 scripts/validate_partitioned_authority_fixture.py` writes `experiments/partitioned_authority/results/2026-07-03-local.json` and checks 3 valid finite partitioned-authority records plus 6 expected-invalid controls for stale grants, revocation-delay quarantine, fresh authority receipt requirements, grant/effect race residual ownership, no-mutation evidence, CAP-style authority consistency boundaries, and support-state non-promotion. No hive scheduler, live registry, portal approval service, network overlay, federation run, deployed partition tolerance, distributed consensus, runtime adapter enforcement, revocation propagation, rented-node sandbox, cross-router connectivity test, portal-continuity test, energy measurement, dropout recovery run, privacy guarantee, or security result has been run. |
...
This repository now contains the record-level first step for that experiment: `device_resource_card.schema.json`, `portal_card.schema.json`, `hive_job_contract.schema.json`, `hive_job_bid.schema.json`, `hive_scheduling_decision.schema.json`, `hive_approval_receipt.schema.json`, and `hive_federation_lease.schema.json`, each with a matching valid fixture. These fixtures establish only that the public record shapes can be validated by the local protocol validator.

It also contains the bounded partitioned-authority fixture. That fixture is the minimum viable implementation of governance under partition: a finite record check that stale grants, revocation-delay cases, and grant/effect races cannot quietly become dispatch authority unless fresh receipt, residual ownership, audit refs, no-mutation evidence, support-state non-promotion, and non-claim boundaries are present.

## Beyond the State of the Art

Personal compute hives become valuable as an owned compute fabric for personal, family, project, and rented capacity. It lets the ASI Stack place work near the right data and device without turning reachability into authority, convenience into surveillance, or cheap compute into a privacy leak.

Owned and rented devices form a policy-bounded compute fabric only when scheduling respects capability, locality, trust, cost, energy, data class, physical risk, and authority. Persons, devices, portals, workers, stores, datasets, tools, jobs, and external hives need signed identity and policy cards. Jobs are classified by data sensitivity, tool authority, physical-world risk, federation scope, cost, energy, and verification requirement before scheduling; eligible devices bid only after policy filtering; the scheduler chooses the best lawful node rather than the fastest reachable node.

Phones, laptops, tablets, dashboards, and headsets act as portals. Desktops, NAS boxes, old machines, workshop computers, and rented nodes act as bounded workers or stores. Device roles, family-sensitive portals, public project jobs, and hive memory classes stay separate so owned substrate does not collapse into surveillance or botnet behavior. Portal cards, approval receipts, job bids, federation leases, sandbox manifests, artifact manifests, evidence bundles, payment or credit records, reputation records, and revocations form the Hive-to-Hive protocol surface.

VCM declares what context is needed and what data may move. Talos and PlanForge lower tasks into typed jobs with resource, authority, and verification requirements. Octopus/RMI routing and TokenMana-style resource records score eligible devices and rented nodes. Field-of-God and SCF governance define family, child, privacy, physical-tool, and federation membranes. Project hives and artifact stewards interact through explicit job contracts, sandbox manifests, evidence bundles, and revocation paths.

Reachability is not authority, policy filtering precedes optimization, external work receives sandboxed job slots rather than private network access, family and child-facing portals preserve dignity and review boundaries, secrets remain handles and approvals, and every temporary federation link has an expiration, revocation path, and audit record. Botnet-like execution, family-surveillance drift, private-data leakage to rented or public compute, battery or thermal abuse, authority confusion, and ungoverned artifact swamps should trigger device quarantine, approval reset, data revocation, escalation, or residual records instead of becoming normal capacity management.

The mature hive also needs partition-aware authority service behavior. Authority state, revocations, approvals, leases, and effect receipts would have freshness windows, partition indicators, denial-first high-impact defaults, reconciliation receipts, and human-visible residuals. The logical conclusion is not "always available action"; it is a hive that can keep working on safe, stale-tolerant tasks while refusing to let stale authority mutate protected state.

This personal compute hive is still a proposed architecture. Hive support remains at `argument` until node inventories, policy-first scheduler traces, privacy and offline-mode tests, energy/budget records, contribution ledgers, and portal-continuity checks show that owned edge compute can join the stack without becoming an unmanaged compute swarm.
...
These targets are implemented as finite record predicates. The current Lean envelope now includes negative-case theorems for missing bound approval on high-risk execution, missing lease-boundary fields on external hive access, lifecycle-route theorems for modeled hive-work admission gaps, and a partitioned-authority fixture bridge for stale-grant quarantine and fresh-receipt routing. It still does not prove a distributed scheduler, network boundary, rented-node sandbox, family-governance policy, privacy guarantee, approval-service behavior, partition tolerance, energy-aware scheduling, dropout recovery, audit replay, revocation propagation, or live federation behavior.
