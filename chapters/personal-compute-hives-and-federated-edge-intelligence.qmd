...
| Test state | Record-shape fixtures for `DeviceResourceCard`, `HiveJobContract`, `HiveSchedulingDecision`, `PortalCard`, `HiveApprovalReceipt`, `HiveJobBid`, and `HiveFederationLease` validate locally; `AsiStackProofs.PersonalComputeHives` builds four finite predicates plus negative-case theorems rejecting high-risk execution without a bound approval receipt and external access with a missing lease boundary. No hive scheduler, live registry, portal approval service, federation, rented-node test, cross-router connectivity test, child-routing test, dropout recovery test, or energy-aware scheduling test has been run. |
...
These targets are implemented as finite record predicates. The current Lean envelope now includes negative-case theorems for missing bound approval on high-risk execution, missing lease-boundary fields on external hive access, and lifecycle-route theorems for modeled hive-work admission gaps. It still does not prove a distributed scheduler, network boundary, rented-node sandbox, family-governance policy, privacy guarantee, approval-service behavior, energy-aware scheduling, dropout recovery, audit replay, or live federation behavior.
