...
open_evidence_gaps:
  - "Appendix C now records exact passage-reviewed mappings for all assigned Personal Compute Hives sources; support remains at argument until a live device registry, policy-first scheduler, portal approval service, federation protocol, family-governance policy engine, rented-node sandbox, or behavioral tests exist."
  - "Only record-shape schemas, valid fixtures, and narrow finite Lean predicates have been implemented; no personal hive scheduler, network overlay, device registry, approval portal, rented-node sandbox, or federation run has been executed from this repository."
...
After MoECOT gives the book a runtime evidence shape, this chapter asks where that runtime can live. The answer is not "any reachable device." It is a policy-bounded hive where portals, workers, stores, rented nodes, and project machines participate only through explicit identity, data, tool, network, approval, and revocation records.

::: {.asi-human-only}
## Human Reading Path

The runtime chapter asks what an orchestrator should emit. This chapter asks where such orchestration can safely live. A personal compute hive is not every device the system can reach; it is a governed federation of portals, workers, storage, rented nodes, family surfaces, and project machines.

For the reader, the practical question is consent and containment. Edge intelligence becomes useful only when work placement respects data locality, owner policy, family roles, cost, privacy, rollback, approval, and authority boundaries instead of treating personal hardware as free ambient compute.
:::
...
The mature version of Personal Compute Hives and Federated Edge Intelligence is an owned compute fabric for personal, family, project, and rented capacity. It lets the ASI Stack place work near the right data and device without turning reachability into authority, convenience into surveillance, or cheap compute into a privacy leak.

The operational contract is that owned and rented devices form a policy-bounded compute fabric only when scheduling respects capability, locality, trust, cost, energy, data class, physical risk, and authority. The personal-hive control surface makes that fabric inspectable through:

- Operating mechanism: Represent persons, devices, portals, workers, stores, datasets, tools, jobs, and external hives with signed identity and policy cards; Classify jobs by data sensitivity, tool authority, physical-world risk, federation scope, cost, energy, and verification requirement before scheduling; Let eligible devices bid on work after policy filtering, then choose the best lawful node rather than the fastest reachable node; Treat phones, laptops, tablets, dashboards, and headsets as portals; desktops, NAS boxes, old machines, workshop computers, and rented nodes as bounded workers or stores; Represent device roles, family-sensitive portals, public project jobs, and hive memory classes separately so owned substrate does not collapse into surveillance or botnet behavior; Use portal cards, approval receipts, job bids, federation leases, sandbox manifests, artifact manifests, evidence bundles, payment or credit records, reputation records, and revocations as the Hive-to-Hive protocol surface; Preserve audit records, data-placement decisions, approval events, and residuals for every cross-device or cross-hive action.
- Stack interface: VCM declares what context is needed and what data may move; Talos and PlanForge lower tasks into typed jobs with resource, authority, and verification requirements; Octopus/RMI routing and TokenMana-style resource records score eligible devices and rented nodes; Field-of-God and SCF governance define family, child, privacy, physical-tool, and federation membranes; Project hives and artifact stewards interact through explicit job contracts, sandbox manifests, evidence bundles, and revocation paths.
- Preserved invariants: Reachability is not authority; Policy filtering precedes optimization; External work receives sandboxed job slots, not private network access; Family and child-facing portals preserve dignity, consent boundaries, and parent/guardian review for sensitive transitions; Secrets remain handles and approvals, not strings in model context; Every temporary federation link has an expiration, revocation path, and audit record.
- Failure closure: detect and route failure modes such as Personal hive becomes a botnet or malware execution surface; Family governance becomes surveillance; Rented or public project compute leaks private data; Wrong-device scheduling drains batteries, overheats machines, exposes data, or bypasses approvals; Identity confusion grants child, guest, agent, or project authority to the wrong principal; Hive memory becomes an ungoverned swamp of artifacts, logs, models, and personal data to quarantine, rollback, escalation, or residual records instead of hidden degradation.

This personal compute hive remains a target architecture, not a current-result claim. Support should stay at `argument` until node inventories, policy-first scheduler traces, privacy and offline-mode tests, energy/budget records, contribution ledgers, and portal-continuity checks show that owned edge compute can join the stack without becoming an unmanaged compute swarm.
