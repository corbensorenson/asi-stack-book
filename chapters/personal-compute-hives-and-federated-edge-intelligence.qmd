...
**Diagram reading note:** The hive scheduler filters devices by policy before optimization begins. Phones, storage, local GPUs, rented nodes, portals, and workers are not interchangeable; each selected execution slot needs a scoped lease, approval when required, and evidence back into memory.
...
Owned and rented devices form a policy-bounded compute fabric only when scheduling respects capability, locality, trust, cost, energy, data class, physical risk, and authority. The personal-hive control surface makes that fabric inspectable through:
...
- Hive closure: botnet-like execution, family-surveillance drift, private-data leakage to rented or public compute, battery or thermal abuse, authority confusion, and ungoverned artifact swamps should trigger device quarantine, approval reset, data revocation, escalation, or residual records instead of becoming normal capacity management.

This personal compute hive remains a target architecture, not a current-result claim. Keep the hive architecture at `argument` until node inventories, policy-first scheduler traces, privacy and offline-mode tests, energy/budget records, contribution ledgers, and portal-continuity checks show that owned edge compute can join the stack without becoming an unmanaged compute swarm.
