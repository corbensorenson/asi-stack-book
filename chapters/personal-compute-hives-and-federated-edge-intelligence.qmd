...
open_evidence_gaps:
  ...
  - "Record-shape schemas, valid fixtures, narrow finite Lean predicates, and the synthetic hive admission harness have been implemented; no personal hive scheduler, network overlay, device registry, approval portal, rented-node sandbox, federation run, energy measurement, dropout recovery run, privacy guarantee, or security result has been executed from this repository."
...
| Test state | Record-shape fixtures for `DeviceResourceCard`, `HiveJobContract`, `HiveSchedulingDecision`, `PortalCard`, `HiveApprovalReceipt`, `HiveJobBid`, and `HiveFederationLease` validate locally; `AsiStackProofs.PersonalComputeHives` builds four finite predicates plus negative-case theorems rejecting high-risk execution without a bound approval receipt and external access with a missing lease boundary. `python3 scripts/validate_hive_admission.py` passes on 2 valid and 8 expected-invalid synthetic hive admission fixtures for policy-first scheduling, data locality/rented-node denial, approval receipts, guardian portal routing, federation lease boundaries, job bidding, energy/dropout residuals, audit replay refs, and support-state non-promotion. No hive scheduler, live registry, portal approval service, network overlay, federation run, rented-node sandbox, cross-router connectivity test, portal-continuity test, energy measurement, dropout recovery run, privacy guarantee, or security result has been run. |
