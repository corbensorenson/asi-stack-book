...
open_evidence_gaps:
  ...
  - "The compressed_artifact_record fixture validates schema/example consistency for task family, access pattern, admission state, declared use envelope, metadata costs, fallback trigger, exact-replay status, consumer policy, support-state effect, evidence refs, and non-claims only; `AsiStackProofs.ArtifactCompression` now includes a finite admission lifecycle route envelope over compressed-artifact use boundaries; no RankFold/NeuralFold codec, deterministic decoder, corpus benchmark, or utility probe has been run."
  - "The RankFold artifact import records three existing local `.rfa` archive observations for the same 100,000,000-byte decoded artifact digest, with `rfa verify` reporting 1 OK, 0 failed for each archive and `rfa inspect` reporting `NEURAL0`; this is metadata/hash/import evidence only, not a fresh benchmark or codec-correctness proof."
  - "The RankFold public-safe replay probe records a fresh local RAW0 pack/verify/list/unpack roundtrip over one generated synthetic text fixture plus a rejected single-byte archive mutation; this proves only that the local CLI replayed that tiny fixture exactly under a license-disabled NeuralFold boundary, not NeuralFold compression, compression advantage, codec correctness, downstream utility, fallback execution, or deployed compression behavior."
  - "RankFold/NeuralFold and RankFold Compressor source-reported ratio, losslessness, latency, benchmark, code, and reproducibility claims are not promoted here; the local import is treated as a narrow public-safe artifact observation and the broader claims remain source-reported architecture/lineage context."
...
| Test state | `compressed_artifact_record.valid.json` passes repository-level protocol fixture validation for task family, access pattern, admission state, declared use envelope, metadata costs, fallback trigger, exact-replay status, consumer policy, support-state effect, evidence refs, and non-claims; `AsiStackProofs.ArtifactCompression` implements finite probe/fallback, residual/fallback metadata, and admission-lifecycle route predicates plus negative-case theorems blocking failed-probe/no-fallback use, promotion with missing residual or fallback metadata, and modeled admission gaps; `python3 scripts/validate_rankfold_public_safe_probe.py` validates a fresh RAW0 pack/verify/list/unpack replay over a generated public-safe synthetic text fixture, roundtrip-exact digest preservation, no compression advantage, license-disabled NeuralFold boundary, and a rejected single-byte archive mutation; `python3 scripts/validate_rankfold_artifact_import.py` validates the public-safe RankFold artifact import for a 100,000,000-byte decoded artifact digest, archive ratios up to 2.76634019 decoded/archive, `rfa verify` summaries of 1 OK, 0 failed, and `NEURAL0` inspect metadata; NeuralFold compression reproduction, behavioral probe-route fallback, corpus benchmark, and downstream-utility tests remain planned. |
...
| Artifact compression admission lifecycle route | Check that a modeled compressed-artifact admission review routes missing preserved full artifacts, missing manifests, use-envelope and access-pattern gaps, unadmitted states, decoder-determinism gaps, exact-replay readiness failures, failed probes, missing fallback artifacts, residual metadata gaps, utility-evidence gaps, support-promotion gaps, and missing non-claim boundaries to explicit finite outcomes. | implemented in `AsiStackProofs.ArtifactCompression`; no compression ratio, decoder correctness, downstream utility, behavioral fallback, benchmark, support-state-promotion, or codec-performance claim |
...
These Lean hooks are implemented as finite-record predicates over artifact-use, metadata, and admission-review fields. The module now includes negative-case theorems showing that a compressed artifact use is invalid when the probe failed and no fallback is routed, that a promotion candidate is invalid when residual or fallback metadata is missing, and that modeled admission reviews route missing full-artifact preservation, manifest, use-envelope, access-pattern, admission-state, decode-determinism, exact-replay readiness, probe, fallback, residual, utility, evidence-transition, and non-claim-boundary gaps to explicit outcomes. They prove only those record-level gates; they do not prove a compression ratio, decoder correctness, task utility, behavioral fallback execution, codec performance, support-state promotion, or benchmark result.
