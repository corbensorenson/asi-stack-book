...
| Test state | `compressed_artifact_record.valid.json` passes repository-level protocol fixture validation for task family, access pattern, admission state, declared use envelope, metadata costs, fallback trigger, exact-replay status, consumer policy, support-state effect, evidence refs, and non-claims; `AsiStackProofs.ArtifactCompression` implements finite probe/fallback and residual/fallback metadata predicates plus negative-case theorems blocking failed-probe/no-fallback use and promotion with missing residual or fallback metadata; compression-ratio, behavioral probe-route fallback, and downstream-utility tests remain planned. |
...
These Lean hooks are implemented as finite-record predicates over artifact-use and metadata fields. The module now includes negative-case theorems showing that a compressed artifact use is invalid when the probe failed and no fallback is routed, and that a promotion candidate is invalid when residual or fallback metadata is missing. They prove only those record-level gates; they do not prove a compression ratio, decoder correctness, task utility, behavioral fallback execution, or benchmark result.
