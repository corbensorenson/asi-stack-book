...
| Test state | `compressed_artifact_record.valid.json` passes repository-level protocol fixture validation; compression-ratio, probe-route fallback, and downstream-utility tests remain planned. |
...
These Lean hooks remain planned. The current schema can require residual and fallback metadata, but it does not mechanize the route decision or prove that a compressed artifact is useful for a task. A first proof should model the finite probe-or-fallback predicate, not a compression-performance claim.
