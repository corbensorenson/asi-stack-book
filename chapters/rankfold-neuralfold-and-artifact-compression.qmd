...
| Test state | `compressed_artifact_record.valid.json` passes repository-level protocol fixture validation for task family, access pattern, admission state, declared use envelope, metadata costs, fallback trigger, exact-replay status, consumer policy, support-state effect, evidence refs, and non-claims; `AsiStackProofs.ArtifactCompression` implements finite probe/fallback, residual/fallback metadata, and admission-lifecycle route predicates plus negative-case theorems blocking failed-probe/no-fallback use, promotion with missing residual or fallback metadata, and modeled admission gaps; `python3 scripts/validate_rankfold_artifact_import.py` validates the public-safe RankFold artifact import for a 100,000,000-byte decoded artifact digest, archive ratios up to 2.76634019 decoded/archive, `rfa verify` summaries of 1 OK, 0 failed, and `NEURAL0` inspect metadata; fresh compression-ratio reproduction, behavioral probe-route fallback, and downstream-utility tests remain planned. |
