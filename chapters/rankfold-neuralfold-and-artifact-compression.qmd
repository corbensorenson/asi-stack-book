...
The falsification program compares naïve magnitude rules under
function-preserving reparameterization; parameter distance against behavioral
distance; payload-only against complete accounting; static, layer-wise, mixed,
progressive, and dynamically routed precision; and average parity against
protected-slice failures. A strong static policy wins if it preserves the same
contract at lower total cost. Dynamic precision loses if routing overhead,
instability, attack surface, or fallback burden exceeds the saved work.
