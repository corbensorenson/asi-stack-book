...
| Test state | Runtime adapter fixture validation now includes invocation state, impact class, risk tier, approval scope/expiry, effect lease, pre/post state refs, external side effects, verification refs, irreversible residuals, incident/audit refs, support-state effect, and non-claims; finite permission and approval predicates plus negative-case theorems are implemented in `AsiStackProofs.RuntimeAdapters` for missing parent-job permission and high-impact unapproved adapter calls that try to remain unrejected; `python3 scripts/validate_runtime_adapter_permissions.py` checks synthetic typed-job, adapter-invocation, and authority-use-receipt consistency for permissions, approval gates, expiry markers, receipts, rollback handles, irreversible residuals, and authority receipt alignment. Deployed adapter execution, sandbox isolation, approval-service behavior, rollback execution, and secret-handle safety remain untested. |

## Drafting guardrail

This chapter treats runtime adapters as effect-boundary records, not as proof that tools were safely executed. The repo validates an adapter-invocation fixture and now proves two finite predicates plus two negative-case theorems over permission and approval records. Seven assigned source mappings have passage-reviewed refs, while `moecot` remains connector/source-note mapped. The fixture does not call a tool, inject a secret, enforce a live approval service, or produce a real rollback handle.
...
The runtime-adapter fixture checks the invocation envelope. `AsiStackProofs.RuntimeAdapters` adds a bounded permission predicate showing that a valid invocation requires the parent job permission list to include the adapter capability, plus a bounded approval predicate requiring a high-impact unapproved invocation to be rejected. It also adds negative-case theorems showing that missing parent-job permission invalidates the modeled invocation and that a high-impact unapproved adapter call cannot remain unrejected while satisfying the approval-rejection predicate. The synthetic runtime-adapter permission harness now adds cross-record checks over typed jobs, adapter invocations, and authority-use receipts: valid fixtures cover low-impact local write and high-impact approved preview-deployment records, while expected-invalid fixtures reject missing permissions, high-impact approval bypass, expired approval, missing effect receipts, and irreversible effects without rollback or residual records. This is still fixture behavior, not tool execution, sandbox isolation, human approval workflow, secret-handle safety, effect receipt validity in a live adapter, or rollback execution.
...
Fixture-shape validation, four bounded Lean theorem declarations over two finite predicates, and the synthetic runtime-adapter permission harness are implemented. The remaining tests require a deployed adapter harness, sandbox, parent job permission lookup against real jobs, high-impact classification against policy, live approval decisions, live effect receipts, rollback execution, and secret-handle safety checks.
...
These proof hooks are implemented as finite-record predicates plus negative-case theorems over modeled records. They do not prove sandboxing, authority-handle mediation, actual tool invocation, effect receipt validity, approval-service quality, secret-handle safety, or rollback behavior.
