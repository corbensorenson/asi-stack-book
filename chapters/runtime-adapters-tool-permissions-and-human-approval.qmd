...
Artifact graphs record work, but runtime adapters create effects. This chapter is the boundary where a typed job may touch tools, files, networks, deployments, payments, messages, credentials, hardware, or other external systems under explicit permission and human approval when needed.

The reader should treat this as the stack's narrow waist for action. A plan can be brilliant and a job can be well typed, but external effect still needs an authority envelope, approval scope, effect lease, pre/post state, incident path, and residual record.
:::
...
The adapter boundary is where the stack stops being interpretive. A claim can be wrong without changing the world. A plan can be flawed without deleting a file. A route can be inefficient without spending money. Once an adapter runs, the system may have produced an external fact. This chapter exists to make that transition narrow, auditable, and reversible where possible.
...
For this chapter, beyond the state of the art means that action becomes lease-bound, inspectable, and revocable where possible. The stack can use tools without giving cognition ambient power over the world. This remains a target architecture. The support state should not rise until adapter harnesses, sandbox checks, approval workflows, secret-handle tests, rollback records, and effect-receipt validation exist for this end state.

## Codex test plan

| Test | Purpose | Status |
|---|---|---|
| Runtime adapter fixture validation | Check that the adapter fixture matches the public schema and declares job, adapter, target, capability, permission, sandbox, invocation state, impact class, risk tier, approval scope/expiry, authority handle, inputs, effect lease, receipt refs, rollback handle, irreversible residuals, incident/audit refs, support-state effect, and non-claims. | implemented by protocol validation; run in this pass |
...
The chapter's rule is that real-world effects require explicit authority. Planning may propose action; only an authorized adapter may perform it. This pass tightened the evidence crosswalk but did not run an adapter. Once real effect traces accumulate, the next chapter asks which recurring patterns deserve to become procedural memory.
