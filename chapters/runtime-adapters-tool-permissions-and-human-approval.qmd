...
The rule is that real-world effects require explicit authority. Planning may propose action; only an authorized adapter may perform it. The evidence map is narrower now, but the book still has not run an adapter. Once real effect traces accumulate, the system can ask which recurring patterns deserve to become procedural memory.
