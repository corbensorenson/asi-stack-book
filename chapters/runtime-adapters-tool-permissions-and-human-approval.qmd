...
| Test state | Runtime adapter fixture validation now includes invocation state, impact class, risk tier, approval scope/expiry, effect lease, pre/post state refs, external side effects, verification refs, irreversible residuals, incident/audit refs, support-state effect, and non-claims; finite permission, approval, effect-lease, and rollback-obligation predicates plus negative-case theorems are implemented in `AsiStackProofs.RuntimeAdapters` for missing parent-job permission, high-impact unapproved adapter calls that try to remain unrejected, mismatched effect leases, expired leases, unsandboxed leases, and high-impact rollback-required calls without rollback handles; `python3 scripts/validate_runtime_adapter_permissions.py` checks synthetic typed-job, adapter-invocation, and authority-use-receipt consistency for permissions, approval gates, expiry markers, receipts, rollback handles, irreversible residuals, and authority receipt alignment. Deployed adapter execution, sandbox isolation, approval-service behavior, rollback execution, and secret-handle safety remain untested. |

## Drafting guardrail

This chapter treats runtime adapters as effect-boundary records, not as proof that tools were safely executed. The repo validates an adapter-invocation fixture and now proves finite permission, approval, effect-lease, and rollback-obligation predicates with negative cases for missing parent permission, high-impact approval bypass, mismatched leases, expired leases, unsandboxed leases, and missing rollback handles. Seven assigned source mappings have passage-reviewed refs, while `moecot` remains connector/source-note mapped. The fixture does not call a tool, inject a secret, enforce a live approval service, enforce a sandbox, or produce a real rollback handle.
...
Fixture-shape validation, ten bounded Lean theorem declarations over finite permission, approval, effect-lease, and rollback-obligation predicates, and the synthetic runtime-adapter permission harness are implemented. The remaining tests require a deployed adapter harness, real sandbox isolation, parent job permission lookup against real jobs, high-impact classification against policy, live approval decisions, live effect receipts, rollback execution, and secret-handle safety checks.
...
These proof hooks are implemented as finite-record predicates plus negative-case theorems over modeled records. They prove only record-level implications: valid leased invocations require parent permission plus an active scoped sandboxed lease; mismatched, expired, or unsandboxed leases do not satisfy the modeled validity predicate; high-impact unapproved calls are rejected; and high-impact rollback-required calls without rollback handles cannot satisfy the rollback-obligation predicate while remaining unrejected. They do not prove deployed sandboxing, authority-handle mediation, actual tool invocation, effect receipt validity, approval-service quality, secret-handle safety, or rollback behavior.
