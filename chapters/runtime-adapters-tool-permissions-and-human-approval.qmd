...
| Test state | Runtime adapter fixture validation now includes invocation state, impact class, risk tier, approval scope/expiry, effect lease, pre/post state refs, external side effects, verification refs, irreversible residuals, incident/audit refs, support-state effect, and non-claims; finite permission and approval predicates are implemented in `AsiStackProofs.RuntimeAdapters`; `python3 scripts/validate_runtime_adapter_permissions.py` checks synthetic typed-job, adapter-invocation, and authority-use-receipt consistency for permissions, approval gates, expiry markers, receipts, rollback handles, irreversible residuals, and authority receipt alignment. Deployed adapter execution, sandbox isolation, approval-service behavior, rollback execution, and secret-handle safety remain untested. |
...
Fixture-shape validation, two bounded Lean predicates, and the synthetic runtime-adapter permission harness are implemented. The remaining tests require a deployed adapter harness, sandbox, parent job permission lookup against real jobs, high-impact classification against policy, live approval decisions, live effect receipts, rollback execution, and secret-handle safety checks.
