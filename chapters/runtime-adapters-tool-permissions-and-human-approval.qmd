...
Artifact graphs record work, but runtime adapters create effects. This chapter is the boundary where a typed job may touch tools, files, networks, deployments, payments, messages, or other external systems under explicit permission and human approval when needed.

The reader should treat this as the stack's narrow waist for action. A plan can be brilliant and a job can be well typed, but external effect still needs an authority envelope, approval scope, pre/post state, incident path, and residual record.
:::
...
The mature version of Runtime Adapters, Tool Permissions, and Human Approval is not the minimal artifact with more scale. It is the chapter's idea turned into a product-grade, evidence-bearing boundary that the rest of the ASI Stack can depend on under ordinary operation and upgrade pressure.

At that end state, the chapter's claim becomes an operational contract: Runtime adapters should enforce typed permissions, sandboxing, human approval, and post-action evidence capture. The mature surface would include:

- Operating mechanism: Map jobs to runtime adapters with declared capabilities; Cover document/software, API/service, firmware/hardware, CAD/fabrication, robotics, and organizational workflow targets where source support exists; Require permission checks and approval gates for high-impact actions; Record deployment evidence and rollback handles.
- Stack interface: Execution owns adapters; Security mediates secrets; Evidence records deployment outcomes.
- Preserved invariants: No adapter executes beyond granted scope; High-impact actions require approval; Rollback handles are captured when available.
- Failure closure: detect and route failure modes such as Tool overreach; Approval bypass; Irreversible deployment without rollback to quarantine, rollback, escalation, or residual records instead of hidden degradation.

For Runtime Adapters, Tool Permissions, and Human Approval, this is a target architecture, not a current-result claim. Its support state should not rise until the source mappings, schemas, Lean predicates, tests, benchmark records, runtime traces, review receipts, or governance artifacts needed for this specific end state exist and validate the claim.
