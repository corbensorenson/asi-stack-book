...
Artifact graphs record work, but runtime adapters create effects. The adapter is the boundary where a typed job may touch tools, files, networks, deployments, payments, messages, credentials, hardware, or other external systems under explicit permission and human approval when needed.

This is the stack's narrow waist for action. A plan can be brilliant and a job can be well typed, but external effect still needs an authority envelope, approval scope, effect lease, pre/post state, incident path, and residual record. The practical promise is not frictionless action; it is action whose authority, evidence, and rollback surface are visible before harm compounds.

The adapter should make powerful action slower when the missing piece is permission, not intelligence.

That deliberate friction is what keeps execution coupled to accountable authority. A slow approval path is sometimes the feature that prevents a fast mistake.

The adapter is trustworthy when denial is as normal as dispatch. Human approval matters only when the adapter can wait. Waiting preserves accountable external power.
:::
