...
open_evidence_gaps:
  ...
  - "The layer_boundary_record fixture and stack-boundary Lean proofs validate narrow record/proof surfaces only; the fixture now records lifecycle state, owner, handoff protocol, contract refs, change policy, source refs, support-state effect, and non-claims. Source-to-layer traceability and claim-support audits remain planned."
...
| Test state | `layer_boundary_record.valid.json` passes protocol fixture validation for lifecycle state, owner, handoff protocol, contract refs, change policy, source refs, support-state effect, and non-claims; stack-boundary Lean hooks build locally; source-to-layer traceability and claim-support audits remain planned. |
...
A layer contract is the mechanism that makes that rule concrete. The artifact is a Layer Boundary Record, and a chapter is mature only when it can answer the same recurring questions: what lifecycle state is this layer in, who owns the boundary, what responsibility does the layer own, what artifact does it emit, how does handoff work, what authority does it have and lack, what invariant must survive implementation changes, and what evidence would justify promotion, replacement, or rollback.
...
The smallest useful implementation is a stack map, a layer boundary schema, one valid fixture, the generated source crosswalk, and Appendix C. This does not prove the architecture. It gives the rest of the book a typed surface for saying which layer owns which lifecycle state, owner, responsibility, artifact, handoff protocol, contract refs, authority ceiling, invariant, source refs, support-state effect, and non-claim.

## Codex test plan

| Test | Purpose | Status |
|---|---|---|
| Layer-boundary record validation | Check that the layer boundary fixture records lifecycle state, owner, responsibility, input/output artifacts, authority ceiling, handoff protocol, contract refs, change policy, invariants, failure modes, evidence gates, downstream interfaces, source refs, support-state effect, and non-claims. | implemented by protocol validation; run in this pass |
