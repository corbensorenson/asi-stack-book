...
**Diagram reading note:** Read this stack map as a responsibility loop, not a capability ladder. Self-improvement is not outside the stack; it returns to governance, where new capabilities must be qualified, bounded, and made reversible before they become defaults.
...
A useful first build is a stack map, a layer boundary schema, one valid fixture, the generated source crosswalk, and Appendix C. That bundle is not evidence that the architecture runs; it gives the rest of the book a typed surface for saying which layer owns which lifecycle state, owner, chapter refs, traceability state, responsibility, artifact, handoff protocol, contract refs, authority ceiling, invariant, integration decision, promotion blockers, source refs, support-state effect, and non-claim.
...
The target doctrine is deliberately simple: efficient ASI is a governed stack of cooperating layers, not one undifferentiated model. In a mature stack doctrine, that discipline is visible in:
...
- Architectural closure: anthology drift, monolith drift, and evidence inflation should produce boundary records, source-note blockers, residuals, or rollback decisions rather than silent degradation.
...
With the stack boundary established, The Efficient ASI Hypothesis asks how capability should grow without turning scale into the only design variable. The next move is from architecture identity to architecture economics: if intelligence is a stack of governed layers, then efficiency means routing, reuse, compression, verification, and residual accounting at the correct layer rather than spending maximum cognition everywhere.
