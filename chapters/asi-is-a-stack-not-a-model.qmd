...
open_evidence_gaps:
  ...
  - "The layer_boundary_record fixture and stack-boundary Lean proofs validate narrow record/proof surfaces only; the fixture now records lifecycle state, owner, chapter refs, traceability state, handoff protocol, contract refs, change policy, integration decision, promotion blockers, source refs, support-state effect, and non-claims, and `AsiStackProofs.StackBoundaries` now includes a finite trace-level unauthorized external-handoff rejection theorem. Source-to-layer traceability and claim-support audits remain planned."
...
These two stack-boundary hooks are implemented in the local Lean workspace. The module now also includes a finite stack-trace theorem that blocks a valid trace from containing an external-action step when the layer lacks external authority and the handoff is unauthorized. These predicates support only the handoff, authority-ceiling, and trace-step boundaries named here; they do not prove whole-system safety, capability, source interpretation, or deployed layer enforcement.
