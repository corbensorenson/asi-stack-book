...
| Reachable stack-boundary authority/effect consumer | Check arbitrary-run invariants, prefix/suffix composition, multi-layer artifact and authority custody, and the request-to-rollback path against source-anchored local evidence and semantic mutations. | implemented by `AsiStackProofs.StackBoundaries` and `python3 scripts/validate_stack_boundary_effect_consumer.py`; 13 invariant prefixes, 13 compositions, 3 multi-layer handoff cases with 2 rejected countermodels, 6 authority fixtures, 3 runtime paths, 10 accepted events, 1 effect, 1 observation, 1 exact rollback, 2 no-mutation denials, 5 revocation entries, and 12/12 rejected runtime mutations; support-state effect `none` |
...
These hooks retain three genuine reusable order/trace lemmas and add an arbitrary-length boundary invariant, exact trace composition, and a finite multi-layer handoff chain. Eighteen former theorem-per-record normalizations have been physically retired; their frozen identities remain in the proof rationalization registry, and the independent consumer preserves their finite route behavior with a generated 18/18 suite. The reachable model and consumer establish only the authored handoff, authority, dispatch, effect-accounting, observation, denial, revocation, local rollback, and adjacent-layer custody boundaries. Recorded identities, artifact digests, approvals, and receipts remain trusted inputs, and the result does not prove authentic authority, complete effects, distributed execution, whole-system safety, capability, source interpretation, support-state promotion, reproduction, transfer, or deployed layer enforcement.
