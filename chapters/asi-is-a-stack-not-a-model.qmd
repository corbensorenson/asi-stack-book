...
frozen repository-wide terminal audit records 16 `blocked_after_full_attempt`; 8 `retained_after_full_attempt`. Its core
atom `asi-is-a-stack-not-a-model.core` remains at `argument` with terminal
disposition `blocked_after_full_attempt`. The authoritative per-atom rows
are the `asi-is-a-stack-not-a-model` slice of
...
16 still lack required proof lanes,
0 were narrowed, and
...
- **Failure or non-promotion case:** 16 atoms
...
|---|---:|---|---|
| Core: `asi-is-a-stack-not-a-model.core` | 1 | `executable`, `formal`, `source-synthesis` | `blocked_after_full_attempt`; missing or unproved: `empirical`, `normative`, `transfer`. |
...
| blocked after full attempt | 16 | `executable`, `formal`, `source-synthesis` | Missing or unproved lanes across this set: `causal`, `empirical`, `executable`, `formal`, `normative`, `transfer`. These are residual proof obligations, not false claims. |
