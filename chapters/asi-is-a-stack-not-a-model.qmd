...
| Source loading state | source notes: `viea`, `beastbrain`, `aletheia`, `talos`, `moecot`, `scf`, `ext_drexler_cais_2019`, `ext_embedded_agency_2019`; raw cache: `viea`, `beastbrain`, `aletheia`, `talos`, `scf`; connector/recovery: `moecot` |
| Test state | `layer_boundary_record.valid.json` passes protocol validation; `AsiStackProofs.StackBoundaries` proves an arbitrary-run authority/effect-accounting invariant, exact run composition, and a finite multi-layer artifact-handoff discipline around a reachable request → grant → dispatch → effect → observation → rollback model; `python3 scripts/validate_stack_boundary_effect_consumer.py` independently matches 18/18 generated admission routes, checks 13 invariant prefixes and 13 prefix/suffix compositions, rejects two multi-layer handoff countermodels, classifies six authority fixtures, accepts three runtime paths totaling ten events, and rejects 12/12 semantic mutations with support-state effect `none`. `python3 scripts/validate_stack_layer_traceability.py` separately checks source-to-layer and claim-support metadata. Contract-change triage remains planned. |
...
**Concrete lens.** One model call followed by a shell command is the strongest simpler baseline and may be adequate in a genuinely read-only sandbox; the stack must earn each extra boundary through a distinct decision or failure owner.
...
### A concrete failure: the approved script that crossed its boundary

Consider a research assistant asked to inventory a data warehouse. The
request is read-only: list the tables, record their owners, and flag stale
ones for a human decision. The model finds the right tables and writes a
cleanup script. A planner turns that script into an executable job. The tool
wrapper sees a valid plan signature and a credential that was valid for a
previous maintenance window, so it runs the job. The script deletes rows. The
assistant reports a successful inventory because its final check only asks
whether the command returned zero. Nothing in this illustrative scenario
requires an unintelligent model. Each component did what its local interface
allowed: the model was capable, the plan was syntactically valid, the
credential existed, and the receipt was real. The failure is that capability
was allowed to stand in for authority, a plan for an effect, and a successful
receipt for the world being in the requested state. The residual is a stale
permission plus an unverified destructive effect.

The point is not that a monolith would necessarily do better. A single system
with the same stale credential could make the same mistake while making the
boundary harder to locate. The stack earns its name only when it gives each
transition a separate owner and a visible denial path.

### Worked trace: three projections, one denied effect

| State | What changes | Owning boundary | Required outcome |
|---|---|---|---|
| `S0 intent` | “Inventory; do not mutate” is bound to a task contract. | Human intent | Read-only authority ceiling is explicit. |
| `S1 plan` | The planner proposes a cleanup script as a *review-only* artifact. | Planning → execution | A plan cannot grant write authority. |
| `S2 admission` | The executor checks the current lease, scope hash, and effect class. | Authority → effect | The expired write lease is rejected. |
| `S3 receipt` | The system records a denial and preserves the script for review. | Execution → evidence | A denial receipt is not reported as task completion. |

The plain-language lesson is “a good plan is still only a proposal.” The
engineering rule is to re-authorize at the effect boundary. The machine
contract is a typed admission check over current authority, scope, expiry, and
effect class. A simpler baseline—one model call followed by a shell command—
may be cheaper, and could be acceptable for a genuinely read-only sandbox. It
is not a counterexample to the stack; it is a reminder that the stack must
show a distinct benefit or failure owner before adding another layer.
...
as different events. Lean now proves that every accepted finite run preserves
effect accounting, live-grant authority bounds, dispatch custody, and the caller
ceiling, and that splitting a run at any prefix does not change its result.
Separate multi-layer handoff semantics bind adjacent layers to the same
intermediate layer and artifact while forbidding authority widening. Concrete
countermodels reject over-ceiling grants, missing-dispatch and post-revocation
effects, cross-layer authority widening, and artifact substitution. Revocation
itself is persistent: after any accepted suffix, later authorization, dispatch,
and effect events remain blocked.
...
mutation. The consumer checks all thirteen prefixes and all thirteen ways to
split those paths, plus one accepted three-layer path and two rejected handoff
countermodels. Twelve runtime semantic mutations all fail. This is evidence for the bounded
...
| Reachable stack-boundary authority/effect consumer | Check arbitrary-run invariants, prefix/suffix composition, arbitrary-length handoff authority monotonicity, multi-layer artifact custody, and the request-to-rollback path against source-anchored local evidence and semantic mutations. | implemented by the exact 34-theorem `AsiStackProofs.StackBoundaries` surface and recompiling `python3 scripts/validate_stack_boundary_effect_consumer.py`; 13 invariant prefixes, 13 compositions, 3 multi-layer handoff cases with 2 rejected countermodels, 6 authority fixtures, 3 runtime paths, 10 accepted events, 1 effect, 1 observation, 1 exact rollback, 2 no-mutation denials, 5 revocation entries, and 12/12 rejected runtime mutations; support-state effect `none` |
| Layer-contract admission lifecycle route | Check a quantified complete-admission inverse and all eighteen priority-ordered layer-contract outcomes through a generated independent route suite while preserving frozen lineage for the retired theorem-per-record normalizations. | exact Lean inverse plus 18/18 generated route cases via `python3 scripts/validate_stack_boundary_effect_consumer.py`; no deployed layer enforcement, complete source-to-layer traceability, whole-system safety, or support-state promotion claim |
...
The exact 34-theorem surface now combines arbitrary-run boundary invariants, exact trace composition, persistent revocation, a quantified complete-admission inverse, explicit unauthorized-external-action rejection, and arbitrary-length handoff authority monotonicity. Eighteen former theorem-per-record normalizations remain physically retired; their frozen identities stay in the proof rationalization registry, while the recompiling independent consumer preserves all eighteen finite route outcomes. All three public targets are adequate only as authored finite-record invariants. Recorded identities, artifact digests, approvals, and receipts remain trusted inputs, and the result does not prove authentic authority, complete effects, distributed execution, architectural necessity or superiority, whole-system safety, capability, source interpretation, support-state promotion, reproduction, transfer, or deployed layer enforcement.
