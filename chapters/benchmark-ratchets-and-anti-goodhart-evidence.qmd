...
| Test state | `benchmark_ratchet_record.valid.json` passes repository-level protocol fixture validation; `AsiStackProofs.BenchmarkRatchets` implements derived readiness-promotion, regression-floor, and contaminated-review consequences plus `lean:benchmarks.ratchet.fixture_bridge`; `python3 scripts/validate_benchmark_antigoodhart.py` validates synthetic saturation, holdout/contamination/mutation-or-transfer, regression-ref, negative-result, and promotion-blocking semantics; `python3 scripts/validate_benchmark_fixture_bridge.py` validates `experiments/benchmark_antigoodhart/results/2026-07-02-fixture-bridge.json` against 2 valid fixtures, 5 expected-invalid controls, and the Lean fixture bridge. Empirical benchmark runs, hidden holdout execution, contamination audits, policy-training runs, steward release execution, source-reported replay, and current Theseus report verification remain planned. |
...
fixture-bridge result aligned to that same set; three proof targets; and six
local BenchmarkRatchets declarations classified as three derived finite
decision cases and three fixture-normalization assertions. There is no natural
...
These Lean hooks are implemented as finite-record consequences over ratchet-decision reviews and the synthetic fixture-bridge summary. They prove only record-level gates: an accepted readiness-promotion decision requires transfer-or-mutation checks plus preserved negative and regression records, saturated benchmarks moved to regression-floor status require preserved regression records, an accepted contaminated review cannot select readiness promotion, and the tracked fixture bridge preserves its expected positive/negative synthetic controls without support promotion. They do not prove benchmark success, transfer, contamination resistance, regression-suite quality, anti-Goodhart effectiveness, policy quality, steward release safety, or empirical capability gains.

The formal audit counts six declarations. Three are derived finite decision
cases for accepted readiness promotion, saturated-floor conversion, and
contaminated-review rejection. Three normalize the fixed two-valid/five-invalid
synthetic bridge and its no-promotion fields. None proves
