...
open_evidence_gaps:
  ...
  - "The benchmark_ratchet_record fixture validates schema/example consistency, AsiStackProofs.BenchmarkRatchets implements finite record predicates, and the benchmark anti-Goodhart harness validates synthetic saturation, holdout/contamination/mutation-or-transfer, regression-ref, negative-result, and promotion-blocking semantics; no empirical benchmark run, hidden holdout, mutation test, transfer check, contamination audit, source-reported benchmark replay, or current Theseus report verification has been run."
...
| Test state | `benchmark_ratchet_record.valid.json` passes repository-level protocol fixture validation; `AsiStackProofs.BenchmarkRatchets` implements finite capability-promotion evidence and saturated-benchmark higher-readiness gates; `python3 scripts/validate_benchmark_antigoodhart.py` validates synthetic saturation, holdout/contamination/mutation-or-transfer, regression-ref, negative-result, and promotion-blocking semantics. Empirical benchmark runs, hidden holdout execution, contamination audits, source-reported replay, and current Theseus report verification remain planned. |
