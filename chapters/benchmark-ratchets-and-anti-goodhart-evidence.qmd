...
The logical end state is a benchmark operating system, not a leaderboard appendix. Every evaluation would become a governed evidence packet with enough history to tell whether the system learned a capability, learned a test, leaked a public calibration surface, or merely improved a proxy.
...
In that final product, a capability would not be "better" because a single score improved. It would be better only within the recorded evidence boundary, with regressions preserved and residuals visible. This remains a target architecture. The support state should not rise until actual ratchet records, contamination reviews, transfer checks, regression-preservation tests, and accepted evidence transitions exist.
