...
Executable specs say what a narrow artifact can certify. Benchmarks ask how measured performance should change future behavior without becoming a game. This chapter treats benchmarks as ratchets: they can move a floor forward only when regressions, scope, baselines, and negative results remain visible.

The reader should watch for the anti-Goodhart theme. A score is not evidence by itself. It becomes useful when the stack records what was measured, what was not measured, what got worse, what incentives changed, and what claim the result is allowed to support.
:::
...
The mature version of Benchmark Ratchets and Anti-Goodhart Evidence is not the minimal artifact with more scale. It is the chapter's idea turned into a product-grade, evidence-bearing boundary that the rest of the ASI Stack can depend on under ordinary operation and upgrade pressure.

At that end state, the chapter's claim becomes an operational contract: Benchmark ratchets should preserve regressions, create harder frontiers, record residuals, and resist Goodhart pressure. The mature surface would include:

- Operating mechanism: Track benchmark mastery thresholds and saturation; Generate regressions and hidden checks; Move residual failures into escrow and new frontier tasks; Record negative and inconclusive results as evidence, not cleanup.
- Stack interface: Routing and SCFs use readiness gates; Evidence matrix records support movement; Changelog records evidence changes.
- Preserved invariants: No test success is claimed without execution; Synthetic and empirical evidence remain distinct; Regressions are preserved.
- Failure closure: detect and route failure modes such as Benchmark overfitting; Hidden regression deletion; Claim support inflation to quarantine, rollback, escalation, or residual records instead of hidden degradation.

For Benchmark Ratchets and Anti-Goodhart Evidence, this is a target architecture, not a current-result claim. Its support state should not rise until the source mappings, schemas, Lean predicates, tests, benchmark records, runtime traces, review receipts, or governance artifacts needed for this specific end state exist and validate the claim.
...
The durable rule is that benchmarks should improve memory, not erase it. A good ratchet makes the next system harder to fool and easier to compare; a bad one gives the next system a number to optimize and a story to overclaim.
