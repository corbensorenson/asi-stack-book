...
The mechanism is a Benchmark Ratchet Record.

```{mermaid}
flowchart LR
  A["Benchmark frontier"] --> B["Run record"]
  B --> C["Mastery / saturation check"]
  C --> D["Residual escrow"]
  D --> E["Regression suite"]
  E --> F["Anti-Goodhart checks"]
  F --> G{"Promote readiness?"}
  G -- "yes" --> H["New regression floor + harder frontier"]
  G -- "no" --> I["Residual / blocked promotion"]
...
The interface is the Benchmark Ratchet Record.
