...
open_evidence_gaps:
  ...
  - "The resource_budget_record fixture validates schema/example consistency, and deterministic generation-mode plus resource-budget ledger harnesses check record-accounting discipline only; no TokenMana load simulation, PlanForge scheduler benchmark, welfare study, KV-cache memory audit, cost-quality experiment, or scheduler trace has been run."
...
| Test state | `resource_budget_record.valid.json` passes repository-level protocol fixture validation; `python3 scripts/validate_generation_mode_baselines.py` checks paired generation-mode/resource-budget fixture alignment; `python3 scripts/validate_resource_budget_ledgers.py` checks deterministic budget-ledger dispatch, escalation, protected-overhead, displaced-cost residualization, review-capacity hoarding, evidence-ref, and no-promotion scenarios; `AsiStackProofs.ResourceEconomics` implements finite required-gate and high-risk verification-budget predicates; scheduler, load-stability, verification-tax optimization, KV-cache, and cost-quality tests remain planned. |
...
These Lean hooks are implemented as finite-record predicates over required gates, risk class, verification-budget sufficiency, and budget decision fields. They prove only the record-level gates: required safety and verification gates cannot be disabled, and high-risk work with insufficient verification budget must route to escalation, deferral, scope reduction, rejection, or residual accounting rather than ordinary dispatch. Together with the deterministic generation-mode/resource-budget alignment harness and the resource-budget ledger harness, they are useful but too narrow for the chapter boundary. They do not prove load stability, economic optimality, welfare effects, scheduler quality, KV-cache performance, or TokenMana performance.
