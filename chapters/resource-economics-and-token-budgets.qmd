...
| Test state | `resource_budget_record.valid.json` passes repository-level protocol fixture validation; budget-allocation, risk-adjusted verification, and load-stability tests remain planned. |
...
These Lean hooks remain planned. The current schema can require safety-gate and verification-tax fields, but it does not prove load stability or optimal allocation. A useful first proof should model a finite policy predicate: required gates cannot be disabled by a low-budget decision.
