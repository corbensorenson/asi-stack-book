...
| Primary source records | 20 assigned records with 17 exact mappings across resource, simulation, serving, memory, recurrent-compute, verification, routing-economics, relational-compute, and harness concerns |
| Claim label | Design rationale |
| Evidence level | argument |
| Source queue | primary: `tokenmana`, `planforge`; supporting: the local resource/simulation lineage, `reflexive_router_whitepaper`, and `relational_dimension_compiler`; external comparators: PagedAttention, Reluplex, Mem0, Recurrent Transformer, Dynamic Compute, and Claw-SWE-Bench |
| Source loading state | 17 exact passage-reviewed mappings cover 20 assigned sources; local and external results remain source-scoped unless a named repository artifact independently supports them. |
...
Appendix C carries seventeen exact passage-reviewed mappings across twenty assigned sources. The mappings support budget records, regenerative capacity, scheduling, physical/fidelity boundaries, execution-spine discipline, hidden-burden warnings, serving and recurrent-compute comparisons, variable-depth allocation, persistent-memory costs, relational-compute accounting, scoped verification, and harness-bound cost reporting. They do not support measured economic outcomes, welfare effects, scheduler quality, serving performance, simulator adequacy, physical feasibility, open-world transfer, cache behavior, model quality, or local superiority.
