...
The book's efficiency thesis depends on this accounting. A route is not efficient because it looks cheap at the model boundary. It is efficient only when the full system cost and the remaining residual burden are visible.
:::
...
The mature version of Resource Economics and Token Budgets is not the minimal artifact with more scale. It is the chapter's idea turned into a product-grade, evidence-bearing boundary that the rest of the ASI Stack can depend on under ordinary operation and upgrade pressure.
...
- Operating mechanism: Track task value, uncertainty, cost of error, and inference cost; Use regenerative or budgeted capacity mechanisms where useful; Escalate verification when risk justifies the cost; Separate aggregate serving throughput and memory pressure from single-request verified-output value.
- Stack interface: Planning allocates budgets; Routing chooses costed specialists; Evidence measures cost-quality tradeoffs; Serving infrastructure reports memory and throughput costs without converting them into quality claims.
- Preserved invariants: Budgets do not override protected safety gates; High-risk tasks pay verification cost; Cost savings are recorded with quality results; Serving-throughput gains remain separate from verified-output and task-success claims.
- Failure closure: detect and route failure modes such as Cost-cutting verification away; Load-synchronized degradation; Resource hoarding by low-value tasks; Aggregate serving throughput mistaken for lower single-request risk or better answer quality to quarantine, rollback, escalation, or residual records instead of hidden degradation.
