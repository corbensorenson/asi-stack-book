...
open_evidence_gaps:
  ...
  - "The resource_budget_record and simulation_contract_record fixtures validate schema/example consistency only; deterministic generation-mode, resource-budget ledger, capacity-smoothing, resource-workflow-trace, resource-live-probe, resource-workload-quality-probe, resource-ci-cost-profile, and simulation-transfer harnesses check record-accounting discipline, toy traces, selected-route accounting, protected high-risk ordering, displaced-cost residuals, capacity-budget-overrun rejection, physical-feasibility overclaim rejection, KV-cache/serving-memory accounting separation, throughput-to-quality overclaim rejection, local command replay, a scoped local workload-quality route choice with a rejected no-op negative control, publication-pipeline metadata, and claim-transport boundaries only; the costed-route result carries field-level Python/Lean fixture alignment, and the workflow result now carries trace-property Python/Lean fixture alignment over finite dispatch events, rollup totals, high-risk-first order, guard flags, over-budget rejection, and no-promotion boundaries; `AsiStackProofs.ResourceEconomics` and `AsiStackProofs.SimulationFidelity` add finite negative cases for disabled required gates, high-risk insufficient-budget dispatch, aggregate-throughput quality overclaim, missing simulation scope, and fidelity overclaim only; no TokenMana load simulation, PlanForge scheduler benchmark, welfare study, KV-cache memory audit, serving-throughput measurement, single-request quality result, simulation benchmark, physical-computation audit, deployed scheduler, stable speedup result, external workload-quality review, or live cost-quality experiment has been run."
...
| Resource workload-quality probe | Measure a scoped Resource workflow trace review route against a broader Resource Economics replay baseline, reject a cheaper no-op success-text route, record residuals and non-claims, and preserve the no-transition boundary. | implemented; passing via `python3 scripts/validate_resource_workload_quality_probe.py`; local single-run repository task only; no stable-speedup, deployed-scheduler, model-quality, economic-outcome, external-review, or support-state-transition claim |
