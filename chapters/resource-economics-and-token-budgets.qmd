...
open_evidence_gaps:
  ...
  - "The resource_budget_record and simulation_contract_record fixtures validate schema/example consistency only; deterministic generation-mode, resource-budget ledger, capacity-smoothing, resource-workflow-trace, resource-live-probe, resource-ci-cost-profile, and simulation-transfer harnesses check record-accounting discipline, toy traces, selected-route accounting, protected high-risk ordering, displaced-cost residuals, physical-feasibility overclaim rejection, local command replay, publication-pipeline metadata, and claim-transport boundaries only; the costed-route and workflow result records now carry explicit Python/Lean fixture-alignment fields; `AsiStackProofs.ResourceEconomics` and `AsiStackProofs.SimulationFidelity` add finite negative cases for disabled required gates, high-risk insufficient-budget dispatch, missing simulation scope, and fidelity overclaim only; no TokenMana load simulation, PlanForge scheduler benchmark, welfare study, KV-cache memory audit, simulation benchmark, physical-computation audit, deployed scheduler, or live cost-quality experiment has been run."
