...
open_evidence_gaps:
  ...
  - "The resource_budget_record and simulation_contract_record fixtures validate schema/example consistency only; deterministic generation-mode, resource-budget ledger, capacity-smoothing, and simulation-transfer harnesses check record-accounting discipline, toy traces, and claim-transport boundaries only; `AsiStackProofs.ResourceEconomics` and `AsiStackProofs.SimulationFidelity` add finite negative cases for disabled required gates, high-risk insufficient-budget dispatch, missing simulation scope, and fidelity overclaim only; no TokenMana load simulation, PlanForge scheduler benchmark, welfare study, KV-cache memory audit, simulation benchmark, physical-computation audit, cost-quality experiment, or scheduler trace has been run."
...
The implemented rows validate fixture/schema consistency, deterministic record-accounting behavior, toy capacity traces, and synthetic simulation-transfer boundaries only. The remaining rows require scheduler behavior, risk-policy checks, a real load model, KV-cache accounting, physical-feasibility review, measured simulation outputs, resource estimates, and review harnesses; they are not reported results. Future resource-economics or simulation-audit tests should link to the command, fixture, environment notes, assumptions, and result summary, and Appendix E should be regenerated or updated accordingly.
