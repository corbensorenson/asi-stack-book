...
open_evidence_gaps:
  ...
  - "The resource_budget_record fixture validates schema/example consistency, deterministic generation-mode plus resource-budget ledger harnesses check record-accounting discipline, and the capacity-smoothing harness checks toy traces only; no TokenMana load simulation, PlanForge scheduler benchmark, welfare study, KV-cache memory audit, cost-quality experiment, or scheduler trace has been run."
...
| Test state | `resource_budget_record.valid.json` passes repository-level protocol fixture validation; `python3 scripts/validate_generation_mode_baselines.py` checks paired generation-mode/resource-budget fixture alignment; `python3 scripts/validate_resource_budget_ledgers.py` checks deterministic budget-ledger dispatch, escalation, protected-overhead, displaced-cost residualization, review-capacity hoarding, evidence-ref, and no-promotion scenarios; `python3 scripts/validate_capacity_smoothing.py` checks toy capacity traces for bounded regeneration arithmetic, priority deferral, overload rejection, and no-promotion boundaries; `AsiStackProofs.ResourceEconomics` implements finite required-gate and high-risk verification-budget predicates; scheduler, real load-stability, verification-tax optimization, KV-cache, and cost-quality tests remain planned. |
...
The generation-mode baseline harness now checks that deterministic generation-mode scenarios keep resource-budget records aligned on task identity, risk class, verification tax, protected overhead, safety gates, residuals, evidence references, and no-promotion boundaries. The resource-budget ledger harness adds deterministic records for low-risk dispatch, high-risk escalation, protected-overhead dispatch, displaced-cost residualization, and review-capacity hoarding. The capacity-smoothing toy harness checks bounded regeneration arithmetic and priority deferral in small traces. That is useful accounting coverage, not a scheduler or economics result.
...
This cognition-budget market is still proposed architecture. Resource economics should remain `argument` until budget ledgers, verification-budget checks, load/friction tests, value-of-computation comparisons, scarce-resource scheduler traces, and rejected-saving records show that resource discipline improves the stack without hiding risk.
...
| Resource budget ledger harness | Check deterministic Resource Budget Record decisions for dispatch, high-risk escalation, protected overhead, displaced-cost residualization, review-capacity hoarding, evidence refs, and no-promotion boundaries. | implemented; passing via `python3 scripts/validate_resource_budget_ledgers.py` |
| Capacity smoothing toy harness | Check deterministic toy capacity traces for bounded regeneration arithmetic, priority deferral under blocked high-risk work, scope reduction, overload rejection, and no-promotion boundaries. | implemented; passing via `python3 scripts/validate_capacity_smoothing.py` |
...
| Real load stability scenario | Simulate or review whether regenerative capacity reduces synchronized overload under explicit workload assumptions beyond the current toy trace. | planned; not run |
...
The implemented rows validate fixture/schema consistency, deterministic record-accounting behavior, and toy capacity traces only. The remaining rows require scheduler behavior, risk-policy checks, or a real load model; they are not reported results. When a resource-economics test is implemented, the chapter should link to the command, fixture, environment notes, assumptions, and result summary, and Appendix E should be regenerated or updated accordingly.
...
These Lean hooks are implemented as finite-record predicates over required gates, risk class, verification-budget sufficiency, and budget decision fields. They prove only the record-level gates: required safety and verification gates cannot be disabled, and high-risk work with insufficient verification budget must route to escalation, deferral, scope reduction, rejection, or residual accounting rather than ordinary dispatch. Together with the deterministic generation-mode/resource-budget alignment harness, the resource-budget ledger harness, and the capacity-smoothing toy harness, they are useful but too narrow for the chapter boundary. They do not prove real load stability, economic optimality, welfare effects, scheduler quality, KV-cache performance, or TokenMana performance.
