...
open_evidence_gaps:
  - "Appendix C now records exact passage-reviewed mappings for the folded resource-economics and simulation-fidelity source union; support remains at argument until TokenMana simulations, PlanForge scheduler benchmarks, welfare/load studies, serving-memory audits, simulation-transfer reviews, physical-computation audits, cost-quality experiments, or accepted evidence transitions exist."
  - "The resource_budget_record and simulation_contract_record fixtures validate schema/example consistency only; deterministic generation-mode, resource-budget ledger, and capacity-smoothing harnesses check record-accounting discipline and toy traces only; `AsiStackProofs.ResourceEconomics` and `AsiStackProofs.SimulationFidelity` add finite negative cases for disabled required gates, high-risk insufficient-budget dispatch, missing simulation scope, and fidelity overclaim only; no TokenMana load simulation, PlanForge scheduler benchmark, welfare study, KV-cache memory audit, simulation benchmark, physical-computation audit, cost-quality experiment, or scheduler trace has been run."
...
| Test state | `resource_budget_record.valid.json` and `simulation_contract_record.valid.json` pass repository-level protocol fixture validation; `python3 scripts/validate_generation_mode_baselines.py` checks paired generation-mode/resource-budget fixture alignment; `python3 scripts/validate_resource_budget_ledgers.py` checks deterministic budget-ledger dispatch, escalation, protected-overhead, displaced-cost residualization, review-capacity hoarding, evidence-ref, and no-promotion scenarios; `python3 scripts/validate_capacity_smoothing.py` checks toy capacity traces for bounded regeneration arithmetic, priority deferral, overload rejection, and no-promotion boundaries; `AsiStackProofs.ResourceEconomics` and `AsiStackProofs.SimulationFidelity` implement finite record-level predicates plus negative cases for disabled required gates, high-risk insufficient-budget dispatch, missing simulation scope, and fidelity overclaim; scheduler, real load-stability, verification-tax optimization, KV-cache, simulation-transfer, physical-feasibility, and cost-quality tests remain planned. |

## Drafting guardrail

This chapter uses TokenMana and PlanForge as resource-governance and planning sources, Simulation Scaling as a contract-relative physical/fidelity source, and PagedAttention/vLLM plus Reluplex as external comparators for serving economics and scoped formal verification. It does not reproduce TokenMana formal results, run load simulations, deploy vLLM, audit KV-cache behavior, run a simulator, reproduce Reluplex, validate physical feasibility, or claim measured welfare, profit, latency, throughput, productivity, transfer, or safety effects.
...
The resource-budget fixture does not establish load stability or economic optimality. The simulation-contract fixture does not establish feasibility or transfer. Together they give future PlanForge, TokenMana, simulator, and benchmark work a shared budget and fidelity contract.
...
A first budget ledger shows the rejected route alongside the chosen route, because hidden savings are not governed savings. A first simulation receipt suite should add one unit-invariant result, one benchmark result with approximation limits, one scenario-only result, one blocked transfer because a bottleneck is missing, and one downgraded claim after instrumentation cost is recorded.
...
| Resource-bound simulation sanity check | Check that demand and capacity bottlenecks are named before simulation feasibility is claimed. | planned; not run |
| Simulation approximation audit | Check that conclusions do not exceed approximation, instrumentation, and fidelity boundaries. | planned; not run |

The implemented rows validate fixture/schema consistency, deterministic record-accounting behavior, and toy capacity traces only. The remaining rows require scheduler behavior, risk-policy checks, a real load model, simulation claims, resource estimates, and review harnesses; they are not reported results. When a resource-economics or simulation-audit test is implemented, the chapter should link to the command, fixture, environment notes, assumptions, and result summary, and Appendix E should be regenerated or updated accordingly.
