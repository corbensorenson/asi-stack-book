...
### Price reuse over its break-even horizon

Reusable knowledge moves cost rather than deleting it. Its construction bill
includes mining, independent validation, indexing, documentation, storage, and
admission. Its operating bill includes retrieval, vocabulary growth, search,
candidate verification, compilation or lowering, monitoring, maintenance, and
retirement. A shorter future derivation can still lose economically if the
library makes the search space harder to navigate or if a rarely used artifact
never repays its fixed cost.

Two comparisons answer different questions. An equal-downstream-budget test
asks whether the frozen library helps once construction is treated as sunk.
An equal-total-budget test charges construction and maintenance to the
candidate and asks whether the complete system is preferable. Both should be
reported, followed by the task volume or time horizon at which cumulative
savings, if any, exceed the fixed and continuing cost. A favorable latency
result does not erase verification capacity, human review, storage, energy,
wrong-route recovery, or opportunity cost.

*From Compression to Forward Transfer* specifies this ledger and break-even
analysis but supplies no measured costs. It therefore sharpens what a future
resource claim must retain; it does not establish that a learned library,
compressed artifact, or compiled procedure is economical.
...
| `forward_transfer_program_synthesis` | Passage-reviewed comparator: [From Compression to Forward Transfer: Evaluating Reusable Knowledge in Program Synthesis](../papers/forward_transfer_program_synthesis.html). Adds full lifecycle costing for reusable knowledge, equal-downstream and equal-total-budget comparisons, vocabulary and retrieval overhead, and an explicit break-even horizon. | The paper provides a cost schema and comparison discipline, not measured construction cost, operating cost, break-even time, or resource superiority. No local implementation, reproduction, performance, safety, deployment, support-state, or ASI result is established by this reconciliation row. |
