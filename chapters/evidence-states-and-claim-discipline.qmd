...
| Accepted live transition review | Check that any actual support-state movement has claim-specific evidence, independent review, changelog refs, and no overbroad support effect. | planned; not run |
...
These evidence-state hooks are implemented in the local Lean workspace. The module models `deprecated` and `refuted` as terminal support states, proves narrow terminal/downgrade guard theorems over finite records, includes a finite transition-lifecycle route envelope, and now carries fixture bridges for the evidence-bundle completeness probe and claim-ledger completeness audit. Their scope is narrow: they check transition discipline and fixture alignment, not the truth of any source-derived or benchmark-backed claim, not reviewer quality, and not the correctness of any live promotion, demotion, deprecation, or refutation decision.
