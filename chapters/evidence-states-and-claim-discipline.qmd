...
| Source queue | primary: `benchmaxxing`, `spinoza`, `viea`; supporting: `uat`, `coherence_exchange`, `verification_bandwidth`; external comparators: `ext_model_cards_2019`, `ext_datasheets_datasets_2021`, `ext_ml_reproducibility_program_2021`, `ext_proof_carrying_code_1997` |
| Source loading state | source notes: `benchmaxxing`, `spinoza`, `viea`, `uat`, `coherence_exchange`, `verification_bandwidth`, `ext_model_cards_2019`, `ext_datasheets_datasets_2021`, `ext_ml_reproducibility_program_2021`, `ext_proof_carrying_code_1997`; raw cache: `benchmaxxing`, `spinoza`, `viea`, `uat`, `verification_bandwidth`; connector/recovery: `coherence_exchange` |
...
This chapter is therefore not a housekeeping chapter. It is the book's methodological claim-control layer, paired with Living Book Methodology: the method by which a living technical text can keep changing without turning clearer writing into stronger evidence.
...
The support-state harness exercises synthetic no-change, upward, downward, and terminal refutation transitions, including invalid controls for unreviewed promotion, failed verification, reasonless demotion, and unsupported refutation. The evidence-bundle probe adds two valid synthetic evidence bundles for no-change and blocked promotion, plus seven expected-invalid controls for missing claim IDs, artifact/result refs, commands, changelog refs, limitations, non-claims, stale changelogs, and fixture overclaiming. The claim-ledger audit checks the real Appendix C table against all 44 manifest chapter core claims and rejects seven synthetic mutations that would hide missing, duplicate, unknown, mislabeled, support-mismatched, open-gap-missing, or promotion-path-missing rows. The accepted-transition audit checks the existing 36 accepted transition records, the five bounded non-core upward transitions, the accepted no-promotion ledger, and seven mutation controls for core promotion, missing review artifacts, unresolved blockers, support movement, and no-promotion drift. The Lean module proves narrow support-state invariants in this repository, including that terminal states are not promotion targets and that a finite transition-lifecycle route sends missing claim records, scope boundaries, support-state effects, effect mismatches, missing review, missing required evidence, missing negative evidence, missing downgrade triggers, wrong terminal effects, missing changelog refs, and missing non-claim boundaries to explicit modeled outcomes. These artifacts leave chapter core claims unchanged unless claim-specific evidence and accepted review exist.
...
These evidence-state hooks are implemented in the local Lean workspace. The module models `deprecated` and `refuted` as terminal support states, proves narrow terminal/downgrade guard theorems over finite records, includes a finite transition-lifecycle route envelope, and now carries fixture bridges for the evidence-bundle completeness probe, claim-ledger completeness audit, and accepted-transition review audit. Their scope is narrow: they check transition discipline and fixture alignment, not the truth of any source-derived or benchmark-backed claim, not reviewer quality, and not the correctness of any live promotion, demotion, deprecation, or refutation decision.
