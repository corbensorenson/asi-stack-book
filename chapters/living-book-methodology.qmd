...
| Source loading state | source notes: `benchmaxxing`, `viea`, `bugbrain`, `cognitive_loop_closure`, `moecot`, `moecot_md`, `road_to_agi`, `ext_openunlearning_2025`, `ext_literate_programming_1984`, `ext_jupyter_book_docs`, `ext_quarto_books_docs`, `ext_nist_ai_rmf_1_0_2023`, `ext_frontier_ai_regulation_2023`, `ext_helm_2022`, `ext_livebench_2024`, `ext_benchmark_contamination_2023`; raw cache: `benchmaxxing`, `viea`, `bugbrain`, `cognitive_loop_closure`, `moecot_md`, `road_to_agi`; connector/recovery: `moecot` |
...
**Concrete lens.** The filename-order baseline renumbers neighbors and breaks derivatives. The manifest compiler derives order while preserving stable identities.
...
**Reader claim.** A living book stays trustworthy when edits can change order, prose, evidence, and presentation without changing chapter identity or silently promoting claims.

**Operational rule.** Drive structure from one canonical manifest, keep chapter IDs stable, derive display order, bind each editorial and publication change to current digests, and record its evidence and release effects separately. A green build is necessary but never sufficient for editorial acceptance or public truth.

### Worked structural edit: insert a chapter without renaming its neighbors

A new chapter is inserted between display positions 20 and 21. The manifest compiler assigns contiguous display ordinals, but the stable IDs and file identities of every existing chapter remain unchanged. Cross-references, evidence packets, videos, and citations continue to address those IDs rather than old ordinal numbers. A thin summary containing only the new manifest digest and chapter count cannot decide whether all required editorial, render, rights, and release gates passed.

The local change-packet harness checks three valid and six invalid packets, a middle insertion, five trace splits, nine reachable states through 162 transitions, and five terminal states through 90 absorbing transitions. It rejects fifteen semantic mutations. These repository-process checks establish deterministic structure and custody for the authored workflow; they do not establish source interpretation, prose quality, accessibility, public availability, reader approval, or chapter truth.
...
[Artifact Steward Agents and Living Project Governance](artifact-steward-agents-and-living-project-governance.qmd)
is the technical-detail owner for the general problem of maintaining a durable
artifact: chartered work, contributor and treasury records, bounded execution,
release gates, transfer, federation, maintenance, retirement, and sunset. This
chapter owns the narrower reflexive method by which this book changes its own
manifest, prose, claims, sources, proofs, editions, and publication surfaces
without confusing a successful build with editorial or evidentiary approval.

The publication placement makes the book a worked specialization of governed
artifact stewardship without treating it as proof that the general steward
design works. The living-book pipeline may supply a concrete record discipline
and failure surface; it does not transfer project-governance, treasury,
continuity, capture-resistance, or federation support. The technical route, in
turn, does not transfer source truth, manuscript quality, claim support,
accessibility, or release authority to this chapter. Both owners retain their
claim identities, proof and test surfaces, evidence states, implementation
horizons, and stable URLs.
...
| `moecot` | MoECOT-Agent Architecture Whitepaper | implementation_reference | Runtime-reference context for ledgers, replay, readiness gates, residuals, handoff, and promotion blockers. | source note available; connector or recovery required |
| `moecot_md` | moecot_agent_whitepaper.md | implementation_reference_variant | Markdown export/source version for terminology normalization and variant comparison only. | source note available; local raw cache available |
| `road_to_agi` | Road To AGI | strategic_roadmap | Remaining-work, source-reported status, unreproduced benchmark, and promotion-blocker context. | source note available; local raw cache available |
| `ext_literate_programming_1984` | Literate Programming | literate_programming | External lineage comparator for coupling human-readable exposition with executable artifact structure. | source note available |
| `ext_jupyter_book_docs` | Jupyter Book Documentation | executable_books | External comparator for computational/executable books built from notebooks or Markdown with code execution, cross references, and web publication. | source note available |
| `ext_quarto_books_docs` | Quarto Books Documentation | technical_publishing | External comparator for multi-chapter Quarto manuscripts, book-style HTML, cross references, and publication-format targets. | source note available |
| `ext_nist_ai_rmf_1_0_2023` | Artificial Intelligence Risk Management Framework (AI RMF 1.0) | governance_evals | External comparator for lifecycle, risk, trustworthiness, and Govern/Map/Measure/Manage vocabulary. | source note available |
| `ext_frontier_ai_regulation_2023` | Frontier AI Regulation | governance_evals | External comparator for reporting, standards, scrutiny, compliance, pre-deployment assessment, and post-deployment monitoring vocabulary. | source note available |
| `ext_helm_2022` | Holistic Evaluation of Language Models | benchmark_science | External comparator for transparent multi-scenario, multi-metric reporting and missing-coverage disclosure. | source note available |
| `ext_livebench_2024` | LiveBench | benchmark_science | External comparator for living benchmark pressure and contamination-limited update practice. | source note available |
| `ext_benchmark_contamination_2023` | Investigating Data Contamination in Modern Benchmarks for Large Language Models | benchmark_contamination | External comparator for stale/leaky evaluation surfaces and score-interpretation limits. | source note available |
