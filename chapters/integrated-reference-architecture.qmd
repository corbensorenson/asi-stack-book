...
The integrated architecture should first exist as the reference flow diagram plus `reference_trace_record.valid.json`. The reference-trace fixture checks record shape, trace state, execution boundary, parentage, deltas, blockers, source refs, and support-state effect only: it does not execute the architecture, replay a runtime trace, or prove that every layer handoff is implemented.
