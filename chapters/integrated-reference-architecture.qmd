...
**Diagram reading note:** The reference trace is an end-to-end handoff chain, not a single agent loop. Each participant receives and emits a typed artifact, and the final SCF gate can still promote, roll back, quarantine, or require review based on evidence and authority.
...
A tiny dry-run trace set is the first integration artifact worth building. One approved flow should carry stable artifact IDs from intent through plan, context, route, work order, audit log, evidence update, and release note. One blocked flow should fail because a required authority, context adequacy check, verification predicate, or rollback path is absent. The trace set would remain a fixture rather than a runtime result, but it would make the architecture's stop conditions executable.
