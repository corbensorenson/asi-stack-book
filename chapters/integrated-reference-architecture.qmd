...
| Test state | `reference_trace_record.valid.json` passes repository-level protocol fixture validation; end-to-end trace, artifact continuity, and authority stop-condition tests remain planned. |
...
These Lean hooks remain planned. The reference trace schema can require handoff fields, but it does not prove that governance gates are enforced. A first proof should model a finite trace predicate: a trace missing a governance gate cannot be marked valid.
