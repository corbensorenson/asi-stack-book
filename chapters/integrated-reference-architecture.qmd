...
| End-to-end intent trace test | Check deterministic reference-trace fixtures for required layer terms from intent through SCF, stable artifact refs, source-note refs, validation commands, evidence deltas, and residual deltas. | implemented by `python3 scripts/validate_reference_trace.py`; no integrated runtime trace or replayed demo exists |
...
The implemented rows validate deterministic fixture behavior only. They do not execute the architecture or replay a deployed demo with typed artifacts across layers. A future runtime integration test still needs the command, environment notes, trace artifacts, and result summary for an actual run.
