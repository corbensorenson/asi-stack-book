...
| Source loading state | source notes: `rmi`, `cognitive_loop_closure`, `ext_gemini_robotics_2025`, `ext_foundation_robotics_physical_risk_2025`, `ext_control_barrier_functions_2019`, `ext_simplex_architecture_1998`, `ext_safe_reinforcement_learning_survey_2015`, `ext_ai_simulation_digital_twins_2025`, `viea`; raw cache: `rmi`, `cognitive_loop_closure`, `viea` |
...
**Concrete lens.** The simpler controller executes the same command because the plan is unchanged. The lease path rechecks current timing and stopping margin and degrades when either fails.
...
**Reader claim.** A physically safe command is time-dependent: the same action can become inadmissible when its observation is stale, its deadline slips, or its stopping margin shrinks.

**Operational rule.** Recheck plant identity, observation age, latency, state envelope, actuator limit, and stopping distance every control period. A deadline miss or lost margin immediately narrows the lease to the independent hold or stop path; the learned controller cannot vote itself more time.

### Worked control period: miss the deadline, enter the safe state

A controller receives a bounded move command at tick `5`. Its plant lease expires at tick `8`; the state was observed at tick `4` with maximum age `2`; worst-case latency is `2` inside a control period of `3`. The estimated state interval `[4, 7]` lies within the declared safe interval `[2, 10]`, the requested magnitude `4` is below actuator limit `6`, and stopping distance upper bound `3` fits inside remaining margin `5`. With fallback ready and an independent stop armed, the modeled lease admits one bounded period.

Now increase worst-case latency beyond the period or shrink remaining margin below stopping distance. The advanced action is no longer eligible even if its symbolic plan is unchanged. The route degrades to the independent fallback and records the failed timing or margin predicate. The local review rejects 13 admission-axis mutations and 105 lifecycle mutations, but it trusts the authored plant, estimator, and fallback fields. It therefore checks lease logic, not real deadline satisfaction, safe-set truth, braking effectiveness, or physical safety.
...
`AsiStackProofs.EmbodiedPhysicalSafety` with 41 theorem declarations: 22 in the
finite control-lease admission model and 19 in a separate simulation-trial
review lifecycle. A complete authored lease derives current-version and
...
The eight-stage simulation-only lifecycle binds nine plant, lease, controller,
estimator, policy, envelope, actuator, observer, and result identities from
proposal through closure. Arbitrary successful runs preserve those identities
and zero support/external-effect authority, account for exactly one receipt per
accepted event, keep stop-receipt counts monotone, expose an accepted trace,
compose across all eight prefix/suffix splits, and reject every event after
closure. A seven-event witness closes with seven receipts and one stop receipt;
any of the thirteen missing safety axes prevents the trial from starting. The
independent consumer rejects 105/105 lifecycle mutations spanning identity,
stage, safety-axis, replay, authority, support, effect, and post-closure cases.
...
| Simulation-trial review lifecycle | Recompile the exact 41-theorem surface; execute the seven-event review; check identity, non-authority, receipts, stop monotonicity, eight trace splits, closure, and 105/105 rejecting mutations. | implemented and passed locally |
...
|---|---|---|
| `rmi` | Passage-reviewed comparator: [Ratcheting Modular Intelligence](../papers/rmi.html). Supplies a hierarchical embodied-log design separating raw telemetry, salient events, semantic state, active skill/controller identity, and residuals so loop learning and safety review do not consume an undifferentiated sensor stream. | Conceptual architecture only; no logger, event detector, semantic encoder, raw-ring buffer, trace join, incident reconstruction, storage result, privacy result, physical controller, or hardware campaign has been implemented or measured. No local implementation, reproduction, performance, safety, deployment, support-state, or ASI result is established by this reconciliation row. |
| `cognitive_loop_closure` | Passage-reviewed comparator: [Cognitive Loop Closure](../papers/cognitive_loop_closure.html). Supplies the detailed embodied trajectory policy: raw telemetry separated from cognitive/event/residual logs, rolling multi-resolution buffers, triggered pre/post-event retention, semantic eventization, feature compression, reflex-state custody, memory budgets, and explicit warning against loss of safety-relevant information. | Conceptual architecture only; no telemetry corpus, eventizer, feature compressor, raw-buffer policy, reflex logger, incident reconstruction, privacy evaluation, real-time controller, simulator, or hardware result has been implemented or measured. No local implementation, reproduction, performance, safety, deployment, support-state, or ASI result is established by this reconciliation row. |
