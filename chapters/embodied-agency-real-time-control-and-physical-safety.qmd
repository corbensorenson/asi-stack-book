...
| Source loading state | One passage-reviewed Corben architecture source supplies hierarchical embodied logging, and five external records cover generalist robotics, foundation-model physical risks, barrier-function safety filters, independent runtime fallback, and safe-RL taxonomy. No local physical or logging result is claimed. |
...
`lean:embodiment.missing_safety_state_blocks_control` will reject a finite
control lease missing required timing, state, envelope, fallback, or stop
evidence. It will trust the authored predicates and cannot establish plant
dynamics, safe-set validity, deadline satisfaction, recovery, or physical safety.
