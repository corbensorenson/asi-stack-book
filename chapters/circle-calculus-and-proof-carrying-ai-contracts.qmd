...
open_evidence_gaps:
  - "Appendix C now records exact passage-reviewed mappings for all five assigned proof-carrying contract sources; support remains at argument until local Circle theorem-id resolution, clean replay from this repo, Circle Lean builds, downstream workloads, or accepted chapter-core evidence transitions exist."
  - "The proof_target_record and proof_contract_receipt_record fixtures validate proof-record and receipt-record schema/example consistency; the receipt fixture now records receipt state, proof boundary, fingerprint status, consumer state, staleness policy, source refs, support-state effect, and non-claims. `AsiStackProofs.ProofCarryingContracts` implements local finite-record boundary predicates plus negative cases for missing receipt fields, missing contract readiness, stale/unsupported consumer gates, and replay artifacts. `scripts/validate_circle_public_replay.py` validates one public ASI-side consumer-gate fixture by pinned digest and rejects digest-mismatch, missing-theorem, stale-contract, and unsupported-transfer mutations; `scripts/validate_circle_contract_pack_archive.py` validates a public-safe archived Circle contract pack and acceptance-policy report by digest, contract fingerprints, public-safety hygiene, and expected-invalid controls. No external Circle Lean build, theorem-id resolver, Circle contract command, or sidecar regeneration has been run from this repo."
