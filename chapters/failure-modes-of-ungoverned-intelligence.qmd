...
open_evidence_gaps:
  ...
  - "The implemented FailureModes Lean predicates cover finite-record invariant failure, unbounded-authority detection, and incident routing for authority overreach, tainted context, subject-modified evaluators, and unverified claim promotion only; they do not prove deployed failure detection or containment."
...
| Test state | `failure_boundary_map.valid.json` passes protocol fixture validation with failure class, affected contract refs, promotion blocker, source refs, support-state effect, and non-claims, and `AsiStackProofs.FailureModes` builds locally with finite invariant, governance-failure, and incident-route predicates; no runtime authority gate, context-admission detector, evaluator-independence probe, claim-verification harness, or deployed failure detector has run. |
...
These proof targets are implemented only over finite Lean records in `AsiStackProofs.FailureModes`. They prove that a modeled component with a failed required invariant is not promotable, that an unbounded authority field is represented as a governance failure, and that four modeled incident states route to review, quarantine, frozen review, or blocked promotion. They do not prove runtime scenario coverage, evaluator independence, context-pollution containment, claim-verifier quality, or deployed failure detection; those remain executable-test obligations.
