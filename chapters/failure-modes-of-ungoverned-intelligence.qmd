...
| Failure recovery lifecycle refinement | Check rejected-state preservation, arbitrary-run identity and authority custody, exact receipt accounting, trace validity, batch composition, five reachable recovery stages, guarded readmission, recurrence re-isolation, and independently encoded route mutations. | implemented by the exact 19-theorem `AsiStackProofs.FailureRecoveryRefinement` surface and `python3 scripts/validate_failure_recovery_refinement.py`; five accepted transitions and 31/31 rejecting mutations; no event-truth, detector-quality, mitigation-effectiveness, deployed-recovery, safety, or support-state claim |
...
The original proof targets remain finite records in `AsiStackProofs.FailureModes`. They establish invariant rejection, explicit incident routes, and an input-general close-state inverse. The exact 19-theorem `AsiStackProofs.FailureRecoveryRefinement` surface adds a reachable five-stage transition system and accepted-run semantics: rejected events preserve exact state; every successful finite run preserves all nine incident/version identities and zero support or external authority, adds exactly one receipt per accepted event, yields a valid trace, and composes across event batches; accepted detection disables modeled effects; accepted readmission requires current assurance, current taxonomy, residual custody, and explicit authority; and one bounded recurrence re-isolates the system. The independent validator recompiles that exact surface before reproducing five accepted transitions and 31 state-preserving rejections. The former direct `GovernanceFailure` field projection remains retired; the public authority target binds to the retained incident-route theorem.

The original module contains twenty-one declarations and the recovery module
contains nineteen. The former detector-probe summary
