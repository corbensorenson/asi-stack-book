...
These proof targets are implemented only over finite Lean records in `AsiStackProofs.FailureModes`. They prove that a modeled component with a failed required invariant is not promotable, that an explicit authority-over-ceiling incident routes to authority review, that three other modeled incident states route to quarantine, frozen review, or blocked promotion, and that modeled failure recurrence records route missing class, boundary, receipt, owner, containment, residual, learning-path, normalization-guard, review, quarantine, evidence-transition, and non-claim-boundary fields to explicit outcomes. The close-state result is now an input-general inverse: returning `closeFailureRecord` entails every required field and excludes every earlier guard, rather than merely normalizing an authored all-green constant. The former direct `GovernanceFailure` field projection is retired; the public authority target now binds to the retained incident-route theorem.

The module contains twenty-one declarations. The former detector-probe summary
