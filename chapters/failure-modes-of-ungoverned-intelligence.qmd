...
This failure-radar layer remains a target architecture, not a current-result claim. The support state remains `argument` until failure records are mapped to invariant checks, falsification tests, evaluator-capture probes, negative-result retention, and residual/escalation routing that preserve failures instead of smoothing them away.
