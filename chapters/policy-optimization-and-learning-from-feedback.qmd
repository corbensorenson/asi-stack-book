...
open_evidence_gaps:
  ...
  - "The policy_optimization_record fixture validates schema/example consistency and AsiStackProofs.PolicyOptimization implements finite record predicates only; neither artifact proves reward quality, optimizer convergence, benchmark improvement, policy safety, or reward-hacking resistance."
---
...
| Test state | `policy_optimization_record.valid.json` passes repository-level protocol fixture validation; `AsiStackProofs.PolicyOptimization` implements finite policy-update record and reward/governance promotion gates; no policy optimization experiment has been run. |
...
Policy optimization enters the stack as a governed update ledger. Before any behavior is changed, the stack records which policy is being updated, which feedback artifact is admissible, what drift bound applies, how reward hacking will be probed, which residuals remain, and which governance gate can promote or reject the update.
...
- Governance refs: authority limits, high-impact review, rollback plan, and promotion decision.
...
Every update should produce a policy-delta receipt. The receipt records what behavior changed, which feedback authorized the change, which probes tried to break the reward, which regressions were checked, which authority boundary remained unchanged, and how rollback would restore the prior policy.
...
The fixture records target layer, policy ref, training mode, feedback source, reward or preference signal, verifier refs, update constraint, drift bound, evaluation refs, governance gate refs, rollback plan, promotion decision, measurement status, residuals, and non-claims. It validates the record shape only. It does not run PPO, DPO, GRPO, RLVR, router-policy RL, context-policy RL, or reasoning-budget RL.
...
| Test | Purpose | Status |
|---|---|---|
| Policy optimization record fixture validation | Validate that a policy update records target layer, policy ref, training mode, feedback source, reward signal, verifier refs, update constraint, drift bound, evaluation refs, governance gates, rollback, residuals, and non-claims. | implemented; passing via `python3 scripts/validate_protocol_examples.py` |
