...
| Test state | `intent_contract.valid.json`, `command_contract.valid.json`, and `intent_execution_trace.valid.json` pass protocol fixture validation; `python3 scripts/validate_plan_execution_contracts.py` passes 2 valid and 8 expected-invalid synthetic plan-execution contract checks, including unresolved-ambiguity dispatch blocking, unrejected-hidden-override blocking, and intent-authority-ceiling preservation; `AsiStackProofs.IntentToExecution` and `AsiStackProofs.CommandContracts` implement finite record predicates including a dispatch-route envelope, missing-field negative case, and hidden-override precedence negative case; behavioral parser, dispatcher, approval-enforcement, tool-effect, prompt-injection, authority-extraction, and runtime execution tests remain planned. |
...
| Field-confidence audit | Check that confirmed, policy-imposed, source-derived, defaulted, inferred, and missing fields preserve dispatch consequences. | planned; not run beyond record-shape checks |
| Authority-inference block test | Check that inferred means, tools, disclosure, publication, or side effects route to draft-only or re-contract states. | planned; not run beyond finite record and fixture checks |
...
- `AsiStackProofs.CommandContracts` for required command fields, missing-field
  ...
  instructions, and hidden-override rejection:
...
approval-service correctness, runtime execution correctness, model behavior, or
