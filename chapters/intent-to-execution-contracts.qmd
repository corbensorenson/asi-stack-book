...
open_evidence_gaps:
  ...
  - "Source notes, schemas, fixtures, a synthetic plan-execution harness, and finite Lean predicates including a dispatch-route envelope support record discipline only; they do not prove parser correctness, semantic extraction quality, prompt-injection resistance, approval enforcement, runtime dispatcher behavior, tool-effect control, or deployed execution safety."
...
| Command schema validation test | Check that command contracts expose objective, constraints, output contract, verification, and failure behavior before planning input is accepted. | implemented in `AsiStackProofs.CommandContracts` and protocol fixtures |
...
| Prompt override scenario | Check that hidden, retrieved, or conflicting instructions cannot override explicit contract constraints. | planned; not run against a deployed parser or model |
