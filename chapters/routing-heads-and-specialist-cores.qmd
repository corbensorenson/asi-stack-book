...
`AsiStackProofs.Routing` adds a bounded selection predicate requiring a selected route to satisfy authority and readiness, plus a bounded readiness-failure predicate requiring fallback or residual rather than promotion. It also proves that a selected route missing authority or readiness cannot satisfy the modeled selected-route admissibility predicate. Actual router implementation, specialist selection, and routing-quality measurement remain outside this finite receipt predicate.
...
Fixture-shape validation, seven bounded Lean theorem declarations over four finite predicates, the readiness/residual harness's synthetic route authority/fallback/readiness-mapping checks, and the synthetic routing decision lease harness are implemented. Routing accuracy and MoECOT runtime claims still require real routing tasks, candidate specialists, readiness states, learned-router or policy-router code, imported runtime packets, concrete replay refs, benchmark records, and logged route outcomes.
