...
This chapter is architecture argument grounded in available source notes and passage-reviewed mappings. Octopus Router Architecture and RMI supply the routing vocabulary; Benchmaxxing, SCF, Talos, Project Theseus, and MoECOT supply runtime-evidence pressure. The folded MoECOT material is treated as an implementation-reference crosswalk, not as reproduced runtime evidence. The repository proves finite predicates and routes, runs synthetic lease/readiness fixtures, and records bounded matched and ambiguous routing workloads. No deployed authority enforcement, trained-specialist utility, independent evaluation, MoECOT replay, orchestration benchmark, natural multi-model routing, or production transfer has run.
...
|---|---:|---|---|
| Core: `routing-heads-and-specialist-cores.core` | 1 | `causal`, `empirical`, `executable`, `formal`, `source-synthesis` | `blocked_after_full_attempt`; missing or unproved: `normative`, `transfer`. |
