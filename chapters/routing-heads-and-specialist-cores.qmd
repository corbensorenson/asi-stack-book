...
`physical_route_plan_ref`. Evaluation must compare the full QCSA route with
flat IDs, one semantic tree, multi-view identifiers without stable identity,
and ablations without active questions, soft routing, physical translation, or
migration governance. The `qcsa_whitepaper` proposes that test; it reports no
routing advantage, so the support state does not move.
