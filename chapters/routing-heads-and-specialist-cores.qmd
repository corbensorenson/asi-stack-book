...
open_evidence_gaps:
  ...
  - "Protocol schema/fixture validation, narrow Lean routing/readiness predicates, and the readiness/residual gate harness's synthetic authority-bounded/fallback route checks are implemented; no specialist-routing benchmark, learned-router harness, runtime route, or deployed authority-enforcement test has been run for this chapter."
...
| Test state | Specialist registry and routing decision fixture validation implemented; finite routing/readiness predicates implemented in `AsiStackProofs.Routing`; `python3 scripts/validate_readiness_residual_gates.py` validates synthetic authority-bounded and fallback route behavior. Routing accuracy, learned-router, runtime-route, and deployed authority-enforcement tests remain planned. |
...
Routing is trustworthy only when it preserves the reasons a specialist was chosen and the reasons it may be rejected. Recorded rejection matters because unused routes shape future trust and later qualification.
:::
...
Fixture-shape validation, two bounded Lean predicates, and the readiness/residual harness's synthetic route authority/fallback checks are implemented. Routing accuracy still requires synthetic routing tasks, candidate specialists, readiness states, learned-router or policy-router code, and logged route outcomes.
