...
open_evidence_gaps:
  ...
  - "Protocol schema/fixture validation, narrow Lean routing/readiness predicates, MoECOT runtime source-boundary predicates, and the readiness/residual gate harness's synthetic authority-bounded/fallback route checks are implemented; no specialist-routing benchmark, learned-router harness, runtime route, MoECOT replay, orchestration benchmark, or deployed authority-enforcement test has been run for this chapter."
...
The folded MoECOT slice adds an orchestration fixture and finite source-boundary predicates. The smallest honest runtime-crosswalk fixture records one accepted command, one denied route, one missing replay reference, one residual handoff, one failed gate, and one promotion blocker. That fixture checks record shape and evidence partitioning only; it is not a runtime replay, orchestration benchmark, specialist-quality result, or deployment claim.
...
Fixture-shape validation, four bounded Lean predicates, and the readiness/residual harness's synthetic route authority/fallback/readiness-mapping checks are implemented. Routing accuracy and MoECOT runtime claims still require synthetic routing tasks, candidate specialists, readiness states, learned-router or policy-router code, imported runtime packets, replay refs, benchmark records, and logged route outcomes.
...
These proof hooks are implemented as finite-record predicates. They do not prove routing accuracy, specialist quality, learned-router behavior, runtime enforcement, MoECOT runtime behavior, replay correctness, benchmark results, specialist-core quality, deployment readiness, or source-reported performance.
