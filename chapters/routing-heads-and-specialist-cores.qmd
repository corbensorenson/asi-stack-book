...
open_evidence_gaps:
  ...
  - "Only protocol schema/fixture validation is implemented; no specialist-routing benchmark or learned-router harness has been run for this chapter."
...
| Test state | Specialist registry and routing decision fixture validation implemented; routing accuracy tests remain planned. |

## Drafting guardrail

This chapter is architecture argument grounded in the available source notes. Octopus Router Architecture and RMI supply the routing vocabulary; Project Theseus and MoECOT supply implementation-reference pressure; no local specialist-routing benchmark or runtime reproduction has been run in this repository for this chapter.
...
| Authority-bounded routing test | Check that a specialist outside the authority envelope is rejected even if semantically suitable. | planned; not run |
| Fallback route test | Check that failed readiness or quality predicates emit fallback or residual records. | planned; not run |

Only fixture-shape validation is implemented. The planned tests require synthetic routing tasks, candidate specialists, authority envelopes, readiness states, and logged route outcomes.
...
These proof hooks remain planned. The current schemas expose authority and readiness fields; they do not yet prove route admissibility.
