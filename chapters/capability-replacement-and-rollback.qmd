...
| Test state | synthetic and formal baselines remain unchanged. Bounded outcomes include 15/15 P3 and 35/35 M7 exact local 24-surface rollback transactions, 32/36 attack-control rollback with 2/36 useful release, and 12/12 nine-surface harness probes with all model calls failing before candidate creation. The P5 local slice adds 8/8 real-subprocess cases spanning observed effects, crash recovery, exact rollback, compensation, revocation, full-state restore, and descendant deletion. No useful replacement, semantic recovery, real model-state recovery, privacy repair, external erasure, or production transfer follows. |
...
The supported conclusion is narrow: prospective inventories can make local rollback equality testable, and the current programs expose both inventory success and usefulness/recovery failure. The new slice also shows that exact reversal, compensation, and residual history can remain separate in one executable local lifecycle. These records do not establish complete semantic or external-effect recovery, monitor/evaluator validity, production replacement, useful governed throughput, acceptable cost, heterogeneous transfer, or chapter-core support.

The next executable slice should replace the deterministic state payloads with one real stateful, side-effecting field: frozen prior and candidate runtimes; exact dependency/environment locks; a prospective checkpoint over actual learning state; independently implemented evaluator and monitor; delayed outcomes; state/cache/data migration; one downstream consumer and descendant; an irreversible-effect control; rehearsed reverse migration and compensation; and strong conventional rollout baselines. It must report usefulness, unsafe release, rollback coverage, recovery time, privacy/rights effects, cost, and residuals jointly.
