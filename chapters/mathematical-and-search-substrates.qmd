...
The human reading rule is simple: a substrate enters through an interface, not through fascination. It needs a task boundary, baseline, evidence packet, failure mode, and reason why it improves the governed system rather than merely adding a beautiful internal story.
:::
...
The mature version of Mathematical and Search Substrates is not the minimal artifact with more scale. It is the chapter's idea turned into a product-grade, evidence-bearing boundary that the rest of the ASI Stack can depend on under ordinary operation and upgrade pressure.
...
- Operating mechanism: Frame each substrate as a specialist core or IR option; Define expected advantage, baseline, and falsification condition; Route experiments through evidence gates before adoption; Distinguish backbone or sequence-substrate efficiency from draft-token acceleration and downstream task quality.
- Stack interface: Routing treats substrate as specialist; Compression tests representation efficiency; Evidence compares against baselines; Fast-generation routes may consume substrate evidence, but adoption remains gated by substrate-specific A/B records.
- Preserved invariants: Exploratory claims stay exploratory; Baselines are recorded; Failed hypotheses remain visible; Backbone-efficiency claims do not imply search, routing, compression, or reasoning-quality gains.
- Failure closure: detect and route failure modes such as Performance overclaiming; Opaque math treated as proof; Adoption without regression tests; Sequence-model throughput treated as evidence for unrelated substrate quality to quarantine, rollback, escalation, or residual records instead of hidden degradation.
