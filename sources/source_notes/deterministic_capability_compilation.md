...
| `adjudicated-persistence-and-the-adaptive-commit-boundary` | Separates the decision to persist a lesson, its selected locus, and its evidence-qualified commitment from the later act of compiling an accepted representation into an executable capability. | §§6, 9-10, 14-17, 20 | Does not establish that a selected locus is optimal or that compilation preserves the intended lesson. |
